		...
		If (cSRDRot)->(!Eof())
			...
			While (cSRDRot)->(!Eof())
				...
				If !lVerRJ5 .Or. (lVerRJ5 .And. lRJ5Ok)
					...
					Endif
					CorpoXML(cOpcTab, cCompete, lGeraRes, aResCompl, @aLogDiss, aEstb, lComplTSV, cTpFolha, Nil, @lCPFDepOk, @aDepAgreg, cOptSimp, aTabInss, @lRJ5Ok, @aRJ5CC, @lRAZOk, lRelat, lComplCont, lExiste2300, @lDissMV, @aErros, @aLogPeric,, cTpRes, cNumBen, lTemRRA,.F. )
				EndIf
...
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
ßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßß*/
Static Function CorpoXML(cOpcTab, cCompete, lGeraRes, aResCompl, aLogDiss, aEstb, lComplTSV, cTpFolha, lTem132, lCPFDepOk, aDepAgreg, cOptSimp, aTabInss, lRJ5Ok, aErrosRJ5, lRAZOk, lRelat, lComplCont, lExiste2300, lDissMV , aErros, aLogPeric, lFer, cTpRes, cNumFer, lTemRRA, lGeraRRA)
	...
	Local cMatAnt		:= ""
	Local cIdsPerIns	:= ""
	Local cIdsRRA		:= "0974/0975/0976/0977/0978/0979/0980/0981/0982/0983/0986/0987"
	Local cVerbRRA      := ""
	Local cVerbRFC		:= ""
	Local nPosRaz		:= 0
	Local nPosEstb		:= 0
	...
	For nContRes := 1 To nTotVezes
		...
		If lGeraRes .And. !lDtPgto .And. cTpRotBkp != "F"
			If Select( cSRDAlias ) > 0
				(cSRDAlias)->( dbCloseArea() )
			EndIf
		...
		While (cSRDAlias)->(!Eof())
			...
			If cCCAnt <> (cSRDAlias)->RD_CC .Or. lPgIN2110
				cTpInscr	:= ""
				cInscr		:= ""
				...
				If lGeraRes .Or. lPagPost .Or. lPgIN2110
					...
					For nD := 1 to Len(aDissidio)
						...
						If lAchouRHH .And. Empty(aLogDiss)
							...
							If !Empty(cDscAc) .And. !Empty(cTpAco)
								...
								Else
									If !Empty(aTransfFun) .And. aScan(aTransfFun, { |x| x[12] >= cPerIni .And. x[12] <= SubStr(cCompete,3,4) + SubStr(cCompete,1,2) .And. (x[1] != x[4] .Or. x[8] != x[10]) }) > 0
										lTransEmp := .T.
									Endif
									cQrySt := "SELECT RHH.RHH_FILIAL,RHH.RHH_MAT,RHH.RHH_MESANO,RHH.RHH_DATA,RHH.RHH_VB,RHH.RHH_CC,RHH.RHH_IDCMPL,RHH.RHH_GRRA,RHH.RHH_RRA,SUM(RHH.RHH_VALOR) AS RHH_VALOR "
									cQrySt += "FROM " + RetSqlName('RHH') + " RHH "
...
If nNumReg == 0 .And. SRA->RA_SITFOLH == "D" .And. SRA->RA_RESCRAI $ "30|31"
    If __oSt19 == Nil
       ...
       cQrySt := "SELECT RH_FILIAL,RH_MAT,RH_PROCES,RH_PERIODO,RH_ROTEIR,RH_DTRECIB,RH_DFERIAS,RH_DATAINI,RH_DTRECIB, RE_CCD FROM "
	...
	While (cAliasSRH)->(!Eof())
		...
		If SRR->( DbSeek( (cAliasSRH)->RH_FILIAL + (cAliasSRH)->RH_MAT + "F" + (cAliasSRH)->(RH_DATAINI) ) )
			...
			While SRR->(!Eof() .and. RR_FILIAL+RR_MAT+RR_TIPO3+DTOS(RR_DATA) == (cAliasSRH)->RH_FILIAL+(cAliasSRH)->RH_MAT+"F"+(cAliasSRH)->RH_DATAINI )
				...
				aAdd(aFer[nPos,6],{ SRR->RR_PERIODO, If(lTransf, cCCOrig, SRR->RR_CC), SRR->RR_PD, SRR->RR_PERIODO, SRR->RR_ROTEIR, SRR->RR_HORAS, SRR->RR_VALOR, dtos(SRR->RR_DATAPAG), 0, "FER", "01", SRR->RR_NUMID})
...
@return cCCRJ5
/*/
Static Function fBsCCRJ5(cSRDFil,cSRDCC, lFilTit, cPeriod)
Local aArea			:= GetArea()
Local cQueryAlias 	:= GetNextAlias()
Local cCCRJ5		:= ""
