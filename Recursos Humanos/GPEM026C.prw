...
Function fInt2299( oModel, aErros, cReg, cCodDslg, cTpRes, aPd, dDataRes, cDiaInde, cVersaoEnv, cIndAvPrv, lResComp, lRetif, nOpca, lNT15, lRel)
	...
	Begin Transaction
		...
		If( cTpRes == "1" ) .And. nOperation != 5
			...
			If !Empty(cFilEnv)
				...
				If lXmlVerbas .And. (!lIntermit .Or. (lIntermit .And. (lCMesAtual .Or. len(aCols) > 0 .And. !Empty(aCols[1,13])))) .And.;
					...
					fDis2299( dDataRes, @cVBDiss, aDadosCCT, cIndSimp, @cInfoDiss, @cMsgDiss, @lRJ5Ok, @aErrosRJ5, cTpRes, , @cDtEfei, @cCompAc)
					If !Empty(aErrosRJ5)
					...
					If (lResComp .And. !(dDataHom == M->RG_DATAHOM)) .Or. lRetif
						fResCom(@cXml, oModel, aDadosCCT, cVBDiss, cIndSimp, lRetif, @aCols,lRJ5Ok)
					EndIf
					If cVersaoEnv >= '2.3'
					...
					If lTemRRA
						...
						lRetRRA := fRRA2299( @cXml, oModel, aDadosCCT, cIndSimp, dDataRes,cCompTrab, nMesRRA, cTpRes)
						If lRetRRA
							//Validação para verificar se gera o dmDev do Dissidio
		...
		IF lRel .And. nOperation <> 5
			fGeraRelat(alltrim(cXml), cDtEfei, cCompAc)
		Endif
...
/*/
Function fDis2299( dDataRes, cVBDiss, aDadosCCT, cIndSimp, cXmlAux, cMsgErro, lRJ5Ok, aErrosRJ5, cTpRes, aPd, cDtEfei, cCompAc)
...
/*/
Static Function fResCom(cXml, oModel, aDadosCCT, cVBDiss, cIndSimp, lRetif, aColsRes,lRJ5Ok)
...
//Somente gera os dados de 13º na rescisão se a rescisão NÃO for em dezembro com data de demissão maior ou igual ao pagamento do 13º (roteiro 132)
...
While (cAliasSRG)->(!Eof())
	...
	If !( &( cValidFil ) )
		...
		Loop
	EndIf
	If SRA->( dbSeek( SRG->RG_FILIAL+SRG->RG_MAT ) ) .And. (!lRel .Or. (lRel .And. !(SRA->RA_CATEFD $ cTrabSVinc)))
...
If cPaisLoc == "BRA" .And. nOper != 5 .And. !lRel
...
@version 1.0
/*/
Static Function fGeraRelat(cXML, cDtEfei, cCompAc)
					...
					While oXml:XPathHasNode(cinfoPerApur)
						...
						While oXml:XPathHasNode(cideEstabLot)
							...
							While oXml:XPathHasNode(cdetVerbas)
								...
								oExcel:AddRow(cAba1, cTabela1, { cCodFil, cCPF, cNome, cCatEFD, cideDmDev, ctpInsc + " - " + cnrInsc, ccodLotacao, cCodMat + " - " + cMateSocial, cindSimples, ccodRubr, cCodFol, cTpCod, nvrRubr, cCodNat, cCodINCCP, cCodINCIRF, cCodINCFGT, cIncINS, cIncIRF, cIncFGT } )
							...
							While oXml:XPathHasNode(cidePeriodo)
								...
								While oXml:XPathHasNode(cideEstabLot)
									...
									While oXml:XPathHasNode(cdetVerbas)
										...
										oExcel:AddRow(cAba2, cTabela2, { cCodFil, cCPF, cNome, cCatEFD, cideDmDev, stod(replace(cdtAcConv,"-","")), ctpAcConv, cCompAc, stoD(cDtEfei), cperRef, ctpInsc + " - " + cnrInsc, ccodLotacao, cCodMat + " - " + cMateSocial, cindSimples, ccodRubr, cCodFol, cTpCod, nvrRubr, cCodNat, cCodINCCP, cCodINCIRF, cCodINCFGT, cIncINS, cIncIRF, cIncFGT } )
...
Function fRRA2299( cXml, oModel, aDadosCCT, cIndSimp, dDataRes,cCompTrab, nMesRRA, cTipo, aPdRRA )
	...
	If cTipo == "1"
		...
		For nB := 1 To oGrid:Length()
			...
			If Dtos(oGrid:GetValue("RR_DATA")) <= dToS(dDataGer) .And. oGrid:GetValue("RR_ROTEIR") == 'RES' .And.;
				oGrid:GetValue("RR_TIPO3") == 'R' .And. oGrid:GetValue("RR_PD") $ aCodFol[0978,1]
		...
		For nB := 1 To Len( aPdRRA )
			If cRoteiro == 'RES' .And. aPdRRA[nB,1] $ aCodFol[0978,1]
				If dLastDate != dDataGer
					dLastDate := dDataGer
