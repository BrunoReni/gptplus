...
If lIntTaf .And. cVersEnvio >= "9.1.00"
	...
	cStatus := TAFGetStat( "S-2500", RE0->RE0_PROJUD,cEmpAnt,cFilEnv, 7)
	If cStatus == '4'
		DbSelectArea("RD0")
		...
		InExc3500(@cXml,'S-2500', cRecib, Alltrim(RE0->RE0_PROJUD), cCpf)
	...
	ElseIf cStatus == "2"
    	...
    	lRetXML := .F.
	ElseIf cStatus == "3"
		aAdd( aErros, OemToAnsi(STR0138) )//"Registro de exclusao S-3500 desprezado pois o evento S-2500 foi transmitido com inconsistencias."
		lRetXML := .F.
	ElseIf cStatus <> "-1"
		aAdd( aErros, OemToAnsi(STR0139) ) //"Registro de exclusao S-3500 desprezado pois o evento S-2500 não transmitido"
		lRetXML := .F.
	Endif
	
	If lRetXML .Or. cStatus = '-1'  // gerado S3500 OK ou nao encontrou... excluir 
...
If ((nOpcx == 3 .OR. nOpcx== 4) .AND. aEfd[1] .AND. cEFDAviso <> '2' .And. !Empty(M->RE0_DTDECI) )
	...
	aCampos		:= {M->RE0_TPPROC	,M->RE0_PROJUD	, IIf(Type("lIndSu2") == "L" .And. lIndSu2, M->RE0_INDSU2, M->RE0_INDSUS)	,M->RE0_DTDECI	,M->RE0_IDDEP}
	aCamposTit	:= {"RE0_TPPROC"	,"RE0_PROJUD"	, IIf(Type("lIndSu2") == "L" .And. lIndSu2, "RE0_INDSU2"	, "RE0_INDSUS")	,"RE0_DTDECI"	,"RE0_IDDEP"}
...
If lESProc 
	...
	If  M->RE0_TPACAO <> "1" // Tipo de Ação diferente de 1=Individual
...
Function InExc3500(cXml, ctpEvento, cRecibo, cProJud, cCpf, cPerApur )
...
fVersEsoc( "S3500", .T., /*aRetGPE*/, /*aRetTAF*/, @cVersEnv, Nil, @cVersMw )
