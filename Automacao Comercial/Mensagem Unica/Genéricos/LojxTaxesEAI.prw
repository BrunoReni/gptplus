...
Function LjTaxEAI(aCab, aItem, lTabTemp)
    ...
    If Len(aItem) > 0
        ...
        If nPosCaFre > 0
            MaFisLoad("NF_FRETE"  , aCab[nPosCaFre][2])
        EndIf
        ...
        For nItem:=1 To Len(aItem)
            ...
            If SF4->F4_IPI == 'S'
                If SF4->F4_IPIFRET == 'S'                     
                    ...
                    LjGrvLog("LjTaxEAI", "O item " + CValToChar(nItem) + ", esté configurado para incluir o frete no IPI do Produto.") 
                EndIf            
                If SF4->F4_DESPIPI == 'S'
            ...
            If lAtualiza
                aItem[nItem][nPosVrUnit][2] := MaFisRet(nItem, "IT_PRCUNI" ) - Iif(SF4->F4_IPIFRET == 'S', (nVlrItFrete/aItem[nItem][nPosQuant][2]), 0) - Iif(SF4->F4_DESPIPI == 'S', (nValDesp/aItem[nItem][nPosQuant][2]), 0)//Preço Unitário
                aItem[nItem][nPosPrcTab][2] := MaFisRet(nItem, "IT_PRCUNI" ) - Iif(SF4->F4_IPIFRET == 'S', (nVlrItFrete/aItem[nItem][nPosQuant][2]), 0) - Iif(SF4->F4_DESPIPI == 'S', (nValDesp/aItem[nItem][nPosQuant][2]), 0)//Preço de Tabela
                aItem[nItem][nPosVlrIte][2] := MaFisRet(nItem, "IT_VALMERC") - Iif(SF4->F4_IPIFRET == 'S', nVlrItFrete, 0) - Iif(SF4->F4_DESPIPI == 'S', nValDesp, 0)//Valor do Item
        ...
        If lAtualiza
            ...
            aCab[nPosCaMer][2] := MaFisRet(,"NF_VALMERC") + Iif(SF4->F4_IPIFRET == 'S', MaFisRet(,"NF_FRETE") *-1, 0) + Iif(SF4->F4_DESPIPI == 'S', MaFisRet(,"NF_DESPESA") *-1, 0)

            //Loja701c, refaz o calculos destes campos, então é necessário o tratamento abaixo
            If !FWIsInCallStack("Envia")

                aCab[nPosCaTot][2]  := MaFisRet(,"NF_TOTAL") + MaFisRet(,"NF_DESCTOT") + Iif(SF4->F4_IPIFRET == 'S', MaFisRet(,"NF_FRETE") *-1, 0) + Iif(SF4->F4_DESPIPI == 'S', MaFisRet(,"NF_DESPESA") *-1, 0)
                aCab[nPosCaLiq][2]  := MaFisRet(,"NF_TOTAL") + MaFisRet(,"NF_DESCTOT") + Iif(SF4->F4_IPIFRET == 'S', MaFisRet(,"NF_FRETE") *-1, MaFisRet(,"NF_FRETE")) + Iif(SF4->F4_DESPIPI == 'S', MaFisRet(,"NF_DESPESA") *-1, 0)
                aCab[nPosCaBru][2]  := MaFisRet(,"NF_TOTAL") + MaFisRet(,"NF_DESCTOT") + Iif(SF4->F4_IPIFRET == 'S', MaFisRet(,"NF_FRETE") *-1, MaFisRet(,"NF_FRETE")) + Iif(SF4->F4_DESPIPI == 'S', MaFisRet(,"NF_DESPESA") *-1, 0)
            EndIf
