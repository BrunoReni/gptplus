...
Method Consulta(cAssinante) Class RmiGrvMsgPubObj
    ...
    If Empty(cDB)
        LjGrvLog(" RmiEnvObj ", "Não há conexão ativa!" )
        Return Nil
    Else        
        LjGrvLog(" RmiEnvObj ", "Conectado com banco de dados: " + cDB )
        Do Case
            Case cDB == "MSSQL" 
                cSelect := "SELECT TOP(50) MHQ.R_E_C_N_O_ "
            Case cDB == "ORACLE"
                cSelect := " SELECT MHQ.R_E_C_N_O_ "
                cWhere  := " WHERE ROWNUM <= 50 AND MHQ_ORIGEM = '" + cAssinante + "'"                          
            OTherWise
                cSelect := " SELECT MHQ.R_E_C_N_O_ "
                cOrder  := " LIMIT 50"
        EndCase
    ...
    Self:cQuery += IIF(!Empty(cWhere),cWhere, " WHERE MHQ_ORIGEM = '" + cAssinante + "'" )
    Self:cQuery += "   AND MHQ_STATUS = '0' "
    Self:cQuery += "   AND MHQ.D_E_L_E_T_ = ' ' "
    Self:cQuery += IIF(!Empty(cOrder),cOrder, "")
...
Method Gravar() Class RmiGrvMsgPubObj
    ...
    Else
        ...
        If ElapTime( MHQ->MHQ_HORGER, Time() ) <= "01:00:00" .And. ( ("DE#PARA" $ cErro) .Or. ("SA1" $ cErro) .Or. ("SA3" $ cErro) .Or. ("SA6" $ cErro) .Or. ("SM0" $ cErro) )
