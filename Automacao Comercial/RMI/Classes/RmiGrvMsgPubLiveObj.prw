...
If !(self:cAliasQuery)->( Eof() )
    ...
    While !(self:cAliasQuery)->( Eof() )
        ...
        If Self:lSucesso .AND. oBuscaObj:lSucesso
            ...
            EndIf
        Else
            If ValType(oBuscaObj) == "O" .AND. Len(oBuscaObj:aArrayFil) == 0
                oBuscaObj:oRegistro := Self:ExecParseXml(MHQ->MHQ_MSGORI)
                Self:lSucesso := .F.
                Self:cErro := STR0001 + AllTrim(MHQ->MHQ_ORIGEM) + STR0002 + AllTrim(oBuscaObj:oRegistro:_CodigoLoja:Text) //"De/Para(MHM) não encontrado para a busca realizada. Origem: " # " | Alias: SM0 | Valor: "
                LjGrvLog("RMIGRVPUBLIVEOBJ",Self:cErro)
            Else
                LjGrvLog("RMIGRVPUBLIVEOBJ","Sem sucesso na verificação dentro do objeto")
            EndIf
        EndIf
