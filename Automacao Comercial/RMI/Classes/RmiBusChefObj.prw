        ...
        If lDtIntegra
            ...
            If !Empty(self:cLayoutFil) .And. SubStr(self:cLayoutFil, 1, 1) == "{"
                ...
                If !(self:oLayoutFil == Nil) .and. Valtype(self:oLayoutFil["Filiais"]) == "A"
                    ...
                    Next nItem
                Else
                    LjGrvLog("RMIBUSCHEFOBJ", "Conteúdo de cLayoutFil deve ser diferente de NIL")
                    LjGrvLog("RMIBUSCHEFOBJ", 'Conteúdo de self:oLayoutFil["Filiais"] deve ser um array')
                EndIf
            Else
                LjGrvLog("RMIBUSCHEFOBJ","Conteúdo do campo MHP_LAYFIL fora do formato esperado", self:cLayoutFil)
            EndIf
        Else
            LjGrvLog( "RMIBUSCHEFOBJ", "Não esta configurado para utilizar o metodo ListPorDataIntegracaoChefWeb", SubStr(self:oConfProce["url"],Rat("/",self:oConfProce["url"]) + 1) )
        EndIf
    Else
        LjGrvLog("RMIBUSCHEFOBJ", "Campo MHP_LAYFIL não encontrado na tabela MHP - Efetue a criação do campo")
    EndIf
...
Method Venda() Class RmiBusChefObj
    ...
    If !self:oRetorno["Sucesso"]
        ...
        self:lSucesso := .F.
        self:cRetorno := IIF(Valtype(self:oRetorno["Sucesso"]) =="U",AllTrim(self:cProcesso) + " - " + STR0004,AllTrim(self:cProcesso) + " - " + self:oRetorno["Erros"][1]["DescricaoErro"])//"Mensagem Recebida inválida verifique na Origem o Json Enviado"
        LjGrvLog("RMIBUSCHEFOBJ", self:cRetorno)
