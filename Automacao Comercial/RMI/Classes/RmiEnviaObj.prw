            ...
            If Self:lSucesso
                ...
                While !(self:cAliasQuery)->( Eof() )
                    ...
                    If !MHQ->( Eof() )
                        ...
                        //Atualiza o processo que será distribuido                        
...
Method Grava(cStatus) Class RmiEnviaObj
    ...
    Default cStatus    := IIF(self:lSucesso, "2", "3")    //1=A processar, 2=Processado, 3=Erro, 6=Aguardando Confirmação
...
Method SetArrayFil() Class RmiEnviaObj
    ...
    Else
        ...
        self:lSucesso := .F.
        self:cRetorno := I18n(STR0007, {"cProcesso", self:cAssinante})  //Campo Obrigatorio de Filiais não preenchido no cadastro de Assinantes
    EndIf
...
Method DePara(  cAlias  , cValor    , nRetIxdex , nIndice ,;
    ...
    EndIf
    //quando existe integração de envio e recebimento de cliente
    //é preciso validar o De/para e se existe no protheus.
