	...
	If cTipo == "F"
		...
		Else
			...
			If Empty(SA6->A6_DATAFCH)
				If !LjExOrc(Nil ,dDataMov) .And. iIf(lLjExMov, !LjExMov(,dDataMov), .T.) .AND. !Lj7ExMvSE5(dDataMov,aID[1][1])
					lAtuSLW := .T.
					RestArea(aArea)
					Return lRet
				EndIf
		...
		While !SE5->(EOF()) .And. cChaveSE5 == SE5->(E5_FILIAL+DTOS(E5_DATA)+E5_BANCO)
			...
			If AllTrim(SE5->E5_NUMMOV) == AllTrim(cNumMov)
				...
				ElseIf Upper(AllTrim(SE5->E5_HISTOR)) == Upper(AllTrim("BAIXA REF. DEVOLUCAO")) .And. Upper(AllTrim(SE5->E5_NATUREZ)) $ cNatDevol //Devolu็ใo a dinheiro
					...
					cDesForma	:= DescFP(AllTrim(cForma))
					nValFP		:= nValFP * (-1) 	// Transforma o valor em negativo para subtrair o valor e a quantidade
				ElseIf SE5->E5_TIPO == "NCC" .AND. SE5->E5_MOEDA == "CR" .AND. AllTrim(SE5->E5_NATUREZ) == "NOTA DE CR" .AND. SE5->E5_TIPODOC == "BA" .AND.;
...
If SE5->( DbSeek( cChaveSE5 ) )
	While !SE5->(EOF()) .And. cChaveSE5 == SE5->(E5_FILIAL+DTOS(E5_DATA)+E5_BANCO)
		...
		If AllTrim(SE5->E5_NUMMOV) == AllTrim(cNumMov)
			If SE5->E5_TIPO == "NCC" .AND. SE5->E5_MOEDA == "CR" .AND. AllTrim(SE5->E5_NATUREZ) == "NOTA DE CR" .AND. SE5->E5_TIPODOC == "BA" .AND.;
			 	SE5->E5_MOTBX == "CMP" .AND. Alltrim(SE5->E5_ORIGEM) == "LOJXREC"		
				lRet := .T.		
				Exit
			EndIf	
