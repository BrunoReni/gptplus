...
Do While !SL1->(Eof())
	...
	If	(AllTrim(SL1->L1_SERIE) # AllTrim(aID[3][1]) .AND. AllTrim(SL1->L1_SERPED) # AllTrim(aID[5][1]) .AND. Empty(SL1->L1_DOCRPS)).OR.;
...
If ChkFile("MHJ") .AND. ChkFile("MHK")
	...
	DbSelectArea("MHK")
	cFiltro := "MHK_FILIAL = '" + aChave[1] + "' .AND. DToS(MHK_DATMOV) >= '" + aChave[2] + "' .AND. DToS(MHK_DATMOV) <= '" + DToS(dDataBase) + "' " +;
