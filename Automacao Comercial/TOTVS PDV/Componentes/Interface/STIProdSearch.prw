...
Static cGetProduct	:= Space(40)				// Variável do Get 
...
oGetSearch:= TGet():New(	oPanelMVC:nHeight/14.955,POSHOR_1,{|u| If(PCount()>0,(lBusca := .F.,cGetProduct:=u), cGetProduct)}, ;
							oMainPanel,120 ,ALTURAGET,"@!",,,,,,,.T.,,,,,,,,,,"cGetProduct") //colocar o valid
...
oLblInfProd := TSay():New(oPanelMVC:nHeight/3.6378,POSHOR_1,{||cDados},oMainPanel,,,,,,.T.,,,,100)	// Descrição e Código do Produto
...
If !Empty(cGetProduct) .OR. lFindAll
	...
	aInfoItem	:= STWFindItem( cGetProduct, lPAFECF, 		lHomolPaf )
	...
	If !lFindAll .AND. aInfoItem[ITEM_ENCONTRADO]
		If !aInfoItem[ITEM_BLOQUEADO]
			...
			EndIf		
		EndIf
	ElseIf lMinGetSize .OR. lFindAll 
...
If nPos > 0 .AND. nPos <= Len(aRecno)
	...
	Else
		oLblProdDados:= TSay():New(oPanelMVC:nHeight/3.6378+10,POSHOR_1,{||cProdData},oMainPanel,,,,,,.T.,,,,100)
		oLblProdDados:SetCSS( POSCSS (GetClassName(oLblProdDados), CSS_LABEL_FOCAL ))
				
		oLblProdVal := TSay():New(oPanelMVC:nHeight/3.6378+10,POSHOR_1+130,{||cProdVal},oMainPanel,,,,,,.T.,,,,100)
