...
If !lMobile 
	...
	oListTpPaym := TListBox():Create(oPanPayment, POSVERT_GET1, POSHOR_1, Nil, aPaym, nLargura, nTamAltListBox,,,,,;
					.T.,,{||lListWhen := .F., lDisableBtn := .F.,STIRestVlr(), If( !Empty(oModel) .AND. (Empty(STIGetPgOr()) .Or. STFPROFILE(41)[1]), STIAddNewPan(oPanPayment, oListTpPaym), .F. )},,,,{||lListWhen == .T.})
...
lZeraDesc:= SuperGetMv("MV_LJRGDES",,.F.) .AND. !lDescGrupo .AND. !STBPromPgto(cPgtoSelected) .AND. !STIGetRecTit() .AND. STIDescRegVar() .AND. !lTemPagto
...
If cFrmPag=='BOL' .OR. cFrmPag=='CR' .OR. cFrmPag=='DC' .OR. cFrmPag== 'BF' 
	lRet:= .T. 
Endif
