...
Static nGetJurAdm	:= Space(TAMSx3("L1_JUROS")[1])			//Vaiavel Get Juros Adm Fin
...
If lContTef .Or. nCards == 1
	...
	oGetData := TGet():New(POSVERT_PAYGET1,POSHOR_PAYCOL1,{|u| If(PCount()>0,cGetData:=u,cGetData)},oPnlAdconal,LARG_GET_DATE,ALTURAGET,,,,,,,,.T.,,,{||.F.},,,,,,,,,,,,.T.)
	...
	oGetValor := TGet():New(POSVERT_PAYGET1,POSHOR_PAYCOL2,{|u| If(PCount()>0,cGetVal:=u,cGetVal)},oPnlAdconal,LARG_GET_VALOR,ALTURAGET,"@E 99,999,999.99",bChange,,,,,,.T.,,,,,,,lReadOnly,,,,,,,,,.T.)
	...
	oLblParcels := TSay():New(POSVERT_PAYLABEL1, oPnlAdconal:nWidth/2.5, {||STR0003}, oPnlAdconal,, ,,,,.T.,,,,8)
	oGetParcels := TGet():New(POSVERT_PAYGET1,oPnlAdconal:nWidth/2.5,{|u| If(PCount()>0,nGetParc:=u,nGetParc)},oPnlAdconal,40,ALTURAGET,"@E 9999",bChange,,,,,,.T.,,,,,,,lReadOnly,,,,,,,,.T.)
	...
	oGetJurAdm := TGet():New(POSVERT_PAYGET3,POSHOR_PAYCOL1,{|u| nGetJurAdm := nVlJuroAdm },oPnlAdconal,60,ALTURAGET,"@E 99,999,999.99",,,,,,,.T.,,,,,,,.T.,,,,,,,,.T.)
	...
	If Len(aAdmFin) > 0
		...
		oCboAdmFin := TComboBox():New(	POSVERT_PAYGET2	, POSHOR_PAYCOL1, {|u| If(PCount()>0,cCboAdmFin:=u,cCboAdmFin)}	, aAdmFin		,;
										LARG_GET_DATE+40, 100			, oPnlAdconal									, Nil			,; 
	...
	If lContTef .OR. ((!STWChkTef("CC") .AND. cTypeCard == "CC") .Or. (!STWChkTef("CD") .AND. cTypeCard == "CD"))  .OR. (!STWChkTef("PD") .AND. cTypeCard == "PD") .OR. (!STWChkTef("PX") .AND. cTypeCard == "PX")
		...
		Endif 
		/* NSU */ 
		oLblNSU := TSay():New(POSVERT_PAYLABEL2, POSHOR_PAYCOL2, {||STR0011}, oPnlAdconal,, ,,,,.T.,,,,8) // "NSU" 
		oGetNSU := TGet():New(POSVERT_PAYGET2,POSHOR_PAYCOL2,{|u| If(PCount()>0,cGetNSU:=u,cGetNSU)},oPnlAdconal,80,ALTURAGET,"@! " + Replicate("N",Len(cGetNSU)),,,,,,,.T.,,,,,,,,,,,,,,,.T.)
		
		oLblNSU:SetCSS( POSCSS (GetClassName(oLblNSU), CSS_LABEL_FOCAL )) 
		oGetNSU:SetCSS( POSCSS (GetClassName(oGetNSU), CSS_GET_NORMAL )) 
		
		/* AutorizaÁ„o */
		oLblAutoiz  := TSay():New(POSVERT_PAYLABEL2, oPnlAdconal:nWidth/2.5, {|| STR0012}, oPnlAdconal,, ,,,,.T.,,,,8) // "AutorizaÁ„o" 
		oGetAutoriz := TGet():New(POSVERT_PAYGET2,oPnlAdconal:nWidth/2.5,{|u| If(PCount()>0,cGetAutoriz:=u,cGetAutoriz)},oPnlAdconal,40,ALTURAGET,"@! " + Replicate("N",Len(cGetAutoriz)),,,,,,,.T.,,,,,,,,,,,,,,,.T.)
