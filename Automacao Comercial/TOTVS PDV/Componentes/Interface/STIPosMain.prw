...
DEFINE DIALOG oDlg TITLE STR0008 PIXEL STYLE nOr(WS_VISIBLE,WS_POPUP)  //Grafico Radar
	...
	oPOSDesk:SetTInfo( STR0013 + " " + cEstacao + "    |    " + STR0014 + " " + cUserName )   //"PDV:"  ...  "Operador:"
...
Local cClientDir	:= GetRemoteIniName()			//Caminho do diretorio do smartclient exe: "C:\totvs\bin\SmartClient\smartclient.ini"
...
@ nTop+(nX-1)*(nHeight+nMargin), nLeft+(nY-1)*(nWidth+nMargin) BUTTON oBtnConProd PROMPT IIf(lExibAtalho,STR0020,STR0042) SIZE nWidth,nHeight ACTION { || STIProdSearch() } WHEN {|| lBtnActive } OF o PIXEL
...
@ (o:nClientHeight/2)-50,10 BITMAP oSemaforo RESOURCE "FRTOFFLINE" NOBORDER SIZE 16,16 OF o ADJUST PIXEL
...
@ nTop+2,nLeft SAY oSay PROMPT STR0022 SIZE 040,010 RIGHT OF o PIXEL HTML   //"Volumes:"
oSay:SetCSS( POSCSS (GetClassName(oSay), IIf(lMobile,CSS_LABEL_SUBTITLE,CSS_LABEL_NORMAL))) 

@ nTop,nLeft+oSay:nWidth/2 GET oGetQtde VAR STIQtdItens() SIZE 100,010 OF o PIXEL
oGetQtde:SetCSS( POSCSS (GetClassName(oGetQtde), IIf(lMobile,CSS_GET_TOTAL,CSS_GET_NORMAL)))  
oGetQtde:lActive := .F.

nTop += 11

@ nTop+2,nLeft SAY oSay PROMPT STR0023 SIZE 040,010 RIGHT OF o PIXEL HTML   //"Subtotal:"
oSay:SetCSS( POSCSS (GetClassName(oSay), IIf(lMobile,CSS_LABEL_SUBTITLE,CSS_LABEL_NORMAL)))

@ nTop,nLeft+oSay:nWidth/2 GET oGetSubTot VAR AllTrim(Str(oTotal:GetValue("L1_VALMERC"),12,2)) SIZE 100,010 OF o PIXEL
oGetSubTot:SetCSS( POSCSS (GetClassName(oGetSubTot), IIf(lMobile,CSS_GET_TOTAL,CSS_GET_NORMAL))) 
oGetSubTot:lActive := .F.

nTop += 11

@ nTop+2,nLeft SAY oSay PROMPT STR0024 SIZE 040,010 RIGHT OF o PIXEL HTML   //"Desconto:"
oSay:SetCSS( POSCSS (GetClassName(oSay), IIf(lMobile,CSS_LABEL_SUBTITLE,CSS_LABEL_NORMAL))) 
...
If FindFunction("STIQtdBrindes")
	...
	@ nTop,nLeft+oSay:nWidth/2 GET oGetQtdBrinde VAR STIQtdBrindes() SIZE 100,010 OF o PIXEL
...
Do Case
	...
	Case nPanelActive == 2
		...
		If ValType(oPanel2) == "O" .And. !lItCkAct
			oPanel2:Hide()  //O Panel2 é que exibe a tela de brinde e qdo a variavel nPanelActive é igual a 2 ele nao exibe  e ocorre erro log no oPanel2
		EndIf
