...
METHOD Activate() CLASS POSMsgBarTop
	...
	Local nHeigthBtn := Val(FWResAdjSize(40))
	Local aColors := GetPOSColors()
	Local cColorBackGround 	:= '#' + aColors[5] //"#FFA200" //FFFFFF"		
	Local cColorSeparator 	:= '#' + aColors[2] //"#F37021" //C0C0C0"		
	Local cColorText		:= "#FFFFFF" 
	...
	//-------------------------------------------------------------
	For nX := 1 To Len(::aLOptions)
		oMenuItem := TMenuItem():New(,OemToAnsi(::aLOptions[nX][1]+Space(300)),OemToAnsi(::aLOptions[nX][1]+Space(300)),,,,,,,,,,,,.T.)
