...
Function POSCSS( cAdvPLComp, nType, xPar01 )
	...
	Do Case
		Case nType == CSS_BG
			...
			Else
				cCSS += "Q3Frame { border: none; background-color: #"+__aColors[2]+"; }"
			EndIf
		...
		Case nType == CSS_BAR_TOP
			...
			Else
				cCSS += "Q3Frame { border: none; background-color: #"+__aColors[3]+"; }"
			EndIf
			cCSS := FWCSSVerify( cAdvPLComp, "Q3Frame", cCSS )
		...
		Case nType == CSS_BAR_ALERT
			...
			Else
				cCSS += "QLabel { font: bold 32px; color: #FFFFFF; }"
			EndIf
		...
		Case nType == CSS_BAR_BUTTON
			...
			Else
				cCSS +=	 "QPushButton{ font: bold 12px; color: #FFFFFF; background-color: #"+__aColors[2]+"; border: none; }"
		...
		Case nType == CSS_BAR_INFO
			...
			Else
				cCSS += "QLabel { font: 12px; color: #FFFFFF; }"
			EndIf
		...
		Case nType == CSS_BAR_MENU
			...
			Else
				cCSS += 'QMenu{ font: bold; font-size: 13px; font-family: "Arial"; color: #FFFFFF; background-color: #'+__aColors[2]+'; ' + ;
		...
		Case nType == CSS_BTN_FOCAL
			...
			Else
				cCSS += "QPushButton{ font: 13px; padding: 6px; background-color: #"+__aColors[4]+"; " + ;
		...
		Case nType == CSS_BTN_NORMAL
			...
			Else
				cCSS +=	 "QPushButton{ font-size: 12px; font: bold large; padding: 4px; color: #858585; background-color: #FFFFFF; " + ;
									"outline :none; "+;
		...
		Case nType == CSS_BTN_ATIVO
			...
			Else
				cCSS += "QPushButton{ font-size: 12px; font: bold large; padding: 6px; color: #FFFFFF; " + ; 
								"background-color: qlineargradient(x1: 0, y1: 0, x2: 0, y2: 1, " + ;
								"stop: 0.0 #757575, stop: 0.6 #878787, stop: 0.7 #878787, stop: 1.0 #909090); " + ;
		...
		Case nType == CSS_BROWSE
			...
			Else
				cCSS += "QHeaderView::section { font: bold 12px; color: #FFFFFF; background-color: #"+__aColors[3]+"; " + ;
								"border: 1px solid #"+__aColors[3]+"; height: 30px; }"
				cCSS += "QTableView{ font: "+Str(nPxBrowse)+"px; alternate-background-color: #E9E9E9; background: #FFFFFF; color: #000000; border: none; " + ;
								"selection-background-color: #FFFFFF; gridline-color: white; }"
			EndIf
		...
		Case nType == CSS_LISTBOX
			...
			Else
				...
				cCSS += "Q3ListBox{ font: 18px; padding: 4px; background-color: #FFFFFF; margin: 0px; border-radius: 4px; " + ;
		...
		Case nType == CSS_PANEL_BTN_FOOTER
			...
			//cCSS += "QFrame{ margin: "+FWResAdjSize(0)+"px -20px "+FWResAdjSize(50)+"px "+FWResAdjSize(50)+"px; border: "+FWResAdjSize(2)+"px solid #D1D1D1;background-color: transparent; border-radius: 0px; }"
		...
		Case nType == CSS_BREADCUMB
			...
			Else
				cCSS += "QLabel{ font: bold 16px; color: #C0C0C0; background-color: transparent; border: none; margin: 0px; }"
			EndIf
			cCSS := FWCSSVerify( cAdvPLComp, "QLabel", cCSS )

		Case nType == CSS_LABEL_HEADER
			cCSS += "QLabel{ font:  "+FWResAdjSize(14)+"px; background-color: transparent; color: #000000; border: none; margin: 0px; }"
		...
		Case nType == CSS_LABEL_NORMAL
			...
			Else
				cCSS += "QLabel{ font: "+Str(nPxNormal)+"px; color: #B2B2B2; background-color: transparent; border: none; margin: 0px; }" 
			EndIf
			cCSS := FWCSSVerify( cAdvPLComp, "QLabel", cCSS )
		...
		Case nType == CSS_GET_FOCAL
				cCSS +=	 "QLineEdit {  font: bold; font-size:  "+FWResAdjSize(16)+"px; color: #656565; "+;
		...
		Case nType == CSS_GET_NORMAL
			If FWIsMobile()
				...
				cCSS +=	 GET_CSS_FOCAL
			Else
				cCSS +=	 "QLineEdit { font: bold; font-size: "+Str(nPxNormal)+"px; text-align: right; color: #656565; background-color: #FFFFFF; border: 1px solid #9C9C9C; border-radius: 4px; padding: 4px; }"
		...
		Case nType == CSS_LABEL_TOTAL
			...
			Else
				cCSS += "QLabel{ font: 35px; color: #656565; background-color: transparent; border: none; margin: 0px; }"
			EndIf			
			cCSS := FWCSSVerify( cAdvPLComp, "QLabel", cCSS )
		...
		Case nType == CSS_LABEL_ALERT
			...
			Else
				cCSS += "QLabel{ font: 22px; color: #656565; background-color: transparent; border: none; margin: 0px; }"
			EndIf			
			cCSS := FWCSSVerify( cAdvPLComp, "QLabel", cCSS )
