...
Static Function ExisteJob() As Logical
    ...
    // Não existe índice por nome da Rotina
    While XX1->(!(EoF()))
        If AllTrim(Upper(XX1->XX1_ROTINA)) == "FINI136O"
            If XX2->(DbSeek(XX1->XX1_CODIGO))
                While XX2->XX2_AGEND == XX1->XX1_CODIGO
                    If cEmpAnt + "/" $ XX2->XX2_EMPFIL
                        lCriado := .T.
                        Exit
                    EndIf
                    XX2->(DbSkip())
