            ...
            If LockByName( cLock, .T., .F. )       
                ...
                If !lConciliation
                    ...
                    For nType := 1 To Len( aEntityList )                      
                        ...
                        //--------------------------------------------------------------     
                        RSKUpdEntity( cHost, aEntityList[ nType ], lAutomato, aParam ) 
                    Next                                     
                Else   
...
/*/
//-------------------------------------------------------------------
Static Function RSKUpdEntity( cHost As Character, nType As Numeric, lAutomato As Logical, aParam As Array )
...
Function GetRSKItems( cHost As Character, nEntity As Numeric, lAutomato As Logical, aParam As Array ) As Array
    ...
    ElseIf nEntity == AFTERSALES    // 4=Pos venda
        If __lRegistry
            cAction := __oTFRegistry:oUrlTF["risk-antecipa-bearers-V2"]
        Else
    ...
    If !Empty( cHost ) .Or. lAutomato
        Do Case    
            ...
            Case nEntity == CONCILIATION        // 9=Conciliação
                If __lRegistry
                    cAction := __oTFRegistry:oUrlTF["risk-protheusapi-conciliation-V3"]
                Else
                    cAction := '/v3/conciliation'
                EndIf
...
Static Function GetAccounts( jItems, cContent, aItems ) 
    ...
    aSubProperties := { 'parcela', 'numeroParcelas', 'numeroNotaFiscal', 'codigoTransacao', 'tipoEvento', ;
        ...
        'cnpjParceiro', 'cnpjCpf', 'eventoDivergenciaComercial', 'lancamentoId', 'erpId' }
...
Function RSKAction( nType As Numeric, cHost As Character, aRecords As Array, lAutomato As Logical, aParam As Array)
    ...
    Local lConfirmation As Logical
    ...
    If !Empty( aRecordInfo )
        ...
        //-------------------------------------------------------------------       
        If lConfirmation     
            RSKPlatConfirm( cHost, nType, aRecordInfo, lAutomato, aParam )     
        EndIf
    EndIf
...
Static Function RSKPlatConfirm( cHost As Character, nType As Numeric, aRecords As Array, lAutomato As Logical, aParam As Array) 
    ...
    If !Empty( cHost ) .Or. lAutomato
        ...
        Elseif nType == CONCILIATION    // 9=Conciliação 
            nOrder := 2
            If __lRegistry
                cAction := __oTFRegistry:oUrlTF["risk-protheusapi-conciliation-V3"] + '/confirmation'
            Else
                cAction := '/v3/conciliation/confirmation'
            EndIf            
            cAlias := 'AR4'
...
Static Function RSKRecbyBranch( nType, aRecords )
    ...
    For nRec := 1 To nLenRecords
        ...
        cGrpEmp := IIF( nType == CONCILIATION .And. lSeekBranch, aRecords[ nRec ][ COD_EMP ], cEmpAnt ) //[9]-Conciliação # [33]-Código Empresa
...
Function RskJobBranch( aParam As Array, cFil As Character, lAutomato As Logical )
    ...
    If lLock
        ...
        If AliasInDic( "AR5" ) .And. AR5->( ColumnPos( "AR5_RCOUNT" ) ) > 0
            If __lRegistry
                cEndPoint := __oTFRegistry:oUrlTF["risk-riskapi-creditconcession-V3"]
            Else
