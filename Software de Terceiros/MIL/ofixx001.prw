...
Static lFuncOM030GRUSRV := FindFunction("OM030GRUSRV")

Static lFOM020ArmazemOri:= FindFunction("OM0200065_ArmazemOrigem")
...
::nAt:=0
return self
// Luis Delorme         : Alterado em 29-04-2010 - 09:05  (FIM)
//-----------------------------------------------------------
...
Private lDupOrc := .f.
Private cApSegMod := ""
Private cApAno := Replicate("0",IIf(VE3->(FieldPos("VE3_ANOMOD"))>0,TamSX3("VE3_ANOMOD")[1],4))
// Variaveis Fiscais
...
if !(FindFunction("FMX_CNPJLB")) .or. !FMX_CNPJLB()
	If !AMIIn(14,41) .or. !FMX_AMIIn({"OFIXA018","OFIXA012","OFIXA011","OFIOA330","OFIXA014","OFIXA016","OFIOC060","OFIOC270","OFIOC470","VEICC500","VEIVC220","VEIVA620","OFIOM350","M_CONSPEC","VEIXA018","VEIXA030","OFIOC500","VEIXA019","OFIOC520","OFIOC526","OFIXC001","MATA030","OFIOC150","OFIOC430"})
	    Return()
	EndIf
endif
...
if lInconveniente
	Do Case
		Case cMV_VERIORC == "2"
			AADD( aNewBot , {"AVGBOX1", {|| OX001XC008()  } , ("<F7> "+STR0254) })
		//Case cMV_VERIORC == "3"
		//	AADD( aNewBot , {"AVGBOX1", {|| OX001XC009()  } , ("<F7> "+STR0254) })
		Case cMV_VERIORC == "M_CONSPEC"
			AADD( aNewBot , {"AVGBOX1", {|| OX001CONSPEC()} , ("<F7> "+STR0254) })
		OtherWise
...
Else
	Do Case
		Case cMV_VERIORC == "2"
			AADD( aNewBot , {"AVGBOX1", {|| OFIXC008()    } , ("<F7> "+STR0254) })
		//Case cMV_VERIORC == "3" .and. ExistFunc("OFIXC009")
		//	AADD( aNewBot , {"AVGBOX1", {|| OFIXC009(2)   } , ("<F7> "+STR0254) })
		Case cMV_VERIORC == "M_CONSPEC"
			AADD( aNewBot , {"AVGBOX1", {|| U_M_CONSPEC() } , ("<F7> "+STR0254) })
    	OtherWise
			AADD( aNewBot , {"AVGBOX1", {|| IIf(ExistBlock("OX001CPC"),ExecBlock("OX001CPC",.f.,.f.,{""}),OFIXC001()) , SETKEY(VK_F6,{|| OX001KEYF6(.f.) } ) , FG_MEMVAR(aHeaderP,oGetPecas:aCols,oGetPecas:nAt) } , STR0004 })
	EndCase
Endif
...
AADD( aNewBot , {"COMPTITL",  {|| Processa( {|| OX001RECALC(nOpc) })  } , STR0009 })
...
//
If nOpc <> 3 .and. VS1->(FieldPos("VS1_OPORTU")) > 0
...
if !FM_PILHA("OFIXA011") .and. !FM_PILHA("OFIXA018") .and. !FM_PILHA("OFIXC008") .and. !FM_PILHA("OFIXC009") .and. !FM_PILHA("U_M_CONSPEC")
	aNewBot := {}
endif
...
IF ALTERA .and. VS1->VS1_TIPORC == "2" .and. VO1->(FieldPos("VO1_GARMUT")) <> 0
...
// ####################################################################################################
if FECHA
	if cVS1Status == "5"	// Se for liberar divergencia deixa alterar (nOpc = 4)
		nOpc := 4
...
Local cPesqPromo  := ""
Local lVS3PESPRO  := VS3->(FieldPos("VS3_PESPRO")) > 0 
...
//aAdd(aOrc, {'MaFisRet(,"NF_DIFAL")', STR0342, 0, "VS1->VS1_DIFAL"}) // Diferença de ICMS
...
If VAI->(FieldPos("VAI_ALTFML")) > 0 .and. VAI->VAI_ALTFML == "0" // Usuario NAO pode alterar a Formula
	lAltFormula := .f.
EndIf
...
cVS1nMostra += IIF(VAI->(FieldPos("VAI_RESERV")) > 0 .and. VAI->VAI_RESERV != "1", "VS1_RESERV,","")
//
...
While !Eof().and.(x3_arquivo=="VS1")
	...
	If ( (x3_context != "V" .OR. (x3_context == "V" .AND. X3_VISUAL == "A"))  .or. x3_campo $ "VS1_GETKEY,VS1_OBSERV")
		if !(Alltrim(x3_campo) $ cVS1nEdit) .and.  !(Alltrim(x3_campo)+"," $ cVS1nMostra)
			if FECHA .and. cVS1Status $ "F/G"
				if VS1->(FieldPos("VS1_MENNOT")) > 0 .and.;
				   Alltrim(x3_campo)+"," $ "VS1_MENNOT,VS1_MENPAD,VS1_KILOME,VS1_VEICUL,VS1_TRANSP,VS1_VOLUM1,VS1_VOLUM2,VS1_VOLUM3,VS1_VOLUM4,VS1_ESPEC1,VS1_ESPEC2,VS1_ESPEC3,VS1_ESPEC4,VS1_PESOL,VS1_PESOB,VS1_USUENT,VS1_NATURE,VS1_OBSERV,"
					aAdd(aCpos,X3_CAMPO)
				endif
			else
				aAdd(aCpos,X3_CAMPO)
...
While !Eof().And.(x3_arquivo=="VS3")
	If  X3USO(x3_usado) .and. !(Alltrim(x3_campo)+"," $ cVS3nMostra) .And. ( cNivel>=x3_nivel .or. IIF(cNivel < x3_nivel .and. Alltrim(x3_campo) == "VS3_MARLUC",.t.,.f.) )
		...
		if x3_usado != "V" .and. (INCLUI .or. ALTERA)
			if !(cVS1Status == "5" .and. FECHA) .and. !lFaturaPVP .and.  !lCancelPVP
...
If !INCLUI // deve-se montar a acols com os registros da base
	...
	While !eof() .and. VS3->VS3_FILIAL == xFilial("VS3").and. VS3->VS3_NUMORC == VS1->VS1_NUMORC
		...
		If SF4->F4_OPEMOV == "05" .and. ALTERA
			...
			If cPesqPromo <> "0" .and. lVS3PESPRO .and. VS3->VS3_PESPRO == "0"
				cPesqPromo := "0"
			EndIf
			aTempPro := OX005PERDES(SBM->BM_CODMAR,VS1->VS1_CENCUS,VS3->VS3_GRUITE,VS3->VS3_CODITE,VS3->VS3_QTDITE,VS3->VS3_PERDES,.f.,VS1->VS1_CLIFAT,VS1->VS1_LOJA,IIF(VS1->VS1_TIPORC == "2","3",VS1->VS1_TIPVEN),VS3->VS3_VALTOT/VS3->VS3_QTDITE,2,VS1->VS1_FORPAG,,,,cPesqPromo,dDatRefPD,nVS1PERREM)
			reclock("VS3",.f.)
		...
		For nCntFor:=1 to nUsadoPX01
			if aHeaderP[nCntFor,2] =="VS3_QTDEST"
				SB1->(DBSetOrder(7))
				SB1->(DBSeek(xFilial("SB1")+VS3->VS3_GRUITE+VS3->VS3_CODITE))
				aColsP[Len(aColsP),nCntFor] := OX001SLDPC(xFilial("SB2")+SB1->B1_COD+VS3->VS3_LOCAL)
			elseif IsHeadRec(aHeaderP[nCntFor,2])
				aColsP[Len(aColsP),nCntFor] := VS3->(RecNo())
			elseif IsHeadAlias(aHeaderP[nCntFor,2])
				aColsP[Len(aColsP),nCntFor] := "VS3"
			elseif aHeaderP[nCntFor,10] == "V"
				SX3->(DBSetOrder(2))
				SX3->(DBSeek(aHeaderP[nCntFor,2]))
				aColsP[Len(aColsP),nCntFor] := &(sx3->x3_relacao)
			else
				aColsP[Len(aColsP),nCntFor] := FieldGet(FieldPos(aHeaderP[nCntFor,2]))
			endif
		Next
	...
	While !eof() .and. VS4->VS4_FILIAL == xFilial("VS4") .and. VS4->VS4_NUMORC == VS1->VS1_NUMORC
		...
		For nCntFor:=1 to nUsadoS
			if aHeaderS[nCntFor,10] == "V"
				...
				aColsS[Len(aColsS),nCntFor] := FieldGet(FieldPos(aHeaderS[nCntFor,2]))
			endif
		Next
	...
	Else // ... caso contrário deve-se montar a acols com os registros da base
		...
		While !eof() .and. VST->VST_FILIAL == xFilial("VST") .and. VST->VST_TIPO == "1" .and. VST->VST_CODIGO == VS1->VS1_NUMORC
			...
			For nCntFor:=1 to nUsadoI
				if aHeaderI[nCntFor,10] == "V"
					...
					aColsI[Len(aColsI),nCntFor] := &(sx3->x3_relacao)
				else
					aColsI[Len(aColsI),nCntFor] := FieldGet(FieldPos(aHeaderI[nCntFor,2]))
				endif
			Next
		...
		If VS1->VS1_TIPORC == "2" .and. !Empty(cIncDefault) // Orcamento de Oficina e possui Inconveniente Default
			...
			For nCntFor := 1 to len(aColsP)
				If Empty(aColsP[nCntFor,FG_POSVAR("VS3_GRUINC","aHeaderP")]+aColsP[nCntFor,FG_POSVAR("VS3_CODINC","aHeaderP")]+aColsP[nCntFor,FG_POSVAR("VS3_DESINC","aHeaderP")]) // Descricao do Incoveniente
					aColsP[nCntFor,FG_POSVAR("VS3_SEQINC","aHeaderP")] := aColsI[nPosInc,FG_POSVAR("VST_SEQINC","aHeaderI")] // Sequencia
					aColsP[nCntFor,FG_POSVAR("VS3_DESINC","aHeaderP")] := aColsI[nPosInc,FG_POSVAR("VST_DESINC","aHeaderI")] // SEM INSTALACAO
			    EndIf
			Next
        EndIf
	EndIf
...
If !lOX001Auto
	...
	oEnch := MSMGet():New( cAlias ,nReg,IIF((cVS1Status == "5" .and. FECHA).or. (lPediVenda .and. (nOpc != 3 .and. nOpc != 4)) ,2,IIF((cVS1Status $ "F/G" .and. FECHA).or.(If(VS1->(FieldPos("VS1_PEDREF"))>0.and.!Empty(VS1->VS1_PEDREF),.t.,.f.) .and. FECHA),3,nOpc)),,,,aCpoEncS, aPosEnchoice,aCpos,nModelo,,,cTudoOk,oDlgXX001,lF3,lMemoria,lColumn,caTela,lNoFolder, lProperty)
	//#############################################################################
	//# Folder das GetDados                                                       #
	//#############################################################################
...
//		cDesInc := space(TamSX3("VST_DESINC")[1])
	...
	If lLibPV
		...
		for nCntFor := 1 to Len(oGetPecas:aCols)
			...
			M->VS3_QTDITE := oGetPecas:aCols[nCntFor,fg_posvar("VS3_QTDITE","aHeaderP")]
...
If ReadVar() == "M->VS1_RESERV"
	...
	If !lMsgGrv // se não vai apresentar mensagem de gravação com sucesso
		if INCLUI
...
If ReadVar() == "M->VS1_TIPVEN"
	...
	if DBSeek(xFilial("VAI")+M->VS1_CODVEN)
		...
		if lRet
			if !Empty(oGetPecas:aCols[1,FG_POSVAR("VS3_CODITE","aHeaderP")]) .and. !oGetPecas:aCols[1,Len(oGetPecas:aCols[1])]
				...
				if nPos1 < nPosicao .or. nPosicao == 0
					...
					if !Empty(oGetPecas:aCols[oGetPecas:nAt,FG_POSVAR("VS3_FORMUL","aHeaderP")])
						if oGetPecas:aCols[oGetPecas:nAt,FG_POSVAR("VS3_FORMUL","aHeaderP")] <> &(cFormul)
						    cMensagem := STR0259+CHR(13)+CHR(10)+CHR(13)+CHR(10)
						    cMensagem += STR0260+CHR(13)+CHR(10)
						    cMensagem += STR0261
							...
							ACTIVATE MSDIALOG oDlg8 CENTER
 			            	if nOpca == 1
	    		        	    For nPosic := 1 to Len(oGetPecas:aCols)
									...
									//
									SBM->(dbSetOrder(1))
									SBM->(dbSeek(xFilial("SBM")+oGetPecas:aCols[oGetPecas:nAt,FG_POSVAR("VS3_GRUITE","aHeaderP")]))
									oGetPecas:nAt := nPosic
									if !oGetPecas:aCols[oGetPecas:nAt,Len(oGetPecas:aCols[oGetPecas:nAt])]
										FG_MEMVAR(aHeaderP,oGetPecas:aCols,oGetPecas:nAt)
										if !Empty(SBM->BM_FORMUL)
											oGetPecas:aCols[oGetPecas:nAt,FG_POSVAR("VS3_FORMUL","aHeaderP")] := SBM->BM_FORMUL
										else
											oGetPecas:aCols[oGetPecas:nAt,FG_POSVAR("VS3_FORMUL","aHeaderP")] := &(cFormul)
										endif
...
if ReadVar()=="M->VS1_PGTFRE"
	...
	If oGetPecas:aCols[1,fg_posvar("VS3_QTDITE","aHeaderP")] != 0
...
if ReadVar()=="M->VS1_FORPAG"
	...
	If SE4->(FieldPos("E4_MSBLQL")) > 0 .AND. !Empty(M->VS1_FORPAG)
		 DbSelectArea("SE4")
		 DbSetOrder(1)
	...
	if SA1->A1_COND != M->VS1_FORPAG .and. !Empty(SA1->A1_COND)
		if !lOX001Auto
			nOperacao := Aviso( STR0028, STR0029, { STR0030 , STR0031 } )
			if nOperacao <> 1
				M->VS1_FORPAG := SA1->A1_COND
				DBSelectArea("SE4")
				DBSetOrder(1)
	...
	if !Empty(oGetPecas:aCols[1,FG_POSVAR("VS3_CODITE","aHeaderP")]) .and. !oGetPecas:aCols[1,Len(oGetPecas:aCols[1])]
		...
		if nPos1 < nPosicao .or. nPosicao == 0
			...
			lAchou := .f.
       	    For nPosic := 1 to Len(oGetPecas:aCols)
       	        if !oGetPecas:aCols[nPosic,Len(oGetPecas:aCols[nPosic])]
					if !Empty(oGetPecas:aCols[nPosic,FG_POSVAR("VS3_FORMUL","aHeaderP")])
						if oGetPecas:aCols[nPosic,FG_POSVAR("VS3_FORMUL","aHeaderP")] <> &(cFormul)
							if SB1->(FieldPos("B1_RECALC")) > 0
								dbSelectArea("SB1")
								DBSetOrder(7)
								DBSeek(xFilial("SB1")+oGetPecas:aCols[nPosic,FG_POSVAR("VS3_GRUITE","aHeaderP")]+oGetPecas:aCols[nPosic,FG_POSVAR("VS3_CODITE","aHeaderP")])
        	                    if SB1->B1_RECALC <> "1"
            	                   lAchou := .t.
                	            Endif
                    	    Endif
						...
						oGetPecas:nAt := nPosic
	           	        if !oGetPecas:aCols[oGetPecas:nAt,Len(oGetPecas:aCols[oGetPecas:nAt])]
							...
							if SB1->(FieldPos("B1_RECALC")) > 0
								dbSelectArea("SB1")
								DBSetOrder(7)
								DBSeek(xFilial("SB1")+oGetPecas:aCols[oGetPecas:nAt,FG_POSVAR("VS3_GRUITE","aHeaderP")]+oGetPecas:aCols[oGetPecas:nAt,FG_POSVAR("VS3_CODITE","aHeaderP")])
	                            if SB1->B1_RECALC <> "1"
									if !Empty(SBM->BM_FORMUL)
										oGetPecas:aCols[oGetPecas:nAt,FG_POSVAR("VS3_FORMUL","aHeaderP")] := SBM->BM_FORMUL
									else
...
if ReadVar()=="M->VS1_PERDES" .or. ReadVar()=="M->VS1_VALPRE"
	...
	// ###########################################################################################
	// Se for digitado valor pretendido tenta ratear o valor entre os itens
	lValPre := .f.
	lValMaior := .f.
	if ReadVar()=="M->VS1_PERDES" .and. M->VS1_PERDES != 0
		nValDesAtu := M->VS1_PERDES
		//
		M->VS1_PERDES := 0
		__ReadVar := 'M->VS1_PERDES'
		OX001VLDENC()
		//
		M->VS1_PERDES := nValDesAtu
	endif
	//
	if ReadVar()=="M->VS1_VALPRE"
		...
		//
		lValPre := .t.
		if M->VS1_VALPRE > MaFisRet(,"NF_VALMERC")
			nValDif := M->VS1_VALPRE
			lValMaior := .t.
		else
			M->VS1_PERDES := 100- (100 * M->VS1_VALPRE / MaFisRet(,"NF_VALMERC"))
		endif
	endif
	if lValMaior
		for nCntFor = 1 to Len(oGetPecas:aCols)
		    // So processa a linha que Interessa
			If nPosVetor > 0
				If nCntFor <> nPosVetor
					Loop
				Endif
			Endif
			if !oGetPecas:aCols[nCntFor,Len(oGetPecas:aCols[nCntFor])]
				if oGetPecas:aCols[nCntFor,fg_posvar("VS3_VALDES","aHeaderP")] > 0
					MsgStop(STR0176,STR0025)
					return .f.
				endif
			endif
		next
	...
	for nCntFor = 1 to Len(oGetPecas:aCols)
		...
		if lValMaior
			...
			if !oGetPecas:aCols[nCntFor,Len(oGetPecas:aCols[nCntFor])]
				...
				oGetPecas:aCols[nCntFor,fg_posvar("VS3_VALPEC","aHeaderP")] := A410Arred(nValDif * oGetPecas:aCols[nCntFor,fg_posvar("VS3_VALPEC","aHeaderP")]  / nValPRat,"C6_PRUNIT")
				oGetPecas:nAt := nCntFor
				...
				next
				M->VS3_VALPEC := oGetPecas:aCols[nCntFor,fg_posvar("VS3_VALPEC","aHeaderP")]
			...
			if !oGetPecas:aCols[nCntFor,Len(oGetPecas:aCols[nCntFor])]
				...
				nVTotPecAntes += M->VS3_VALPEC * M->VS3_QTDITE
				//If Type("aVetItDesc[nCntFor]") <> "U"
					aVetItDesc[Len(aVetItDesc),1] := M->VS3_VALPEC * M->VS3_QTDITE
				//Endif
				// armazena grupo e código do item
				cCodIte := 	oGetPecas:aCols[nCntFor,fg_posvar("VS3_CODITE","aHeaderP")]
				cGruIte := 	oGetPecas:aCols[nCntFor,fg_posvar("VS3_GRUITE","aHeaderP")]
				SBM->(dbSetOrder(1))
				SBM->(dbSeek(xFilial("SBM")+cGruIte))
				// calcula o valor mínimo possível para o produto em questão
				nValMin := OX005RETMIN(SBM->BM_CODMAR,M->VS1_CENCUS,M->VS3_GRUITE,M->VS3_CODITE,M->VS3_QTDITE,M->VS3_PERDES,.f.,M->VS1_CLIFAT,M->VS1_LOJA,M->VS1_TIPVEN,M->VS3_VALPEC,2,M->VS1_FORPAG,,.f.,dDatRefPD,nVS1PERREM)

				// coloca o valor mínimo possível no item da acols
				if M->VS3_VALPEC != 0
					M->VS3_PERDES := ( 1 - nValMin/M->VS3_VALPEC)*100
				endif
				// executa o fieldok para atualizar os campos
				oGetPecas:aCols[nCntFor,fg_posvar("VS3_PERDES","aHeaderP")] := M->VS3_PERDES
				oGetPecas:nAt := nCntFor
				__ReadVar := 'M->VS3_PERDES'
				OX001FPOK(.f.,,.f.,.f.)
				// armazena o valor total e a diferença depois da alteração
				nVTotPecDepois += oGetPecas:aCols[nCntFor,fg_posvar("VS3_VALTOT","aHeaderP")]
				//If Type("aVetItDesc[nCntFor]") <> "U"
					aVetItDesc[Len(aVetItDesc),2] := oGetPecas:aCols[nCntFor,fg_posvar("VS3_VALTOT","aHeaderP")]
				//Endif
				//If Type("aVetItDif[nCntFor]") <> "U"
	...
	if lValMaior
		...
		if nDifFinal != 0
			for nCntFor = 1 to Len(oGetPecas:aCols)
				...
				//
				if !oGetPecas:aCols[nCntFor,Len(oGetPecas:aCols[nCntFor])]
					oGetPecas:aCols[nCntFor,fg_posvar("VS3_VALPEC","aHeaderP")] := nDifFinal/oGetPecas:aCols[nCntFor,fg_posvar("VS3_QTDITE","aHeaderP")] + oGetPecas:aCols[nCntFor,fg_posvar("VS3_VALPEC","aHeaderP")]
					oGetPecas:nAt := nCntFor
					M->VS3_VALPEC := oGetPecas:aCols[nCntFor,fg_posvar("VS3_VALPEC","aHeaderP")]
					__ReadVar := 'M->VS3_VALPEC'
					OX001FPOK(.f.,,.f.,.f.)
	...
	if !lValMaior
		...
		nValorDesejado := nVTotPecAntes * (1 - M->VS1_PERDES/100)
		...
		if nValFalta > nValSobra
			...
			For nCntFor = 1 to Len(oGetPecas:aCols)
				...
				Endif
			Next
			If nUltPosaCols > 0 .and. nTotFinal <> M->VS1_VALPRE .and. abs(nTotFinal - M->VS1_VALPRE) <= 0.05 .and. oGetPecas:aCols[nUltPosaCols,FG_POSVAR("VS3_VALDES","aHeaderP")] > 0.05
				M->VS3_VALDES := oGetPecas:aCols[nUltPosaCols,FG_POSVAR("VS3_VALDES","aHeaderP")]
				...
				Endif
				oGetPecas:aCols[nUltPosaCols,FG_POSVAR("VS3_VALDES","aHeaderP")] := M->VS3_VALDES 
...
===============================================================================
*/
Function OX001FPOK(lMensagem, lLibPv, lRefNF, lRecalc, lMsgLotMin, lVerPromo , lAgrMsgSld ,aProdSdEst)
Local nCntFor, nCntFor2, nCntFor3, nCntFor4
...
if VS3->(FieldPos("VS3_RESERV")) > 0 .and. VS1->VS1_STATUS<>'5' .and. !lRecalc
	if oGetPecas:aCols[oGetPecas:nAt,fg_posvar("VS3_RESERV","aHeaderP")] == "1" .and. ;
		( ReadVar() != "M->VS3_VALPEC" .and.;
		  ReadVar() != "M->VS3_FORMUL" .and.;
		  ReadVar() != "M->VS3_VALDES" .and.;
		  ReadVar() != "M->VS3_PERDES" )
		MsgStop(STR0274,STR0025)
		return .f.
	endif
endif

if VS3->(FieldPos("VS3_MOTPED")) > 0
	if !Empty(oGetPecas:aCols[oGetPecas:nAt,fg_posvar("VS3_MOTPED","aHeaderP")])
		MsgStop(STR0292,STR0025)
		return .f.
	endif
endif

if VS3->(FieldPos("VS3_QTDAGU")) > 0
	if oGetPecas:aCols[oGetPecas:nAt,fg_posvar("VS3_QTDAGU","aHeaderP")] > 0
		MsgStop(STR0225,STR0025)
		return .f.
	endif
endif
//
if VS3->(FieldPos("VS3_QTDTRA")) > 0
	if oGetPecas:aCols[oGetPecas:nAt,fg_posvar("VS3_QTDTRA","aHeaderP")] > 0 .and. Readvar() $ "M->VS3_GRUITE,M->VS3_CODITE,M->VS3_QTDITE"
		MsgStop(STR0225,STR0025)
		return .f.
	endif
endif
...
If Readvar() == "M->VS3_CODITE" .and. ( M->VS3_GRUITE + M->VS3_CODITE ) <> ( oGetPecas:aCols[oGetPecas:nAt,fg_posvar("VS3_GRUITE","aHeaderP")] + oGetPecas:aCols[oGetPecas:nAt,fg_posvar("VS3_CODITE","aHeaderP")] ) .and. !Empty(oGetPecas:aCols[oGetPecas:nAt,fg_posvar("VS3_CODITE","aHeaderP")])
...
If Readvar() == "M->VS3_CODTES" .and. ( M->VS3_GRUITE + M->VS3_CODITE ) <> ( oGetPecas:aCols[oGetPecas:nAt,fg_posvar("VS3_GRUITE","aHeaderP")] + oGetPecas:aCols[oGetPecas:nAt,fg_posvar("VS3_CODITE","aHeaderP")] )
...
If ReadVar() == "M->VS3_LOCAL" .and. !Empty(M->VS3_LOCAL) .and. !Empty(M->VS3_GRUITE+M->VS3_CODITE)
	SB1->(DbSetOrder(7))
...
// ##############################################################################
	if !FS_QTDLIN("1")
		Return(.f.)
	Endif
	If ( VS1->(FieldPos("VS1_PEDREF")) > 0 .and. !Empty(M->VS1_PEDREF) ) .and. GetNewPar("MV_MIL0011","0") == "1"
		If lMensagem
			MsgAlert(STR0270,STR0025)
			return .f.
		EndIf
	Endif
	...
	DbSelectArea("SB1")
	DbSetOrder(7)
	if !MsSeek( xFilial("SB1") + M->VS3_GRUITE + M->VS3_CODITE )
		M->VS3_CODITE := space(TamSX3("VS3_CODITE")[1])
		oGetPecas:aCols[oGetPecas:nAt,fg_posvar("VS3_CODITE","aHeaderP")] := space(TamSX3("VS3_CODITE")[1])
		M->VS3_DESITE := space(TamSX3("VS3_DESITE")[1])
		oGetPecas:aCols[oGetPecas:nAt,fg_posvar("VS3_DESITE","aHeaderP")] := space(TamSX3("VS3_DESITE")[1])
		if fg_posvar("VS3_KEYALT","aHeaderP") > 0
		...
		// TODO: Zera o resto da linha pois apenas o grupo está preenchido
		if !lPediVenda
			M->VS3_QTDEST := 0
			oGetPecas:aCols[oGetPecas:nAt,fg_posvar("VS3_QTDEST","aHeaderP")] := 0
		endif
	else
		// TODO: Alteração do grupo encontrou outra correspondência. Faz algumas verificações e apenas altera os campos necessários
		lGrAchou := .t.
		oGetPecas:aCols[oGetPecas:nAt,FG_POSVAR("VS3_DESITE","aHeaderP")] := M->VS3_DESITE := SB1->B1_DESC
		If !lRecalc
			oGetPecas:aCols[oGetPecas:nAt,FG_POSVAR("VS3_LOCAL","aHeaderP")]  := M->VS3_LOCAL  := OX0010105_ArmazemOrigem()
		EndIf
	endif
EndIf
...
// TODO: retirar isso quando possível
DbSelectArea("SB1")
DbSetOrder(7)
MsSeek( xFilial("SB1") + M->VS3_GRUITE + M->VS3_CODITE )

If ReadVar() == "M->VS3_CODITE" .or. lGrAchou
// ##############################################################################
// Verifica se estourou tamanho do Acols de Peças para de acordo com MV_NUMITEN #
// ##############################################################################
	if !FS_QTDLIN("1")
	...
	oGetPecas:aCols[oGetPecas:nAt,FG_POSVAR("VS3_FORMUL","aHeaderP")] := M->VS3_FORMUL
	...
	Else
		...
		If !lRecalc
			oGetPecas:aCols[oGetPecas:nAt,FG_POSVAR("VS3_LOCAL","aHeaderP")]  := M->VS3_LOCAL  := OX0010105_ArmazemOrigem()
		EndIf
		oGetPecas:aCols[oGetPecas:nAt,FG_POSVAR("VS3_CENCUS","aHeaderP")] := M->VS3_CENCUS := SB1->B1_CC
		oGetPecas:aCols[oGetPecas:nAt,FG_POSVAR("VS3_CONTA","aHeaderP")]  := M->VS3_CONTA  := SB1->B1_CONTA
		oGetPecas:aCols[oGetPecas:nAt,FG_POSVAR("VS3_ITEMCT","aHeaderP")] := M->VS3_ITEMCT := SB1->B1_ITEMCC
		oGetPecas:aCols[oGetPecas:nAt,FG_POSVAR("VS3_CLVL","aHeaderP")]   := M->VS3_CLVL   := SB1->B1_CLVL
	endif
	...
	if FG_POSVAR("VS3_LOCPRO","aHeaderP") > 0
		M->VS3_LOCPRO := OX0010211_Retorna_LOCALI2( M->VS3_GRUITE , M->VS3_CODITE )
		oGetPecas:aCols[oGetPecas:nAt,FG_POSVAR("VS3_LOCPRO","aHeaderP")] := M->VS3_LOCPRO
	Endif

	// #################################
	// # Verifica Substituicao do Item #
	// #################################
	If !FM_PILHA("OX001RECALC")
		if !lOX001Auto
			...
			if Len(aItensKit) > 0
	            ...
	            If nValKit > 0
					for nCntFor := 1 to Len(aItensKit)
						nValVdK += (aItensKit[nCntFor,5]*(aItensKit[nCntFor,4]/aItensKit[nCntFor,8]))
					Next
					nPDsKit := (1 - (nValKit / nValVdK )) * 100
        	    Endif
	       		//
				OX001RefNF()
				//
				for nCntFor := 1 to Len(aItensKit)
					...
					for nCntFor2 := 1 to Len(oGetPecas:aCols)
						...
						if !oGetPecas:aCols[nCntFor2,Len(oGetPecas:aCols[nCntFor2])]
							...
							if aItensKit[nCntFor,1] == oGetPecas:aCols[nCntFor2,FG_POSVAR("VS3_GRUITE","aHeaderP")] .and.;
								aItensKit[nCntFor,2] == oGetPecas:aCols[nCntFor2,FG_POSVAR("VS3_CODITE","aHeaderP")]
								...
								next
								// atualiza quantidade
								M->VS3_QTDITE := aItensKit[nCntFor,4] + oGetPecas:aCols[nCntFor2,FG_POSVAR("VS3_QTDITE","aHeaderP")]
					...
					DBSelectArea("SB1")
					DBSetOrder(7)
					MsSeek(xFilial("SB1")+aItensKit[nCntFor,1]+aItensKit[nCntFor,2])
					DBSetOrder(1)
					//
					// Verifica se existe o Tipo de Operação no Vetor de Retorno do KIT //
					cTipOperac := ""
					If len(aItensKit[nCntFor]) > 8
					...
					Else
						aAdd(aVetCmp,{"VS3_PERDES",0,M->VS3_PERDES} )
					Endif
					RegToMemory("VS3",.t.)
...
//					Next
				...
				if lPrimLinha .and. (Len(oGetPecas:aCols) > 0 .and. Empty(oGetPecas:aCols[1,FG_POSVAR("VS3_CODITE","aHeaderP")]))
					aSize(oGetPecas:aCols,Len(oGetPecas:aCols)-1)
				endif
				for nCntFor := 1 to Len(aItensKit)
					for nCntFor2 := 1 to Len(oGetPecas:aCols)
						...
						if !oGetPecas:aCols[nCntFor2,Len(oGetPecas:aCols[nCntFor2])]
							...
							if aItensKit[nCntFor,1] == oGetPecas:aCols[nCntFor2,FG_POSVAR("VS3_GRUITE","aHeaderP")] .and.;
								aItensKit[nCntFor,2] == oGetPecas:aCols[nCntFor2,FG_POSVAR("VS3_CODITE","aHeaderP")]
								oGetPecas:aCols[nCntFor2,FG_POSVAR("VS3_PECKIT","aHeaderP")] := aItensKit[nCntFor,10]
				...
				if !lOX001Auto .AND. lExibeKit
					...
					if DBSeek(xFilial("VE8")+M->VS3_GRUITE+M->VS3_CODITE)
						while !eof() .and. xFilial("VE8")+M->VS3_GRUITE+M->VS3_CODITE == VE8->VE8_FILIAL+VE8->VE8_GRUITE+VE8->VE8_CODITE
							for nCntFor := 1 to len(oGetPecas:aCols)
								If !oGetPecas:aCols[nCntFor,len(oGetPecas:aCols[nCntFor])]
									if ALLTRIM(VE8->VE8_GRUKIT) + ALLTRIM(VE8->VE8_CODKIT) == AllTrim(oGetPecas:aCols[nCntFor,FG_POSVAR("VS3_GRUITE","aHeaderP")]);
										+ AllTrim(oGetPecas:aCols[nCntFor,FG_POSVAR("VS3_CODITE","aHeaderP")])
	...
	// Fim do tratamento do kit
	oGetPecas:aCols[oGetPecas:nAt,FG_POSVAR("VS3_GRUITE","aHeaderP")] := M->VS3_GRUITE
	oGetPecas:aCols[oGetPecas:nAt,FG_POSVAR("VS3_CODITE","aHeaderP")] := M->VS3_CODITE
	If Empty(oGetPecas:aCols[oGetPecas:nAt,FG_POSVAR("VS3_PECKIT","aHeaderP")])
		oGetPecas:aCols[oGetPecas:nAt,FG_POSVAR("VS3_PECKIT","aHeaderP")] := "0"
	EndIf
	// Nesse momento a peca ja foi escolhida. Assim vamos setar o fiscal e preencher os demais...
	// ...campos antes de continuar as verificacoes.
	DBSelectArea("SB1")
	DBSetOrder(7)
	MsSeek(xFilial("SB1")+M->VS3_GRUITE+M->VS3_CODITE)
	//
...
If ReadVar() == "M->VS3_QTDITE" .OR. ReadVar() == "M->VS3_QTD2UM" .OR. ReadVar() == "M->VS3_CODTES" .OR. ReadVar() == "M->VS3_OPER" // tes e oper aqui é por conta do calculo fiscal do Tipo de Frete
	...
	If ReadVar() == "M->VS3_QTD2UM"           //FNC 15783 - boby - 29/07/10 - 2a UNIDADE MEDIDA
		//FNC 18940 -Boby - 20/08 - Alterado o calculo que estava invertido no exemplo do help da microsiga
	...
	if SF4->F4_ESTOQUE == "S" .and. cVS1Status != "5"
		if !lOX001Auto
			...
			if lAprMsg .and. M->VS3_QTDITE > 0 // o M->VS3_QTDITE > 0 vai funcionar para o nível de atendimento, mostrando mensagem somente depois de mostrado o saldo
				if M->VS3_QTDITE > nSdoPecas
					if !lAgrMsgSld
						MsgInfo( STR0041 +": "+alltrim(SB1->B1_DESC)+" "+ STR0042 ;
						+": "+str(nSdoPecas,6)+" "+ STR0182, STR0025 )
					else
				...
				elseif M->VS3_QTDITE > (nSdoPecas-nEstSeg)
					MsgInfo( STR0041+": "+SB1->B1_GRUPO + " " + alltrim(SB1->B1_CODITE)+ ": " + alltrim(SB1->B1_DESC)+" "+ STR0043;
					+": "+str(nSdoPecas-nEstSeg,6)+" "+STR0044 , STR0025 )				  	
				endif
			endif
			if At("T",GetNewPar("MV_FASEORC","0FX")) > 0 .and. M->VS3_QTDITE > nSdoPecas .AND. M->VS1_TIPORC != "2" // removi a checagem de nivel de atendimento devido ao novo tratamento
				if MsgYesNo(STR0243,STR0025)
			   		aPedTra2 := OXA020LBOX(xFilial("VS1"),SB1->B1_GRUPO,SB1->B1_CODITE, M->VS3_QTDITE - (nSdoPecas-nEstSeg))
					if ! Empty(aPedTra2)
						...
						oGetPecas:aCols[oGetPecas:nAt,FG_POSVAR("VS3_QTDTRA","aHeaderP")] := nQtdPTra
	...
	Endif
	MaFisRef("IT_QUANT","VS300",M->VS3_QTDITE)
	OX001FisPec()
	OX001PREPEC()
EndIf

If ReadVar() $ "M->VS3_VALPEC,M->VS3_QTDITE,M->VS3_QTD2UM,M->VS3_PERDES,M->VS3_VALDES"
	lRepPerDes := .t.
	While .t.
		if VAI->(FieldPos("VAI_ALTVPC")) > 0 .and. ReadVar() == "M->VS3_VALPEC"
			DBSelectArea("VAI")
			DBSetOrder(6)
			DBSeek(xFilial("VAI") + M->VS1_CODVEN)
			if VAI->VAI_ALTVPC == "0" .or. Empty(VAI->VAI_ALTVPC)
				return .f.
			elseif VAI->VAI_ALTVPC == "2"
				if M->VS3_VALPEC < oGetPecas:aCols[oGetPecas:nAt,FG_POSVAR("VS3_VALPEC","aHeaderP")]
					return .f.
				endif
			elseif VAI->VAI_ALTVPC == "3"
				if M->VS3_VALPEC > oGetPecas:aCols[oGetPecas:nAt,FG_POSVAR("VS3_VALPEC","aHeaderP")]
					return .f.
				endif
			endif
		...
		SB1->(DBSetOrder(7))
		SB1->(MsSeek(xFilial("SB1")+M->VS3_GRUITE+M->VS3_CODITE))
		if M->VS3_QTDITE == 0 .or. M->VS3_VALPEC == 0
			If !lLibPV
		...
		//
		If GetNewPar("MV_MIL0026","S") == "S"
			nValItem := FtDescItem( @M->VS3_VALPEC,;			//ExpN1: Preco de lista aplicado o desconto de cabecalho
		 	@M->VS3_VALPEC,; 			//ExpN2: Preco de Venda
			M->VS3_QTDITE,;				//ExpN3: Quantidade vendida
			@M->VS3_VALTOT,;			//ExpN4: Valor Total (do item)
			iif(ReadVar()=="M->VS3_VALDES",M->VS3_PERDES,@M->VS3_PERDES),;           	//ExpN5: Percentual de desconto
			@M->VS3_VALDES,;			//ExpN6: Valor do desconto
			@M->VS3_VALDES,;			//ExpN7: Valor do desconto original
			iif(ReadVar()=="M->VS3_VALDES",2,1))	//ExpN8: Tipo de Desconto (1 % OU 2 R$)
			//
			If ReadVar()=="M->VS3_VALDES"
		...
		Else
			...
			nValItem := M->VS3_VALTOT / M->VS3_QTDITE
		Endif

		oGetPecas:aCols[oGetPecas:nAt,FG_POSVAR("VS3_PERDES","aHeaderP")] := M->VS3_PERDES
		oGetPecas:aCols[oGetPecas:nAt,FG_POSVAR("VS3_VALPEC","aHeaderP")] := M->VS3_VALPEC
		oGetPecas:aCols[oGetPecas:nAt,FG_POSVAR("VS3_VALDES","aHeaderP")] := M->VS3_VALDES
		oGetPecas:aCols[oGetPecas:nAt,FG_POSVAR("VS3_VALTOT","aHeaderP")] := M->VS3_VALTOT
		...
		IF VS3->(FieldPos("VS3_VALLIQ")) <> 0
			M->VS3_VALLIQ := oGetPecas:aCols[oGetPecas:nAt,FG_POSVAR("VS3_VALLIQ","aHeaderP")] := nValItem
		ENDIF
		SB1->(DBSetOrder(7))
		SB1->(MsSeek(xFilial("SB1")+M->VS3_GRUITE+M->VS3_CODITE))
		OX001PREPEC()
...
If ReadVar() $ "M->VS3_FORMUL,M->VS3_OPER,M->VS3_CODTES"
	if ReadVar() == "M->VS3_FORMUL"
	...
	OX001PREPEC()
	If !FM_PILHA("OX001RECALC")              // Manoel 02/08/2016 - Mantomac
		if VS3->(FieldPos("VS3_SITTRI")) > 0 // .and. Empty(M->VS3_SITTRI)
			DbSelectArea("SB1")
			DbSetOrder(7)
			MsSeek( xFilial("SB1") + M->VS3_GRUITE + M->VS3_CODITE )
			cBx_ORIGEM := FM_PRODSBZ(SB1->B1_COD,"SB1->B1_ORIGEM")
...
If ReadVar() == "M->VS3_INSTAL"
	if M->VS1_TIPORC == "2" // Orcamento de Oficina
		If M->VS3_INSTAL == "1"
			If oGetPecas:aCols[oGetPecas:nAt,fg_posvar("VS3_QTDITE","aHeaderP")] <= 0
				MsgInfo(STR0051,STR0025)
				Return .f.
			EndIf
			If ( Empty(oGetPecas:aCols[oGetPecas:nAt,FG_POSVAR("VS3_DESINC","aHeaderP")] ) .or. Alltrim(oGetPecas:aCols[oGetPecas:nAt,FG_POSVAR("VS3_DESINC","aHeaderP")]) == cIncDefault ) // Instalar Peça? 1-Sim
				DbSelectArea("SB1")
				DbSetOrder(7)
				MsSeek( xFilial("SB1") + oGetPecas:aCols[oGetPecas:nAt,FG_POSVAR("VS3_GRUITE","aHeaderP")] + oGetPecas:aCols[oGetPecas:nAt,FG_POSVAR("VS3_CODITE","aHeaderP")] )
				DbSetOrder(1)
		...
		ElseIf M->VS3_INSTAL == "0" // Instalar Peça? 0-Nao
			If !Empty(oGetPecas:aCols[oGetPecas:nAt,FG_POSVAR("VS3_DESINC","aHeaderP")]) .and. Alltrim(oGetPecas:aCols[oGetPecas:nAt,FG_POSVAR("VS3_DESINC","aHeaderP")]) <> cIncDefault
				If MsgYesNo(STR0318,STR0025) // Deseja excluir o Servico relacionado a instalacao da Peca? / Atencao
					...
					nPos := aScan(oGetServ:aCols, {|x| x[FG_POSVAR("VS4_SEQINC","aHeaderS")] == oGetPecas:aCols[oGetPecas:nAt,FG_POSVAR("VS3_SEQINC","aHeaderP")] })
					//
					If nPos > 0
					...
					nPos := aScan(oGetInconv:aCols, {|x| x[FG_POSVAR("VST_SEQINC","aHeaderI")] == oGetPecas:aCols[oGetPecas:nAt,FG_POSVAR("VS3_SEQINC","aHeaderP")] })
					If nPos > 0
					...
					If nPos > 0
						oGetPecas:aCols[oGetPecas:nAt,FG_POSVAR("VS3_SEQINC","aHeaderP")] := oGetInconv:aCols[nPos,FG_POSVAR("VST_SEQINC","aHeaderI")]
						oGetPecas:aCols[oGetPecas:nAt,FG_POSVAR("VS3_DESINC","aHeaderP")] := oGetInconv:aCols[nPos,FG_POSVAR("VST_DESINC","aHeaderI")]
						oGetPecas:oBrowse:Refresh()
					EndIf
				Else
					Return .f.
...
if M->VS1_TIPORC == "2"
	...
	If !FM_PILHA("OX001RECALC")
		If ( VS3->(FieldPos("VS3_DEPINT")) > 0 .and. ReadVar() == "M->VS3_DEPINT" ) .or.( VS3->(FieldPos("VS3_DEPGAR")) > 0 .and. ReadVar() == "M->VS3_DEPGAR" )
			...
			if VOI->VOI_SITTPO == "3"
				...
				If ReadVar() == "M->VS3_DEPINT"
					For nCntFor := 1 to len(oGetPecas:aCols)
						If !oGetPecas:aCols[nCntFor,len(oGetPecas:aCols[nCntFor])]// .and. oGetPecas:nAt <> nCntFor
							if AllTrim(oGetPecas:aCols[nCntFor,FG_POSVAR("VS3_DEPINT","aHeaderP")]) <> M->VS3_DEPINT .and. ;
								!Empty(AllTrim(oGetPecas:aCols[nCntFor,FG_POSVAR("VS3_DEPINT","aHeaderP")]))
				...
				If ReadVar() == "M->VS3_DEPGAR"
					For nCntFor := 1 to len(oGetPecas:aCols)
						If !oGetPecas:aCols[nCntFor,len(oGetPecas:aCols[nCntFor])] .and. oGetPecas:nAt <> nCntFor
							if AllTrim(oGetPecas:aCols[nCntFor,FG_POSVAR("VS3_DEPGAR","aHeaderP")]) <> M->VS3_DEPGAR .and. ;
								!Empty(AllTrim(oGetPecas:aCols[nCntFor,FG_POSVAR("VS3_DEPGAR","aHeaderP")]))
								if MsgYEsNo(STR0251+"'"+M->VS3_DEPINT+"'"+CHR(13)+CHR(10)+CHR(13)+CHR(10)+STR0250)
								   OF001DEPGAR()
								   Return .t.
								Else
...
If !FM_PILHA("OX001RECALC")
	...
	if !Empty(M->VS3_CODTES) .and. !Empty(M->VS3_CODITE) .and. !Empty(M->VS3_VALPEC)
		SF4->(dbseek(xFilial("SF4")+M->VS3_CODTES))
		If GetNewPar("MV_LBVACB","S") == "N" .and. SF4->F4_OPEMOV == "05"  //Libera Venda Abaixo do Custo (Balcao)
			DbSelectArea("SB1")
			DbSetOrder(7)
			if MsSeek( xFilial("SB1") + M->VS3_GRUITE + M->VS3_CODITE )
				//
				...
				While !eof() .and. xFilial("SB2")+SB1->B1_COD == SB2->B2_FILIAL+SB2->B2_COD
					nSdoPecB2 += OX001SLDPC(xFilial("SB2")+SB1->B1_COD+M->VS3_LOCAL) //SB2->B2_QATU
					dbSkip()
				Enddo
				DbSeek(xFilial("SB2")+SB1->B1_COD+Iif(!Empty(M->VS3_LOCAL),M->VS3_LOCAL,OX0010105_ArmazemOrigem()))
				If Round(M->VS3_VALPEC,2) < Round(SB2->B2_CM1,2) .and. nSdoPecB2 > 0 .AND. !Empty(M->VS3_FORMUL)
					if lMens
						MsgStop(STR0131)
					endif
				Endif
	...
	If ReadVar() $ "M->VS3_VALPEC,M->VS3_QTDITE,M->VS3_QTD2UM,M->VS3_PERDES,M->VS3_VALDES,M->VS3_FORMUL,M->VS3_OPER,M->VS3_CODTES,M->VS3_SITTRI"
		OX001PecFis()
		OX001RecFis("IT_TES",M->VS3_CODTES) // Manoel 14/06/2016
		...
		Endif
		If FG_POSVAR("VS3_VALIRR","aHeaderP") > 0
			oGetPecas:aCols[oGetPecas:nAt,FG_POSVAR("VS3_BASIRR","aHeaderP")] := M->VS3_BASIRR
			oGetPecas:aCols[oGetPecas:nAt,FG_POSVAR("VS3_ALIIRR","aHeaderP")] := M->VS3_ALIIRR
			oGetPecas:aCols[oGetPecas:nAt,FG_POSVAR("VS3_VALIRR","aHeaderP")] := M->VS3_VALIRR
		EndIf
		If FG_POSVAR("VS3_VALCSL","aHeaderP") > 0
			oGetPecas:aCols[oGetPecas:nAt,FG_POSVAR("VS3_BASCSL","aHeaderP")] := M->VS3_BASCSL
			oGetPecas:aCols[oGetPecas:nAt,FG_POSVAR("VS3_ALICSL","aHeaderP")] := M->VS3_ALICSL
			oGetPecas:aCols[oGetPecas:nAt,FG_POSVAR("VS3_VALCSL","aHeaderP")] := M->VS3_VALCSL
		EndIf
		OX001FisPec()
	Endif
...
Endif
// ###############################################################################################
if ReadVar() == "M->VS3_GRUITE" .or. lGrAchou
	DbSelectArea("SB1")
	DbSetOrder(7)
	MsSeek( xFilial("SB1") + M->VS3_GRUITE + M->VS3_CODITE )
endif
...
DBSelectArea("VOI")
DBSetOrder(1)
DBSeek(xFilial("VOI")+Iif(FieldPos("VS1_TIPTSV")>0 , M->VS1_TIPTSV , M->VS1_TIPTEM))
if VO7->(FieldPos("VO7_APLICA")) > 0 .and. VO7->(FieldPos("VO7_CONTRO")) > 0 .and. VO7->(FieldPos("VO7_SEQSER")) > 0 .and. VO7->(RecCount()) > 0
...
If ReadVar()=="M->VS4_CODSER" .or. ReadVar()=="M->VS4_TIPSER"
	...
	if lKilometragem
		...
		if VO7->(FieldPos("VO7_APLICA")) > 0 .and. VO7->(FieldPos("VO7_CONTRO")) > 0 .and. VO7->(FieldPos("VO7_SEQSER")) > 0 .and. VO7->(RecCount()) > 0 .and. M->VS1_CODMAR == FG_MARCA("CHEVROLET",,.f.)
			if Empty(M->VS4_SEQSER)
				cQryAl001 := GetNextAlias()
	...
	MaFisRef("IT_TES","VS300",VOI->VOI_CODTES)
...
If ReadVar()=="M->VS4_VALVEN"
	if VOK->VOK_INCMOB == "2" // Servico de Terceiro
		oGetServ:aCols[oGetServ:nAt,FG_POSVAR("VS4_VALTOT","aHeaderS")] := M->VS4_VALTOT := M->VS4_VALVEN // Valor de Venda
	EndIf
EndIf
...
EndIf
//
If cPaisLoc == "BRA" .and. VS4->(FieldPos("VS4_VALISS")) > 0 // Carregar Impostos por Serviço
...
//
if M->VS1_TIPORC == "2"
	If ( VS4->(FieldPos("VS4_DEPINT")) > 0 .and. ReadVar() == "M->VS4_DEPINT" ) .or.( VS4->(FieldPos("VS4_DEPGAR")) > 0 .and. ReadVar() == "M->VS4_DEPGAR" )
		...
		if VOI->VOI_SITTPO == "3"
			...
			If ReadVar() == "M->VS4_DEPINT"
				For nCntFor := 1 to len(oGetPecas:aCols)
					If !oGetPecas:aCols[nCntFor,len(oGetPecas:aCols[nCntFor])]// .and. oGetPecas:nAt <> nCntFor
						if AllTrim(oGetPecas:aCols[nCntFor,FG_POSVAR("VS3_DEPINT","aHeaderP")]) <> M->VS4_DEPINT .and. ;
							!Empty(AllTrim(oGetPecas:aCols[nCntFor,FG_POSVAR("VS3_DEPINT","aHeaderP")]))
			...
			If ReadVar() == "M->VS4_DEPGAR"
				For nCntFor := 1 to len(oGetPecas:aCols)
					If !oGetPecas:aCols[nCntFor,len(oGetPecas:aCols[nCntFor])] .and. oGetPecas:nAt <> nCntFor
						if AllTrim(oGetPecas:aCols[nCntFor,FG_POSVAR("VS3_DEPGAR","aHeaderP")]) <> M->VS4_DEPGAR .and. ;
							!Empty(AllTrim(oGetPecas:aCols[nCntFor,FG_POSVAR("VS3_DEPGAR","aHeaderP")]))
...
If GetNewPar("MV_LBVACB","S") == "N" .and. SF4->F4_OPEMOV == "05"  //VENDA  //Libera Venda Abaixo do Custo (Balcao)
	DbSelectArea("SB1")
	DbSetOrder(7)
	if DbSeek( xFilial("SB1") + cGrupo + cPeca )
		//
...
for nCntFor := 1 to Len(oGetPecas:aCols)
	if !oGetPecas:aCols[nCntFor,len(oGetPecas:aCols[nCntFor])] .and. !Empty(oGetPecas:aCols[nCntFor,nVS3CODITE])
		nValSeq ++ //incrementa +1 par adicionar no VS3_SEQUEN
		oGetPecas:aCols[nCntFor,nVS3SEQUEN] := STRZERO(nValSeq,TamSX3("VS3_SEQUEN")[1])
...
For nCntFor:=1 to Len(aHeaderP)
	If X3Obrigat(aHeaderP[nCntFor,2])  .and. (Empty(oGetPecas:aCols[oGetPecas:nAt,nCntFor]))
		If lAltPedVda
...
// ############################################################
If !Empty(oGetPecas:aCols[oGetPecas:nAt,fg_posvar("VS3_GRUITE","aHeaderP")])  .and. oGetPecas:aCols[oGetPecas:nAt,fg_posvar("VS3_QTDITE","aHeaderP")] <= 0 .and. !lPediVenda
	if !lOX001Auto
		MsgInfo(STR0051,STR0025)
	endif
	Return .f.
...
// ############################################################
If M->VS1_TIPORC == "2" .and. oGetPecas:aCols[oGetPecas:nAt,fg_posvar("VS3_VALPEC","aHeaderP")] <= 0
	if !lOX001Auto
...
if lInconveniente
	...
	// Verifica duplicidade com grupo / codigo do item e INCONVENIENTE
	// ################################################################
	if lVldDPec .and. OX001PDUPL(oGetPecas:nAt,oGetPecas:aCols[oGetPecas:nAt,fg_posvar("VS3_GRUITE","aHeaderP")] ,;
		oGetPecas:aCols[oGetPecas:nAt,fg_posvar("VS3_CODITE","aHeaderP")] ,;
		oGetPecas:aCols[oGetPecas:nAt,fg_posvar("VS3_GRUINC","aHeaderP")] ,;
		oGetPecas:aCols[oGetPecas:nAt,fg_posvar("VS3_CODINC","aHeaderP")] ,;
		oGetPecas:aCols[oGetPecas:nAt,fg_posvar("VS3_DESINC","aHeaderP")] ,;
		oGetPecas:aCols[oGetPecas:nAt,fg_posvar("VS3_SEQINC","aHeaderP")] ,;
		oGetPecas:aCols[oGetPecas:nAt,fg_posvar("VS3_NUMLOT","aHeaderP")] ,;
		oGetPecas:aCols[oGetPecas:nAt,fg_posvar("VS3_LOTECT","aHeaderP")] ,;
		@nLinDup )
		if !lOX001Auto  .and. !lDupOrc
...
Endif
// ############################################################
// # altera valor do campo VS4_SEQUEN                         #
// ############################################################
for nCntFor := 1 to Len(oGetServ:aCols)
	if !oGetServ:aCols[nCntFor,len(oGetServ:aCols[nCntFor])] .and. !Empty(oGetServ:aCols[nCntFor,FG_POSVAR("VS4_CODSER","aHeaderS")])
		nValSeq ++ //incrementa +1 par adicionar no VS3_SEQUEN
		oGetServ:aCols[nCntFor,FG_POSVAR("VS4_SEQUEN","aHeaderS")] := STRZERO(nValSeq,TamSX3("VS4_SEQUEN")[1])
		//VS3_SEQUEN := STRZERO(nVS3Seq,TamSX3("VS3_SEQUEN")[1])
	else
		oGetServ:aCols[nCntFor,FG_POSVAR("VS4_SEQUEN","aHeaderS")] := " "
	endif
next
// ############################################################
// # Verifica campos obrigatorios                             #
...
lVS3RESERV := ( VS3->(FieldPos("VS3_RESERV")) > 0 )
...
for nCntFor := 1 to Len(oGetPecas:aCols)
	if !oGetPecas:aCols[nCntFor,len(oGetPecas:aCols[nCntFor])] .and. !Empty(oGetPecas:aCols[nCntFor,nVS3CODITE])
		SB1->(DbSetOrder(7))
		SB1->(MsSeek(xFilial("SB1")+oGetPecas:aCols[nCntFor,nVS3GRUITE]+oGetPecas:aCols[nCntFor,nVS3CODITE]))
		If !MaAvalPerm( 3 , { oGetPecas:aCols[nCntFor,nVS3LOCAL] , SB1->B1_COD } )
			SB1->(DbSetOrder(1))
		...
		If lPediVenda .and. lFaturaPVP .and. lVS3RESERV .and. SF4->F4_ESTOQUE == "S"		
			if oGetPecas:aCols[nCntFor,nVS3RESERV] != "1"
				nQtdEst := OX001SLDPC(xFilial("SB2")+SB1->B1_COD+Iif(!Empty(oGetPecas:aCols[nCntFor,nVS3LOCAL]),oGetPecas:aCols[nCntFor,nVS3LOCAL],OX0010105_ArmazemOrigem()))
				If oGetPecas:aCols[nCntFor,nVS3QTDITE] > nQtdEst
					aadd(aPecSemEst,{SB1->B1_GRUPO,SB1->B1_CODITE,oGetPecas:aCols[nCntFor,nVS3QTDITE],nQtdEst})
				Endif
			 Endif
	    Endif

		// Verificação da Fórmula informada
		If !Empty(oGetPecas:aCols[nCntFor,nVS3FORMUL])
			If OFP8600016 .And. !OFP8600016_VerificacaoFormula(oGetPecas:aCols[nCntFor,nVS3FORMUL])
				Return .f. // A mensagem já é exibida dentro da função
			EndIf
		EndIf
	Endif
Next
If lPediVenda .and. lFaturaPVP .and. lVS3RESERV .and. Len(aPecSemEst) > 0
	aIntCab := {}
...
if M->VS1_TIPORC=="2"
	...
	if lInconveniente
		for nCntFor := 1 to Len(oGetInconv:aCols)
			if !oGetInconv:aCols[nCntFor,Len(oGetInconv:aCols[nCntFor])] .and. !Empty(oGetInconv:aCols[nCntFor,FG_POSVAR("VST_DESINC","aHeaderI")])
				lSemInconv := .f.
				exit
			endif
		Next nCntFor
...
//if lSemPeca .and. lSemSrv .and. lSemInconv .and. GetNewPar("MV_MIL0011","0") == "0" /*nivel de atendimento deixa gravar sem peças*/
if lSemPeca .and. lSemSrv .and. lSemInconv // 19/04/2018 - chamado 007773 - não deixa mais passar em branco - orientação do Otávio
	if !lOX001Auto .and. !lLibPV
...
// # Verifica Quantidade                                      #
// ############################################################
 // 19/04/2018 - chamado 007773 - não deixa mais passar em branco - orientação do Otávio
 for nCntFor := 1 to len(oGetPecas:aCols)

	If LinhaSemQuantidade(nOpc,nCntFor)
		MsgInfo(STR0051,STR0025)
		Return(.f.)
	Endif
...
// ############################################################
// # Valida a linha atual antes da gravacao                   #
// ############################################################
//if oFoldX001:nOption == 2
if !lOX001Auto
	if oFoldX001:nOption == nFolderS
		lRet := OX001LINSOK()
	endif
...
if VS3->(FieldPos("VS3_RESERV")) > 0
	for nCntFor := 1 to Len(oGetPecas:aCols)
		if oGetPecas:aCols[nCntFor,len(oGetPecas:aCols[nCntFor])]
			if oGetPecas:aCols[nCntFor,FG_POSVAR("VS3_RESERV","aHeaderP")] == "1" .and. oGetPecas:aCols[nCntFor,FG_POSVAR("VS3_QTDITE","aHeaderP")] > 0
				DBSelectArea("VS3")
				DBSetOrder(2)
				if DBSeek(xFilial("VS3") + M->VS1_NUMORC + oGetPecas:aCols[nCntFor,FG_POSVAR("VS3_GRUITE","aHeaderP")] + oGetPecas:aCols[nCntFor,FG_POSVAR("VS3_CODITE","aHeaderP")] )
		...
		if lBo
			oGetPecas:aCols[nCntFor,FG_POSVAR("VS3_QTDRES","aHeaderP")] := FM_SQL(" select VS3_QTDRES from "+retsqlName('VS3')+" WHERE VS3_FILIAL = '"+xFilial('VS3')+"' AND VS3_NUMORC = '"+cNumOrcPed+"' AND VS3_GRUITE = '"+oGetPecas:aCols[nCntFor,FG_POSVAR("VS3_GRUITE","aHeaderP")]+"' AND VS3_CODITE = '"+oGetPecas:aCols[nCntFor,FG_POSVAR("VS3_CODITE","aHeaderP")]+"' AND VS3_SEQUEN = '"+oGetPecas:aCols[nCntFor,FG_POSVAR("VS3_SEQUEN","aHeaderP")]+"' AND D_E_L_E_T_=' '")
		EndIf
	next
endif
if Len(aResDel) > 0
...
if lFaturaPVP .and. !lImp
	...
	for nCntFor := 1 to Len(oGetPecas:aCols)
		if !oGetPecas:aCols[nCntFor,len(oGetPecas:aCols[nCntFor])] .and. !Empty(oGetPecas:aCols[nCntFor,FG_POSVAR("VS3_CODITE","aHeaderP")])
			...
			DBSelectArea("VS3")
			DBSetOrder(2)
			if DBSeek(xFilial("VS3") + cNumOrcPed + oGetPecas:aCols[nCntFor,FG_POSVAR("VS3_GRUITE","aHeaderP")] + oGetPecas:aCols[nCntFor,FG_POSVAR("VS3_CODITE","aHeaderP")] )
				reclock("VS3",.f.)
				VS3->VS3_QTDPED := VS3->VS3_QTDPED - oGetPecas:aCols[nCntFor,FG_POSVAR("VS3_QTDITE","aHeaderP")]
				VS3->VS3_QTDITE := oGetPecas:aCols[nCntFor,FG_POSVAR("VS3_QTDITE","aHeaderP")]
				VS3->VS3_VALTOT := (VS3->VS3_VALPEC*VS3->VS3_QTDITE) - VS3->VS3_VALDES
				msunlock()
			endif
		endif
	next
elseif lCancelPVP .and. lCallPed
	if Len(oGetPecas:aCols) > 1
		for nCntFor := 1 to Len(oGetPecas:aCols)
			if oGetPecas:aCols[nCntFor,len(oGetPecas:aCols[nCntFor])] .and. !Empty(oGetPecas:aCols[nCntFor,FG_POSVAR("VS3_CODITE","aHeaderP")])
				if Empty(oGetPecas:aCols[nCntFor,FG_POSVAR("VS3_MOTPED","aHeaderP")])
					lMotivo := .t.
					Exit
				Endif
			Endif
      ...
      if lMotivo
			...
			if cMotPed01 == ""
				for nCntFor := 1 to Len(oGetPecas:aCols)
					if oGetPecas:aCols[nCntFor,len(oGetPecas:aCols[nCntFor])] .and. oGetPecas:aCols[nCntFor,FG_POSVAR("VS3_RESERV","aHeaderP")] == "0"
						cSeq := FM_SQL(" select VS3_SEQUEN from "+retsqlName('VS3')+" WHERE VS3_FILIAL = '"+xFilial('VS3')+"' AND VS3_NUMORC = '"+VS1->VS1_NUMORC+"' AND VS3_GRUITE = '"+oGetPecas:aCols[nCntFor,FG_POSVAR("VS3_GRUITE","aHeaderP")]+"' AND VS3_CODITE = '"+oGetPecas:aCols[nCntFor,FG_POSVAR("VS3_CODITE","aHeaderP")]+"' AND  D_E_L_E_T_=' '")
					   If ascan(aResDel,cSeq) > 0
		...
		for nCntFor := 1 to Len(oGetPecas:aCols)
			if oGetPecas:aCols[nCntFor,len(oGetPecas:aCols[nCntFor])] .and. !Empty(oGetPecas:aCols[nCntFor,FG_POSVAR("VS3_CODITE","aHeaderP")])
				...
				if DBSeek(xFilial("VS3") + M->VS1_NUMORC + oGetPecas:aCols[nCntFor,FG_POSVAR("VS3_GRUITE","aHeaderP")] + oGetPecas:aCols[nCntFor,FG_POSVAR("VS3_CODITE","aHeaderP")] )
					...
					oGetPecas:aCols[nCntFor,FG_POSVAR("VS3_MOTPED","aHeaderP")] := cMotPed01
					msunlock()
				endif
			endif
		next
...
//		VS1->VS1_DIFAL  := MaFisRet(,"NF_DIFAL")
	...
	For nCntFor := 1 to Len(aVetCortS)
		...
		MsUnlock()
	Next
	//Deleta quando o item for informado novamente no orcamento.
	For nCntFor := 1 to Len(oGetPecas:aCols)
	...
	Next
	aVetCort := {}
	//Deleta quando o servico for informado novamente no orcamento.
	For nCntFor := 1 to Len(oGetServ:aCols)
		if oGetServ:aCols[nCntFor,Len(oGetServ:aCols[nCntFor])] == .f.
			dbSelectArea("VPM")
			dbSetOrder(1)
	...
	for nCntFor := 1 to Len(oGetPecas:aCols)
		...
		if (lNaOfi .AND. !Empty(cMot)) .OR. ( !Empty(cMot) .AND. !lDeleted ) .OR. ( !Empty(oGetPecas:aCols[nCntFor,FG_POSVAR("VS3_CODITE","aHeaderP")]) .AND. !lDeleted )
			...
			Else
				...
				//
			EndIf
			if lResOrc // .or. VS1->VS1_RESERV == "1"
		...
		EndIf
	next
	if (lResOrc .or. VS1->VS1_RESERV == "1") .and. len(aResSeq) > 0
...
===============================================================================
*/
Function OX001FAT(nOpc, lSoGravar, lMsgGrv)
// ############################################################
...
Local nRecVO1     := 0
Local nVS3SEQVEN  := FG_POSVAR("VS3_SEQVEN","aHeaderP")
//
...
if EXCLUI
	if lOX001Auto .or. MsgYesNo(STR0064,STR0025)
		...
		If VS1->VS1_TIPORC == "2" .AND. VO1->(FieldPos("VO1_GARMUT")) <> 0
...
if M->VS1_TIPORC == "1" .and. !lPediVenda
	If ( "C5_NATUREZ" $ Upper(SuperGetMv("MV_1DUPNAT",.F.,"")) ) .and. SC5->(FieldPos("C5_NATUREZ")) <> 0 .and. Empty(M->VS1_NATURE)
		if !lOX001Auto
...
if M->VS1_TIPORC == "2" .and. !lPediVenda
	...
	If !Empty(M->VS1_TIPTSV)
		...
		if VOI->VOI_CONVOX == "1" .or. ( VOI->(FieldPos("VOI_CONGSV")) > 0 .and. VOI->VOI_CONGSV == "1")
			DbSelectArea("VS4")
			DbSetOrder(1)
			DbSeek(xFilial("VS4")+VS1->VS1_NUMORC)
	...
	If !lOX001Auto
		...
		Else // OS ja existente - Orcamento Complementear
			...
			cNumOrd := cNroOS
			DbSelectArea("VS1")
			If !OX001VLABERT(cNumOrd,VS1->VS1_TIPTEM, VS1->VS1_CHAINT, VS1->VS1_CLIFAT, VS1->VS1_LOJA, cOrcAtu,If(VS1->(FieldPos("VS1_TIPTSV"))>0.and.!Empty(VS1->VS1_TIPTSV),VS1->VS1_TIPTSV,VS1->VS1_TIPTEM),VS1->VS1_CODBCO,VS1->VS1_FORPAG,VS1->VS1_FRANQU,IIF(VS1->(FieldPos("VS1_FRANQP")) <> 0, VS1->VS1_FRANQP , 0 ))
				return .f.
			EndIf
...
// ##########################################################################
	if lOX001Auto .or. Empty(cNroOS)
		If !OX001VERQMX(cNumOrd)
			...
			Return .f.
		EndIF
		cNumOrd := FM_ABREOSV( @aCodErro ,,M->VS1_CHAINT , M->VS1_KILOME , M->VS1_CLIFAT, M->VS1_LOJA , M->VS1_OBSMEM, IIf(Type("M->VS1_DATENT")<>"U",M->VS1_DATENT,Ctod("")),IIf(VS1->(FieldPos("VS1_HORTRI"))>0,M->VS1_HORTRI,0), .F. )
		if Empty(cNumOrd)
			if !lOX001Auto
	...
	Else
		...
		if VS1->VS1_FRANQU > 0 .or. (VS1->(FieldPos("VS1_FRANQP")) <> 0 .and. VO1->(FieldPos("VO1_FRANQP")) <> 0 .and. VS1->VS1_FRANQP > 0)
			VS1->(dbSetOrder(1))
			...
			if reclock("VO1",.f.)
				...
				IF VS1->(FieldPos("VS1_FRANQP")) <> 0 .and. VO1->(FieldPos("VO1_FRANQP")) <> 0 .and. VS1->VS1_FRANQP > 0
			...
			If oGetPecas:aCols[oGetPecas:nAt,nAuxPos] == "1" // Sim
				If !Empty(oGetPecas:aCols[oGetPecas:nAt,FG_POSVAR("VS3_DESINC","aHeaderP")]) .and. Alltrim(oGetPecas:aCols[oGetPecas:nAt,FG_POSVAR("VS3_DESINC","aHeaderP")]) <> cIncDefault
					If MsgYesNo(STR0318,STR0025) // Deseja excluir o Servico relacionado a instalacao da Peca? / Atencao
						...
						nPos := aScan(oGetInconv:aCols, {|x| Alltrim(x[FG_POSVAR("VST_DESINC","aHeaderI")]) == cIncDefault })
						If nPos > 0
							oGetPecas:aCols[oGetPecas:nAt,FG_POSVAR("VS3_SEQINC","aHeaderP")] := M->VS3_SEQINC := oGetInconv:aCols[nPos,FG_POSVAR("VST_SEQINC","aHeaderI")]
							oGetPecas:aCols[oGetPecas:nAt,FG_POSVAR("VS3_DESINC","aHeaderP")] := M->VS3_DESINC := oGetInconv:aCols[nPos,FG_POSVAR("VST_DESINC","aHeaderI")]
							oGetPecas:oBrowse:Refresh()
						EndIf
					Else
						Return .f.
...
If oPedido:isBO({; // é back order? se for pode cancelar dependendo do VAI
		...
		{ 'VS3_CODITE', oGetPecas:aCols[oGetPecas:nAt,fg_posvar("VS3_CODITE","aHeaderP")] },;
		{ 'VS3_GRUITE', oGetPecas:aCols[oGetPecas:nAt,fg_posvar("VS3_GRUITE","aHeaderP")] },;
	...
	if MsgYesNo( STR0308 /* "Tem certeza que deseja cancelar um item com pedido pendente? O pedido será perdido, essa ação não poderá ser desfeita." */ , STR0025 /* "Atenção" */)
		...
		BEGIN TRANSACTION // ação e log devem ser feitos
			...
			SB1->(dbSetOrder(7))
			SB1->(dbSeek( xFilial('SB1') + oGetPecas:aCols[oGetPecas:nAt,fg_posvar("VS3_GRUITE","aHeaderP")] + oGetPecas:aCols[oGetPecas:nAt,fg_posvar("VS3_CODITE","aHeaderP")] ))
			//
			if Empty(cMotCan)
...
//
if VS3->(FieldPos("VS3_QTDTRA")) > 0
	if oGetPecas:aCols[oGetPecas:nAt,fg_posvar("VS3_QTDTRA","aHeaderP")] > 0
		MsgStop(STR0224,STR0025)
		return .f.
	endif
endif
...
If oGetPecas:aCols[oGetPecas:nAt,Len(oGetPecas:aCols[oGetPecas:nAt])]
	...
	if lInconveniente
		...
		if lVldDPec .and. OX001PDUPL(oGetPecas:nAt,oGetPecas:aCols[oGetPecas:nAt,fg_posvar("VS3_GRUITE","aHeaderP")] ,;
			oGetPecas:aCols[oGetPecas:nAt,fg_posvar("VS3_CODITE","aHeaderP")] ,,,,,;
			oGetPecas:aCols[oGetPecas:nAt,fg_posvar("VS3_NUMLOT","aHeaderP")] ,;
			oGetPecas:aCols[oGetPecas:nAt,fg_posvar("VS3_LOTECT","aHeaderP")] , @nLinDup )
			oGetPecas:aCols[oGetPecas:nAt,Len(oGetPecas:aCols[oGetPecas:nAt])] := .t.
			MsgInfo(STR0097 + chr(13) + chr(13) + STR0146 + AllTrim(Str(nLinDup)) ,STR0025)
			return .f.
		endif
	...
	if lInconveniente
		...
		for nCntFor := 1 to Len(oGetInconv:aCols)
			if Alltrim(oGetInconv:aCols[nCntFor,FG_POSVAR("VST_SEQINC","aHeaderI")]) == Alltrim(oGetPecas:aCols[oGetPecas:nAt,FG_POSVAR("VS3_SEQINC","aHeaderP")]) .and. ;
				Alltrim(oGetInconv:aCols[nCntFor,FG_POSVAR("VST_GRUINC","aHeaderI")]) == Alltrim(oGetPecas:aCols[oGetPecas:nAt,FG_POSVAR("VS3_GRUINC","aHeaderP")]) .and. ;
				Alltrim(oGetInconv:aCols[nCntFor,FG_POSVAR("VST_CODINC","aHeaderI")]) == Alltrim(oGetPecas:aCols[oGetPecas:nAt,FG_POSVAR("VS3_CODINC","aHeaderP")]) .and. ;
				Alltrim(oGetInconv:aCols[nCntFor,FG_POSVAR("VST_DESINC","aHeaderI")]) == Alltrim(oGetPecas:aCols[oGetPecas:nAt,FG_POSVAR("VS3_DESINC","aHeaderP")]) .and. ;
...
Else
	cTipoKit := oGetPecas:aCols[oGetPecas:nAt,fg_posvar("VS3_PECKIT","aHeaderP")]
...
if (lPediVenda.OR.(M->VS1_TIPORC=="2" .AND. GetNewPar("MV_MIL0011", "0") == "1" )) .and. !lCancelPVP
	if !Empty(M->VS1_NUMORC) .and. !Empty(M->VS3_GRUITE) .and. !Empty(M->VS3_CODITE)
		...
		if DBSeek(xFilial("VS3") + M->VS1_NUMORC + M->VS3_GRUITE + M->VS3_CODITE)
			...
			endIf
			VS3->VS3_MOTPED := cMotPed01
			oGetPecas:aCols[oGetPecas:nAt,FG_POSVAR("VS3_MOTPED","aHeaderP")] := cMotPed01
	        if ! ( VS1->VS1_TIPORC=="2" .AND. GetNewPar("MV_MIL0011", "0") == "1" )
				VS3->VS3_QTDITE := 0
				oGetPecas:aCols[oGetPecas:nAt,FG_POSVAR("VS3_QTDITE","aHeaderP")] := 0
			EndIf
			If VS1->VS1_TIPORC <>"2"
				VS3->VS3_QTDPED := 0
...
for nCntFor := 1 to Len(oGetPecas:aCols)
	if !oGetPecas:aCols[nCntFor,len(oGetPecas:aCols[nCntFor])] .and. !Empty(oGetPecas:aCols[nCntFor,FG_POSVAR("VS3_CODITE","aHeaderP")])
		nValSeq ++ //incrementa +1 par adicionar no VS3_SEQUEN
		oGetPecas:aCols[nCntFor,FG_POSVAR("VS3_SEQUEN","aHeaderP")] := STRZERO(nValSeq,TamSX3("VS3_SEQUEN")[1])
		//VS3_SEQUEN := STRZERO(nVS3Seq,TamSX3("VS3_SEQUEN")[1])
	else
...
If !oGetPecas:aCols[oGetPecas:nAt,Len(oGetPecas:aCols[oGetPecas:nAt])]
	If !Empty(oGetPecas:aCols[oGetPecas:nAt,FG_POSVAR("VS3_CODITE","aHeaderP")])
		OX001PecFis()
		OX001RecFis("IT_TES",oGetPecas:aCols[oGetPecas:nAt,FG_POSVAR("VS3_CODTES","aHeaderP")]) // nova atualização do Fiscal - Junho 2016
		OX001FisPec()
	EndIf
Else
	OX001ATUF1() // Atualiza informacoes adicionais (dependentes do fiscal)
...
for nCntFor := 1 to Len(oGetPecas:aCols)
	if nCntFor != oGetPecas:nAt .and. !(oGetPecas:aCols[nCntFor,len(oGetPecas:aCols[nCntFor])])
		if oGetPecas:aCols[nCntFor,fg_posvar("VS3_CODITE","aHeaderP")] == oGetPecas:aCols[oGetPecas:nAt,fg_posvar("VS3_CODITE","aHeaderP")] .and. ;
			oGetPecas:aCols[nCntFor,fg_posvar("VS3_GRUITE","aHeaderP")] == oGetPecas:aCols[oGetPecas:nAt,fg_posvar("VS3_GRUITE","aHeaderP")]
			OX001DELP()
...
If readvar() == "M->VS4_CODSER" .and. FindFunction("OM030CSRV")
...
if readvar() $ "M->VS3_LOTECT,M->VS3_NUMLOT"
	DBSelectArea("SB1")
	DBSetOrder(7)
	DBSeek(xFiliaL("SB1")+ oGetPecas:aCols[oGetPecas:nAt,FG_POSVAR("VS3_GRUITE","aHeaderP")]+;
	oGetPecas:aCols[oGetPecas:nAt,FG_POSVAR("VS3_CODITE","aHeaderP")])
	F4LOTE(,,,"OX001",SB1->B1_COD,Iif(!Empty(oGetPecas:aCols[oGetPecas:nAt,FG_POSVAR("VS3_LOCAL","aHeaderP")]),oGetPecas:aCols[oGetPecas:nAt,FG_POSVAR("VS3_LOCAL","aHeaderP")],OX0010105_ArmazemOrigem()))
 	M->VS3_NUMLOT := oGetPecas:aCols[oGetPecas:nAt,FG_POSVAR("VS3_NUMLOT","aHeaderP")]
	M->VS3_LOTECT := oGetPecas:aCols[oGetPecas:nAt,FG_POSVAR("VS3_LOTECT","aHeaderP")]
...
if readvar() $ "M->VS3_LOCALI"
	...
	oGetPecas:aCols[oGetPecas:nAt,FG_POSVAR("VS3_CODITE","aHeaderP")])
	SB1->(DBSetOrder(1))
	F4LOCALIZ( ,,, "A260",SB1->B1_COD,Iif(!Empty(oGetPecas:aCols[oGetPecas:nAt,FG_POSVAR("VS3_LOCAL","aHeaderP")]),oGetPecas:aCols[oGetPecas:nAt,FG_POSVAR("VS3_LOCAL","aHeaderP")],OX0010105_ArmazemOrigem()),,"M->VS3_LOCALI" )
...
===============================================================================
*/
Function OX001PREPEC()
Local cBx_ORIGEM    := ""
Local cGrupoVal     := oGetPecas:aCols[oGetPecas:nAt,FG_POSVAR("VS3_GRUITE","aHeaderP")]
Local cCodIteVal    := oGetPecas:aCols[oGetPecas:nAt,FG_POSVAR("VS3_CODITE","aHeaderP")]
Local nVS3PROMOC    := FG_POSVAR("VS3_PROMOC","oGetPecas:aHeader")  // Posicao no oGetPecas:aHeader  VS3_PROMOC
Local cEraPromoc    := "0"
Local lAltValorPeca := FM_PILHA("OX001RECALC") .or. GetNewPar("MV_MIL0117","S") <> "N" .or. INCLUI
Local nVS3PESPRO    := 0
Local nVS1PERREM    := 0
Local cVS1CONPRO    := "2"
Local dDatRefPD     := dDataBase
...
//if Readvar() $ "M->VS3_GRUITE,M->VS3_CODITE"
//	oGetPecas:aCols[oGetPecas:nAt,FG_POSVAR("VS3_DESITE","aHeaderP")] := M->VS3_DESITE := SB1->B1_DESC
//	oGetPecas:aCols[oGetPecas:nAt,FG_POSVAR("VS3_LOCAL","aHeaderP")]  := M->VS3_LOCAL  := FM_PRODSBZ(SB1->B1_COD,"SB1->B1_LOCPAD")
//endif
if ReadVar()=="M->VS3_FORMUL"
	oGetPecas:aCols[oGetPecas:nAt,FG_POSVAR("VS3_FORMUL","aHeaderP")] := M->VS3_FORMUL
elseif ReadVar()=="M->VS3_CODTES"
	oGetPecas:aCols[oGetPecas:nAt,FG_POSVAR("VS3_CODTES","aHeaderP")] := M->VS3_CODTES
elseif ReadVar()=="M->VS3_OPER" .or. ReadVar()=="M->VS3_CODITE" .or. ReadVar()=="M->VS1_CLIFAT" .or. ReadVar()=="M->VS1_LOJA" .or. ReadVar()=="M->VS1_TIPCLI"
	oGetPecas:aCols[oGetPecas:nAt,FG_POSVAR("VS3_OPER","aHeaderP")]   := M->VS3_OPER
	If !Empty(M->VS3_OPER)
		oGetPecas:aCols[oGetPecas:nAt,FG_POSVAR("VS3_CODTES","aHeaderP")] := M->VS3_CODTES := OX001TESINT(M->VS3_OPER)
	EndIf
	if VS3->(FieldPos("VS3_SITTRI")) > 0
		cBx_ORIGEM := FM_PRODSBZ(SB1->B1_COD,"SB1->B1_ORIGEM")
		if !Empty(cBx_ORIGEM) .and. !Empty(M->VS3_CODTES)
...
If OX0010016_MostraQuantidadeEmEstoque(oGetPecas:nAt, M->VS1_TIPORC)
	nSdoPecas := OX001SLDPC(xFilial("SB2") + SB1->B1_COD + oGetPecas:aCols[oGetPecas:nAt, FG_POSVAR("VS3_LOCAL", "aHeaderP")])
	// dependendo do TES, questiona-se o saldo da peca
	SF4->(DbSeek(xFilial("SF4") + oGetPecas:aCols[oGetPecas:nAt, FG_POSVAR("VS3_CODTES", "aHeaderP")]))
	if nSdoPecas <= 0 .and. SF4->F4_ESTOQUE == "S"
		if Type("M->VS3_QTDEST") == "U"
	...
	M->VS3_QTDEST := nSdoPecas
	oGetPecas:aCols[oGetPecas:nAt, FG_POSVAR("VS3_QTDEST", "aHeaderP")] := nSdoPecas
EndIf

//
// Verifica Segunda Linha em diante
//
if oGetPecas:nAt > 1
	if !oGetPecas:aCols[oGetPecas:nAt - 1,Len(oGetPecas:aCols[oGetPecas:nAt - 1])]
		if Empty(M->VS3_OPER) .and. !Empty(oGetPecas:aCols[oGetPecas:nAt - 1,FG_POSVAR("VS3_OPER","aHeaderP")])
			M->VS3_OPER :=  oGetPecas:aCols[oGetPecas:nAt - 1,FG_POSVAR("VS3_OPER","aHeaderP")]
			oGetPecas:aCols[oGetPecas:nAt,FG_POSVAR("VS3_OPER","aHeaderP")] := M->VS3_OPER
			M->VS3_CODTES := MaTesInt(2,M->VS3_OPER,M->VS1_CLIFAT,M->VS1_LOJA,"C",SB1->B1_COD)
			oGetPecas:aCols[oGetPecas:nAt,FG_POSVAR("VS3_CODTES","aHeaderP")] := M->VS3_CODTES
		endif
...
//###################################################################
//# Se nao foi digitada a formula, preenche com o valor do parametro#
//###################################################################
if Empty(oGetPecas:aCols[oGetPecas:nAt,FG_POSVAR("VS3_FORMUL","aHeaderP")])
	if !Empty(M->VS1_FORMUL)
		oGetPecas:aCols[oGetPecas:nAt,FG_POSVAR("VS3_FORMUL","aHeaderP")] := M->VS1_FORMUL
	else
		cFormul := FS_FORMULA()
		SBM->(dbSetOrder(1))
		SBM->(dbSeek(xFilial("SBM")+oGetPecas:aCols[oGetPecas:nAt,FG_POSVAR("VS3_GRUITE","aHeaderP")]))
	...
	M->VS3_FORMUL := oGetPecas:aCols[oGetPecas:nAt,FG_POSVAR("VS3_FORMUL","aHeaderP")]
Endif
...
if M->VS1_TIPORC == "2"
	...
	DBSetOrder(1)
	DBSeek(xFilial("VOI")+M->VS1_TIPTEM)
	if FieldPos("VOI_CODOPE")>0 .and.Empty(M->VS3_OPER) .and. !Empty(VOI->VOI_CODOPE)
...
//###################################################################
if Empty(oGetPecas:aCols[oGetPecas:nAt,FG_POSVAR("VS3_OPER","aHeaderP")]) .and. Empty(oGetPecas:aCols[oGetPecas:nAt,FG_POSVAR("VS3_CODTES","aHeaderP")])
	if ReadVar() $ "M->VS3_GRUITE,M->VS3_CODITE"
		M->VS3_CODTES := FM_PRODSBZ(SB1->B1_COD,"SB1->B1_TS")//SB1->B1_TS
		oGetPecas:aCols[oGetPecas:nAt,FG_POSVAR("VS3_CODTES","aHeaderP")] := FM_PRODSBZ(SB1->B1_COD,"SB1->B1_TS")//SB1->B1_TS
	endif
endif
//
cPesqPromo := cVS1CONPRO
If cPesqPromo <> "0" 
	nVS3PESPRO := FG_POSVAR("VS3_PESPRO","aHeaderP")
	If nVS3PESPRO > 0 .and. oGetPecas:aCols[oGetPecas:nAt,nVS3PESPRO] == "0"
		cPesqPromo := "0"
	EndIf
EndIf
//
if  ( oGetPecas:aCols[oGetPecas:nAt,FG_POSVAR("VS3_VALPEC","aHeaderP")] == 0 )     .or.;
	( readvar() == "M->VS3_GRUITE" .and. M->VS3_GRUITE <> oGetPecas:aCols[oGetPecas:nAt,FG_POSVAR("VS3_GRUITE","aHeaderP")]) .or.;
	( readvar() == "M->VS3_CODITE" .and. M->VS3_GRUITE <> oGetPecas:aCols[oGetPecas:nAt,FG_POSVAR("VS3_CODITE","aHeaderP")]) .or.;
	( readvar() == "M->VS3_FORMUL" .and. M->VS3_GRUITE <> oGetPecas:aCols[oGetPecas:nAt,FG_POSVAR("VS3_FORMUL","aHeaderP")])
	...
	If SF4->F4_OPEMOV == "05"
		SA1->(DbSetOrder(1))
		SA1->(MsSeek(xFilial("SA1")+M->VS1_CLIFAT+M->VS1_LOJA))
		aTempPro := OX005PERDES(SBM->BM_CODMAR,M->VS1_CENCUS,cGrupoVal,cCodIteVal,M->VS3_QTDITE,M->VS3_PERDES,.f.,M->VS1_CLIFAT,M->VS1_LOJA,IIF(M->VS1_TIPORC == "2","3",M->VS1_TIPVEN),M->VS3_VALTOT/M->VS3_QTDITE,2,M->VS1_FORPAG, GetNewPar("MV_FORMALO",""),,,cPesqPromo,dDatRefPD,nVS1PERREM)
		nValPec := aTempPro[1]
		nMarLuc := aTempPro[2]
		If len(aTempPro) > 4
			cSequenVEN := aTempPro[5]
		EndIf
	else
		nValPec := 0
		nMarLuc := 0
	endif
	If Empty(M->VS3_FORMUL) .and. !Empty(oGetPecas:aCols[oGetPecas:nAt,FG_POSVAR("VS3_FORMUL","aHeaderP")])
		M->VS3_FORMUL := oGetPecas:aCols[oGetPecas:nAt,FG_POSVAR("VS3_FORMUL","aHeaderP")]
	Endif
	if ValType(nValPec) == "N"
		if nValPec == 0
			M->VS3_VALPEC := FG_VALPEC(M->VS1_TIPTEM,"M->VS3_FORMUL",cGrupoVal,cCodIteVal,,.f.,.t.)
			oGetPecas:aCols[oGetPecas:nAt,FG_POSVAR("VS3_VALPEC","aHeaderP")] := M->VS3_VALPEC
		else
...
// ----------------------------------------------------------------------------------------
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
// RETIRADO POIS NA MAQNELSON E NA ESCANDINAVIA O VALOR ESTA CORRETO ENTAO O PROBLEMA PROVAVALMENTE E CONFIGURACAO DO CLIENTE//
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
//If  Left(MaFisRet(n,"IT_CLASFIS"),1) $ "1.2.3.8" .and. !MaFisRet(n,"IT_CF") $ cExceRes13
//	MaFisRef("IT_ALIQICM","VS300",0)            // FOI NECESSARIO ZERAR PARA QUE NA MATXFIS A ALIQUOTA SEJA CARREGADA CORRETAMENTE
//Endif
...
if oGetPecas:aCols[oGetPecas:nAt,FG_POSVAR("VS3_QTDITE","aHeaderP")] > 0 
	...
	//###################################################################
	//# verifica se a peca esta em promocao para retornar o valor       #
	//###################################################################
	DBSelectArea("VAI")
	DBSetOrder(6)
	...
	If lAltValorPeca
		...
		If SF4->F4_OPEMOV == "05"
			...
			If Empty(M->VS3_FORMUL) .and. !Empty(oGetPecas:aCols[oGetPecas:nAt,FG_POSVAR("VS3_FORMUL","aHeaderP")])
				M->VS3_FORMUL := oGetPecas:aCols[oGetPecas:nAt,FG_POSVAR("VS3_FORMUL","aHeaderP")]
			Endif
			If ReadVar() == "M->VS3_QTDITE" .and. M->VS3_VALPEC == 0
				M->VS3_VALPEC := FG_VALPEC(M->VS1_TIPTEM,"M->VS3_FORMUL",cGrupoVal,cCodIteVal,,.f.,.t.)
			Endif
			oGetPecas:aCols[oGetPecas:nAt,FG_POSVAR("VS3_VALPEC","aHeaderP")] := M->VS3_VALPEC
			aTempPro := OX005PERDES(SBM->BM_CODMAR,M->VS1_CENCUS,M->VS3_GRUITE,M->VS3_CODITE,M->VS3_QTDITE,M->VS3_PERDES,.f.,M->VS1_CLIFAT,M->VS1_LOJA,IIF(M->VS1_TIPORC == "2","3",M->VS1_TIPVEN),M->VS3_VALTOT/M->VS3_QTDITE,2,M->VS1_FORPAG, GetNewPar("MV_FORMALO",""),,,cPesqPromo,dDatRefPD,nVS1PERREM)
			nValPec := aTempPro[1]
			nMarLuc := aTempPro[2]
			nDesPro := aTempPro[3]
			If len(aTempPro) > 3
				If aTempPro[4] == "0" // 0 = NAO PODE DAR DESCONTO EM PROMOCAO
				...
				EndIf
			EndIf
			If nValPec == 0 .and. nDesPro > 0
				nValUni := FG_VALPEC(M->VS1_TIPTEM,"M->VS3_FORMUL",cGrupoVal,cCodIteVal,,.f.,.t.)
				nValPec := nValUni-(nValUni*(nDesPro/100)) // Quando há % de Desconto na Promoção
			Endif
			If nVS3PROMOC > 0
				If nValPec == 0
					cEraPromoc := M->VS3_PROMOC
					M->VS3_PROMOC := "0"
					oGetPecas:aCols[oGetPecas:nAt,nVS3PROMOC]	:= "0"
					If cEraPromoc == "1"  // Era promoção e preciso voltar o valor de antes
						nValPec := FG_VALPEC(M->VS1_TIPTEM,"M->VS3_FORMUL",cGrupoVal,cCodIteVal,,.f.,.t.)
				...
				Else
					cEraPromoc := M->VS3_PROMOC
					If nValPec < M->VS3_VALPEC .and. cEraPromoc == "1"
						nValPec := M->VS3_VALPEC
					Endif
					M->VS3_PROMOC := "1"
					oGetPecas:aCols[oGetPecas:nAt,nVS3PROMOC]	:= "1"
				Endif
			Endif
			If len(aTempPro) > 4
				cSequenVEN := aTempPro[5]
		...
		if ValType(nValPec) != "N" .or. ValType(nMarLuc) != "N"
			MsgStop(STR0222)
			if ValType(nValPec) != "N"
				nVAlPec := 0
			endif
			if ValType(nMarLuc) != "N"
				nMarLuc := 0
			endif
		endif
		if readvar() == "M->VS3_VALPEC"
			M->VS3_VALPEC := IIF(nValPec==0 .or. M->VS3_VALPEC > nValPec ,M->VS3_VALPEC,nValPec)
		else
	...
	if FG_POSVAR("VS3_VALPIS","aHeaderP") > 0
		oGetPecas:aCols[oGetPecas:nAt,FG_POSVAR("VS3_VALPIS","aHeaderP")] := M->VS3_VALPIS
	endif
	if FG_POSVAR("VS3_VALCOF","aHeaderP") > 0
		oGetPecas:aCols[oGetPecas:nAt,FG_POSVAR("VS3_VALCOF","aHeaderP")] := M->VS3_VALCOF
	endif
	if FG_POSVAR("VS3_ICMCAL","aHeaderP") > 0
		oGetPecas:aCols[oGetPecas:nAt,FG_POSVAR("VS3_ICMCAL","aHeaderP")] := M->VS3_ICMCAL
	endif
	if FG_POSVAR("VS3_VALCMP","aHeaderP") > 0
		oGetPecas:aCols[oGetPecas:nAt,FG_POSVAR("VS3_VALCMP","aHeaderP")] := M->VS3_VALCMP
	endif
	if FG_POSVAR("VS3_DIFAL","aHeaderP") > 0
		oGetPecas:aCols[oGetPecas:nAt,FG_POSVAR("VS3_DIFAL","aHeaderP")] := M->VS3_DIFAL
	endif
	...
	if FG_POSVAR("VS3_MARLUC","aHeaderP") > 0
		...
		oGetPecas:aCols[oGetPecas:nAt,FG_POSVAR("VS3_MARLUC","aHeaderP")] := M->VS3_MARLUC
	endif
Else
	oGetPecas:aCols[oGetPecas:nAt,FG_POSVAR("VS3_PERDES","aHeaderP")] := M->VS1_PERPEC
...
cGrupoVal  := oGetPecas:aCols[oGetPecas:nAt,FG_POSVAR("VS3_GRUITE","aHeaderP")]
cCodIteVal := oGetPecas:aCols[oGetPecas:nAt,FG_POSVAR("VS3_CODITE","aHeaderP")]
cForVal    := oGetPecas:aCols[oGetPecas:nAt,FG_POSVAR("VS3_FORMUL","aHeaderP")]
...
//
if Empty(oGetPecas:aCols[oGetPecas:nAt,FG_POSVAR("VS3_CODITE","aHeaderP")])
	oGetPecas:aCols[oGetPecas:nAt,FG_POSVAR("VS3_DESITE","aHeaderP")] := ""
	M->VS3_DESITE := ""
Endif
...
for nCntFor := 1 to Len(oGetPecas:aCols)
	if  !(oGetPecas:aCols[nCntFor,len(oGetPecas:aCols[nCntFor])])
		if oGetPecas:aCols[nCntFor,fg_posvar("VS3_CODITE","aHeaderP")] == cCodIteVal .and. ;
			oGetPecas:aCols[nCntFor,fg_posvar("VS3_GRUITE","aHeaderP")] == cGrupoVal
			MsgInfo(Alltrim(cGrupoVal)+ " "+Alltrim(cCodIteVal)+ " - "+ STR0104,STR0025)
			return .f.
		endif
	endif
next
...
M->VS3_CODITE := oGetPecas:aCols[oGetPecas:nAt,FG_POSVAR("VS3_CODITE","aHeaderP")]
//
oGetPecas:aCols[oGetPecas:nAt,FG_POSVAR("VS3_GRUITE","aHeaderP")] := cGrupoVal
...
if !lRetorna
	oGetPecas:aCols[oGetPecas:nAt,FG_POSVAR("VS3_CODITE","aHeaderP")] := cCodSalva
	M->VS3_CODITE := cCodSalva
	oGetPecas:aCols[oGetPecas:nAt,FG_POSVAR("VS3_GRUITE","aHeaderP")] := cGruSalva
	M->VS3_GRUITE := cGruSalva
	oGetPecas:aCols[oGetPecas:nAt,FG_POSVAR("VS3_DESITE","aHeaderP")] := cDesSalva
	M->VS3_DESITE := cDesSalva
	oGetPecas:aCols[oGetPecas:nAt,FG_POSVAR("VS3_CODTES","aHeaderP")] := cTesSalva
	M->VS3_CODTES := cTesSalva
...
If Empty(cAux)
	If M->VS1_TIPORC == "2" .and. ( Empty(M->VS1_TIPTEM) .or. lAltEnc )
		...
		cAux2:= cTpTSv := Iif(VS1->(FieldPos("VS1_TIPTSV"))>0,M->VS1_TIPTSV,M->VS1_TIPTEM)
...
Local lVS3DEPGAR := ( VS3->(FieldPos("VS3_DEPGAR")) <> 0 )
...
If VO1->(FieldPos("VO1_GARMUT")) <> 0 .and. VO1->VO1_GARMUT == "1"
...
if VO1->(FieldPos("VO1_FRANQP")) <> 0 .and. VO1->VO1_FRANQP > 0 .and. nVlFraPeca <> 0
	if !lOX001Auto
		MsgStop(STR0151)
	endif
	sRestArea(aArea)
...
if lValidTTPeca
	If lVS3DEPGAR
		For nCntFor := 1 to len(oGetPecas:aCols)
			If !oGetPecas:aCols[nCntFor,len(oGetPecas:aCols[nCntFor])]
				cDepInt := oGetPecas:aCols[nCntFor,fg_posvar("VS3_DEPINT","aHeaderP")]
				cDepGar := oGetPecas:aCols[nCntFor,fg_posvar("VS3_DEPGAR","aHeaderP")]
			endif
		Next
	EndIf
...
if Len(aItensNew) > 1
	...
	If lMsErroAuto
		...
		if Type("lOX001Auto") == "U" .or. !lOX001Auto
			For i := 2 to Len(aItensNew)
				if OX001SLDPC(xFilial("SB2")+aItensNew[i,1]+aItensNew[i,4]) < aItensNew[i,16]
	...
	Else
		...
		If VS3->(FieldPos("VS3_DOCSDB")) > 0
			...
			Else
				// Atualiza VS3_DOCSDB
				nGruIte :=	FG_POSVAR("VS3_GRUITE","aHeaderP")
				nCodIte :=	FG_POSVAR("VS3_CODITE","aHeaderP")
...
//
//
if ( FM_PILHA("OFIXA011") .or. FM_PILHA("OFIXA018") ) .and. !FM_PILHA("OFIXC001")
	//
	if  VISUALIZA
		return .f.
	endif
	...
	if oGetPecas:nAt > 0 .and. oGetPecas:nAt <= Len(oGetPecas:aCols)
		If Empty(xGru+xCod) .and. xQtd <= 0
			xGru    := oGetPecas:aCols[oGetPecas:nAt,FG_POSVAR("VS3_GRUITE","aHeaderP")]
			xCod    := oGetPecas:aCols[oGetPecas:nAt,FG_POSVAR("VS3_CODITE","aHeaderP")]
			xQtd    := oGetPecas:aCols[oGetPecas:nAt,FG_POSVAR("VS3_QTDITE","aHeaderP")]
			xValPec := oGetPecas:aCols[oGetPecas:nAt,FG_POSVAR("VS3_VALPEC","aHeaderP")]
			xFormul := oGetPecas:aCols[oGetPecas:nAt,FG_POSVAR("VS3_FORMUL","aHeaderP")]
		EndIf
...
===============================================================================
*/
Function OX001RECALC(nOpc,lCalcSemGrv)
Local nCntFor, nCntFor2
...
if lCalcSemGrv .or. OX001GRV(nOpc,.f.,.t.,,.f.)
	...
	for nCntFor := 1 to Len(oGetPecas:aCols)
		if !oGetPecas:aCols[nCntFor,Len(oGetPecas:aCols[nCntFor])]
			...
			//
			aVetCmp := {}
			aAdd(aVetCmp,{"VS3_GRUITE",oGetPecas:aCols[oGetPecas:nAt,FG_POSVAR("VS3_GRUITE","aHeaderP")] } )
			aAdd(aVetCmp,{"VS3_CODITE",oGetPecas:aCols[oGetPecas:nAt,FG_POSVAR("VS3_CODITE","aHeaderP")] } )
			aAdd(aVetCmp,{"VS3_QTDITE",oGetPecas:aCols[oGetPecas:nAt,FG_POSVAR("VS3_QTDITE","aHeaderP")] } )
			aAdd(aVetCmp,{"VS3_CODTES",oGetPecas:aCols[oGetPecas:nAt,FG_POSVAR("VS3_CODTES","aHeaderP")] } )
			aAdd(aVetCmp,{"VS3_LOCAL", oGetPecas:aCols[oGetPecas:nAt,FG_POSVAR("VS3_LOCAL","aHeaderP")] } )
			M->VS3_OPER := oGetPecas:aCols[oGetPecas:nAt,FG_POSVAR("VS3_OPER","aHeaderP")]
			SFM->(DBSetOrder(1))
			SFM->(DBSeek(xFilial("SFM")+M->VS3_OPER))
			M->VS3_CODTES := OX001TESINT(M->VS3_OPER)
			oGetPecas:aCols[oGetPecas:nAt,FG_POSVAR("VS3_CODTES","aHeaderP")] := M->VS3_CODTES
			aAdd(aVetCmp,{"VS3_FORMUL", oGetPecas:aCols[oGetPecas:nAt,FG_POSVAR("VS3_FORMUL","aHeaderP")] } )
			For nCntFor2 := 1 to Len(aHeaderP)
				if !IsHeadRec(aHeaderP[nCntFor2,2]) .and. !IsHeadAlias(aHeaderP[nCntFor2,2]) // não pode criar M->  do VS3_REC_WT e VS3_ALI_WT, senão da erro no FG_MEMVAR
					&("M->"+aHeaderP[nCntFor2,2]) := oGetPecas:aCols[nCntFor,nCntFor2]
				Endif
			next
			// faz o laço para cada item, preenchendo os valores e chamando o FieldOk
	...
	next
	// Atualiza Browse
	If nAuxnATPec > 0 .and. nAuxnATPec <= Len(oGetPecas:aCols)
...
For ix_:=1 to Len(oGetPecas:aCols)
	...
	cLocalP := Iif(!Empty(oGetPecas:aCols[oGetPecas:nAt,FG_POSVAR("VS3_LOCAL","aHeaderP")]),oGetPecas:aCols[oGetPecas:nAt,FG_POSVAR("VS3_LOCAL","aHeaderP")],OX0010105_ArmazemOrigem())
	dbSelectArea("SB2")
	...
	VEC_NUMREL := cNumREL
	VEC_NUMIDE := cNumIDE
	VEC_GRUITE := oGetPecas:aCols[oGetPecas:nAt,FG_POSVAR("VS3_GRUITE","aHeaderP")]
	VEC_CODITE := oGetPecas:aCols[oGetPecas:nAt,FG_POSVAR("VS3_CODITE","aHeaderP")]
	VEC_VALVDA := oGetPecas:aCols[oGetPecas:nAt,FG_POSVAR("VS3_VALTOT","aHeaderP")]
	VEC_VALDES := oGetPecas:aCols[oGetPecas:nAt,FG_POSVAR("VS3_VALDES","aHeaderP")]
	VEC_QTDITE := oGetPecas:aCols[oGetPecas:nAt,FG_POSVAR("VS3_QTDITE","aHeaderP")]
	VEC_VALICM := nValICM
	VEC_VALCOF := nValCof
	VEC_VALPIS := nValPis
	VEC_VALIPI := nValIPI
	VEC_TOTIMP := VEC_VALICM + VEC_VALCOF + VEC_VALPIS + VEC_DIFAL + VEC_VALCMP + VEC_VALIPI
	VEC_CUSMED := SB2->B2_CM1 * oGetPecas:aCols[oGetPecas:nAt,FG_POSVAR("VS3_QTDITE","aHeaderP")]
	VEC_JUREST := 0
...
if lGrava
	if cPecSer == "P"  //Pecas
		...
		Else
			cCont := 1
		Endif
		aAdd( aVetCort,{xFilial("VPJ"),M->VS1_NUMORC,dDataBase,val(left(time(),2)+substr(time(),4,2)),oGetPecas:aCols[oGetPecas:nAt,FG_POSVAR("VS3_GRUITE","aHeaderP")],oGetPecas:aCols[oGetPecas:nAt,FG_POSVAR("VS3_CODITE","aHeaderP")],oGetPecas:aCols[oGetPecas:nAt,FG_POSVAR("VS3_QTDITE","aHeaderP")],oGetPecas:aCols[oGetPecas:nAt,FG_POSVAR("VS3_VALTOT","aHeaderP")],cMotCort,STRZERO(cCont,3),M->VS1_NCLIFT} )
	Else       // Servicos
		cCont := 0
		DBSelectArea("SX2")
		if DBSeek("VPM")
...
if Type("oGetPecas") != "U"
	If MaFisFound('NF')
		...
		if cGru == "DEFAULT"  // RUBENS
			cGru := oGetPecas:aCols[oGetPecas:nAt,fg_posvar("VS3_GRUITE","aHeaderP")]
		endif
		if cCod == "DEFAULT"
			cCod := oGetPecas:aCols[oGetPecas:nAt,fg_posvar("VS3_CODITE","aHeaderP")]
		endif
		if nQtd == 0
			if !lPediVenda
				nQtd := oGetPecas:aCols[oGetPecas:nAt,fg_posvar("VS3_QTDITE","aHeaderP")] - oGetPecas:aCols[oGetPecas:nAt,fg_posvar("VS3_QTDEST","aHeaderP")]
			endif
		endif
		//
		If !lItem
...
if !(empty(_cGrupo) .and. empty(_cCodigo))
	...
	aRet := OFIOM420( _cGrupo , _cCodigo , M->VS1_CHAINT , M->VS1_KILOME , M->VS1_TIPTEM , _cDescricao , , Iif(FieldPos("VS1_TIPTSV")>0 , M->VS1_TIPTSV , M->VS1_TIPTEM) , .T. , .F. , "" , "" , "" , .T. ) // Inconvenientes
	aPeca := aRet[1]
	aServ := aRet[2]
else
	aPeca := {}
...
If Len(aPeca) > 0
	...
	for nCntFor := 1 to Len(aPeca)
		...
		RegToMemory("VS3",.t.)
		//
		oGetPecas:aCols[oGetPecas:nAt,FG_POSVAR("VS3_GRUITE","aHeaderP")] := M->VS3_GRUITE := aPeca[nCntFor,1]
		oGetPecas:aCols[oGetPecas:nAt,FG_POSVAR("VS3_CODITE","aHeaderP")] := M->VS3_CODITE := aPeca[nCntFor,2]
		oGetPecas:aCols[oGetPecas:nAt,FG_POSVAR("VS3_QTDITE","aHeaderP")] := M->VS3_QTDITE := aPeca[nCntFor,3]
		oGetPecas:aCols[oGetPecas:nAt,FG_POSVAR("VS3_GRUINC","aHeaderP")] := M->VS3_GRUINC := aPeca[nCntFor,4]
		oGetPecas:aCols[oGetPecas:nAt,FG_POSVAR("VS3_CODINC","aHeaderP")] := M->VS3_CODINC := aPeca[nCntFor,5]
		oGetPecas:aCols[oGetPecas:nAt,FG_POSVAR("VS3_DESINC","aHeaderP")] := M->VS3_DESINC := aPeca[nCntFor,6]
		oGetPecas:aCols[oGetPecas:nAt,FG_POSVAR("VS3_CODTES","aHeaderP")] := M->VS3_CODTES := cTesCmp
		oGetPecas:aCols[oGetPecas:nAt,FG_POSVAR("VS3_SEQINC","aHeaderP")] := M->VS3_SEQINC := cSeqInc
		//
...
If _lInstalar // Preencher Inconveniente da Peca com o mesmo do Servico
	oGetPecas:aCols[oGetPecas:nAt,FG_POSVAR("VS3_SEQINC","aHeaderP")] := cSeqInc
	oGetPecas:aCols[oGetPecas:nAt,FG_POSVAR("VS3_DESINC","aHeaderP")] := _cDescricao
EndIf
//
VE1->(dbSetOrder(1))
...
Local nPosGruIte := FG_POSVAR("VS3_GRUITE","aHeaderP")
Local nPosCodIte := FG_POSVAR("VS3_CODITE","aHeaderP")
Local nPosGruInc := FG_POSVAR("VS3_GRUINC","aHeaderP")
Local nPosCodInc := FG_POSVAR("VS3_CODINC","aHeaderP")
Local nPosSeqInc := FG_POSVAR("VS3_SEQINC","aHeaderP")
Local nPosDesInc := FG_POSVAR("VS3_DESINC","aHeaderP")
...
if _cTp == "P" .and. ;
	((!Empty(oGetPecas:aCols[oGetPecas:nAt,FG_POSVAR("VS3_GRUINC","aHeaderP")]) .and. !Empty(oGetPecas:aCols[oGetPecas:nAt,FG_POSVAR("VS3_CODINC","aHeaderP")]) );
	.or.;
	!Empty(oGetPecas:aCols[oGetPecas:nAt,FG_POSVAR("VS3_DESINC","aHeaderP")]) )
	Return
endif
...
if _cTp == "P"
	oGetPecas:aCols[oGetPecas:nAt,FG_POSVAR("VS3_GRUINC","aHeaderP")] := M->VS3_GRUINC := aInconv[1]
	oGetPecas:aCols[oGetPecas:nAt,FG_POSVAR("VS3_CODINC","aHeaderP")] := M->VS3_CODINC := aInconv[2]
	oGetPecas:aCols[oGetPecas:nAt,FG_POSVAR("VS3_SEQINC","aHeaderP")] := M->VS3_SEQINC := aInconv[4]
	oGetPecas:aCols[oGetPecas:nAt,FG_POSVAR("VS3_DESINC","aHeaderP")] := M->VS3_DESINC := aInconv[3]
endif
...
For nCntFor := 1 to Len(oGetPecas:aCols)
	if !oGetPecas:aCols[nCntFor,Len(oGetPecas:aCols[nCntFor])]
		...
		if oGetPecas:aCols[nCntFor,FG_POSVAR("VS3_SEQINC","aHeaderP")] == oGetInconv:aCols[oGetInconv:nAt,FG_POSVAR("VST_SEQINC","aHeaderI")] .and.;
			oGetPecas:aCols[nCntFor,FG_POSVAR("VS3_GRUINC","aHeaderP")] == oGetInconv:aCols[oGetInconv:nAt,FG_POSVAR("VST_GRUINC","aHeaderI")] .and.;
			oGetPecas:aCols[nCntFor,FG_POSVAR("VS3_CODINC","aHeaderP")] == oGetInconv:aCols[oGetInconv:nAt,FG_POSVAR("VST_CODINC","aHeaderI")] .and.;
			oGetPecas:aCols[nCntFor,FG_POSVAR("VS3_DESINC","aHeaderP")] == oGetInconv:aCols[oGetInconv:nAt,FG_POSVAR("VST_DESINC","aHeaderI")]
...
If M->VS1_TIPORC != "1" // Balcão pode repetir os itens
	for nCntFor := 1 to Len(oGetPecas:aCols)
		if  !(oGetPecas:aCols[nCntFor,len(oGetPecas:aCols[nCntFor])]) .and. nCntFor <> nAuxLinha
			...
			if AllTrim(oGetPecas:aCols[nCntFor,fg_posvar("VS3_GRUITE","aHeaderP")]) == AllTrim(cPGruIte) .and. ;
				AllTrim(oGetPecas:aCols[nCntFor,fg_posvar("VS3_CODITE","aHeaderP")]) == AllTrim(cPCodIte) .and. ;
				AllTrim(oGetPecas:aCols[nCntFor,fg_posvar("VS3_NUMLOT","aHeaderP")]) == AllTrim(cNumLot) .and. ;
				AllTrim(oGetPecas:aCols[nCntFor,fg_posvar("VS3_LOTECT","aHeaderP")]) == AllTrim(cLoteCtl)
				...
				if lInconveniente
					...
					if AllTrim(oGetPecas:aCols[nCntFor,fg_posvar("VS3_GRUINC","aHeaderP")]) == AllTrim(cPGruInc) .and. ;
						AllTrim(oGetPecas:aCols[nCntFor,fg_posvar("VS3_CODINC","aHeaderP")]) == AllTrim(cPCodInc) .and. ;
						AllTrim(oGetPecas:aCols[nCntFor,fg_posvar("VS3_DESINC","aHeaderP")]) == AllTrim(cPDesInc) .and. ;
						AllTrim(oGetPecas:aCols[nCntFor,fg_posvar("VS3_SEQINC","aHeaderP")]) == AllTrim(cPSeqInc) .and. ;
						AllTrim(oGetPecas:aCols[nCntFor,fg_posvar("VS3_NUMLOT","aHeaderP")]) == AllTrim(cNumLot) .and. ;
						AllTrim(oGetPecas:aCols[nCntFor,fg_posvar("VS3_LOTECT","aHeaderP")]) == AllTrim(cLoteCtl)
	...
	for nCntFor := 1 to Len(oGetPecas:aCols)
		...
		if  !(oGetPecas:aCols[nCntFor,len(oGetPecas:aCols[nCntFor])]) .and. nCntFor <> nAuxLinha
			if AllTrim(oGetPecas:aCols[nCntFor,fg_posvar("VS3_GRUITE","aHeaderP")]) == AllTrim(cPGruIte) .and. ;
				...
				If cFaseConfer $ cFaseOrc .or. "R" $ cFaseOrc .or. (oGetPecas:aCols[nCntFor,fg_posvar("VS3_RESERV","aHeaderP")]=="1")
...
for nCntFor = 1 to Len(oGetPecas:aCols)
	...
	if !oGetPecas:aCols[nCntFor,Len(oGetPecas:aCols[nCntFor])]
		...
		if !lPediVenda .OR. lRegBO // se for registrar backorder pode fazer sim sugs. compra nao sendo pedido de venda
			if oGetPecas:aCols[nCntFor,fg_posvar("VS3_QTDEST","aHeaderP")] < oGetPecas:aCols[nCntFor,fg_posvar("VS3_QTDITE","aHeaderP")]
				DBSelectArea("SB1")
				DBSetOrder(7)
				...
				SB1->B1_DESC,;
				oGetPecas:aCols[nCntFor,fg_posvar("VS3_QTDITE","aHeaderP")] - oGetPecas:aCols[nCntFor,fg_posvar("VS3_QTDEST","aHeaderP")],;
				SBM->BM_CODMAR,;
			...
			if lFaseReserv .OR. lRegBO // e tem fase R lReg
				if lAguardar
					...
					if DBSeek(aSugest[nCntFor,9])
						...
						msunlock()
						oGetPecas:aCols[aSugest[nCntFor,10],FG_POSVAR("VS3_QTDAGU","aHeaderP")] := aSugest[nCntFor,5]
					endif
				endif
			endif
			nTmp := n
...
Else
	...
	While !eof() .and. xFilial("VS3") + VS1->VS1_NUMORC == VS3->VS3_FILIAL + VS3->VS3_NUMORC
		...
		If SF4->F4_OPEMOV == "05"
			...
			cPesqPromo := IIf( cPesqPromo <> "0" .and. lVS3PESPRO .and. VS3->VS3_PESPRO == "0" , "0" , cVS1CONPRO )




			lRetDes := OX005PERDES(SBM->BM_CODMAR, VS1->VS1_CENCUS, VS3->VS3_GRUITE, VS3->VS3_CODITE,;
				VS3->VS3_QTDITE, VS3->VS3_PERDES, .t., VS1->VS1_CLIFAT, VS1->VS1_LOJA,;
				IIF(VS1->VS1_TIPORC == "2", "3", VS1->VS1_TIPVEN), VS3->VS3_VALTOT / VS3->VS3_QTDITE,, VS1->VS1_FORPAG,,,,cPesqPromo,dDatRefPD,nVS1PERREM)
...
Static Function OX001GERLB()
	...
	Local lVS1NUMLIS  := VS1->(FieldPos("VS1_NUMLIS")) > 0
	Local lGeraSep    := ( (GetNewPar("MV_GLIBVEN","N") == "S") .and. lVS1NUMLIS )
	Local lConfLib
	...
	if VOK->VOK_INCMOB == "5"
		...
		if VOK->VOK_INCMOB <> "7" // Calcular se for diferente de Valor Informado
			...
			Else
				...
				if VO7->(FieldPos("VO7_APLICA")) > 0 .and. VO7->(FieldPos("VO7_CONTRO")) > 0 .and. VO7->(FieldPos("VO7_SEQSER")) > 0 .and. VS1->VS1_CODMAR == FG_MARCA("CHEVROLET",,.f.)
	...
	For nCont := 1 to Len(oVO4GetServ:aCols)
		...
		Aadd(aRetSrvc,;
			{;
				...
				IIF(VO7->(FieldPos("VO7_SEQSER")) > 0, oVO4GetServ:aCols[nCont, FG_POSVAR("VO4_SEQSER","aVO4Header")], ""),; // 17
...
if !( (cQryAlSC1)->(eof()) )
	...
	while !((cQryAlSC1)->(eof()))
		...
		for nCntFor2 := 1 to Len(oGetPecas:aCols)
			...
			if !oGetPecas:aCols[nCntFor2,Len(oGetPecas:aCols[nCntFor2])]
				// se o item já foi lançado no orçamento deve-se apenas alterar a quantidade
				if cGrupoSCA == oGetPecas:aCols[nCntFor2,FG_POSVAR("VS3_GRUITE","aHeaderP")] .and.;
					Alltrim((cQryAlSC1)->(VD4_PARTNO)) == Alltrim(oGetPecas:aCols[nCntFor2,FG_POSVAR("VS3_CODITE","aHeaderP")])
					// salva valores da acols para restauração
					nAtual := oGetPecas:nAt
					oGetPecas:nAt := nCntFor2
					n := nCntFor2
					...
					next
					// atualiza quantidade
					M->VS3_QTDITE := (cQryAlSC1)->(VD4_QUANT) // + oGetPecas:aCols[nCntFor2,FG_POSVAR("VS3_QTDITE","aHeaderP")]
					// executa o fieldok com os valores posicionados
					__ReadVar := "M->VS3_QTDITE"
					OX001FPOK(.f.,,.f.,.f.)
					// restaura a posição anterior da acols
		...
		// quando o item é lançado pela primeira vez ele deve ocupar o lugar do item atual (kit)
		// caso contrário deve-se criar uma nova linha
		if Len(oGetPecas:aCols) > 0 .and. !Empty(oGetPecas:aCols[1,FG_POSVAR("VS3_CODITE","aHeaderP")])
			lPrimLinha := .f.
		endif
		if !lPrimLinha
			// adiciona a linha com os valores default
...
Local lPRODIA  := ( VEN->(FieldPos("VEN_PRODIA")) > 0 )
...
while !( ( cQAlVEN )->(eof()) )
	...
	If lPRODIA .and. ( cQAlVEN )->(VEN_PRODIA) == "1" // Promocao do Dia
		aAdd( aPromoD, { SB1->B1_GRUPO, SB1->B1_CODITE, SB1->B1_DESC, SB1->B1_FABRIC, OX001SLDPC(xFilial("SB2")+SB1->B1_COD+OX0010105_ArmazemOrigem()), ( cQAlVEN )->(VEN_VALPRO), ( cQAlVEN )->(VEN_QTDITE), ( cQAlVEN )->(VEN_PERDES) })
	Else
		aAdd( aPromoG, { SB1->B1_GRUPO, SB1->B1_CODITE, SB1->B1_DESC, SB1->B1_FABRIC, OX001SLDPC(xFilial("SB2")+SB1->B1_COD+OX0010105_ArmazemOrigem()), ( cQAlVEN )->(VEN_VALPRO), ( cQAlVEN )->(VEN_QTDITE), ( cQAlVEN )->(VEN_PERDES) })
	EndIf
	( cQAlVEN )->( dbSkip() )
enddo
( cQAlVEN )->( dbCloseArea() )
...
If ReadVar() == "M->VS3_DEPINT"
	For nCntFor := 1 to len(oGetPecas:aCols)
		If !oGetPecas:aCols[nCntFor,len(oGetPecas:aCols[nCntFor])]// .and. oGetPecas:nAt <> nCntFor
			if AllTrim(oGetPecas:aCols[nCntFor,FG_POSVAR("VS3_DEPINT","aHeaderP")]) <> M->VS3_DEPINT .and. ;
				!Empty(AllTrim(oGetPecas:aCols[nCntFor,FG_POSVAR("VS3_DEPINT","aHeaderP")]))
		         oGetPecas:aCols[nCntFor,FG_POSVAR("VS3_DEPINT","aHeaderP")] := M->VS3_DEPINT
	        Endif
    	Endif
	Next
	For nCntFor := 1 to len(oGetServ:aCols)
...
Elseif ReadVar() == "M->VS4_DEPINT"
	For nCntFor := 1 to len(oGetPecas:aCols)
		If !oGetPecas:aCols[nCntFor,len(oGetPecas:aCols[nCntFor])]// .and. oGetPecas:nAt <> nCntFor
			if AllTrim(oGetPecas:aCols[nCntFor,FG_POSVAR("VS3_DEPINT","aHeaderP")]) <> M->VS4_DEPINT .and. ;
				!Empty(AllTrim(oGetPecas:aCols[nCntFor,FG_POSVAR("VS3_DEPINT","aHeaderP")]))
				oGetPecas:aCols[nCntFor,FG_POSVAR("VS3_DEPINT","aHeaderP")] := M->VS4_DEPINT
			endif
		endif
	Next
	For nCntFor := 1 to len(oGetServ:aCols)
...
If ReadVar() == "M->VS3_DEPGAR"
	For nCntFor := 1 to len(oGetPecas:aCols)
		If !oGetPecas:aCols[nCntFor,len(oGetPecas:aCols[nCntFor])] .and. oGetPecas:nAt <> nCntFor
	    	if AllTrim(oGetPecas:aCols[nCntFor,FG_POSVAR("VS3_DEPGAR","aHeaderP")]) <> M->VS3_DEPGAR .and. ;
		    	!Empty(AllTrim(oGetPecas:aCols[nCntFor,FG_POSVAR("VS3_DEPGAR","aHeaderP")]))
		         oGetPecas:aCols[nCntFor,FG_POSVAR("VS3_DEPGAR","aHeaderP")] := M->VS3_DEPGAR
	        Endif
	    Endif
	Next
	For nCntFor := 1 to len(oGetServ:aCols)
...
Elseif ReadVar() == "M->VS4_DEPGAR"
	For nCntFor := 1 to len(oGetPecas:aCols)
		If !oGetPecas:aCols[nCntFor,len(oGetPecas:aCols[nCntFor])] .and. oGetPecas:nAt <> nCntFor
			if AllTrim(oGetPecas:aCols[nCntFor,FG_POSVAR("VS3_DEPGAR","aHeaderP")]) <> M->VS4_DEPGAR .and. ;
				!Empty(AllTrim(oGetPecas:aCols[nCntFor,FG_POSVAR("VS3_DEPGAR","aHeaderP")]))
				oGetPecas:aCols[nCntFor,FG_POSVAR("VS3_DEPGAR","aHeaderP")] := M->VS4_DEPGAR
			endif
		endif
	Next
	For nCntFor := 1 to len(oGetServ:aCols)
...
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
±±ÉÍÍÍÍÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍÍËÍÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍËÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍÍÍÍ»±±
±±ºPrograma  ³OX001XC008 ºAutor  ³Thiago              º Data ³  04/12/12   º±±
±±ÌÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±±
±±ºDesc.     ³Chamada da OFIXC008 e teclas de atalhos.      			   º±±
±±ÈÍÍÍÍÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍ¼±±
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
ßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßß
*/
...
If oGetPecas:nAt == 1  .or. VISUALIZA
   Return(.t.)
Endif
If Empty(oGetPecas:aCols[oGetPecas:nAt ,fg_posvar("VS3_DEPINT","aHeaderP")])
	oGetPecas:aCols[oGetPecas:nAt ,fg_posvar("VS3_DEPINT","aHeaderP")] := M->VS3_DEPINT := oGetPecas:aCols[oGetPecas:nAt-1 ,fg_posvar("VS3_DEPINT","aHeaderP")]
EndIf
If Empty(oGetPecas:aCols[oGetPecas:nAt ,fg_posvar("VS3_DEPGAR","aHeaderP")])
	oGetPecas:aCols[oGetPecas:nAt ,fg_posvar("VS3_DEPGAR","aHeaderP")] := M->VS3_DEPGAR := oGetPecas:aCols[oGetPecas:nAt-1 ,fg_posvar("VS3_DEPGAR","aHeaderP")]
EndIf
...
/*				M->VS3_FORMUL := VOI->VOI_VALPEC
				...
				If OX001VLDENC(nPosAltTT)
					...
					oGetPecas:aCols[oGetPecas:nAt,FG_POSVAR("VS3_LOCAL","aHeaderP")] := M->VS3_LOCAL
	...
	if !Empty(M->VS1_CHASSI)
	    ...
	    For nPosAltTT := 1 to Len(oGetPecas:aCols)
			If !oGetPecas:aCols[nPosAltTT,len(aHeaderP)+1] // Peça ativa
				oGetPecas:nAt := nPosAltTT
				FG_MEMVAR(aHeaderP,oGetPecas:aCols,oGetPecas:nAt)
				oGetPecas:aCols[oGetPecas:nAt,FG_POSVAR("VS3_OPER","aHeaderP")] := VOI->VOI_CODOPE
				M->VS3_OPER := VOI->VOI_CODOPE
				lPPrepec := .t.
...
Local nCntFor   := 0
Local cQueryTPF := ""
Local lDifal := VS1->(FieldPos("VS1_DIFAL")) > 0
Default cTip := ""

If cPar == "ON" // On line

	For nCntFor := 1 to Len(oGetPecas:aCols)
		if !oGetPecas:aCols[nCntFor,len(oGetPecas:aCols[nCntFor])] .and. oGetPecas:aCols[nCntFor,FG_POSVAR("VS3_QTDITE","aHeaderP")] > 0  .and. Empty(oGetPecas:aCols[nCntFor,FG_POSVAR("VS3_MOTPED","aHeaderP")])
			nValRet += oGetPecas:aCols[nCntFor,FG_POSVAR("VS3_VALTOT","aHeaderP")]
			if lDifal
				nValRetD += oGetPecas:aCols[nCntFor,FG_POSVAR("VS3_DIFAL","aHeaderP")]
			Endif
		Endif
	Next
	for nCntFor := 1 to Len(oGetServ:aCols)
...
Static Function OX001CriRLt(nSldLote,nPosAcolsLt)
	...
	For nY:=1 to nUsadoPX01
		...
		Else
			oGetPecas:aCols[Len(oGetPecas:aCols),nY]:= FieldGet(FieldPos(aHeaderP[nY,2]))
		Endif
...
Return MaTesInt(2,cCodOper,M->VS1_CLIFAT,M->VS1_LOJA,"C",SB1->B1_COD,,M->VS1_TIPCLI)
/*
ÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜ
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
±±ÉÍÍÍÍÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍËÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍËÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍ»±±
	...
	For ny := 1 to Len(oGetPecas:aCols)
			...
			if !oGetPecas:aCols[ny,Len(oGetPecas:aCols[ny])] .and. !Empty(oGetPecas:aCols[ny,FG_POSVAR("VS3_CODITE","aHeaderP")])
				...
				If cChamadoPor == "OX001LVS34"
					...
					n := aNumP[nItem,2]
					//
					cACodGrp := oGetPecas:aCols[ny,FG_POSVAR("VS3_GRUITE","aHeaderP")]
					cACodIte := oGetPecas:aCols[ny,FG_POSVAR("VS3_CODITE","aHeaderP")]
					//
					DBSelectArea("SB1")
					DBSetOrder(7)
					DBSeek(xFilial("SB1")+cACodGrp+cACodIte)

					SF4->(dbSeek(xFilial("SF4")+oGetPecas:aCols[ny,FG_POSVAR("VS3_CODTES","aHeaderP")]))
					MaFisIniLoad(n,{	SB1->B1_COD,;
					oGetPecas:aCols[ny,FG_POSVAR("VS3_CODTES","aHeaderP")],;
					" "  ,;
					oGetPecas:aCols[ny,FG_POSVAR("VS3_QTDITE","aHeaderP")],;
					"",;
					"",;
					SB1->(RecNo()),;	//IT_RECNOSB1
					SF4->(RecNo()),;	//IT_RECNOSF4
					...
					MaFisLoad("IT_QUANT",oGetPecas:aCols[ny,FG_POSVAR("VS3_QTDITE","aHeaderP")],n)
					MaFisLoad("IT_TES",oGetPecas:aCols[ny,FG_POSVAR("VS3_CODTES","aHeaderP")],n)
					MaFisLoad("IT_PRCUNI",oGetPecas:aCols[ny,FG_POSVAR("VS3_VALPEC","aHeaderP")],n)
					MaFisLoad("IT_VALMERC",Round(oGetPecas:aCols[ny,FG_POSVAR("VS3_VALPEC","aHeaderP")]*oGetPecas:aCols[ny,FG_POSVAR("VS3_QTDITE","aHeaderP")],2),n)
					MaFisLoad("IT_DESCONTO",oGetPecas:aCols[ny,FG_POSVAR("VS3_VALDES","aHeaderP")],n)
					MaFisRecal("",n)
					MaFisEndLoad(n,1)
				...
				oGetPecas:aCols[ny,FG_POSVAR("VS3_PICMSB","aHeaderP")] := M->VS3_PICMSB
				oGetPecas:aCols[ny,FG_POSVAR("VS3_VICMSB","aHeaderP")] := M->VS3_VICMSB
				oGetPecas:aCols[ny,FG_POSVAR("VS3_BICMSB","aHeaderP")] := M->VS3_BICMSB
				oGetPecas:aCols[ny,FG_POSVAR("VS3_VALPIS","aHeaderP")] := M->VS3_VALPIS
				oGetPecas:aCols[ny,FG_POSVAR("VS3_VALCOF","aHeaderP")] := M->VS3_VALCOF
				oGetPecas:aCols[ny,FG_POSVAR("VS3_ICMCAL","aHeaderP")] := M->VS3_ICMCAL
				oGetPecas:aCols[ny,FG_POSVAR("VS3_VALCMP","aHeaderP")] := M->VS3_VALCMP
				oGetPecas:aCols[ny,FG_POSVAR("VS3_DIFAL","aHeaderP")]  := M->VS3_DIFAL
				If FG_POSVAR("VS3_PIPIFB","aHeaderP") > 0 .and. FG_POSVAR("VS3_VIPIFB","aHeaderP") > 0
					oGetPecas:aCols[ny,FG_POSVAR("VS3_PIPIFB","aHeaderP")] := M->VS3_PIPIFB
					oGetPecas:aCols[ny,FG_POSVAR("VS3_VIPIFB","aHeaderP")] := M->VS3_VIPIFB
				Endif
...
Local lVS1_DEXPOS := (VS1->(FieldPos("VS1_DEXPOS")) <> 0) // Data de Exportação do Orçamento para a OS
...
For i:= 1 to Len(oGetPecas:aCols)
	if !oGetPecas:aCols[i,Len(oGetPecas:aCols[i])]
		if !Empty(oGetPecas:aCols[i,fg_posvar("VS3_CODITE","aHeaderP")])
			nQtdLin+=1
		Endif
	Endif
Next
If M->VS1_TIPORC == "1" .or. lPediVenda
	If nQtdLin > 0 .and. nMaxItNF > 0 .and. nQtdLin >= nMaxItNF .and. cOpcao <> "2"
		if ReadVar() == "M->VS3_GRUITE" .or. ReadVar() == "M->VS3_CODITE"
			...
			M->VS3_CODITE := space(TamSx3("VS3_CODITE")[1])
			oGetPecas:aCols[oGetPecas:nAt,fg_posvar("VS3_GRUITE","aHeaderP")]  := space(TamSx3("VS3_GRUITE")[1])
			oGetPecas:aCols[oGetPecas:nAt,fg_posvar("VS3_CODITE","aHeaderP")] := space(TamSx3("VS3_CODITE")[1])
		Endif
		lRet := .f.
	...
	if !lRet
		oGetPecas:oBrowse:refresh()
		ShowHelpDlg ( "OX001NUMITEM", { STR0316 }) // "Estourou o número maximo de Itens pra geração de NF, conforme parâmetro MV_NUMITEN."
		Return .f.
	Endif
	if cOpcao == "2"
		if Empty(oGetPecas:aCols[Len(oGetPecas:aCols),fg_posvar("VS3_CODITE","aHeaderP")])
...
Static function LinhaSemQuantidade(nOpc,nCntFor)

	If lPediVenda .and. ; //Executado pelo Pedido de Orçamento
		nOpc == 3 .and. ; // Inclusão de Pedido
		!oGetPecas:aCols[nCntFor,len(oGetPecas:aCols[nCntFor])] .and.; // Não Deletado
		oGetPecas:aCols[nCntFor,FG_POSVAR("VS3_QTDITE","aHeaderP")] == 0 // Quantidade Requisitada
		Return .t.
	EndIf

	If !lPediVenda .and. ; // Não executado pelo Pedido de Orçamento
		!oGetPecas:aCols[nCntFor,len(oGetPecas:aCols[nCntFor])] .and.; // Não Deletado
		!Empty(oGetPecas:aCols[nCntFor,fg_posvar("VS3_GRUITE","aHeaderP")]) .and.; // Não existir peças
		oGetPecas:aCols[nCntFor,FG_POSVAR("VS3_QTDITE","aHeaderP")] == 0 // Quantidade Requisitada
		Return .t.
	EndIf
...
Local cMVMIL0011 := GetNewPar("MV_MIL0011","0")
Local lNaForte   := GetNewPar("MV_MIL0132",.F.)
Default nLinha := 0
...
If nLinha > 0
	...
	If cMVMIL0011 == "1" .And. oGetPecas:aCols[nLinha, FG_POSVAR("VS3_REC_WT","aHeaderP")] > 0
		return .t.
	elseif cMVMIL0011 == "1" .and. lPediVenda // se pedido de venda não deve mostrar
		return .f.
	elseif lNaForte .And. oGetPecas:aCols[nLinha, FG_POSVAR("VS3_REC_WT","aHeaderP")] == 0
	 	return .f.
	EndIf
EndIf
Return .t.
...
Local nPosGru := FG_POSVAR("VS3_GRUITE","aHeaderP")
Local nPosCod := FG_POSVAR("VS3_CODITE","aHeaderP")
Local nPosQtd := FG_POSVAR("VS3_QTDITE","aHeaderP")
//
M->VS1_PESOL  := 0
M->VS1_PESOB  := 0
	...
	For nX := 1 to nUsadoPX01
		...
		if ! lFeito
			if cField == "VS3_QTDEST" .and. GetNewPar("MV_MIL0011", "0") == "0"
				aColsPec[nLastIdx, nX] := OX001SLDPC(xFilial("SB2")+SB1->B1_COD+VS3->VS3_LOCAL)
...
If GetNewPar("MV_MIL0172",.F.) .and. FindFunction("OFA420031_Remuneracao") .and. VAI->(FieldPos("VAI_PSCREM")) > 0 // Trabalha com Remuneração?
	If OFA420051_ClienteConsideraRemuneracao( M->VS1_CLIFAT , M->VS1_LOJA )
...
Local lRet       := .t.
Local nVS3SEQVEN := FG_POSVAR("VS3_SEQVEN","oGetPecas:aHeader")
Local cSeqVEN    := ""
Local cCodVBM    := ""
Local nQtdMov    := 0
Local nCntFor    := 0
...
If nVS3SEQVEN > 0
	For nCntFor := 1 to len(oGetPecas:aCols)
 		If oGetPecas:aCols[nCntFor,FG_POSVAR("VS3_PROMOC","oGetPecas:aHeader")] == "1" // Promoção
			...
			If !Empty(cSeqVEN)
				...
				If !Empty(cCodVBM)
					...
					nQtdMov := oGetPecas:aCols[nCntFor,FG_POSVAR("VS3_QTDITE","oGetPecas:aHeader")]
					If nSldPro < nQtdMov
						aAdd(aProSemSld,{	oGetPecas:aCols[nCntFor,FG_POSVAR("VS3_SEQUEN","oGetPecas:aHeader")] ,;
											oGetPecas:aCols[nCntFor,FG_POSVAR("VS3_GRUITE","oGetPecas:aHeader")] ,;
											oGetPecas:aCols[nCntFor,FG_POSVAR("VS3_CODITE","oGetPecas:aHeader")] ,;
											oGetPecas:aCols[nCntFor,FG_POSVAR("VS3_DESITE","oGetPecas:aHeader")] ,;
											nQtdMov ,;
											nSldPro })
					Else
						If lUtiPromoc
							aAdd(aProComSld,{	cSeqVEN ,;
												nQtdMov ,;
												oGetPecas:aCols[nCntFor,FG_POSVAR("VS3_SEQUEN","oGetPecas:aHeader")] })
						EndIf
					EndIf
				EndIf
			EndIf
...
Local cCodVBM    := ""
Local nVS3SEQVEN := FG_POSVAR("VS3_SEQVEN","oGetPecas:aHeader")
Local nVS3PESPRO := FG_POSVAR("VS3_PESPRO","oGetPecas:aHeader")
...
If Empty(M->VS1_STATUS) .or. M->VS1_STATUS == "0" // Somente para Orçamento com Status 0=Digitado
	If GetNewPar("MV_MIL0172",.F.) .and. FindFunction("OFA420021_LevantaRemuneracao") .and. VS1->(FieldPos("VS1_PERREM")) > 0 // Trabalha com Remuneração?
