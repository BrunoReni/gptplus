...
===============================================================================
*/
Function OX005PERDES(cMarca,cCenRes,cGrupo,cCodite,nQtd,nPercent,lHlp,cCliente,cLoja,cTipVen,nValUni,nTipoRet,cForPag,cFormAlu,lFechOfi,lConMrgLuc,cConPromoc,dDatRefPD,nPERREM)
//
Local nCntFor
Local aRet := {}
...
If lVZO
	...
	if DBSeek(xFilial("VZO")+SA1->A1_COD+SA1->A1_LOJA)
		while xFilial("VZO")+SA1->A1_COD+SA1->A1_LOJA == VZO_FILIAL+VZO_CLIENT+VZO_LOJA
			...
			aAdd(aRet, OX005PDU(cMarca,cCenRes,cGrupo,cCodite,nQtd,nPercent,lHlp,cCliente,cLoja,cTipVen,nValUni,nTipoRet,cTipNeg,cForPag,cFormAlu,lFechOfi,lConMrgLuc,cConPromoc,dDatRefPD,nPERREM))
			DBSelectArea("VZO")
			DBSkip()
		enddo
...
//
return OX005PDU(cMarca,cCenRes,cGrupo,cCodite,nQtd,nPercent,lHlp,cCliente,cLoja,cTipVen,nValUni,nTipoRet,cTipNeg,cForPag,cFormAlu,lFechOfi,lConMrgLuc,cConPromoc,dDatRefPD,nPERREM)
/*
===============================================================================
###############################################################################
##+----------+------------+-------+-----------------------+------+----------+##
...
##|Descri‡„o | Retorna Politica de Desconto                                 |##
*/
Function OX005PDU(cMarca,cCenRes,cGrupo,cCodite,nQtd,nPercent,lHlp,cCliente,cLoja,cTipVen,nValUni,nTipoRet,cTipNeg,cForPag,cFormAlu,lFechOfi,lConMrgLuc,cConPromoc,dDatRefPD,nPERREM)
Local nCntFor    := 0
Local cQuery     := ""
...
For nCntFor := 1 to 4
	...
	While !(cSQLAlias)->(Eof())
		...
		//
		Do Case
			Case (cSQLAlias)->VEN_GRUITE + (cSQLAlias)->VEN_CODITE == cGrupo + cCodite .and. ( cConPromoc <> "0" ) // cConPromoc = Considera Promoção? ( 0=Não / 1=Sim e Não Acrescenta Percentual / 2=Sim e com Acrescimo de Remuneração )
				if nQtd >= (cSQLAlias)->VEN_QTDITE
					if nQtdTopo == 0 .or. nQtdTopo < (cSQLAlias)->VEN_QTDITE
						...
						EndIf
						If lVENSEQUEN
							aSequenVEN[1] := (cSQLAlias)->VEN_SEQUEN
						EndIf
					endif
				endif
			Case !Empty((cSQLAlias)->VEN_CODCAI)
				if SB5->B5_CODCAI == (cSQLAlias)->VEN_CODCAI
					...
					If lVENSEQUEN
						aSequenVEN[2] := (cSQLAlias)->VEN_SEQUEN
					EndIf
				endif
			Case !Empty((cSQLAlias)->VEN_GRUPEC)
				if cGrupo == (cSQLAlias)->VEN_GRUPEC
					...
					If lVENSEQUEN
						aSequenVEN[3] := (cSQLAlias)->VEN_SEQUEN
					EndIf
				endif
			Case !Empty((cSQLAlias)->VEN_GRUDES)
				if FM_PRODSBZ(SB1->B1_COD,"SB1->B1_GRUDES") == (cSQLAlias)->VEN_GRUDES
					aMaxDesc[4]  := (cSQLAlias)->VEN_PERDES
					aMargMin[4]  := (cSQLAlias)->VEN_MARMIN
					aFormulas[4] := cFormAlu
					If lVENSEQUEN
						aSequenVEN[4] := (cSQLAlias)->VEN_SEQUEN
					EndIf
				endif
			Case !Empty((cSQLAlias)->VEN_CLAFIN)
				if SBL->BL_ABCVEND+SBL->BL_ABCCUST == (cSQLAlias)->VEN_CLAFIN
					...
					If lVENSEQUEN
						aSequenVEN[5] := (cSQLAlias)->VEN_SEQUEN
					EndIf
				endif
			Case lMLF
				If !Empty((cSQLAlias)->VEN_MARPEC) .and. !Empty((cSQLAlias)->VEN_CODLIN) .and. !Empty((cSQLAlias)->VEN_CODFAM)
					if  cMarPec == (cSQLAlias)->VEN_MARPEC .and. cCodLin == (cSQLAlias)->VEN_CODLIN .and. cCodFam == (cSQLAlias)->VEN_CODFAM
						...
						If lVENSEQUEN
							aSequenVEN[6] := (cSQLAlias)->VEN_SEQUEN
						EndIf
						lContMlf := .f.
					endif
				else
...
===============================================================================
*/
Function OX005RETMIN(cMarca,cCenRes,cGrupo,cCodite,nQtd,nPercent,lHlp,cCliente,cLoja,cTipVen,nValUni,nTipoRet,cForPag,cFormAlu,lFechOfi,dDatRefPD,nPERREM)
Local aRetFunc := {}
Private lSBZ := ( SuperGetMV("MV_ARQPROD",.F.,"SB1") == "SBZ" )
...
MsSeek( xFilial("SB2") + SB1->B1_COD + FM_PRODSBZ(SB1->B1_COD,"SB1->B1_LOCPAD") )
//
aRetFunc := OX005PERDES(cMarca,cCenRes,cGrupo,cCodite,nQtd,nPercent,lHlp,cCliente,cLoja,cTipVen,0,3,cForPag,cFormAlu,lFechOfi,,,dDatRefPD,nPERREM)
