...
Static Function fGetDiagn( cAcidente )
	...
	If lAchou .And. lDiagnostico .Or. IsInCallStack( "MDTR832" ) //Caso ache o registro
...
Static Function fGetAtest( cAcidente )
	...
	If lAchou .And. lAtestado .Or. IsInCallStack( "MDTR832" ) //Caso ache o registro
...
Static Function fVerMaisRec( cAcidente )
	...
	Local dDtDiag := SToD( "" )
	Local dDtAtes := SToD( "" )
	Local lExclu  := IIf( IsInCallStack( "MDTR832" ), .F., !( INCLUI .Or. ALTERA ) ) //Verifica se é exclusão

	//Busca a data do atendimento do Diagnóstico
	If lDiagnostico
		dDtDiag := IIf( lExclu, SToD( "" ), M->TMT_DTATEN ) //Caso for exclusão do diagnóstico, pega a data como vazia
	Else
	...
	If lAtestado
		dDtAtes := IIf( lExclu, SToD( "" ), M->TNY_DTCONS ) //Caso for exclusão do atestado, pega a data como vazia
	Else
	...
	ElseIf !Empty( dDtDiag ) .And. !Empty( dDtAtes ) //Caso exista os dois, verifica qual o mais atual
		...
		Else
			cRet := "2"
		EndIf
	ElseIf Empty( dDtDiag ) .And. Empty( dDtAtes )
...
Static Function fGetDesDia( nOpc )
	...
	If aArea[ 1 ] <> "TMT"
		...
		RegToMemory( "TMT", .F., , .F. ) //Adiciona os registros da TMT a memória para pegar o código da SYP
	EndIf
