		...
		If nHrFim > 0 .And. nDtFim > 0
			...
			If aCols[n, nDtFim] > Date() .Or. (aCols[n, nDtFim] == Date() .And. aCols[n, nHrFim] > Time())
				...
				Help( NIL, 1, STR0114, NIL, STR0219, 1, 0, NIL, NIL, NIL, NIL, NIL, {STR0220} ) // "ATENCAO"###"Hora final gerada maior que a hora atual."###
																								// "O cálculo que atribui valor é realizado se baseando na hora 
...
If l103Exclui
	...
	BeginSQL Alias cAliSt9

		SELECT	T9_FILIAL, T9_CODBEM, T9_STATUS FROM %table:ST9%
		WHERE 	T9_SERIE	= %exp:cSerie%
		AND		T9_NFCOMPR 	= %exp:SD1->D1_DOC%
		AND		T9_FORNECE	= %exp:SD1->D1_FORNECE%
		AND		T9_LOJA		= %exp:SD1->D1_LOJA%
		AND		%NotDel%
