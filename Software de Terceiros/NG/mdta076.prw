...
Function MDTA076()
	...
	Else
		If SuperGetMV( "MV_NG2SEG", .F., "2" ) == "1"
			If lValid
				ShowHelpDlg( STR0003,;        //"ATENÇÃO"
							 { STR0006 }, 2,; //"Usuário não possui agenda médica cadastrada."
							 { STR0007 }, 2 ) //"Favor cadastrar agenda médica."
			EndIf
		Else
			ShowHelpDlg( STR0003,;        //"ATENÇÃO"
						 { STR0008 }, 2,; //"Não existe médico com agenda cadastrada."
						 { STR0007 }, 2 ) //"Favor cadastrar agenda médica."
		EndIf
	EndIf
	oTempMed:Delete()
			...
			If nRet == 2
				...
				If dbSeek( xFilial( "TMJ" ) + cMedico + DTOS( dDiaAtu ) + cHora )
					If Len( AllTrim( TMJ->TMJ_HRCHGD ) ) != 5
						ShowHelpDlg( STR0003, { STR0023 }, 2,; //"Atenção"###"O campo Horário de Chegada não foi informado."
									 { STR0024 }, 2 )          //"Favor informar o Horário de Chegada do Funcionário."
						lInfor := .T.
					EndIf
				EndIf
			EndIf
