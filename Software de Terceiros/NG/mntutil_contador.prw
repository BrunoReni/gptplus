...
//Variáveis de criação/deleção de tabelas temporárias para evitar
...
// contador. Ex.: Verificação de histórico, retorno de contador, etc.
// Antes de adicionar uma função aqui, verifique se atende a este
// requisito.
//---------------------------------------------------------------------
...
Posiciona no intervalo de valores válidos do historico (STP)

@param cVBEM	- Código do bem             - Obrigatório
@param nPOSCON	- Valor do contador         - Obrigatório
@param dVLEIT	- Data da leitura         	- Obrigatório
@param cVHORA	- Hora da leitura           - Obrigatório
@param nTIPOC	- Tipo do contador ( 1/2 )	- Obrigatório
...
/*/
	...
	±±³Fun‡…o    ³NGACUMEHIS³ Autor ³In cio Luiz Kolling    ³ Data ³11/03/2003³±±
	±±ÃÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄ´±±
	±±³Descri‡…o ³Procura o contador acumulado especial ( ANTES/DEPOIS )      ³±±
...
/*/
	...
	±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
	±±ÚÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄ¿±±
	±±³Fun‡…o    ³NGACUMEHI2³ Autor ³In cio Luiz Kolling    ³ Data ³11/03/2003³±±
	±±ÃÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄ´±±
	±±³Descri‡…o ³Procura o contador acumulado especial ( ANTES/DEPOIS )      ³±±
	±±ÃÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ´±±
...
/*/
	...
	±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
	±±ÚÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄ¿±±
	±±³Fun‡…o    ³NGACUMEHEX³ Autor ³In cio Luiz Kolling    ³ Data ³08/10/2009³±±
	±±ÃÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄ´±±
	±±³Descri‡…o ³Alimenta a variavel lExit e faz a consistencia do registro  ³±±
...
/*/
	...
	±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
	±±ÚÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄ¿±±
	±±³Fun‡…o    ³NGACUMAVET³ Autor ³In cio Luiz Kolling    ³ Data ³11/03/2003³±±
	±±ÃÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄ´±±
	±±³Descri‡…o ³Alementa o vetor do acumulado especial                      ³±±
	±±ÃÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ´±±
...
/*/
	...
	±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
	±±ÚÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄ¿±±
	±±³Fun‡…o    ³NGACUMHIST³ Autor ³In cio Luiz Kolling    ³ Data ³11/03/2003³±±
	±±ÃÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄ´±±
	±±³Descri‡…o ³Procura o contador acumulado e/ou projeto                   ³±±
	±±ÃÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ´±±
	±±³Parametros³cBEM     - C¢digo do bem                       - Obrigat¢rio³±±
	±±³          ³dDATA    - Data da leitura                     - Obrigat¢rio³±±
	±±³          ³cHORA    - Hora da leitura                     - Obrigat¢rio³±±
...
@param  nPOSCONT  , Numérico, Valor do contador.
@param  cVHORA    , Caracter, Hora da leitura.
@param  nTIPOC    , Numérico, Tipo do contador.
@param  [vNREPAC] , Array   , Vetor com os elementos não repassar.
@param  lGERAUT   , Lógico  , Gera O.S. automática.
@param  cTipoLan  , Caracter, Tipo lançamento.
@param  cFilTroc  , Caracter, Filial de troca de acesso.
@param  [lProcEst], Lógico  , Reprocessa estrutura do bem pai.
@param  [cOrigem] , Caracter, Rotina que está gerando o historico de contador.
@para   [aTrbEst] , Array   , Array possuindo as tabelas temporárias responsaveis por montar a estrutura do bem.
...
Function NGTRETCON( cBEM, dLEIT, nPOSCONT, cHORAL, nTIPOC, vNREPAC, lGERAUT, cTIPOLAN, cFilTroc, lProcEst, cOrigem, aTrbEst )
	...
	Local nACUMULPE   := 0
	Local lENTRADA    := .F. // Variavel que indica se Bem Filho não participou do incremento do contador
	...
	If nPOSCONT > 0
		...
		If dbSeek(cFilCon+cBEM)
					...
					If &(vARQCON[3]) == cFilHis .And. &(vARQCON[4]) == cBEM
						If &(vARQCON[5]) <= dLEIT
							If &(vARQCON[5]) == dLEIT .And. &(vARQCON[6]) == nPOSCONT
								...
								lRecalhist := .F. // data e contador são iguais ao lançamento anterior
			...
			Else
				...
				If &(vARQCON[3]) == cFilHis .And. &(vARQCON[4]) == cBEM
					...
					If &(vARQCON[5]) == dLEIT .And. &(vARQCON[6]) == nPOSCONT
						...
						// registro anterior para pegar diferença
						dbSkip(-1)
			...
			// Se a chamada for via transferência do ATF060 não precisa verificar estrutura pois Ativos
			// relacionados com bens/veículos que possuam estrutura não podem ser transferido por essa rotina,
			// dessa forma não precisa refazer essa validação.
			...
			If Len(aESTSTZ) > 0
				...
				For nn := 1 To Len(aESTREC)
					...
					If dbSeek(cFilCon+aESTREC[nn][1])
						...
						// VERIFICAR QUAL A POSI€ÇO QUE IRµ CAIR PARA PEGAR O ACUMULADO
						...
						//Define lista de filhos que será reprocessado o contador conforme o pai da estrutura.
						aAdd( aComponents, { aESTREC[nn,1], (vARQCON[2])->( Recno() ), nDiffFilho, lViraFilho, nPOSCONT, dLEIT, nTIPOC, .T., .T., Nil, cFilCon, cFilHis, .T., aESTREC[nn,6], aESTREC[nn,5] } )
			...
			/* Verifica se esta utlizando o MV_NGLANEX com a opção "A" e se o bem possui filhos que entraram posteriormente 
			   ao lançamento do contador. 			   
			   Caso exista componentes que entraram anterior a esta data já é tratado no IF. 
			*/ 
			ElseIf cUseLanex $ "A" .And. lProcEst 

				//Query para verificar e buscar os componentes que entraram na estrutura posterior ao lançamento deste contador.
				...
				While (cAliasSTZ)->( !EoF() )
					...
					If dbseek((cAliasSTZ)->TZ_FILIAL + cFather + (cAliasSTZ)->TZ_DATAMOV + (cAliasSTZ)->TZ_HORAENT)
						...
						// Atualiza a diferença do contador.
						nDifContad := nPosFilho - nPosPai

					EndIf

					aAdd( aComponents, { (cAliasSTZ)->TZ_CODBEM, (cAliasSTZ)->R_E_C_N_O_ , nDifContad, .F., nPosCont, StoD((cAliasSTZ)->TZ_DATAMOV), 1, .F., .T., Nil, cFilCon, cFilHis,.T., (cAliasSTZ)->TZ_TEMCONT, cFather } )
...
/*/
	...
	±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
	±±ÚÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄ¿±±
	±±³Fun‡…o    ³NGATUCONT ³ Autor ³In cio Luiz Kolling    ³ Data ³11/03/2003³±±
	±±ÃÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄ´±±
	±±³Descri‡…o ³Atualiza o contador                                         ³±±
	±±ÃÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ´±±
	...
	±±³          ³nVARDT9  - Valor da varia‡Æo dia               - Obrigat¢rio³±±
...
/*/
	...
	±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
	±±ÚÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄ¿±±
	±±³Fun‡Æo    ³NGGRAVAHIS³ Autor ³In cio Luiz Kolling    ³ Data ³11/03/2003³±±
	±±ÃÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄ´±±
	±±³Descri‡…o ³Gera registro de historico ( STP )                          ³±±
	±±ÃÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ´±±
	...
	If nTIPOC = 1
		...
		If dbseek(cFICONG+cVBEM)
			...
			If !dbSeek(cFIHISG+cVBEM+DTOS(dVDLEIT)+cVHORA)
				...
				//Variação dia nao pode ser menor que 1
				If nVVARD < 1
					nVVARD := 1
				EndIf
...
Identifica o fonte chamador para função NGGravaHis, quando não informado

@since  21/09/17
@author Felipe Nathan Welter
@version P12
@return  cOrigem: código do programa origem
/*/
//---------------------------------------------------------------------
...
@param   nTIPOC,    Numérico, Tipo do contador ( 1 / 2 )
@param   nACUMSTP,  Numérico, Contador acumulado
@param   lRECAL,    Lógico,   Indica se dever  incluir na array o
                              Contador Acumulado do STP ou o contador
do parâmetro nACUMSTP\
@param   lESTOUR,   Lógico,   Indica se dever  considerar o estouro
                              da variação dia
@param   [cFilNov], Caracter, Código da filial

@return nVarDia, Numérico,    Cálculo da variação dia.
/*/
//-------------------------------------------------------------------
FUNCTION NGVARIADT(cCODBEM,dDATALEI,nTIPOC,nACUMSTP,lRECAL,lESTOUR,cFilNov,cFilMulEmp)
	...
	//Atualiza filial se enviado como parâmetro na chamada da função
...
Realiza a chamada do processo de recalculo do histórico de contadores, utilizando-se de threads ou não.
@type function
...
@param [cVBem]      , Caracter, Código do bem.
@param [nVDIF]      , Númerico, Valor a ser somado no acumulado.
@param nVPOSC       , Númerico, Posição do contador.
@param dDLEIT       , Data    , Leitura do contador.
@param nTIPOC       , Númerico, Tipo do contador.
@param lSKIP        , Lógico  , Indica se deve selecionar um registro anterior ao atual ( bem/filho).
@param [lVIRA]      , Lógico  , Indice se houve virada.
@param lATUST9      , Lógico  , Indice se sempre atualiza ST9 ao final do processo.
@param [nACUMEX]    , Númerico, Aculumador na alteracao do historico.
...
@param [aEquipments], Array   , Lista de bens para reprocessamento em lote dos contadores. Quando este parametro
	for utilizado, dispensa o preenchimento dos parâmetros anteriores. Mesmo assim caso sejam informados juntamente
será priorizado o conteudo do parâmetro a aEquipments
...
Function NGRECALHIS( cVBem, nVDIF, nVPOSC, dDLEIT, nTipoC, lSKIP, lVIRA, lATUST9, nACUMEX, cFilCon, cFilHis, aEquipments )
	...
	Local lStartJob := IsInCallStack( 'MNTA435' ) .Or.;
...
@param aEquipments, Array   , Contém registros para reprocessamento em lote.
					...
					[13] - Se atualiza TQS após recálculo
					[14] - Contador controlado por: (P-Pai ,I-Imediato, N-Não tem, S-Proprio)
					[15] - Código do bem Pai
@return .T.
/*/
//----------------------------------------------------------------------------------------------------------
Function NGRECALREG( lJob, cEmpAtu, cFilAtu, aEquipments )
	...
	If lJob
		// Adição do sleep, pois há cenários em que o processo demora muito para finalizar, então ao começar o job algumas informações não existem
		Sleep( 6000 )
	EndIf
	...
	For nX := 1 to Len( aEquipments )
		...
		cVBem     := aEquipments[nX,1]  // Código do Bem
		nVDIF     := aEquipments[nX,3]  // Diferença somada ao acumulado
		...
		If Len( aEquipments[nX] ) >= 14
			...
			cBemPai   := aEquipments[nX,15] // Código do bem PAI
		...
		cLanex := ""
		If nTIPOC == 1
			cLanex := NGUSELANEX( STP->TP_CODBEM, .F., STP->TP_DTLEITU, STP->TP_HORA ) // verifica parâmetro
		EndIf
		...
		While !Eof() .And. cFilHist == &(aARQHist[2]) .And. &(aARQHist[3]) == cVBem
			...
			//------------------------------------------------------------------------------
			// Validação específica quando for componente que acaba de entrar na estrutura
			// não incrementar o contador, devendo ser igual ao anterior a este registro
			//------------------------------------------------------------------------------
			...
			If !lAcumulou
				...
				If cTipLanAtu == "Q" .Or. cTipLanAtu == "V"
					...
					dbSkip(-1)
					If &(aARQHist[2]) == cFilHist .And. &(aARQHist[3]) == cVBem
						//Se o atual for uma Quebra, o acumulado atual é o mesmo do anterior
						If cTipLanAtu == "Q"
							nACUMSTP := &(aARQHist[5])
							lAcumulou := .T.
						ElseIf cTipLanAtu == "V" //Caso o atual seja uma virada, verifica o registro anterior para fazer o acumulo do contador
							...
							If dbSeek(cFilHist+cVBem)							
								...
								Else
									/*	Quando ocorre uma VIRADA e o bem não possui contador proprio, não poderá ser utilizado o campo T9_LIMICON para o calculo
										Será realizado a busca pelo pai da entutura e realizado o seguinte calculo:
										Diferença := Acumulado Pai Posterior a Virada - Acumulado Pai Anterior a Virada
									*/
									nACUMSTP  := &(aARQHist[5])
									BeginSql Alias cAliasSTZ
										SELECT TZ_BEMPAI, TZ_TEMCONT					
											FROM %Table:STZ%
												...
												AND ( 
												// Quando o pneu já saiu da estrutura, onde a dat aem questão precisa estar entre a entrega e saida
													(	%exp:DtoS(dDatLanAtu)+cHorLanAtu%  BETWEEN TZ_DATAMOV || TZ_HORAENT AND TZ_DATASAI || TZ_HORASAI AND TZ_TIPOMOV = 'S') 
												OR 
												// Quando a data em questão é maior que a entrada do pneu ele ainda não saiu da estrutura.
			...
			// quando parametro LANEX está habilitado, altera todos os registros "C"
			// conforme lançamento anterior tipo A Q ou V
...
Retorna a posição do contador ou contador acumulado anterior do tipo
configurado no parâmetro LANEX, Q, V ou I.

@param cCodBem, string, Código do bem
@param dData, date, Data do último registro a ser procurado
@param cHora, Hora do último registro a ser procurado
@param [cLanex], string, conteúdo do parâmetro NGLANEX
@param [lCont], boolean, .T. para retornar a posição do contador, .F. para acumulado
@param [cFilStp], string, campo filial da tabela stp
@param [cFather], string, código do pai (utilizado para pneus)
...
Function NGGetCont( cCodBem, dData, cHora, cLanex, lCont, cFilStp, cFather )
	...
	// Verifica se foi enviado o pai da estrutura, caso não, tenta busca-lo.
	If Empty(cFather)
	...
	/*/ Busca a posição ou acumulado do contador com data e hora menor a enviada no paramentro.
	Se o bem tiver contador proprio, a busca será feita por seu código.
	Se o bem for controlado pelo pai da estrutura ou pai imediato e o mesmo estiver na estrutura, a posição do
	contador será o bem pai e o acumulado dele mesmo.
	Se o bem for controlado pelo pai da estrutura ou pai imediato e o mesmo NÂO estiver na estrutura, a posição do
	...
	If !(cAliasQry)->(Eof()) // Se encontrou movimentação com data anterior
		If lCont // Retorna a posição do contador
...
Valida o lançamento no histórico.

@author  Inacio Luiz Kolling
@since   11/03/2003
@version p12

@param cVBEM, Código do bem, caractere
@param dVDATA, Data da leitura, date
@param nVPOSCONT, Valor do contador, numérico
@param cHORA, Hora da leitura, caractere
@param nTIPOC, Tipo do contador ( 1/2 ), numérico
@param nVITEM, Item do GetDados, numérico
@param lGETVAR, Indica se a saída de erro na tela, lógico
@param cFilTroc, Filial de troca de acesso, caractere
@param cCombus, Código do Combustível, caractere

@return Lógico ou Array, se lGETVAR = .T. retorna Lógico se não retorna aVETOR[1] - lógico, aVETOR[2] - mensagem de erro ou nulo
/*/
//-------------------------------------------------------------------
Function NGCHKHISTO( cVBEM, dVDATA, nVPOSCONT, cHORA, nTIPOC, nVITEM, lGETVAR, cFilTroc, cCombus, lAskAuto )
	...
	If !lPROBPA
		...
		If aVETOR[1] .And. !NGBlCont( cVBEM, .F. )  // Se controla apenas por abastecimento
			...
			cMENSA := fVldLanex(cFilHis, cVBEM, dVDATA, cHORA, nVPOSCONT )
			If !Empty(cMENSA)
				lRETHIS := .F.
				...
				Else
					...
					If &(vARQHI[2]) == cFilHis .And. &(vARQHI[6]) == cVBEM;
						...
						If nVPOSCONT < &(vARQHI[5])
							...
							If !Bof()
								If &(vARQHI[2]) == cFilHis .And. &(vARQHI[6]) == cVBEM
									// Verifica se é menor que o lançamento Anterior
									If nVPOSCONT < &(vARQHI[5])
									...
									Else // Se for Maior ou Igual ao Anterior
										aAreaHist := GetArea()
										dbSkip()
										// Se não houver mais lançamentos para o mesmo Bem, então este é o último e está Ok
										// Caso contrário, verifica se está Ok com o registro Posterior
										If &(vARQHI[2]) == cFilHis .And. &(vARQHI[6]) == cVBEM
											// Se a HORA Atual for MAIOR que a do Posterior, então o CONTADOR deve ser MAIOR
											If cHORA > &(vARQHI[4]) .And. nVPOSCONT < &(vARQHI[5])
												...
												cMENSA := cMENSA +cMENSO
												lRETHIS := .F.
												// Se a HORA Atual for MENOR que a do Posterior, então o CONTADOR deve ser MENOR
											ElseIf cHORA < &(vARQHI[4]) .And. nVPOSCONT > &(vARQHI[5])
												...
												cMENSA := cMENSA +cMENSO
												lRETHIS := .F.
												// Se a HORA Atual IGUAL a do Posterior, então o CONTADOR deve ser IGUAL
											ElseIf cHORA == &(vARQHI[4]) .And. nVPOSCONT <> &(vARQHI[5])
												cMENSA := cMENSP +STR0024+cTIPOC+STR0025+CRLF+CRLF //### //"Ja existe um lancamento para o contador"###"com as caracteristicas:"
												cMENSA := cMENSA +STR0026+Dtoc(&(vARQHI[3]))+CRLF // "Data Lancamento..: "
												cMENSA := cMENSA +STR0027+str(&(vARQHI[5]),9)+CRLF // "Contador................: "
								...
								Else
									...
									Else
										...
										If !NGBlCont( cVBEM, .F. ) // Se controla apenas por abastecimento
											While !Bof()// valida com o registro de Abastecimento, Quebra, Virada ou Inclusão anterior
												If !(&(vARQHI[7]) $ "AQVI")
													Dbskip(-1)
												Else
													Exit
	...
	// PE para incluir uma nova validação na checagem do historico de contador.
	...
	If lGETVAR
		...
		If lCanib
			If nVPOSCONT > &(vARQHI[5]) .And. (DtoS(dVDATA)+cHORA) > (DtoS(&(vARQHI[3]))+&(vARQHI[4]))
				MsgInfo(STR0127+CRLF+; // "Procedimento não permitido para Bens com status de Canibalismo, pois o mesmo não poderá receber reporte de contador."
				STR0128+Str(&(vARQHI[5]),9),STR0044) // "NAO CONFORMIDADE"	//"Para realizar esse procedimento é necessário informar o contador do último lançamento :"
				Return .F.
			EndIf
		EndIf
...
/*/
	...
	±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
	±±ÚÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄ¿±±
	±±³Fun‡…o    ³NGCONTRET ³ Autor ³In cio Luiz Kolling    ³ Data ³11/03/2003³±±
	±±ÃÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄ´±±
	±±³Descri‡…o ³Valida os compos de retorno de contador (1/2)               ³±±
	±±ÃÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ´±±
...
/*/
	...
	±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
	±±ÚÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄ¿±±
	±±³Fun‡…o    ³NGTBEMPAI ³ Autor ³In cio Luiz Kolling    ³ Data ³11/03/2003³±±
	±±ÃÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄ´±±
	±±³Descri‡…o ³Valida os compos de retorno de contador (1/2)               ³±±
	±±ÃÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ´±±
...
/*/
	...
	±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
	±±ÚÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄ¿±±
	±±³Fun‡…o    ³NGTBEMCON ³ Autor ³In cio Luiz Kolling    ³ Data ³11/03/2003³±±
	±±ÃÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄ´±±
	±±³Descri‡…o ³Alimenta vari veis e retorna o c¢digo do bem / contador     ³±±
	±±ÃÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ´±±
	±±³Parametros³cVBEM     - C¢digo do bem                      - Obrigatorio³±±
	±±ÃÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ´±±
...
Checa a variação dia ( intervalo de valores válidos )

@author  Inácio Luiz Kolling
@since   11/03/2003
@version P11/P12
@param   cVBEM,      Caracter, Código do bem
@param   nPOSCON,    Numérico, Valor do contador
@param   dVLEIT,     Data,     Data da leitura
@param   cVHORA,     Caracter, Hora da leitura
@param   nTIPOC,     Numérico, Tipo do contador ( 1/2 )
@param   lSAIDA,     Lógico,   Indica se a sa¡da de erro na tela
@param   [nITEMC],   Numérico, Item do getdados
@param   [cFilTroc], Caracter, Filial de troca de acesso
...
Function NGVALIVARD(cVBEM,nPOSCON,dVLEIT,cVHORA,nTIPOC,lSAIDA,nITEMC,cFilTroc)
	...
	If ExistBlock("NGCHECVAR")
		...
		// Tratativa realizada para não afetar os usuário que já utilizam o P.E.
		If ValType( xValor ) == 'A'
			aVETCHK := xValor
		Else
			lRETCHK := xValor
...
@param cVBEMV   - Código do bem - Obrigatório
@param dDTLEIT  - Data da leitura - Obrigatório
@param nCONTAV  - Valor do contador - Obrigatório
@param cHORAV   - Hora da leitura - Obrigatório
@param nTIPOC   - Tipo do contador ( 1/2 ) - Obrigatório
@param cFilTroc - Filial de troca de acesso - Não obrigatório
@param lGETVAR  - Define se o retorno será lógico ou array - Não obrigatório
@param lRetMen  - Retorno para compor a memo do reprocessamento de contador próprio (mnta876) - Não obrigatório
...
Function NGCHKVIRAD(cVBEMV,dDTLEIT,nCONTAV,cHORAV,nTIPOC,cFilTroc,lGETVAR,lRetMen)
	...
	Else
		...
		If (SuperGetMv("MV_NGPERCV", .F., 0) > 0)
			...
			If(nPercV < SuperGetMv("MV_NGPERCV", .F., 0))
				cMENV3 :=  STR0157 + chr(13); //"Contador anterior não alcançou o mínimo"
				...
				+ STR0028 + &(aVETVIR[6]) //"Hora Lancamento..: "
				cMENSAV := cMENSP+cMENV3+cMENV1
				cMenRet := STR0157 + " " + STR0158 + CRLF +; //"Contador anterior não alcançou o mínimo percentual para realizar a virada:"
	...
	If lRetMen//Retorno para compor a memo do reprocessamento de contador próprio (mnta876)
...
/*/
	...
	±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
	±±ÚÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄ¿±±
	±±³Fun‡…o    ³NGCRETROAT³ Autor ³In cio Luiz Kolling    ³ Data ³14/03/2006³±±
	±±ÃÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄ´±±
	±±³Descri‡…o ³Acerto de contador acumulado de saida de um componente re-  ³±±
	±±³          ³troativo em rela‡Æo ao ultimo reporte de contador           ³±±
	±±ÃÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ´±±
...
Function NGCRETROAT(cVCOMP,dVDATA,cVHORA,nCONT1,nCONT2)
	...
	For nVE := nPLI To nPFL
		...
		If &(vVETCHIS[nVE,2]) = xFILIAL(vVETCHIS[nVE,1]) .And. &(vVETCHIS[nVE,3]) = cVCOMP
			...
			// verifica se o lancamento de contador ‚ retroativo
			dbSelectArea("STZ")
			dbSetOrder(02)
...
@param nPosCont, Posição do contador, numérico
@param lGetVar, Se Verdadeiro o retorno é lógico se não retorna Array e não mostra mensagens em Tela, lógico
@param cCombus, Código do Combustível, caractere
@param nTIPC, Tipo do contador ( 1/2 ), numérico
@param cFiliST9, Filial ST9, caractere
@param lPerg, Perguntar se "Deseja confirmar?", lógico

@return Lógico ou Array, se lGETVAR = .T. retorna Lógico se não retorna aVETRE[1] - lógico, aVETRE[2] - mensagem de erro ou nulo
/*/
//-------------------------------------------------------------------
Function NGAUTONOMIA(cBem,dData,cHora,nPosCont,lGetVar,cCombus,nTIPC,cFiliST9, lPerg)
	...
	/*--------------------------------------------------+
	| 1- Não realiza consistencia de autonomia.         |
	...
	Else
		...
		/*Se a mensagem nao for mostrada em tela e não estiver configurado para bloquear
		a autonomia o retorno sempre será verdadeiro*/
				...
				While !EoF() .And. cFiliST9 == ST9->T9_FILIAL .And. cBem == ST9->T9_CODBEM
					If ST9->T9_SITBEM == "A"
						If FieldPos('T9_MEDIA') > 0 .And. FieldPos('T9_CAPMAX') > 0
							//Caso tenha transferência localiza a filial do último registro de contador e atualiza o cFiliST9
							dbSelectArea("TQ2")
							dbSetOrder(1)
							...
							If lCalc
								...
								If nKmA > nKmB  .And. nPrimCon > 0
									...
									If nTipLAnB = "I" .And. lGetVar .And. FunName() $ "MNTA655/MNTA700/MNTA875"
										lMsg := .T.
										If !MsgYesNo(STR0097+AllTrim(Str(nTIPOCO))+CRLF+STR0122,STR0067)// "Essa posição do contador superou a autonomia, porém é o primeiro abastecimento.Confirma?"//
											lRETURN := .F.
										EndIf
									EndIf
									...
									If !lMsg
										...
										If lMsgAuton
											cMensag:= STR0097+AllTrim(Str(nTIPOCO))+CRLF+STR0093+CRLF+; // "Essa posição do contador superou a autonomia do veículo."
													STR0129+"("+AllTrim(Str(nPercAuto))+"%)."+CRLF+; // "Entretanto está dentro do percentual tolerável "
													IIF(lPerg, STR0130 + Chr(10), "") + Chr(10)+; // "Deseja confirmar?"
													STR0094+AllTrim((cBem))+Chr(10)+; // "Veículo.........: "
...
±±³Fun‡…o    ³ NGRETSTP  ³ Autor ³ Evaldo Cevinscki Jr. ³ Data ³05/12/2006³±±
±±ÃÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄ´±±
±±³Descri‡…o ³Retorna posicao anterior,posterior do contador              ³±±
±±ÃÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ´±±
...
Function NGRETSTP(cFilFro,cFrota,dData,cHora)
	...
	If FunName() $ "MNTA655,MNTA656,MNTA700"
		cQuery += " AND (TP_TIPOLAN='A' OR TP_TIPOLAN='I' OR TP_TIPOLAN='Q') AND D_E_L_E_T_<>'*' "
		cQuery += " AND TP_FILIAL = '"+cFilFro+"' "
	Else
...
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
±±ÚÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄ¿±±
±±³Fun‡…o    ³NGHISTRETR³ Autor ³In cio Luiz Kolling    ³ Data ³13/08/2007³±±
±±ÃÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄ´±±
±±³Descri‡…o ³Consiste se o lancamento do historico/contador ‚ retroativo ³±±
±±ÃÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ´±±
...
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
±±ÚÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄ¿±±
±±³Fun‡„o    ³NGREFMOVR ³ Autor ³Incaio Luiz Kolling    ³ Data ³14/08/2007³±±
±±ÃÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄ´±±
±±³Descri‡„o ³Processa movimentacoes retroativas dos componentes          ³±±
...
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
±±ÚÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄ¿±±
±±³Fun‡…o    ³NGCHKCINC ³ Autor ³Evaldo Cevinscki Jr.   ³ Data ³28/08/2007³±±
±±ÃÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄ´±±
±±³Descri‡…o ³Checa se a data e hora de lancamento de contador nao eh     ³±±
...
Function NGCHKCINC(cVBEM,dVDATA,nVPOSCONT,cHORA,nTIPOC,lGETVAR,cFilB)
	...
	If dbseek(cFilB+cVBEM+"I")
		...
		If &(vARQHI[3]) == dVData
			If &(vARQHI[4]) > cHora
				cMENSA := STR0123 + CRLF+; //"Horário de lançamento é inferior ao informado na inclusão do Contador no Bem."
				STR0124 + cData+CRLF+; //"Inclusão: "
				STR0125 + &(vARQHI[4])+CRLF //"Hora......: "
				lRETHIS   := .F.
			EndIf
		ElseIf &(vARQHI[3]) > dVData
			cMENSA := STR0126 + CRLF+; //"Horário e Data de lançamento são inferiores ao informado na inclusão do Contador no Bem."
			STR0124 + cData+CRLF+; //"Inclusão: "
			STR0125 + &(vARQHI[4])+CRLF //"Hora......: "
			lRETHIS := .F.
		EndIf
...
@param cPosto   - Codigo do Posto                      -Obrigatorio
@param cLoja    - Codigo da Loja do Posto              -Obrigatorio
@param cTanque  - Codigo do Tanque do Posto            -Obrigatorio
@param cBomba   - Codigo da Bomba do Tanque            -Obrigatorio
@param cData    - Data da ocorrencia                   -Obrigatorio
@param cHora    - Hora da ocorrencia                   -Obrigatorio
@param cTipoLan - Tipo de Lancamento 1=Manual/2=Lote   -Obrigatorio
@param nPosCon  - Posicao do contador da bomba na data/hora
@param nQuanti  - Quantidade a ser incrementada no ultimo Contador
@param cNAbast  - Numero do abastecimento              -Nao Obrig.
@param cNumDoc  - Numero do documento de saida         -Nao Obrig.
@param cSerieDoc - Serie do documento de saida         -Nao Obrig.
...
/*/
//---------------------------------------------------------------------
Function NGIncTTV(cPosto,cLoja,cTanque,cBomba,dData,cHora,cTipoLa,nPosCon,nQuanti,cNAbast,cNumDoc,cSerieDoc)//,cFunc)
	Local cMotivo   := "1"
	Local nAcumCo   := 0
	Local nUltPos   := 0
	Local aAreaTTV  := {}
	...
	If dbSeek(xFilial("TQJ")+cPosto+cLoja+cTanque+cBomba)
		...
		If TTV->( !EoF() ) .And. FWxFilial( 'TTV' ) == TTV->TTV_FILIAL .And. TTV->TTV_POSTO == cPosto .And. TTV->TTV_LOJA == cLoja .And.;
			...
			// Função usada para realizar o recálculo dos registros após o incluido na TTV
			NgRecTTV( { cPosto, cLoja, cTanque, cBomba, dData, cHora }, 3 )

			cAliasQry := GetNextAlias()

			BeginSql Alias cAliasQry
				SELECT TTA.TTA_CONBOM,
					   TTA.R_E_C_N_O_ AS RECNO
					FROM %Table:TQN% TQN
				INNER JOIN %Table:TTA% TTA
					ON ( TTA.TTA_FOLHA  = TQN.TQN_NOTFIS
					 AND TTA.TTA_FILIAL = %exp:cFilTTA%
					 AND TTA.TTA_POSTO  = %exp:cPosto%
					 AND TTA.TTA_LOJA   = %exp:cLoja%
					 AND TTA.TTA_TANQUE = %exp:cTanque%
					 AND TTA.TTA_BOMBA  = %exp:cBomba%
					 AND TTA.%NotDel% )
				WHERE TQN_FILIAL = %exp:cFilTQN%
				  AND TQN_NABAST = %exp:TTV->TTV_NABAST%
				  AND TQN.%NotDel%
			EndSql
...
Function NGDelTTV()
	...
	If dbSeek(xFilial("TQJ")+cPosto+cLoja+cTanque+cBomba)
		...
		// Função usada para realizar o recálculo dos registros após o deletado na TTV
...
/*/
	...
	±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
	±±ÚÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄ¿±±
	±±³Funcao    ³NGAltTTVQnt³ Autor ³Vitor Emanuel Batista ³ Data ³09/09/2009³±±
	±±ÃÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄ´±±
	±±³Descricao ³ Altera Consumo em um registro da TTV                       ³±±
	±±ÃÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ´±±
	±±³Parametros³ cPosto  - Codigo do Posto                      -Obrigatorio³±±
	±±³          ³ cLoja   - Codigo da Loja do Posto              -Obrigatorio³±±
	±±³          ³ cTanque - Codigo do Tanque do Posto            -Obrigatorio³±±
	±±³          ³ cBomba  - Codigo da Bomba do Tanque            -Obrigatorio³±±
	±±³          ³ cData   - Data da ocorrencia                   -Obrigatorio³±±
	±±³          ³ cHora   - Hora da ocorrencia                   -Obrigatorio³±±
	±±³          ³ nQuanti - Quantidade a ser incrementada no ultimo Contador ³±±
	±±ÃÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ´±±
	±±³ Uso      ³SIGAMNT                                                     ³±±
	±±ÀÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ±±
	±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
	ßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßß
/*/
Function NGAltTTVQnt(cPosto,cLoja,cTanque,cBomba,dData,cHora,cTipoLa,nQuanti)
	...
	If AliasInDic("TTV")
		...
		If dbSeek(xFilial("TQJ")+cPosto+cLoja+cTanque+cBomba)
			...
			If dbSeek(xFilial("TTV")+cPosto+cLoja+cTanque+cBomba+DtoS(dData)+cHora) .And. nQuanti <> TTV->TTV_CONSUM
				...
				// Função usada para realizar o recálculo dos registros após o alterado na TTV
...
/*/
	...
	±±³Fun‡…o    ³NGCHKLIMVAR³ Autor ³In cio Luiz Kolling    ³ Data ³05/08/2010³±±
	±±ÃÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄ´±±
	±±³Descri‡…o ³Validacao do limite da variacao dia                          ³±±
...
/*/
	...
	±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
	±±ÚÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄ¿±±
	±±³Fun‡…o    ³NGVALABAST  ³ Autor ³ Marcos Wagner Junior  ³ Data ³28/10/2010³±±
	±±ÃÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄ´±±
	±±³Descri‡…o ³Validacoes do abastecimento                                 ³±±
	±±ÃÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ´±±
	±±³Parametros³                                                            ³±±
	±±ÃÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ´±±
	±±³Retorna   ³.T. ou .F.                                                  ³±±
...
Function NGVALABAST(cPosto,cLoja,cTanque,cBomba,dData,cHora,_lTemaCols,_lDel)
	...
	If !lRet
		...
		STR0146+DTOC(dData)+CRLF+STR0147+cHora, 1, 0, NIL, NIL, NIL, NIL, NIL, {STR0031} ) // "ATENCAO"###"Já existe um lançamento com essas características:"
...
Função que incrementa ou decrementa o KM para um Pneu, de
acordo com um registro na STP

@param cPneu, string, Código do Pneu
@param dData, date, Data da leitura do contador
@param cHora, string, Hora da leitura do contador
@param lDel, boolean, Indica se operação é exclusão, decrementando o TQS
@param nKmDif, numérico, Diferença de contador para o registro atual
...
Function NGKMTQS(cPneu,dData,cHora,lDel,nKmDif)
	...
	Local cVida  := "1" //Vida que será incrementada
	Local nVida  := 0  //Vida que será incrementada
	...
	//--------------------------------------------------------
	// Indifica se a operação é exclusão, decrementando o TQS
	//--------------------------------------------------------
	Default lDel   := .F.

	//---------------------------------------
	// Diferença de contador para a operação
	//---------------------------------------
	Default nKmDif := 0

	//--------------------------------------------------------
	// Verifica se Ambiente possui TQS e Bem informado é Pneu
	//--------------------------------------------------------
	dbSelectArea("ST9")
	dbSetOrder(1)
	...
	//------------------------------------------------------------
	// Pega contador anterior ao informado para fazer a diferença
	//------------------------------------------------------------
	dbSkip(-1)
	...
	//------------------------------------------------------------
	// Se o Contador Anterior for o mesmo que o informado não é
	// necessário incrementar TQS pois ela já foi incrementada
	//------------------------------------------------------------
	If nKmAtu == nKmAnt

		//--------------------------------------------------------------
		// Caso o registro de entrada na estrutura for alterada deverá
	...
	//-------------------------------------------
	// Diferança do KM Atual para o Anterior
	//-------------------------------------------
	nKmDif := nKmAtu - nKmAnt

	//----------------------------------------------------------------
	// Verifica se Contador é o último informado
	// Quando não é o último verifica se deve incrementar a banda
	//----------------------------------------------------------------
	dbSelectArea("STP")
	dbSkip(2)
	...
	If nKmDif > 0
		...
		If nVida > 0
			...
			//-----------------------------------------------
			// Se a operação é exclusão, decrementando o TQS
			//-----------------------------------------------
			If lDel
				nKmDif *= -1
			EndIf
	...
	//----------------------------------------------------------------------------------------------------
	// Ponto de Entrada para realizar algum processo específico para a Banda ou mesmo o histórico
	//----------------------------------------------------------------------------------------------------
...
@para  [aTrbEst] , Array   , Array possuindo as tabelas temporárias responsavel por montar a estrutura do bem.
							[1] tabela temporaria do pai da estrutura - cTRBS
							[2] tabela temporaria do pai da estrutura - cTRBF
							[3] tabela temporaria do eixo suspenso    - CTRBEixo
...
Function NGMARKSUSP(_cTpCont ,aTrbEst)
	...
	If lCriaTela
		...
		Define MsDialog oDlg1 Title STR0148+_cTpCont From 300,120 To 480,640 Of oMainWnd Pixel COLOR CLR_BLACK //"Indique os eixos suspensos que não terão reporte de contador "
...
@param Integer nDifer: diferença do acumulado reportado para o pai
		...
		If aSCan(aBens,{|x| x == _aESTSTZ[nI,7] }) == 0
			...
			If dbSeek(xFilial("ST9")+_aESTSTZ[nI,7])
				...
				While !Eof() .And. xFilial("TQ1") == TQ1->TQ1_FILIAL .And. ST9->T9_CODFAMI == TQ1->TQ1_DESENH .And. ST9->T9_TIPMOD == TQ1->TQ1_TIPMOD
					...
					// Adiciona apenas eixo suspenso que pertença ao primeiro nível da estrutura (tratativa temporária)
			...
			// Ajuste temporário para não ocorrer erro no ambiente do cliente, será aberto nova S.S. para tratar
			// eixo suspenso. André Felipe Joriatti
			If .F.
...
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
±±ÚÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄ¿±±
±±³Fun‡…o    ³NGBUSCONTHI³ Autor ³In cio Luiz Kolling    ³ Data ³11/11/2004³±±
±±ÃÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄ´±±
±±³Descri‡…o ³Pesquisa lancamento do historico do contador e/ou projeta    ³±±
±±ÃÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ´±±
±±³Parametros³cVBEM     - C¢digo do bem                       - Obrigatorio³±±
±±³          ³dVDATA    - Data da leitura                     - Obrigat¢rio³±±
±±³          ³cHORA     - Hora da leitura                     - Obrigat¢rio³±±
...
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
±±ÚÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄ¿±±
±±³Fun‡…o    ³NGBUSTPIN1³ Autor ³In cio Luiz Kolling    ³ Data ³11/11/2004³±±
±±ÃÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄ´±±
±±³Descri‡…o ³Projeta contadores quando nACUMULA = 0 e nACUMULC > 0 e     ³±±
...
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
±±ÚÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄ¿±±
±±³Fun‡…o    ³NGBUSTPIN2³ Autor ³In cio Luiz Kolling    ³ Data ³11/11/2004³±±
±±ÃÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄ´±±
±±³Descri‡…o ³Projeta contadores quando nACUMULA > 0 e nACUMULC = 0 e     ³±±
±±³          ³e cHORA > cHORAB ou quando nACUMULA > 0 e nACUMULC > 0 e    ³±±
±±³          ³e cHORA < cHORAB                                            ³±±
...
/*/
	...
	±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
	±±ÚÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄ¿±±
	±±³Fun‡…o    ³NGACOPCONT ³ Autor ³Inacio Luiz Kolling   ³ Data ³01/12/2006³±±
	±±ÃÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄ´±±
	±±³Descri‡…o ³Consistencia do tipo do contador                            ³±±
	±±ÃÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ´±±
...
/*/
	...
	±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
	±±ÚÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄ¿±±
	±±³Fun‡…o    ³ NGCONTAP  ³ Autor ³ Evaldo Cevinscki Jr. ³ Data ³12/11/2007³±±
	±±ÃÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄ´±±
	±±³Descri‡…o ³Checa contador informado em relacao ao anterior e posterior.³±±
	±±ÃÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ´±±
	±±³ Uso      ³ SigaMNT                                                    ³±±
	±±ÃÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ´±±
	±±³         ATUALIZACOES SOFRIDAS DESDE A CONSTRU€AO INICIAL.             ³±±
...
Function NGCONTAP(cBemF, cDtCon, cHrCon, nTipOc, cFilBemF)
	...
	If (IsInCallStack("MNTA655") .Or. IsInCallStack("MNTA656")).And. !Empty(cFilBemF)
		...
		//Deverá usar essa variavel para comparação dos registros, pois caso a chamada seja do MNTA655/656 e a filial cFilBemF esteja marcada
		//o bem poderá estar numa filial diferente do posto, e na hora de fazer a comparação "&(vARQDIA[2]) == xFilial(vARQDIA[1]" está errada,
		//pois não considera a parametrização acima (Chamada via MNTA655/MNTA656 E cFilBemF preenchido).
		//Dessa forma o cFilCons estará sempre preenchido com a informações correta.
		//SS: 027181
		cFilCons := cFilBemF
	Else
		dbSelectArea(vARQDIA[1])
	...
	If !EoF()
		If &(vARQDIA[3]) == cBemF .And. IIf(lTestFi, &(vARQDIA[2]) == cFilCons, .T.)
			...
			If &(vARQDIA[3]) == cBemF .And. IIf(lTestFi, &(vARQDIA[2]) == cFilCons, .T.) .And.;
					(NgBlCont( cBemF, .F. ) .Or. &(vARQDIA[6]) == "A")// Se o parâmetro LANEX estiver habilitado, carrega apenas registro do tipo Abastecimento
				aRetorno := {}
	...
	If !BoF()
		...
		While !BoF() .And. &(vARQDIA[3]) == cBemF .And. IIf(lTestFi, &(vARQDIA[2]) == cFilCons, .T.) .And. &(vARQDIA[7]) == nAcumCon
			...
			If NgBlCont( cBemF, .F. ) .Or. ( &(vARQDIA[6]) == "A" .Or. &(vARQDIA[6]) == "I" )// Se o parâmetro LANEX estiver habilitado, carrega apenas registro do tipo Abastecimento
...
@return numerico, retorna a diferença entre o contador acumulado dos abastecimentos
/*/
//---------------------------------------------------------------------
Function NGCONTPER(cBemF,cDtCon, cHrCon, cTipoCn)
...
/*/
	...
	±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
	±±ÚÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄ¿±±
	±±³Fun‡…o    ³ NGSTPALT  ³ Autor ³ Evaldo Cevinscki Jr. ³ Data ³07/03/2008³±±
	±±ÃÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄ´±±
	±±³Descri‡…o ³Diminui 1 minuto de um lancamento de historico de contador. ³±±
	±±ÃÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ´±±
...
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
±±ÚÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄ¿±±
±±³ Fun‡…o   ³ NGPCONTPE³ Autor ³ Elisangela Costa      ³ Data ³ 31/05/05 ³±±
±±ÃÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄ´±±
±±³ Descri‡…o³ Procura o contador acumulado na data do parametro          ³±±
±±ÃÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ´±±
...
@since   13/04/2011
@version P11/P12
@param   cMarca, Caracter, Define marcação
/*/
//-------------------------------------------------------------------
...
Função para buscar filial da último registro de contador

@author Diego de Oliveira
@since 14/11/2016
@version MP11/MP12
@param   cFrota,  Caracter, Código do bem
@param   dData,   Data,     Data para registro do contador
@param   cHora,   Caracter, Hora para registro do contador
@return  cFilSTP, Caracter, Retorna Filial que será realizada
...
Static Function NGTRANSF(cFrota,dData,cHora)
	...
	Local cAliasQry := ""
	Local cQuery    := ""
	Local _cGetDB 	:= Upper(TCGetDB()) //Obtém qual o Banco de Dados corrente

	//Cria query que pega o primeiro registro do bem sem STP de inclusão
	If AllTrim(_cGetDB) $ "ORACLE/MYSQL"
		cAliasQry := GetNextAlias()
		cQuery := " SELECT TP_FILIAL,TP_CODBEM,TP_TIPOLAN,TP_DTLEITU,TP_HORA,TP_POSCONT,TP_ACUMCON "
	...
	Else //Caso não seja ORACLE/MYSQL, utiliza comando TOP 1

		cAliasQry := GetNextAlias()
		cQuery    := " SELECT TOP 1 TP_FILIAL,TP_CODBEM,TP_TIPOLAN,TP_DTLEITU,TP_HORA,TP_POSCONT,TP_ACUMCON "
		cQuery    += " FROM " + RetSQLName("STP")
		cQuery    += " WHERE TP_CODBEM     = '"+cFROTA+"' "
		cQuery    += "      AND D_E_L_E_T_   <>'*' "
		cQuery 	  += "      AND TP_DTLEITU || TP_HORA < " + ValToSql( DtoS( dData ) + cHora )
...
@param cCBem: Código do bem (obrigatório)
@param cServ: Código do seviço (obrigatório)
@param cSeq: Sequencias da manutencao (obrigatório)
@param dDulM: Data da ultima manutenção (não obrigatório)
@param nCoAm: Acumulado do ultima manutenção (não obrigatório)
...
±±³Fun‡„o    ³NGACERMANSUB³ Autor ³Inacio Luiz Kolling    ³ Data ³25/05/2011³±±
±±ÃÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄ´±±
±±³Descri‡„o ³Atualiza a manutencao padrao e/ou a ser substituida           ³±±
...
@param nTpAcumcon , Númerico, Contador acumulado da STP (utilizado para consistência da ST9)
@param [nTypCount], Númerico, Indica se o processo vai tratar contador 1 ou 2
@param [lChild]   , Lógico  , Replica o contador para tabelas relacionadas aos filhos da estrutura.
...
Function NGPACONT( cCodBem, dData, cHora, cCont, nTpAcumcon, nTypCount, lChild )
	...
	If !lChild
		aAdd(aTables, {"TQN", "TPN", "ST9", "STJ", "STS", "STW", "STY", "STZ", "STZ",; //Tabelas que deverão ser verificadas
...
@param cCont     , Caracter, Valor novo do contador
@param nTpAcumcon, Númerico, Contador acumulado da STP (utilizado para consistência da ST9)
@param cField    , Caracter, Nome do campo posição contador da tabela em uso.
@param nTypCount , Númerico, Indica qual tipo de contado está sendo trabalhado.
...
Cria uma query que altera a existência do registro de histórico.
@type static
...
@param cBem      , Caracter, Código do bem a ser filtrado
@param cField    , Caracter, Nome do campo de Posição do contador
@param dData     , Data    , Data do registro a ser alterado
@param cHora     , Caracter, Hora do registro a ser alterado
@param cCont     , Númerico, Valor novo do contador
...
Static Function fUpdCnt(aCampos, cTabela, cBem, cField, dData, cHora, cCont )
	...
	While (cAliasAtu)->(!EoF())
		...
		//----------------------------------------------------
		// Atualiza manutenção, tratamento para uso do lanex
		//----------------------------------------------------
		If cTabela == 'STJ' .And. (cAliasAtu)->TJ_PLANO != '000000' .And. (cAliasAtu)->TJ_TERMINO == 'S'
			...
			// Atualiza manutenções que foram substituídas, aglutinadas
			dbSelectArea("STJ")
			dbSetOrder(1)
...
When dos campos de contador que retorna o último contador informado,
considerando Quebra e Virada.

@param cCobBem: Código do bem (não obrigatório)
@param dData: Data do registro que está sendo inserido (não obrigatório)
@param cHora: Hora do registro que está sendo inserido (não obrigatório)
@param nAtuCont: Valor atual do contador (não obrigatório)
@author Wexlei Silveira
@since 10/05/2016
...
Function NGTpCont(cCobBem, dData, cHora, nAtuCont)
	...
	//----------------------------------------------------------
	// Validação para os parâmetros obrigatórios
	// Em alguns casos essa função é acionada antes da definição
	//----------------------------------------------------------
	...
	If Empty( cTipoLan ) //Se o parâmetro não existir ou estiver vazio, não busca o contador.
	...
	If cTipoLan != '-1' .And. Len(cTipoLan) > 1// Forma uma lista separada por vírgula dos tipos de contadores a serem listados. Ex.: "A, C"
	...
	Else
		cTpLanLt := cTipoLan
	EndIf
	//TODO quando for implementado mais de um tipo de contador para o parâmetro NGLANEX,
	//será necessário saber qual tipo de contador (A,C, etc.) deve ser considerado no SELECT abaixo.
	//Provavelmente a variável cTpLanLt deverá conter apenas uma letra, referente ao tipo do contador que será filtrado.
...
Bloqueia o campo de contador caso o parâmetro LANEX exista e
contenha o valor correspondente ao tipo de contador a ser bloqueado.

@param cBemSt9, string, código do bem
@author Wexlei Silveira
@since 10/05/2016
@return lRet .F. para bloquear o campo ou .T. para não bloquear
/*/
//----------------------------------------------------------------
Function NGBlCont( cBemSt9, lVerVar )
	...
	//Retorna .F. se o conteúdo for A, bloqueando o campo de contador nas rotinas não relacionadas a abastecimento
...
Cria as tabelas temporárias relacionadas ao contador/estrutura de bens para
garantir que, em banco de dados oracle, os registros não sejam inseridos
quando houver alguma inconsistência durante o processo.
...
Deleta as tabelas temporárias criadas pela função MNTCTemp.
...
@param cCodBem, string, código do bem
@param dDataAtu, date, data do lançamento
@param cHoraAtu, string, hora do lançamento
@param cTipoLan, string, tipo de lançamento
@param dDataPost, date, data do lançamento posterior
@param cHoraPost, string, hora do lançamento posterior
@param cTipPost, string, tipo de lançamento posterior
...
Static Function fIncrement( cCodBem, dDataAtu, cHoraAtu, cTipAtu, dDataPost, cHoraPost, cTipPost )
	...
	//--------------------------------------------------------------------------------------------
	// Verifica se existem reformas após o lançamento do contador e antes do próximo registro da STP
	//--------------------------------------------------------------------------------------------
	BeginSql Alias cAliasQry
	...
	If !lReturn .And. NGUSELANEX( cCodBem, .F., dDataAtu, cHoraAtu ) == "A" .And. cTipAtu == "A"
		...
		If cTipPost == "A" // se o lançamento posterior é A não há necessidade de incrementar o km da banda
			lReturn := .F.
		Else

			//-----------------------------------------------------------------------------
			// Verifica se há pelo menos um lançamento tipo A após a data.
			// Se houver não há necessidade de incrementar o km da banda
			//-----------------------------------------------------------------------------
			cAliasQry := GetNextAlias()
			BeginSql Alias cAliasQry
...
@param dData, Data, Data do lançamento
@param cHora, Caractere, Hora do lançamento
@param nTpCont, Numérico, Tipo de contador
...
Function MntInfCnt(cCodBem, dData, cHora, nTpCont, nContador)
	...
	If nContador <= 0
		lRet := .F.
		Help(Nil, Nil, '', Nil, STR0161, 1, 0)// Posição de contador deve ser maior que zero.
	EndIf

	//------------------------------------------------
	// Valida se o bem existe e está ativo na filial
	//------------------------------------------------
	If lRet
	...
	If lRet
		...
		If dbSeek(xFilial("ST9") + cCodBem)
			...
			If ST9->(FieldPos("T9_MSBLQL")) > 0 .And. ST9->T9_MSBLQL == "1"
				...
				Help(Nil, Nil, STR0015, Nil, STR0163, 1, 0) // O bem está bloqueado e não pode ser utilizado.
				lRet := .F.

			ElseIf ST9->T9_TEMCONT != 'S'

				Help(Nil, Nil, STR0015, Nil, STR0014, 1, 0) // Bem não é controlado por contador.
				lRet := .F.
	...
	// -----------------------------------------------
	// Valida a posição do contador com o limite
	// -----------------------------------------------
	If lRet
	...
	//------------------------------------------------------
	// Valida a variação dia (intervalo de valores válidos)
	//------------------------------------------------------
	If lRet
...
Verifica se bem usa lanex, controle só por abastecimento

@author Maria Elisandra de paula
@since 31/07/2020

@param cBemSt9, string, Código do bem
@param [lBlocking], boolean, indica se é verificação para bloqueio de campo
@param [dDtMov], date, data da movimentação
@param [cHrMov], string, hora da movimentação
@param [cToCheck], string, código do pai, quando já está identificado
@param [cUseLanex], string, conteúdo do parâmetro
...
Function NGUSELANEX( cBemSt9, lBlocking, dDtMov, cHrMov, cToCheck, cUseLanex )
	...
	If dbSeek( xFilial("ST9") + cBemSt9 )
		If ST9->T9_TEMCONT == 'S'
			...
			EndIf
			cToCheck := cBemSt9 // tem contador próprio deve verificar se ele mesmo possui tanque
		ElseIf lBlocking
			RestArea( aArea )
			Return "A" // quando utilizado para bloquear campo contador mas não tem contador próprio
		EndIf
	EndIf
	...
	//--------------------------------------------------
	If Empty( cToCheck )
		// para pneus e comp sem contador próprio verifica o pai
	...
	//----------------------------------------------------------------------------
	// quando não tem tanque, não é controlado por abastecimento, retorna vazio
	//----------------------------------------------------------------------------
	dbSelectArea("TT8")
	dbSetOrder(1)
...
Relação do campo contador, utilizado para lanex

@author Maria Elisandra de paula
@since 05/08/2020
@param cField, string, Campo em uso
@return numérico, valor do contador
/*/
//---------------------------------------------------------------------
...
Validações de contador para ambiente que usa lanex

@author Maria Elisandra de paula
@since 21/12/2021
@param cFilBem, string, filial do bem
@param cCodBem, string, código do bem
@param dDtLeitu, date, data do lançamento
@param cHoraLei, string, hora do lançamento
@param nPosCont, numeric, valor do contador
@return string, mensagem de erro
/*/
//---------------------------------------------------------------------
Static Function fVldLanex( cFilBem, cCodBem, dDtLeitu, cHoraLei, nPosCont  )

	Local aAreaSTP := STP->( FWGetArea() )
	Local cError   := ''

	/*----------------------------------------------------+
	| Verifica se há um lançamento com mesma data e hora. |
	+----------------------------------------------------*/
	dbSelectArea( 'STP' )
	dbSetOrder( 5 ) // TP_FILIAL + TP_CODBEM + TP_DTLEITU + TP_HORA
	If msSeek( cFilBem + cCodBem + DToS( dDtLeitu ) + cHoraLei, .T. )

		cError := fVldMsg( dDtLeitu, cHoraLei, Nil, STP->TP_POSCONT, 1 )

	EndIf
		
	/*--------------------------------------------------------+
	| Verifica se o contador é menor que alguma STP anterior. |
	+--------------------------------------------------------*/
	If Empty( cError )

		STP->( dbSkip( -1 ) )

		If STP->( !BoF() ) .And. STP->TP_FILIAL == cFilBem .And. STP->TP_CODBEM == cCodBem .And.;
			STP->TP_POSCONT > nPosCont

			cError := fVldMsg( dDtLeitu, cHoraLei, Nil, STP->TP_POSCONT, 2 )
	...
	/*---------------------------------------------------------+
	| Verifica se o contador é maior que alguma STP posterior. |
	+---------------------------------------------------------*/
	If Empty( cError )

		While STP->( !EoF() ) .And. STP->TP_FILIAL == cFilBem .And. STP->TP_CODBEM == cCodBem

			If STP->TP_TIPOLAN $ 'Q/V'

				Exit

			ElseIf STP->TP_TIPOLAN == 'A' 

				If STP->TP_POSCONT < nPosCont

					cError := fVldMsg( dDtLeitu, cHoraLei, Nil, STP->TP_POSCONT, 3 )
...
Static Function fVldMsg( dDtLeitu, cHoraLei, cTipoC, cContador, nTipoMsg )
	...
	Do Case
		...
		Case nTipoMsg == 1
			...
			// Já existe um lancamento para o contador ### com as caracteristicas:
...
Função responsável por realizar o recálculo dos campos TTV_POSINI,
...
@since  27/10/2022
@param  aValues, Array, Dados do abastecimento cadastrado / alterado ou excluído.
	aValues[1] = Posto
	aValues[2] = Loja
	aValues[3] = Tanque
	aValues[4] = Bomba
	aValues[5] = Data
	aValues[6] = Hora
@param nOpx, Numérico, Operação realizada no abastecimento.
/*/
//-------------------------------------------------------------------
