...
Function MNTA296(cParFilial, cParSolici, cParTipoSS, cParCodBem)
	...
	//Botoes da tela
	Local oBtnFil, oBtnImp, oBtnHis, oBtnLeg
	Local oBtnCancel
	...
	oBtnFil  := TBtnBmp():NewBar("ng_ico_filtro","ng_ico_filtro",,,,{|o,x,y| oMenuFil:Activate(23,54,oDlg296)},,oPanelLBtn,,,STR0005,,,,,"") //"Filtro"
	...
	{|| MNT296HIST(Substr((aMark296[__OPC_TQB__][__POS_ALIAS__])->TQB_TIPOSS,1,1),(aMark296[__OPC_TQB__][__POS_ALIAS__])->TQB_CODBEM)};
	,,oPanelLBtn,,,STR0007,,,,,"") //"Histórico"
	oBtnHis:Align  := CONTROL_ALIGN_TOP

	oBtnCancel       := TBtnBmp():NewBar("ng_ico_excss","ng_ico_excss",,,,{|| A291Cancel((aMark296[__OPC_TQB__][__POS_ALIAS__])->TQB_FILIAL,(aMark296[__OPC_TQB__][__POS_ALIAS__])->TQB_SOLICI,__cCallDistr)},,oPanelLBtn,,,STR0008,,,,,"") //"Cancelamento da S.S."
	oBtnCancel:Align := CONTROL_ALIGN_TOP
	If !MNT280REST("TUA_CANCSS")
		oBtnCancel:Disable()
	EndIf

	oBtnLeg          := TBtnBmp():NewBar("ng_ico_lgndos","ng_ico_lgndos",,,,{|| A296LEGEND(1)},,oPanelLBtn,,,STR0009,,,,,"") //"Legenda"
	...
	bGravaEnc         := {|| fGravaEnc()}
	oBtnConf          := TBtnBmp():NewBar("ng_ico_confirmar","ng_ico_confirmar",,,,bGravaEnc,,oPanelR1Btn,,,STR0015,,,,,"") //"Confirmar"
	oBtnConf:Align    := CONTROL_ALIGN_TOP
	oBtnConf:lVisible := .F.

	oBtnCanc          := TBtnBmp():NewBar("ng_ico_cancelar","ng_ico_cancelar",,,,{|| fDisable(.F.) },,oPanelR1Btn,,,STR0016,,,,,"") //"Cancelar"
	oBtnCanc:Align    := CONTROL_ALIGN_TOP
	oBtnCanc:lVisible := .F.

	oBtnQuest       := TBtnBmp():NewBar("ng_ico_questionario","ng_ico_questionario",,,,{|| fQuestiSS((aMark296[__OPC_TQB__][__POS_ALIAS__])->TQB_SOLICI)},,oPanelR1Btn,,,STR0017,,,,,"") //"Questionário de Sintomas"
	oBtnQuest:Align := CONTROL_ALIGN_TOP

	oBtnDet         := TBtnBmp():NewBar("ng_ico_tarefas","ng_ico_tarefas",,,,{|| fVisualSS((aMark296[__OPC_TQB__][__POS_ALIAS__])->TQB_SOLICI)},,oPanelR1Btn,,,STR0018,,,,,"") //"Detalhamento Solicitação"
	oBtnDet:Align   := CONTROL_ALIGN_TOP

	oBtnUser        := TBtnBmp():NewBar("ng_ico_info","ng_ico_info",,,,{|| fUserSS((aMark296[__OPC_TQB__][__POS_ALIAS__])->TQB_SOLICI)},,oPanelR1Btn,,,STR0019,,,,,"") //"Informações do Solicitante"
	oBtnUser:Align  := CONTROL_ALIGN_TOP

	oBtnVis         := TBtnBmp():NewBar("ng_ico_conhecimento","ng_ico_conhecimento",,,,{|| fMsDocSS((aMark296[__OPC_TQB__][__POS_ALIAS__])->TQB_SOLICI)},,oPanelR1Btn,,,STR0020,,,,,"") //"Conhecimento"
...
Static Function fCreateFolder(oObjPai, aSize, lInverte, cMarca, cCodSS, lTransfAtend)
	...
	Local oPanelR2Btn
	Local oBtnFilAte, oBtnTrcAte, oBtnLegAte, oBtnVisAte, oBtnLisTer
	Local oFolderAtend
	Local aFoldAtend := {}
	...
	oBtnFilAte          := TBtnBmp():NewBar("ng_ico_filtro","ng_ico_filtro",,,,{|| fSetFilter((oFolderAtend:nOption+1))},,oPanelR2Btn,,,STR0005,,,,,"") //"Filtro"
	oBtnFilAte:Align    := CONTROL_ALIGN_TOP

	oBtnTrcAte          := TBtnBmp():NewBar("ng_os_troca","ng_os_troca",,,,{|| fChangeFil((oFolderAtend:nOption+1),cCodSS)},,oPanelR2Btn,,,STR0024,,,,,"") //"Trocar Filial"
	oBtnTrcAte:Align    := CONTROL_ALIGN_TOP

	oBtnVisAte          := TBtnBmp():NewBar("ng_ico_visualizar","ng_ico_visualizar",,,,{|| fVisualizar((oFolderAtend:nOption+1))},,oPanelR2Btn,,,STR0025,,,,,"") //"Visualizar Atendente"
	oBtnVisAte:Align    := CONTROL_ALIGN_TOP

	oBtnCarga           := TBtnBmp():NewBar("ng_ico_atendimento","ng_ico_atendimento",,,,{|| fCallCarga((oFolderAtend:nOption+1))},,oPanelR2Btn,,,STR0026,,,,,"") //"Carga"
	oBtnCarga:Align	    := CONTROL_ALIGN_TOP

	oBtnLegAte          := TBtnBmp():NewBar("ng_ico_lgndos","ng_ico_lgndos",,,,{|| A296LEGEND(2)},,oPanelR2Btn,,,STR0027,,,,,"") //"Legenda"
	oBtnLegAte:Align    := CONTROL_ALIGN_TOP
...
//---------------------------------------------------------------------
/*/{Protheus.doc} A296AtuQtd
Atualiza a Quantidade de S.S. em Aberto para cada registro de Atendente
da tabela passada como parâmetros, chamando a função 'A296QtdSS'.
* ATENÇÃO
A tabela temporária deve possuir o campo "QTDSS".
...
/*/
//---------------------------------------------------------------------
Function A296AtuQtd(cTblAtend, cTipoAtend, cFilAtend, cCodAtend, cLojAtend)

	// Variáveis auxiliares
	Local cSeekTbl := "('"+cTblAtend+"')->"
	Local cSeekFil := ""
	Local cSeekCod := ""
	Local cSeekLoj := ""
	Local nQtdSS   := 0
	...
	//----------
	// Define os campos
	cSeekFil := If(!Empty(cFilAtend), cSeekTbl+cFilAtend, "Nil")
	cSeekCod := If(!Empty(cCodAtend), cSeekTbl+cCodAtend, "Nil")
	cSeekLoj := If(!Empty(cLojAtend), cSeekTbl+cLojAtend, "Nil")

	// Atualiza a Quantidade de S.S. para cada atendente
	CursorWait()
	dbSelectArea(cTblAtend)
	aAreaMark := GetArea()
	dbGoTop()
	While !Eof()
		// Recebe a Quantidade de S.S.
		nQtdSS := A296QtdSS(cTipoAtend, &(cSeekFil), &(cSeekCod), &(cSeekLoj))

		// Altera apenas se for diferente
		/*If (cTblAtend)->QTDSS <> nQtdSS
		RecLock(cTblAtend, .F.)
		(cTblAtend)->QTDSS := nQtdSS
		MsUnlock(cTblAtend)
		EndIf*/
...
//---------------------------------------------------------------------
/*/{Protheus.doc} fAtuQtdSS
Atualiza a Quantidade de S.S. dos Folders de Atendentes (Equipes,
Atendentes e Terceiros).
...
/*/
//---------------------------------------------------------------------
Static Function fAtuQtdSS()

	// Equipes
	A296AtuQtd(aMark296[__OPC_TP4__][__POS_ALIAS__], "1", "FILATE", "CODATE", )
	// Atendentes (Funcionários)
	A296AtuQtd(aMark296[__OPC_ST1__][__POS_ALIAS__], "2", "FILATE", "CODATE", )
	// Terceiros (Fornecedores)
	A296AtuQtd(aMark296[__OPC_SA2__][__POS_ALIAS__], "3", "FILATE", "CODATE", "LOJATE")
