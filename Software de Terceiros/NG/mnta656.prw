...
Programa de digitaÁ„o de abastecimentos em Lote de posto interno
...
±±≥DescriáÖo ≥ Funcao de inclusao,alteracao e exclusao de abastecimento em≥±±
±±≥          ≥ lote                                                       ≥±±
...
/*/
 Function MNTA656INC(cAlias,nRecno,nOpcx)
	...
	Local aActiveArea   := {}
	Local aAreaTQN      := {}
	Local dDtBlqMov 	:= SuperGetMV( "MV_DBLQMOV",.F.,STOD("") ) //data de bloqueio de movimentaÁıes no
	...
	Local xAferiHod 	:= {}
	Local nVar 			:= 0
	Local LQuitTrans	:= .F. // Verifica se deve continuar a transaÁ„o (Begin/End transaction)
	Local cRelacao      := ""
	...
	Private aOSLub := {}
	Private cFil
	Private cFiliST9   := xFilial("ST9")	//Vari·vel utilizada em chamadas de funÁ„o do MNTA655
	Private cCombust   := ""
	...
	If NGCADICBASE("TTH_FOLHA","A","TTH",.F.)
		Aadd( a656Button , { "TANQUE" , {|| MNT656AFER(nOpcx) } , STR0127 , STR0128 } ) //"SaÌdas de CombustÌveis"###"SaÌdas"
		lAferTTH := .T.
		If !Inclui
			dbSelectArea("TTH")
			dbSetOrder(2)
	...
	EndIf
	//DeclaraÁ„o de variaveis private utilizadas na validaÁ„o de alguns campos de data e hora de abastecimento.
	Private nPDtAbas := aSCAN(aHeadTTH,{|x| Trim(Upper(x[2])) == "TTH_DTABAS"})
	Private nPHrAbas := aSCAN(aHeadTTH,{|x| Trim(Upper(x[2])) == "TTH_HRABAS"})

	//DeclaraÁ„o de Variaveis Private dos Objetos                            SetPrvt("oDlg1","oGrp1","oSay1","oSay2","oLanca","oData","oGrp2","oSay3","oSay4","oSay5","oSay6","oSay13","oSay18","oHora")
	...
	Define MsDialog oDlg1 Title STR0001 From nLinIniTl,nColIniTl TO nLinFimTl,nColFimTl Of oMainWnd Pixel COLOR CLR_BLACK,CLR_WHITE STYLE nOR(WS_VISIBLE,WS_POPUP)  //"Abastecimento de Posto Interno em Lote"
		...
		Else
			...
			oData:bHelp := { || ShowHelpCpo( 'TTA_DTABAS', { STR0186 }, 1, {}, 1 ) } // Data do LanÁamento da folha de abastecimento em lote.
		...
		oHora:bHelp := {|| ShowHelpCpo("TTA_HRABAS", { STR0187 }, 1, {}, 1)} //"Hora do LanÁamento da folha de abastecimento em lote."

		/*----------------+
		| Tipo LanÁamento |
		+----------------*/
		oSay2       := TSay():New( 005, 135,{||STR0004},oPanel,,,.F.,.F.,.F.,.T.,CLR_HBLUE,CLR_WHITE,044,008) //"Tipo LanÁamento"
		bValid      := {|| MNT656LISE()}
		oLanca      := TComboBox():New( 005, 185, { |u| IIf( PCount() > 0, nLanca := u, nLanca ) }, { STR0005,;   // Abastecimento
						 ...
						 oLanca:bHelp := {|| ShowHelpCpo("nLanca", { STR0188 }, 1, {}, 1)} //"Tipo de lanÁamento, podendo ser Abastecimento, Abastecimento + Produto, Produto."
		oLanca:Disable()

		/*------------+
		| Respons·vel |
		+------------*/
		oSay19      := TSay():New( 005, 270,{||STR0160},oPanel,,,.F.,.F.,.F.,.T.,If(lObrigResp,CLR_HBLUE,CLR_BLACK),CLR_WHITE,044,008) //"Respons·vel"
		...
		oResp:bHelp := {|| ShowHelpCpo("TTA_RESPON", { STR0189 }, 1, {}, 1)} // "MatrÌcula do funcion·rio do abastecimento/lubrificaÁ„o. Pressione as teclas [Enter]+[F3] para selecionar um funcion·rio."
		...
		oFolha:bHelp := {|| ShowHelpCpo("TTA_FOLHA", { STR0190 }, 1, {}, 1)} //"CÛdigo da folha de abastecimentos."
		...
		oPosto:bHelp := {|| ShowHelpCpo("TTA_POSTO", { STR0191 }, 1, {}, 1)} //"CÛdigo do posto interno dos abstecimentos. Pressione as teclas [Enter]+[F3] para selecionar um Posto."
		...
		oLoja:bHelp  := {|| ShowHelpCpo("TTA_LOJA", { STR0192 }, 1, {}, 1)} //"CÛdigo identificador de cada uma das unidades (lojas) de um posto."
		...
		oTanque:bHelp  := {|| ShowHelpCpo("TTA_TANQUE", { STR0193 }, 1, {}, 1)} //"Tanque de combustÌveis dos abastecimentos. Pressione as teclas [Enter]+[F3] para selecionar um Tanque."
		...
		oCombst:bHelp  := {|| ShowHelpCpo("cTTAComb", {STR0210}, 1, {}, 1)} //"Informe o cÛdigo do combustÌvel do abastecimento. Pressione as teclas [Enter]+[F3] para selecionar um CombustÌvel."
		...
		oBomba:bHelp   := {|| ShowHelpCpo("TTA_BOMBA", { STR0194 }, 1, {}, 1)} //"Bomba de combustÌveis dos abastecimentos. Pressione as teclas [Enter]+[F3] para selecionar uma Bomba."
		...
		oSay18     := TSay():New( C(051),C(353),{||STR0017},oGrp2,,,.F.,.F.,.F.,.T.,CLR_BLACK,CLR_WHITE,032,008) //"DiferenÁa"
		...
		oCombAnt:bHelp := {|| ShowHelpCpo("nCombAnt", { STR0195 }, 1, {}, 1)} //"Quantidade de combustÌvel j· utilizado em abastecimentos anteriores."
		...
		oCombAtu:bHelp := {|| ShowHelpCpo("TTA_CONBOM", { STR0196 }, 1, {}, 1)} //"Quantidade de combustÌvel abastecido na presente folha somada a quantidade do campo Anterior."
		...
		oCombTot:bHelp := {|| ShowHelpCpo("TTA_TOTCOM", { STR0197 }, 1, {}, 1)} //"Quantidade de combustÌvel total abastecido na presente folha."
		...
		oCombDig:bHelp := {|| ShowHelpCpo("nCombDig", { STR0198 }, 1, {}, 1)} //"Quantidade de combustÌvel abastecido na presente folha conforme soma das Quantidades (TQQ_QUANT) dos registros inclusos."
		...
		oCombDif:bHelp := {|| ShowHelpCpo("nCombDif", { STR0199 }, 1, {}, 1)} //"Quantidade de combustÌvel que falta para completar a presente folha conforme soma das Quantidades (TQQ_QUANT) dos registros inclusos."

		oGrp3      := TGroup():New( C(77),aPosObj[2,2]-1,C(115),aPosObj[2,4],STR0018,oPanel,CLR_BLACK,CLR_WHITE,.T.,.F. ) //"Produto"
		oSay7      := TSay():New( C(092),C(006),{||STR0019},oGrp3,,,.F.,.F.,.F.,.T.,CLR_BLACK,CLR_WHITE,040,008) //"ServiÁo 1-Troca"
		oSay8      := TSay():New( C(105),C(006),{||STR0020},oGrp3,,,.F.,.F.,.F.,.T.,CLR_BLACK,CLR_WHITE,052,008) //"ServiÁo 2-ReposiÁ„o"
		...
		oSay12     := TSay():New( C(096),C(329),{||STR0017},oGrp3,,,.F.,.F.,.F.,.T.,CLR_BLACK,CLR_WHITE,024,008) //"DiferenÁa"
		...
		oDesTroca := TGet():New( c( 091 ), c( 110 ), { |u| IIf( PCount() > 0, cDesTroca := u, cDesTroca ) }, oGrp3, 125,;
								 ...
								 .F., '', 'cDesTroca', , , , .T. )
		oTroca:bHelp := {|| ShowHelpCpo("TTA_SERTRO", { STR0200 }, 1, {}, 1)} //"ServiÁo utilizado para troca de produto (Lubrificante). Pressione as teclas [Enter]+[F3] para selecionar um ServiÁo."
		...
		oDesRepo   := TGet():New( c( 103 ), c( 110 ), { |u| IIf( PCount() > 0, cDesRepo := u, cDesRepo ) }, oGrp3, 125,;
		                          008, '@!', , CLR_BLACK, CLR_LIGHTGRAY, , , , .T., '', , &( cBlKDesc ), .F., .F., , .F.,;
								  .F., '', 'cDesRepo', , , , .T. )
		oReposicao:bHelp := {|| ShowHelpCpo("TTA_SERREP", { STR0201 }, 1, {}, 1)} //"ServiÁo utilizado para reposiÁ„o de produto (Lubrificante). Pressione as teclas [Enter]+[F3] para selecionar um ServiÁo."
		...
		oLubDig:bHelp := {|| ShowHelpCpo("TTA_TOTLUB", { STR0203 }, 1, {}, 1)} //"Quantida de produto (Lubrificante) digitado nos registros inclusos atravÈs do campo Qtde Lub.(TPE_AJUSCO)."
		...
		Else
			...
			If Altera //Se for alteraÁ„o
				//Verifica se h· integraÁ„o entre os mÛdulos ManutenÁ„o de Ativos (SIGAMNT) e Estoque (SIGAEST).
				//Indica o momento que o estoque sera debitado, ou seja, na 'ConciliaÁ„o'.
							...
							If dbSeek( xFilial( "TQN" ) + aCols[nX][nPOSFROTA] + DTOS( aCols[nX][nPOSDATAB] ) + aCols[nX][nPOSHORAB] )
								If !Empty( TQN->TQN_DTCON )
									If TQN->TQN_DTABAS <= dDtBlqMov //Se a data do abastecimento for menor/igual que a data de bloqueio de abastecimento (MV_DBLQMOV);
										//"A folha de abastecimento 'x' n„o pode ser alterada porque a data de abastecimento do veÌculo 'x' È menor ou igual que a data de bloqueio de movimentaÁıes 'y'. Conforme par‚metro (MV_DBLQMOV)."
								...
								Else
									If TQN->TQN_DTABAS <= dDtBlqMov //Se a data do abastecimento for menor/igual que a data de bloqueio de abastecimento (MV_DBLQMOV);
										lReturn	:= MsgYesNo( STR0172 + AllTrim( TTA->TTA_FOLHA ) + STR0207 + DTOC( dDtBlqMov ) + STR0175 + STR0208 ) //" tem data menor ou igual ao bloqueio de movimentaÁıes " ## " Deseja prosseguir? "
										lPrimeiro	:= .F.
										Exit
									EndIf
								EndIf
	...
	Begin Transaction
		...
		//Verifica se cancelou a tela e se est· na inclus„o para "reutilizar" o cÛdigo quando utilizando GETSXENUM().
		If nOpcx == 3
			If nOpca == 0
				RollBackSX8()
			Else
		...
		/* Atualmente a rotina n„o encontra-se preparada para o processo de alteraÁ„o
	   	   quando utilizado apenas o lanÁamento de produtos, desta forma validaÁıes e gravaÁıes
						...
						If !aCols[nI][Len(aHeader)+1]
							...
							If !Empty(aCols[nI][nPOSQUANT])
								...
								If cNGMNTES == 'S' .And. cConEst == "S" .And. !lNao_SD3
									...
									EndIf
									//Verifica se o MV_DOCSEQ est· certo antes de entrar em execuÁ„o autom·tica, para mostrar mensagem de erro e finalizar
									If Empty(ProxNum())
										cEmpAnt := cOldEmp
										cFilAnt := cOldFil
									EndIf
								...
								If !LQuitTrans
									...
									If !dbSeek( cFilTqn + aCols[nI][nPOSFROTA] + DtoS(aCols[nI][nPOSDATAB]) + aCols[nI][nPOSHORAB] )
										...
										TQN->TQN_CODCOM 	:= cCombust //CÛdigo do combustÌvel.
									...
									If !lGeraOSAut
										If nI == 1
											If (cGeraPrev = "S" .Or. cGeraPrev = "C" .Or. cGeraPrev = "A") .And. !Empty(aCols[nI][nPOSHODOM])
												If cGeraPrev = "C"
													If MsgYesNo(STR0123+chr(13)+chr(13); //"Deseja que seja verificado a existÍncia de o.s autom·tica por contador?"
...
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
±±⁄ƒƒƒƒƒƒƒƒƒƒ¬ƒƒƒƒƒƒƒƒƒƒ¬ƒƒƒƒƒƒƒ¬ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ¬ƒƒƒƒƒƒ¬ƒƒƒƒƒƒƒƒƒƒø±±
±±≥FunáÖo    |MNT655Head≥ Autor ≥ Marcos Wagner Junior  ≥ Data ≥ 15/10/08 ≥±±
±±√ƒƒƒƒƒƒƒƒƒƒ≈ƒƒƒƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒƒƒƒ¥±±
±±≥DescriáÖo ≥ Monta aHeader da MsNewGetDados para o Alias: TQN           ≥±±
±±√ƒƒƒƒƒƒƒƒƒƒ≈ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ¥±±
±±≥ Uso      ≥ MNTA656                                                    ≥±±
±±¿ƒƒƒƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒŸ±±
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
...
Static Function MNT655Head(nOpcx)
	...
	//Ponto de Entrada que possibilita mudar ordenaÁ„o ou adicionar um campo de usuario no aHeader
...
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
±±⁄ƒƒƒƒƒƒƒƒƒƒ¬ƒƒƒƒƒƒƒƒƒƒ¬ƒƒƒƒƒƒƒ¬ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ¬ƒƒƒƒƒƒ¬ƒƒƒƒƒƒƒƒƒƒø±±
±±≥FunáÖo    |MNT655Brw ≥ Autor ≥ Marcos Wagner Junior  ≥ Data ≥ 15/10/08 ≥±±
±±√ƒƒƒƒƒƒƒƒƒƒ≈ƒƒƒƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒƒƒƒ¥±±
±±≥DescriáÖo ≥ Monta aCols da MsNewGetDados para o Alias: TQN             ≥±±
±±√ƒƒƒƒƒƒƒƒƒƒ≈ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ¥±±
±±≥ Uso      ≥ MNTA656                                                    ≥±±
±±¿ƒƒƒƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒŸ±±
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
...
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
±±⁄ƒƒƒƒƒƒƒƒƒƒ¬ƒƒƒƒƒƒƒƒƒƒ¬ƒƒƒƒƒƒƒ¬ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ¬ƒƒƒƒƒƒ¬ƒƒƒƒƒƒƒƒƒƒø±±
±±≥FunáÖo    ≥ MenuDef  ≥ Autor ≥ Rafael Diogo Richter  ≥ Data ≥02/02/2008≥±±
±±√ƒƒƒƒƒƒƒƒƒƒ≈ƒƒƒƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒƒƒƒ¥±±
   ...
   Else
      ...
      dbSetOrder(01)
   	If dbSeek(xFilial('STE')+ST4->T4_TIPOMAN)
	   	If STE->TE_CARACTE != 'C'
		   	MsgStop(STR0079,STR0037) //"ServiÁo dever· ser do tipo Corretivo!"###"ATEN«√O"
			   lRet := .F.
		   EndIf
	   EndIf
   EndIf
...
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
±±⁄ƒƒƒƒƒƒƒƒƒƒ¬ƒƒƒƒƒƒƒƒƒƒ¬ƒƒƒƒƒƒƒ¬ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ¬ƒƒƒƒƒƒ¬ƒƒƒƒƒƒƒƒƒƒø±±
±±≥FunáÖo    ≥MNTA656SER≥ Autor ≥ Marcos Wagner Junior  ≥ Data ≥ 15/10/08 ≥±±
±±√ƒƒƒƒƒƒƒƒƒƒ≈ƒƒƒƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒƒƒƒ¥±±
±±≥DescriáÖo ≥ Gatilho dos campos referentes a "Servico"                  ≥±±
±±√ƒƒƒƒƒƒƒƒƒƒ≈ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ¥±±
±±≥ Uso      ≥ MNTA656                                                    ≥±±
±±¿ƒƒƒƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒŸ±±
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
...
Function MNTA656VAL()
	...
	ElseIf Empty(cTTAComb) .And. !lTpLub
		MsgStop(STR0211,STR0037) //"CÛdigo do combustÌvel do abastecimento n„o foi informado"###"ATEN«√O"
		Return .F.
	EndIf
	...
	If Inclui .Or. Altera
		...
		For nI := 1 to Len(aCols)
			...
			If Inclui .OR. (Altera .And. aScan(aOldCols,{|x| DtoS(x[nPOSDATAB])+x[nPOSHORAB]+x[nPOSFROTA] == DtoS(aCols[nI][nPOSDATAB])+aCols[nI][nPOSHORAB]+aCols[nI][nPOSFROTA] }) == 0)
				If !aCols[nI][Len(aHeader)+1]
					/*cFil := MNTA656FIL(aCols[nI][nPOSFROTA],aCols[nI][nPOSDATAB],aCols[nI][nPOSHORAB],aCols[nI][nPOSPLACA])
					 Substituido funÁ„o pela filial corrente da STJ, para que a OS de lubrificaÁ„o ao ser incluida utilize
...
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
±±⁄ƒƒƒƒƒƒƒƒƒƒ¬ƒƒƒƒƒƒƒƒƒƒ¬ƒƒƒƒƒƒƒ¬ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ¬ƒƒƒƒƒƒ¬ƒƒƒƒƒƒƒƒƒƒø±±
±±≥FunáÖo    ≥MNT656LTB ≥ Autor ≥ Marcos Wagner Junior  ≥ Data ≥ 15/10/08 ≥±±
±±√ƒƒƒƒƒƒƒƒƒƒ≈ƒƒƒƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒƒƒƒ¥±±
±±≥DescriáÖo ≥ Limpa campos Loja, Tanque e Bomba quando informado Posto   ≥±±
±±√ƒƒƒƒƒƒƒƒƒƒ≈ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ¥±±
±±≥ Uso      ≥ MNTA656                                                    ≥±±
±±¿ƒƒƒƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒŸ±±
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
...
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
±±⁄ƒƒƒƒƒƒƒƒƒƒ¬ƒƒƒƒƒƒƒƒƒƒ¬ƒƒƒƒƒƒƒ¬ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ¬ƒƒƒƒƒƒ¬ƒƒƒƒƒƒƒƒƒƒø±±
±±≥FunáÖo    |MNT655POIN≥ Autor ≥ Marcos Wagner Junior  ≥ Data ≥ 15/10/08 ≥±±
±±√ƒƒƒƒƒƒƒƒƒƒ≈ƒƒƒƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒƒƒƒ¥±±
±±≥DescriáÖo ≥ Verifica se o Posto/Loja digitado(a) e um Posto Interno    ≥±±
±±√ƒƒƒƒƒƒƒƒƒƒ≈ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ¥±±
±±≥ Uso      ≥ MNTA656                                                    ≥±±
±±¿ƒƒƒƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒŸ±±
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
...
If dbSeek(xFilial("TQF")+cPosto+cLoja) .And. !Empty(cPosto) .And. !Empty(cLoja)
	If TQF->TQF_TIPPOS != '2'
		MsgStop(STR0044,STR0037) //"Posto informado n„o È um Posto Interno!"###"ATEN«√O"
		lRet := .F.
	EndIf
EndIf
...
@since   15/10/08
@version P11/P12
@param	 nPar	  , NumÈrico, 1 - Tanque / 2 - Bomba
...
Function MNT656TABO(nPar, dDataAbs, cHrAbas)
	...
	If Empty( cCombust )
		...
		If dbSeek(xFilial("TQI") + cPosto + cLoja + cTanque + cCombBKP)
			cCombust := TQI->TQI_CODCOM //CÛdigo do combustÌvel.
		EndIf
	EndIf
	...
	If nPar == 1
		...
		If Empty(cTTAComb1)
			// Caso o combustÌvel esteja vazio, pega o primeiro combustÌvel do tanque, visto que em geral, sÛ haver· um combustÌvel por tanque.
			cTTAComb1 := cCodCom
		EndIf

		lChkLote   := .F.

		//ValidaÁ„o para que n„o permita selecionar um produto com controle de Rastro
		If GetNewPar("MV_RASTRO","N") == "S" .And. NGSEEK('SB1',cProComb,1,"B1_RASTRO") <> "N"
			...
			Else
				If SB1->B1_RASTRO <> "N" .And. !Empty(SB1->B1_RASTRO)
					MsgStop(STR0206) //"N„o È possivel infomar um produto que utiliza rastro ou lote/sublote."
					Return .F.
				EndIf
			EndIf
		EndIf
...
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
±±⁄ƒƒƒƒƒƒƒƒƒƒ¬ƒƒƒƒƒƒƒƒƒƒ¬ƒƒƒƒƒƒƒ¬ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ¬ƒƒƒƒƒƒ¬ƒƒƒƒƒƒƒƒƒƒø±±
±±≥FunáÖo    |MNTA656MAR≥ Autor ≥ Marcos Wagner Junior  ≥ Data ≥ 15/10/08 ≥±±
±±√ƒƒƒƒƒƒƒƒƒƒ≈ƒƒƒƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒƒƒƒ¥±±
±±≥DescriáÖo ≥ Alimenta o campo Marcador											  ≥±±
±±√ƒƒƒƒƒƒƒƒƒƒ≈ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ¥±±
±±≥ Uso      ≥ MNTA656                                                    ≥±±
±±¿ƒƒƒƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒŸ±±
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
...
If !aCols[n][Len(aCols[n])] .Or. ReadVar() == 'NCOMBATU' .Or. ReadVar() == 'NLUBINF'
	If nPar == 1
		...
		If aCols[n][nPOSQUANT] <> 0 .And. !Altera
			...
			//InserÁ„o de NoRound, paliativamente, devido ‡ inconsistÍncia de Framework TOTVS
...
@return LÛgico, Define se a folha de abastecimento pode ser salva.
/*/
//----------------------------------------------------------------------------
Function MNT656OK(nPar)
	...
	// No processo de validaÁ„o de linha, caso esteja deletado n„o realiza a validaÁ„o.
	...
	For nI := 1 to Len(aHeader)
		...
		ElseIf !( ( !Empty( oBrw1:aCols[n, nPOSLUBRI] ) .And. !Empty( oBrw1:aCols[n, nPOSQUALU]) .And.;
					...
					ApMsgStop(STR0105,STR0037) //"Dever· ser lanÁado um Produto ou um abastecimento!"###"ATEN«√O"
					Return .F.
		Else
		...
		EndIf
		/*
		MNT659VAL - FunÁ„o que valida abastecimento de acordo com par‚metro MV_NGABAVL
		...
		{{.F.," 47"} - ValidaÁ„o de Esquema Padr„o	 - cÛdigo da inconsistencia pela importaÁ„o
		{.F.," 48"}  - ValidaÁ„o de Estrutura Padr„o - cÛdigo da inconsistencia pela importaÁ„o
		{.F.," 49"}  - ValidaÁ„o de ManutenÁ„o Padr„o- cÛdigo da inconsistencia pela importaÁ„o
		{.F.," 50"}}}- ValidaÁ„o de ManutenÁ„o		 - cÛdigo da inconsistencia pela importaÁ„o
		*/
		...
		//ValidaÁ„o da quantidade de Produto (Aditivo) para o tanque do bem.
		//Verifica se È Produto Aditivo (para o tanque do bem) ou Produto Lubrificante
		dbSelectArea("TT8")
	...
	//Verifica se Data/Hora do abastecimento È valida, de acordo com o parametro MV_MNTQDAB, que indica a data/hora limite para digitaÁ„o de abastecimento.
	lUpdaCols := .F.
		...
		If aRetDtPar[3]
			If ValType(aRetDtPar[2]) == "C"
				If DtoS(oBrw1:aCols[n][nPOSDATAB])+oBrw1:aCols[n][nPOSHORAB] <  DtoS(aRetDtPar[1])+aRetDtPar[2]
					ApMsgStop(STR0090+; //"Nenhuma movimentaÁ„o de abastecimento poder· ser processada antes da data/hora: "
								DtoC(aRetDtPar[1])+" "+aRetDtPar[2]+".",STR0037) //"ATEN«√O"
					Return .F.
				EndIf
			Else
				If oBrw1:aCols[n][nPOSDATAB] <  aRetDtPar[1]
					ApMsgStop(STR0091+; //"Nenhuma movimentaÁ„o de abastecimento poder· ser processada antes da data: "
	...
	If lAfericao  .And. Inclui .And. nLanca <> STR0018 //Verificar se o Tipo LanÁamento È diferente de Produto
		lTemAfer := .F.
		//Verifica a aferiÁ„o da bomba
		dbSelectArea("TQL")
		dbSetOrder(1)
		...
		If !lTemAfer
			ShowHelpDlg("ATEN«√O",{STR0154},2,{STR0155},2)//"ATENCAO"###"Tanque/Bomba n„o possui aferiÁ„o para data do abastecimento."###"Inclua uma aferiÁ„o para a mesma data e com hora infeior ao abastecimento ou selecione um Tanque/Bomba aferido."
			Return .F.
		EndIf
	EndIf

	If nPar == 2 //  ValidaÁ„o TUDOOK
				...
				If (nLanca == STR0006 .OR. nLanca == STR0018) //"Abastecimento+Produto"###"Produto"
					...
					If !Empty(oBrw1:aCols[n][nPOSPLACA])
						...
						If dbSeek(oBrw1:aCols[n][nPOSPLACA] + "A")
							If xFilial("ST9",cFilAnt) <> ST9->T9_FILIAL .And. !Empty(ST9->T9_FILIAL)
								MsgStop(STR0130+Chr(13)+Chr(10)+; //"Para Folha com opÁ„o de lanÁamento de Produtos somente poder„o ser digitados veÌculos da filial em uso."
								STR0132+ST9->T9_FILIAL+Chr(13)+Chr(10)+; //"Filial do VeÌculo: "
					...
					Else
						...
						If dbSeek(xFilial("ST9") + oBrw1:aCols[nI][nPOSFROTA])
							...
							If !lRet
								MsgStop(STR0130+Chr(13)+Chr(10)+; //"Para Folha com opÁ„o de lanÁamento de Produtos somente poder„o ser digitados veÌculos da filial em uso."
									STR0131+oBrw1:aCols[nI][nPOSFROTA]+Chr(13)+Chr(10)+; //"VeÌculo: "
									STR0132+ST9->T9_FILIAL+Chr(13)+Chr(10)+; //"Filial do VeÌculo: "
									STR0133+cFilAnt) //"Filial Em Uso: "
								Return .F.
							EndIf
		...
		If Len(nLanca) != Len(cAbast656) //Se Lancamento diferente de "Abastecimento"
			If !lInformLub
				ApMsgStop(STR0098,STR0037) //"Dever· existir pelo menos 1 lanÁamento de produto!"###"ATEN«√O"
				Return .F.
			EndIf
		EndIf
		If Len(nLanca) != Len(cLubri656) //Se Lancamento diferente de "Produto"
			If	!lInformAba
				ApMsgStop(STR0099,STR0037) //"Dever· existir pelo menos 1 lanÁamento de abastecimento!"###"ATEN«√O"
				Return .F.
			EndIf
		EndIf
	EndIf
	...
	If lChkLote .And. nPar == 1
		...
		If nPosR <> n	//grava as informacoes de rastreabilidade numa array
			...
			//aglutina as qtds dos lotes/localizaÁoes iguais
		...
		Else
			//checa se total por lotes/localizaÁoes existe saldo...
			nPOSX := Ascan(aQtdRas,{|x| x[1]+x[2]+x[3] == aRastro[nPosR][1]+aRastro[nPosR][3]+aRastro[nPosR][2]})
			If nPOSX = 0
...
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
±±⁄ƒƒƒƒƒƒƒƒƒƒ¬ƒƒƒƒƒƒƒƒƒƒ¬ƒƒƒƒƒƒƒ¬ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ¬ƒƒƒƒƒƒ¬ƒƒƒƒƒƒƒƒƒƒø±±
±±≥FunáÖo    |MNT656VALU≥ Autor ≥ Marcos Wagner Junior  ≥ Data ≥ 17/10/08 ≥±±
±±√ƒƒƒƒƒƒƒƒƒƒ≈ƒƒƒƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒƒƒƒ¥±±
±±≥DescriáÖo ≥ Busca o valor unitario do combustivel							  ≥±±
±±√ƒƒƒƒƒƒƒƒƒƒ≈ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ¥±±
±±≥ Uso      ≥ MNTA656                                                    ≥±±
±±¿ƒƒƒƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒŸ±±
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
		...
		If dbSeek(xFilial("TQJ")+cPosto+cLoja+cTanque+cBomba)
			...
			If AliasInDic("TTV")
				...
				If nCombAtu < nCombAnt
					lGravaTQJ := .T.
					//C·lculo para evitar que a vari·vel nCombTot fique negativa (situaÁ„o de quebra de contador de bomba)
...
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
±±⁄ƒƒƒƒƒƒƒƒƒƒ¬ƒƒƒƒƒƒƒƒƒƒ¬ƒƒƒƒƒƒƒ¬ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ¬ƒƒƒƒƒƒ¬ƒƒƒƒƒƒƒƒƒƒø±±
±±≥FunáÖo    |MNT656CLUB≥ Autor ≥Marcos Wagner Junior   ≥ Data ≥ 29/10/08 ≥±±
±±√ƒƒƒƒƒƒƒƒƒƒ≈ƒƒƒƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒƒƒƒ¥±±
±±≥DescriáÖo ≥ Checa o contador do Produto.        						  ≥±±
±±√ƒƒƒƒƒƒƒƒƒƒ≈ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ¥±±
±±≥ Uso      ≥ MNTA656                                                    ≥±±
±±¿ƒƒƒƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒŸ±±
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
...
@param  cFrota       , Caracter, Veiculo a ser utilizado para OS de Lubr.
        cServ        , Caracter, ServiÁo realizado na OS de Lubr.
		cProdLub     , Caracter, Produto utilizado na OS de Lubr.
		nQtdLub      , Numerico, Quantidade do produto lubrificante.
		dDtLub       , Data    , Data de utilizaÁ„o do produto.
		cHrLub       , Caracter, Hora de utilizaÁ„o do produto.
		nPosCont     , Numerico, PosiÁ„o do contador ao utilizar o produto.
		nX           , Numerico, Numero da linha referente ao aCols.
		cAlmoxarifado, Caracter, Almoxarifado de onde obteu-se o produto.
		_nCusto      , Numerico, Custo gerado com a lubrificaÁ„o.
...
Function MNT656LUB(cFrota,cServ,cProdLub,nQtdLub,dDtLub,cHrLub,nPosCont,nX,cAlmoxarifado,_nCusto, cFil, cMoeda)
	...
	//----------------------------------------------------------------------------
	// Deve estar posicionado no SD3 ( MovimentaÁ„o Interna ), para que usu·rio
	// possa manipular o registro, este È o objetivo do PE.
	//----------------------------------------------------------------------------
...
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
±±⁄ƒƒƒƒƒƒƒƒƒƒ¬ƒƒƒƒƒƒƒƒƒƒ¬ƒƒƒƒƒƒƒ¬ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ¬ƒƒƒƒƒƒ¬ƒƒƒƒƒƒƒƒƒƒø±±
±±≥FunáÖo    |MNT656VLUB≥ Autor ≥Evaldo Cecinscki Jr.   ≥ Data ≥ 21/10/08 ≥±±
±±√ƒƒƒƒƒƒƒƒƒƒ≈ƒƒƒƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒƒƒƒ¥±±
±±≥DescriáÖo ≥ Checa se os servicos de Produto foram informados           ≥±±
±±√ƒƒƒƒƒƒƒƒƒƒ≈ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ¥±±
±±≥ Uso      ≥ MNTA656                                                    ≥±±
±±¿ƒƒƒƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒŸ±±
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
...
If M->TQG_ORDENA == "1" .And. Empty(cTroca)
	MsgStop(STR0052,STR0037) //"Para Troca de Produto, deve ser informado o seu respectivo serviÁo."###"ATEN«√O"
	M->TQG_ORDENA := " "
	Return .F.
ElseIf M->TQG_ORDENA == "2" .And. Empty(cReposicao)
	MsgStop(STR0053,STR0037) //"Para ReposiÁ„o de Produto, deve ser informado o seu respectivo serviÁo."###"ATEN«√O"
	M->TQG_ORDENA := " "
	Return .F.
EndIf
...
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
±±⁄ƒƒƒƒƒƒƒƒƒƒ¬ƒƒƒƒƒƒƒƒƒƒ¬ƒƒƒƒƒƒƒ¬ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ¬ƒƒƒƒƒƒ¬ƒƒƒƒƒƒƒƒƒƒø±±
±±≥FunáÖo    |MNT656VIS ≥ Autor ≥Evaldo Cecinscki Jr.   ≥ Data ≥ 22/10/08 ≥±±
±±√ƒƒƒƒƒƒƒƒƒƒ≈ƒƒƒƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒƒƒƒ¥±±
±±≥DescriáÖo ≥ Carrega os campos na Visualizacao                          ≥±±
±±√ƒƒƒƒƒƒƒƒƒƒ≈ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ¥±±
±±≥ Uso      ≥ MNTA656                                                    ≥±±
±±¿ƒƒƒƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒŸ±±
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
		...
		If Len(aCamposU) > 0
			For nX:= 1 to Len(aCamposU)
				If aHeader[aCamposU[nX][2]][10] == 'R'
					//Tratativa realizada para busca de campos de usu·rios quando s„o do Tipo MEMO, pois o mesmo n„o È trazido no resultado da Query.
...
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
±±⁄ƒƒƒƒƒƒƒƒƒƒ¬ƒƒƒƒƒƒƒƒƒƒ¬ƒƒƒƒƒƒƒ¬ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ¬ƒƒƒƒƒƒ¬ƒƒƒƒƒƒƒƒƒƒø±±
±±≥FunáÖo    |MNT656LAN ≥ Autor ≥Evaldo Cecinscki Jr.   ≥ Data ≥ 22/10/08 ≥±±
±±√ƒƒƒƒƒƒƒƒƒƒ≈ƒƒƒƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒƒƒƒ¥±±
±±≥DescriáÖo ≥ Checa o tipo de lancamento para habilitar/desabilitar campo≥±±
±±√ƒƒƒƒƒƒƒƒƒƒ≈ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ¥±±
±±≥ Uso      ≥ MNTA656                                                    ≥±±
±±¿ƒƒƒƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒŸ±±
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
...
@param nPos, NumÈrico, Posicionamento do campo.

@author Evaldo Cevinscki Jr
@since 22/10/08

@return lRet, LÛgico, Retorna verdadeiro caso n„o entre em nenhuma situaÁ„o.
/*/
//-----------------------------------------------------------------------
Function MNT656COM(oGetBrw,nPos)
	...
	//-------------------------------------------------------------------------------------
	// Controle de sem·foro para o cÛdigo do bem
	// Faz lock do cÛdigo do bem, para que seja possÌvel apenas um reporte de combustÌvel
	// para o mesmo bem por vez, esse ajuste foi feito para controlar a integridade de
	// registro do contador, pois ocorria inconsistÍncia na base quando dois usu·rios
	...
	If lRet .And. FunName() == "MNTA656" .And. ReadVar() == "M->TQN_FROTA"
		If !LockByName( "MNTA656" + M->TQN_FROTA ) // STR0168
			ShowHelpDlg(STR0037,{STR0168 + M->TQN_FROTA},3,; // "J· existe um reporte de abastecimento sendo feito para o veÌculo ";
				{STR0169},3) // "Aguarde um instante e tente novamente."
			Return .F.
		Else
			// Adiciona no array para fazer a liberaÁ„o dos cÛdigos quando sair da rotina
	...
	If lRet .And. nPos > 0 .And. nPos != n .And. !aCols[nPos][Len(aCols[nPos])]
		MsgStop(STR0055)  //"J· existe um lanÁamento para esse bem com mesma data e hora!."
		lRet := .F.
	EndIf

	If lRet .And. FunName() == "MNTA656"
		If Altera .And. M->TQN_FROTA != aCols[n][nPOSFROTA] .And. !Empty(aCols[n][nPOSLUBRI])
			MsgStop(STR0062) //"Bem n„o poder· ser alterado, pois o abastecimento relacionado possui O.S. de LubrificaÁ„o"
			lRet := .F.
		EndIf
	EndIf
	...
	// Caso a validaÁ„o do campo tenha retornado falso, ent„o libera o cÛdigo que havia lockado
	If !lRet .And. FunName() == "MNTA656" .And. ReadVar() == "M->TQN_FROTA"
...
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
±±⁄ƒƒƒƒƒƒƒƒƒƒ¬ƒƒƒƒƒƒƒƒƒƒ¬ƒƒƒƒƒƒƒ¬ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ¬ƒƒƒƒƒƒ¬ƒƒƒƒƒƒƒƒƒƒø±±
±±≥FunáÖo    |C         ≥ Autor ≥Evaldo Cecinscki Jr.   ≥ Data ≥ 22/10/08 ≥±±
±±√ƒƒƒƒƒƒƒƒƒƒ≈ƒƒƒƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒƒƒƒ¥±±
±±≥DescriáÖo ≥ Verifica a resolucao do monitor e o tema p/ montar tela    ≥±±
±±√ƒƒƒƒƒƒƒƒƒƒ≈ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ¥±±
±±≥ Uso      ≥ MNTA656                                                    ≥±±
±±¿ƒƒƒƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒŸ±±
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
...
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
±±⁄ƒƒƒƒƒƒƒƒƒƒ¬ƒƒƒƒƒƒƒƒƒƒ¬ƒƒƒƒƒƒƒ¬ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ¬ƒƒƒƒƒƒ¬ƒƒƒƒƒƒƒƒƒƒø±±
±±≥FunáÖo    ≥MNTA656AUT≥ Autor ≥ Marcos Wagner Junior  ≥ Data ≥27/10/2008≥±±
±±√ƒƒƒƒƒƒƒƒƒƒ≈ƒƒƒƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒƒƒƒ¥±±
±±≥DescriáÖo ≥Programa para calcular a autonomia do veiculo.              ≥±±
±±√ƒƒƒƒƒƒƒƒƒƒ≈ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ¥±±
±±≥ Uso      ≥ SigaMNT                                                    ≥±±
±±√ƒƒƒƒƒƒƒƒƒƒ≈ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ¥±±
				...
				If nKmB > 0
					...
					If nKmA > nKmB  .And. aKmsR[1] <> 0
						If !aKmsR[8] .And. lGetVar .And. FunName() $ "MNTA656"
							If MsgYesNo(STR0063,STR0037) //"Essa posiÁ„o do contador superou a autonomia, porÈm È o primeiro lanÁamento de contador. Confirma?"###"ATEN«√O"
								lMsg := .T.
							EndIf
						EndIf
						If !lMsg
							If lMsgAuton
								cMensag:= STR0064+Chr(13)+; //"Essa posiÁ„o do contador superou a autonomia do veÌculo."
								STR0065+"("+AllTrim(Str(nPercAuto))+"%)."+Chr(13)+; //"Entretanto est· dentro do percentual toler·vel "
								STR0066+Chr(10)+Chr(10)+; //"Deseja confirmar?"
								STR0067+".........: "+AllTrim((cBem))+Chr(10)+; //"VeÌculo"
...
FunÁ„o que ir· deletar a Folha de Abastecimento

@param   cAlias, Caracter, Tabela do registro.
@param   nRecno, NumÈrico, NumeraÁ„o do registro em base.
@param   nOpcx , NumÈrico, OperaÁ„o realizada (5-deleÁ„o).
@param   nProgram, NumÈrico, 1 - Rotina MNTA656
							 2 - Rotina MNTA681
@param  cFilProc, Caracter, Informa qual filial far· as validaÁoes
@author  Marcos Wagner Junior
@since   29/10/08
@return  .T.
@version P12
...
//-------------------------------------------------------------------
 Function MNTA656DEL(cAlias,nRecno,nOpcx,nProgram,cFilProc)
	...
	Local lRet       := .T.
	Local cMensag    := STR0074+Chr(13)+Chr(10) //"OperaÁ„o n„o permitida. Para exclus„o da Folha deve-se:"
	Local lExitTQN   := .F.
	Local aRetDtPar  := NgDtAbas()
	Local i          := 0
	Local aEMPFIL    := {}
	Local dDtBlqMov  := SuperGetMV( "MV_DBLQMOV",.F.,STOD("") ) //data de bloqueio de movimentaÁıes no estoque.
	Local aValAbast  := {}
	Local lPrimeiro  := .T.
	Local cOrdens    := ""
	...
	Private cFiliST9 := cFilProc	//Vari·vel utilizada em chamadas de funÁ„o do MNTA655
	...
	While !EoF()
		If cMDataHora < (cAliasQry)->TTA_DTABAS+(cAliasQry)->TTA_HRABAS
			cMensag := STR0125 //"DeleÁ„o permitida apenas para a ˙ltima folha lanÁada."
	...
	For i:=1 to Len(aEMPFIL)
		...
		cQuery += "    AND D_E_L_E_T_ <> '*' "
		//Deve ser avaliado na SS 033890 se deve ser mantido por conta da implementaÁ„o no padr„o.
				...
				If lPrimeiro
					If !Empty( (cAliasQry)->TQN_DTCON )
						If STOD( (cAliasQry)->TQN_DTABAS ) <= dDtBlqMov //Se a data do abastecimento for menor/igual que a data de bloqueio de abastecimento (MV_DBLQMOV);
							//"A folha de abastecimento 'x' n„o pode ser excluÌda porque a data de abastecimento do veÌculo 'x' È menor ou igual que a data de bloqueio de movimentaÁıes 'y'. Conforme par‚metro (MV_DBLQMOV)."
	...
	If (cAlsTQN)->( !EoF() )
		...
		cOrdens += STR0242 + Space( 19 + Len( STJ->TJ_ORDEM ) ) +  STR0243 + Space( 17 + Len( STJ->TJ_PLANO ) ) +;
			STR0244 + CRLF // Ordem ## Plano ## CÛdigo Bem
		...
		/*
			- Reabrir a(s) O.S. de lubrificaÁ„o atravÈs da rotina de Reabertura de O.S. (MNTA880)
			- Cancelar as O.S. reabertas atravÈs de uma das rotinas de Retorno O.S.
			As ordens de serviÁos relacionadas aos abastecimentos s„o:
		*/
	...
	If !lRet
		...
		DEFINE MSDIALOG oDlgl TITLE STR0237 From 3,0 TO 340,417 COLOR CLR_BLACK,CLR_WHITE PIXEL // "Verificar Ordem de ServiÁo"
...
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
±±⁄ƒƒƒƒƒƒƒƒƒƒ¬ƒƒƒƒƒƒƒƒƒƒ¬ƒƒƒƒƒƒƒ¬ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ¬ƒƒƒƒƒƒ¬ƒƒƒƒƒƒƒƒƒƒø±±
±±≥FunáÖo    ≥MNT656LISE≥ Autor ≥ Marcos Wagner Junior  ≥ Data ≥ 26/11/08 ≥±±
±±√ƒƒƒƒƒƒƒƒƒƒ≈ƒƒƒƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒƒƒƒ¥±±
±±≥DescriáÖo ≥ Limpa os campos referentes a Servico 					        ≥±±
±±√ƒƒƒƒƒƒƒƒƒƒ≈ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ¥±±
±±≥ Uso      ≥ MNTA656                                                    ≥±±
±±¿ƒƒƒƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒŸ±±
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
...
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
±±⁄ƒƒƒƒƒƒƒƒƒƒ¬ƒƒƒƒƒƒƒƒƒƒ¬ƒƒƒƒƒƒƒ¬ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ¬ƒƒƒƒƒƒ¬ƒƒƒƒƒƒƒƒƒƒø±±
±±≥FunáÖo    ≥MNT656LIDE≥ Autor ≥ Marcos Wagner Junior  ≥ Data ≥ 05/12/08 ≥±±
±±√ƒƒƒƒƒƒƒƒƒƒ≈ƒƒƒƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒƒƒƒ¥±±
±±≥DescriáÖo ≥ Limpa os campos referentes a Servico 					        ≥±±
±±√ƒƒƒƒƒƒƒƒƒƒ≈ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ¥±±
±±≥ Uso      ≥ MNTA656                                                    ≥±±
...
ElseIf aCols[n][Len(aCols[n])]
	...
	If !LockByName( "MNTA656" + aCols[n][nPOSFROTA] ) // STR0168
		ShowHelpDlg(STR0037,{STR0168 + aCols[n][nPOSFROTA]},3,; // "J· existe um reporte de abastecimento sendo feito para o veÌculo ";
			{STR0169},3) // "Aguarde um instante e tente novamente."
		Return .F.
	EndIf
...
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
±±⁄ƒƒƒƒƒƒƒƒƒƒ¬ƒƒƒƒƒƒƒƒƒƒ¬ƒƒƒƒƒƒƒ¬ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ¬ƒƒƒƒƒƒ¬ƒƒƒƒƒƒƒƒƒƒø±±
±±≥FunáÖo    ≥MNT656CABO≥ Autor ≥ Marcos Wagner Junior  ≥ Data ≥ 15/12/08 ≥±±
±±√ƒƒƒƒƒƒƒƒƒƒ≈ƒƒƒƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒƒƒƒ¥±±
±±≥DescriáÖo ≥ Carrega o campo referente ao abastecimento anterior		  ≥±±
±±√ƒƒƒƒƒƒƒƒƒƒ≈ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ¥±±
±±≥ Uso      ≥ MNTA656                                                    ≥±±
±±¿ƒƒƒƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒŸ±±
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
...
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
±±⁄ƒƒƒƒƒƒƒƒƒƒ¬ƒƒƒƒƒƒƒƒƒƒ¬ƒƒƒƒƒƒƒ¬ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ¬ƒƒƒƒƒƒ¬ƒƒƒƒƒƒƒƒƒƒø±±
±±≥FunáÖo    ≥MNT656ENC ≥ Autor ≥ Marcos Wagner Junior  ≥ Data ≥ 05/12/08 ≥±±
±±√ƒƒƒƒƒƒƒƒƒƒ≈ƒƒƒƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒƒƒƒ¥±±
±±≥DescriáÖo ≥ Valida os abastecimentos com relacao a data e hora da folha≥±±
±±√ƒƒƒƒƒƒƒƒƒƒ≈ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ¥±±
±±≥ Uso      ≥ MNTA656                                                    ≥±±
±±¿ƒƒƒƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒŸ±±
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
...
Local lAferSaida := .F. //AferiÁ„o/SaÌda
...
If !Empty(cPosto) .And. !Empty(cLoja) .And. !Empty(cTanque) .And. ;
	...
	If Inclui
		...
		//Verifica se h· reporte de contador de abastecimento em lote apÛs data
		If NGVDHBomba(cPosto,cLoja,cTanque,cBomba,dData,cHora,"'2'")
			lAbastLote := .T.
		End
		//Verifica se h· reporte de contador de aferiÁ„o/saÌda apÛs data
		If NGVDHBomba(cPosto,cLoja,cTanque,cBomba,dData,cHora,"'3'")
			lAferSaida := .T.
		EndIf
		//Verifica se houve algum reporte apÛs a data do abastecimento
		If lAbastLote .Or. lAferSaida
			// Verifica se existe tanto um abastecimento em lote quanto uma aferiÁ„o/saÌda apÛs a data do abastecimento
			If lAbastLote .And. lAferSaida
				ShowHelpDlg(STR0037,{STR0162},3,; //"J· existe um Abastecimento em Lote e AferiÁ„o(ıes) de Bomba(s)/SaÌda(s) de CombustÌvel com data/hora superior a informada."
										{STR0163},3) //"Exclua os Abastecimento em Lote e as AferiÁıes/SaÌdas cadastradas com data/hora superior ou altere a data/hora deste abastecimento."
			//Verifica se h· um abastecimento em lote apÛs a data do abastecimento
			ElseIf lAbastLote
				ShowHelpDlg(STR0037,{STR0164},3,; //"J· existe um Abastecimento(s) em Lote de CombustÌvel com data/hora superior a informada."
										{STR0165},3) //"Exclua os Abastecimento em Lote cadastrados com data/hora superior ou altere a data/hora deste abastecimento."
			//Se for aferiÁ„o/saÌda
			ElseIf lAferSaida
				ShowHelpDlg(STR0037,{STR0166},3,; //"J· existe AferiÁ„o(ıes) de Bomba(s)/SaÌda(s) de CombustÌvel com data/hora superior a informada."
										{STR0167},3) //"Exclua as AferiÁıes/SaÌdas cadastradas com data/hora superior ou altere a data/hora deste abastecimento."
			EndIf
			Inclui := .T.
			Return .F.
		EndIf
...
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
±±⁄ƒƒƒƒƒƒƒƒƒƒ¬ƒƒƒƒƒƒƒƒƒƒ¬ƒƒƒƒƒƒƒ¬ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ¬ƒƒƒƒƒƒ¬ƒƒƒƒƒƒƒƒƒƒø±±
±±≥FunáÖo    ≥MNTA656CAP≥ Autor ≥ Marcos Wagner Junior  ≥ Data ≥ 18/12/08 ≥±±
±±√ƒƒƒƒƒƒƒƒƒƒ≈ƒƒƒƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒƒƒƒ¥±±
±±≥DescriáÖo ≥ Carrega a capacidade maxima do tanque do veiculo           ≥±±
±±√ƒƒƒƒƒƒƒƒƒƒ≈ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ¥±±
±±≥ Uso      ≥ MNTA656                                                    ≥±±
±±¿ƒƒƒƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒŸ±±
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
...
/*/
 Function MNTA656CAP()
	...
	If !lTpLub //nao valida quando for somente Produto
		...
		If !lComb
			MsgStop(STR0054)  //"CombustÌvel do tanque do posto È incompatÌvel com o combustÌvel do bem."
			lRet := .F.
		EndIf
	EndIf
...
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
±±⁄ƒƒƒƒƒƒƒƒƒƒ¬ƒƒƒƒƒƒƒƒƒƒ¬ƒƒƒƒƒƒƒ¬ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ¬ƒƒƒƒƒƒ¬ƒƒƒƒƒƒƒƒƒƒø±±
±±≥FunáÖo    ≥MNTA656FIL≥ Autor ≥ Marcos Wagner Junior  ≥ Data ≥ 20/12/08 ≥±±
±±√ƒƒƒƒƒƒƒƒƒƒ≈ƒƒƒƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒƒƒƒ¥±±
±±≥DescriáÖo ≥ Carrega a filial do bem                         	        ≥±±
±±√ƒƒƒƒƒƒƒƒƒƒ≈ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ¥±±
±±≥ Uso      ≥ MNTA656                                                    ≥±±
±±¿ƒƒƒƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒŸ±±
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
...
±±≥FunáÑo    ≥MNT656FIL  ≥ Autor ≥Marcos Wagner Junior  ≥ Data ≥20/12/2008≥±±
±±√ƒƒƒƒƒƒƒƒƒƒ≈ƒƒƒƒƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒƒƒƒ¥±±
±±≥DescriáÑo ≥Filtra os abastecimentos em lote.                           ≥±±
±±√ƒƒƒƒƒƒƒƒƒƒ≈ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ¥±±
...
±±√ƒƒƒƒƒƒƒƒƒƒ≈ƒƒƒƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒƒƒƒ¥±±
±±≥DescriáÖo ≥Define posicionamento da getdados                           |±±
±±√ƒƒƒƒƒƒƒƒƒƒ≈ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ¥±±
±±≥Retorno   ≥Nenhum                                                      ≥±±
±±¿ƒƒƒƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒŸ±±
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
...
Local nIReAltu := 339
Local nPorcVert := 1 //% em relaÁ„o a resolucao maxima
...
Function fParam()
	...
	Define MsDialog oDlgPar From 0,0 To 200,420 Title "Par‚metros" Pixel Style DS_MODALFRAME
		...
		/*Barra inferior com os botıes OK/Cancelar*/
...
Efetiva os valores salvos no par‚metro MV_MNTQDAB na funÁ„o fParam
...
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
±±⁄ƒƒƒƒƒƒƒƒƒƒ¬ƒƒƒƒƒƒƒƒƒƒ¬ƒƒƒƒƒƒƒ¬ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ¬ƒƒƒƒƒƒ¬ƒƒƒƒƒƒƒƒƒƒø±±
±±≥FunáÑo    ≥f656Change≥ Autor ≥ Marcos Wagner Junior  ≥ Data ≥20/12/2008≥±±
±±√ƒƒƒƒƒƒƒƒƒƒ≈ƒƒƒƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒƒƒƒ¥±±
±±≥DescriáÑo ≥ Funcao chamada ao trocar a linha da aCols                  ≥±±
±±√ƒƒƒƒƒƒƒƒƒƒ≈ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ¥±±
±±≥ Uso      ≥ MNTA990                                                    ≥±±
±±¿ƒƒƒƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒŸ±±
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
...
±±≥DescriáÖo ≥Checa se lancamento da folha eh somente de Produto           ≥±±
...
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
±±⁄ƒƒƒƒƒƒƒƒƒƒ¬ƒƒƒƒƒƒƒƒƒƒ¬ƒƒƒƒƒƒƒ¬ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ¬ƒƒƒƒƒƒ¬ƒƒƒƒƒƒƒƒƒƒø±±
±±≥FunáÑo    ≥MNTA656BLO≥ Autor ≥ Marcos Wagner Junior  ≥ Data ≥13/01/2009≥±±
±±√ƒƒƒƒƒƒƒƒƒƒ≈ƒƒƒƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒƒƒƒ¥±±
±±≥DescriáÑo ≥ Bloqueia Posto, Loja							                    ≥±±
±±√ƒƒƒƒƒƒƒƒƒƒ≈ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ¥±±
±±≥ Uso      ≥ MNTA990                                                    ≥±±
±±¿ƒƒƒƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒŸ±±
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
...
±±√ƒƒƒƒƒƒƒƒƒƒ≈ƒƒƒƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒƒƒƒ¥±±
±±≥DescriáÖo ≥Valida o campo TL_LOTECTL                                   ≥±±
±±¿ƒƒƒƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒŸ±±
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
ﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂ
/*/
...
Function NG656ALM( nITE )
	...
	For nX := 1 To Len( aCols ) //Percorre o Acols.
		If cCodLubri == oBrw1:aCols[nX][nCodigo] //Verifica se o produto que est· sendo incluso È o mesmo que contÈm no Acols.
			If !aCols[nX][Len(aCols[nX])] //E se a linha n„o estiver deletada.
				nQtdLubr += aCols[nX][nLubrif] //Atribui a quantidade de saldo para a vari·vel que ser· passada como par‚metro para a funÁ„o NGSALSB2().
			EndIf
		EndIf
	Next nX
...
±±√ƒƒƒƒƒƒƒƒƒƒ≈ƒƒƒƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒƒƒƒ¥±±
±±≥DescriáÖo ≥Valida o campo TL_NUMLOTE                                   ≥±±
±±¿ƒƒƒƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒŸ±±
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
ﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂ
/*/
...
±±√ƒƒƒƒƒƒƒƒƒƒ≈ƒƒƒƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒƒƒƒ¥±±
±±≥DescriáÖo ≥Valida o campo TL_LOCALIZ                                   ≥±±
±±¿ƒƒƒƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒŸ±±
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
ﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂ
/*/
...
±±√ƒƒƒƒƒƒƒƒƒƒ≈ƒƒƒƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒƒƒƒ¥±±
±±≥DescriáÖo ≥ Verifica campos obrigatorios                               ≥±±
...
±±√ƒƒƒƒƒƒƒƒƒƒ≈ƒƒƒƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒƒƒƒ¥±±
±±≥DescriáÖo ≥ X3_WHEN dos campos Quantidade, Motorista, Contador         ≥±±
±±¿ƒƒƒƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒŸ±±
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
ﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂ
/*/
...
@param   nI        , NumÈrico, Linha aCols
@param   dDtabas   , Data    , Data do abastecimento
@param   lCCivil   , LÛgico  , Define se È do ConstruÁ„o Civil
...
±±√ƒƒƒƒƒƒƒƒƒƒ≈ƒƒƒƒƒƒƒƒƒƒ1¡ƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒƒƒ¥±±
±±≥DescriáÖo ≥ Abre tela para informar registro de afericao               |±±
±±√ƒƒƒƒƒƒƒƒƒƒ≈ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ¥±±
±±≥Retorno   ≥Nenhum                                                      ≥±±
±±¿ƒƒƒƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒŸ±±
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
...
DEFINE MSDIALOG oDlg TITLE STR0135 From 0,0 To 420,620 OF oMainWnd PIXEL //"Registro de saÌda de combustÌvel na bomba"
...
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
±±⁄ƒƒƒƒƒƒƒƒƒƒ¬ƒƒƒƒƒƒƒƒƒƒ¬ƒƒƒƒƒƒƒ¬ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ¬ƒƒƒƒƒƒ¬ƒƒƒƒƒƒƒƒƒƒø±±
±±≥FunáÖo    ≥MNTTHALLOK ≥ Autor ≥ Denis Hyroshi de Souza≥ Data ≥ 15/01/09≥±±
±±√ƒƒƒƒƒƒƒƒƒƒ≈ƒƒƒƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒƒƒƒ¥±±
±±≥DescriáÖo ≥ Valida confirmacao da tela de Devolucao Parcial            ≥±±
±±¿ƒƒƒƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒŸ±±
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
ﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂ
/*/
Function MNTTHALLOK()
Local nX
Local cMsg := STR0137 //"A quantidade utilizada na aferiÁ„o n„o pode ser maior que o total informado."
Local nTotalDig := 0
...
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
±±⁄ƒƒƒƒƒƒƒƒƒƒ¬ƒƒƒƒƒƒƒƒƒƒ¬ƒƒƒƒƒƒƒ¬ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ¬ƒƒƒƒƒƒ¬ƒƒƒƒƒƒƒƒƒƒø±±
±±≥FunáÖo    ≥MNTTHLINOK≥ Autor ≥ Denis Hyroshi de Souza≥ Data ≥ 15/01/09 ≥±±
±±√ƒƒƒƒƒƒƒƒƒƒ≈ƒƒƒƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒƒƒƒ¥±±
±±≥DescriáÖo ≥ Valida confirmacao da linha de Devolucao Parcial           ≥±±
±±¿ƒƒƒƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒŸ±±
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
ﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂ
/*/
...
Local cMsg := STR0139 //"J· existe registro com a mesma Data/Hora AferiÁ„o."
Local lTemAfer := .F.
...
If Empty(oBrwSaid:aCols[nAc][nPDtAbas])
	HELP(" ",1,STR0138,STR0138,STR0140,3,4) //"ATENCAO"###"ATENCAO"###"O campo Data È obrigatÛrio."
	Return .F.
EndIf

If Empty(oBrwSaid:aCols[nAc][nPHrAbas]) .Or. Alltrim(oBrwSaid:aCols[nAc][nPHrAbas]) == ":"
	HELP(" ",1,STR0138,STR0138,STR0141,3,4) //"ATENCAO"###"ATENCAO"###"O campo Hora È obrigatÛrio."
	Return .F.
EndIf

If Empty(oBrwSaid:aCols[nAc][nPCodCom])
	HELP(" ",1,STR0138,STR0138,STR0142,3,4) //"ATENCAO"###"ATENCAO"###"O campo Combustivel È obrigatÛrio."
	Return .F.
EndIf

If Empty(oBrwSaid:aCols[nAc][nPQuant])
	HELP(" ",1,STR0138,STR0138,STR0143,3,4) //"ATENCAO"###"ATENCAO"###"O campo Quantidade È obrigatÛrio."
	Return .F.
EndIf

If Empty(oBrwSaid:aCols[nAc][nPMotivo])
	HELP(" ",1,STR0138,STR0138,STR0144,3,4) //"ATENCAO"###"ATENCAO"###"O campo Motivo È obrigatÛrio."
	Return .F.
EndIf

If Empty(oBrwSaid:aCols[nAc][nPMatric])
	HELP(" ",1,STR0138,STR0138,STR0145,3,4) //"ATENCAO"###"ATENCAO"###"O campo Matricula È obrigatÛrio."
	Return .F.
EndIf

If oBrwSaid:aCols[nAc][nPDtAbas] > dDataAbast
	MsgStop(STR0146,STR0037) //"Data da AferiÁ„o n„o pode ser maior que a data de encerramento da folha."###"ATEN«√O"
	Return .F.
EndIf
If oBrwSaid:aCols[nAc][nPDtAbas] == dDataAbast .And. oBrwSaid:aCols[nAc][nPHrAbas] > cHrAb656
	MsgStop(STR0147,STR0037) //"Hora da AferiÁ„o n„o pode ser maior que a hora de encerramento da folha."###"ATEN«√O"
	Return .F.
EndIf
...
If lAfericao .And. Inclui .And. nLanca <> STR0018 //Verificar se o Tipo LanÁamento È diferente de Produto
	...
	If !lTemAfer
		ShowHelpDlg(STR0138,{STR0154},2,{STR0155},2)//"ATENCAO"###"Tanque/Bomba n„o possui aferiÁ„o para data do abastecimento."###"Inclua uma aferiÁ„o para a mesma data e com hora infeior ao abastecimento ou selecione um Tanque/Bomba aferido."
		Return .F.
	EndIf
EndIf
...
±±√ƒƒƒƒƒƒƒƒƒƒ≈ƒƒƒƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒƒƒƒ¥±±
±±≥DescriáÖo ≥ Checa se houve quebra/virada de contador e permite lancamen≥±±
...
±±√ƒƒƒƒƒƒƒƒƒƒ≈ƒƒƒƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒƒƒƒ¥±±
±±≥DescriáÖo ≥ Faz alteracoes no dicionario                               ≥±±
±±¿ƒƒƒƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒŸ±±
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
ﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂ
/*/
...
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
±±⁄ƒƒƒƒƒƒƒƒƒƒ¬ƒƒƒƒƒƒƒƒƒƒƒ¬ƒƒƒƒƒƒƒ¬ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ¬ƒƒƒƒƒƒ¬ƒƒƒƒƒƒƒƒƒƒø±±
±±≥FunáÑo    ≥MNA656EXIST≥ Autor ≥Heverson Vitoreti      ≥ Data ≥ 15/02/06 ≥±±
±±√ƒƒƒƒƒƒƒƒƒƒ≈ƒƒƒƒƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒƒƒƒ¥±±
±±≥DescriáÑo ≥Validacao da Frota                                           ≥±±
...
Else
	...
	If M->TQN_FROTA != aCols[n][nPOSFROTA]
		...
		//Atualiza os campos Digitado e DiferenÁa quando alterado o bem
		oLubDig:Refresh()
		oLubDif:Refresh()
	EndIf
...
EndIf
If !lATIVO
	HELP(" ",1,STR0037,,STR0152,3,1) //"ATEN«√O"###"VeÌculo Inativo!"
	Return .F.
EndIf
...
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
±±⁄ƒƒƒƒƒƒƒƒƒƒ¬ƒƒƒƒƒƒƒƒƒƒƒ¬ƒƒƒƒƒƒƒ¬ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ¬ƒƒƒƒƒƒ¬ƒƒƒƒƒƒƒƒƒƒø±±
±±≥FunáÑo    ≥MNA656PLACA≥ Autor ≥Heverson Vitoreti      ≥ Data ≥ 15/02/06 ≥±±
±±√ƒƒƒƒƒƒƒƒƒƒ≈ƒƒƒƒƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒƒƒƒ¥±±
±±≥DescriáÑo ≥Validacao da placa                                           ≥±±
±±√ƒƒƒƒƒƒƒƒƒƒ≈ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ¥±±
±±≥ Uso      ≥MNTA656                                                      ≥±±
±±¿ƒƒƒƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒŸ±±
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
...
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
±±⁄ƒƒƒƒƒƒƒƒƒƒ¬ƒƒƒƒƒƒƒƒƒƒƒ¬ƒƒƒƒƒƒƒ¬ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ¬ƒƒƒƒƒƒ¬ƒƒƒƒƒƒƒƒƒƒø±±
±±≥FunáÑo    ≥MNT656BLOK ≥ Autor ≥ Marcos Wagner Junior  ≥ Data ≥ 28/03/11 ≥±±
±±√ƒƒƒƒƒƒƒƒƒƒ≈ƒƒƒƒƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒƒƒƒ¥±±
±±≥DescriáÑo ≥Bloqueia o campo caso o Produto nao seja preenchido          ≥±±
±±√ƒƒƒƒƒƒƒƒƒƒ≈ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ¥±±
±±≥ Uso      ≥MNTA656                                                      ≥±±
±±¿ƒƒƒƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒŸ±±
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
...
Function MNT656BLOK()
	...
	If GetNewPar("MV_NGMNTCC","N") == "S"
		...
		If !( lCanAplPrd := ( NGSeek('ST9', oBrw1:aCols[n, nPosFrota], 1,'ST9->T9_LUBRIFI') == '1' ) )
			...
			//"AtenÁ„o" ## "Este bem n„o permite aplicaÁ„o de produto."
...
Carrega campo data do abastecimento no getdados da tela de saÌda de combutÌvel.
...
Valida data e hora informada para verficicar se j· n„o h· abastecimentos ou saÌdas para
o mesmo momento da bomba.

@Param cVar  = conte˙do a ser validado
	   nOpcv = opÁ„o da validaÁ„o (1=Data, 2=Hora)
...
If Type("oBrwSaid") == "O" // Verifica e carrega se houver informaÁıes da tela de saÌdas
...
If nOpcv == 1
	...
	EndIf
EndIf
If lTQN // ValidaÁıes dos campos data e hora da primeira tela (tabela TQN)
	nAc := oBrw1:nAt
	...
	ElseIf nOpcv == 2 // Valida Hora
		...
		EndIf
	EndIf
Else // ValidaÁıes para os campas de Data e Hora da tela de saÌdas (TTH)
	nAc := oBrwSaid:nAt
	If nOpcv == 1 // Valida Data
	...
	ElseIf nOpcv == 2 // Valida Hora
		If aSaiCols[nAc][nPDtAbas] == dDataAbast .And. cVar > cHrAb656
			...
			Return .F.
		EndIf
		If (!f656VBomba(cPosto,cLoja,cTanque,cBomba,aSaiCols[nAc][nPDtAbas],cVar))// Consiste se h· saÌdas para a data e hora j· gravadas na base de dados (TTV)
			MsgStop(STR0161) //"J· existe um lanÁamento para essa bomba com mesma data e hora."
			Return .F.
		EndIf
...
Valida h· algum abastecimento ou saÌda superior ou igual a data e hora
que esta sendo reportada.

@Param cPosto = CÛdigo do Posto
	   cLoja = CÛdigo da Loja
	   cTanque = CÛdigo do tanque do posto
	   cBomba = CÛdigo da bomba do tanque
	   dData = Data a ser verificada
	   cHora = Hora a ser verificada
	   cEmp = CÛdigo da empresa
	   cFil = CÛdigo da filial
@author Cezar Augusto Padilha
@since 06/12/2012
...
Function MNTA656FOL( cPostoVal, cLojaVal, cFolhaVal )
	...
	If !Empty(cPostoVal) .And. !Empty(cLojaVal) .And. !Empty(cFolhaVal)
		...
		If !( lRet := !dbSeek(xFilial("TTA") + cPostoVal + cLojaVal + cFolhaVal) )
			...
			//"AtenÁ„o" ## "Esta Folha j· foi informada em outro abastecimento para este posto e loja."
...
V·lida tanque de Combustivel. Verificando se o CombustÌvel do tanque do posto
È incompatÌvel com o combustÌvel do bem.

Essa funÁ„o foi feita para facilitar o entendimento das condiÁıes utilizada
na validaÁ„o.

@param  cCodCom -> CÛdigo do Combustivel.

@author Maicon AndrÈ Pinheiro
@since 25/02/2016
...
@param cFilEmp -> Filial do Bem/Placa que est· executando o abastecimento.
       cCodCom -> CÛdigo do Combustivel.
       cTipCom -> Tipo do Produto (1=CombustÌvel/2=Aditivo)
...
Consulta especÌfica F6 do campo TT_CODIGO para trazer os lubrificantes da tabela TZZ.
	...
	If ReadVar() == "M->TT_CODIGO"
		...
		If lCONDP //Se confirmou a consulta
			...
			If lComb
				...
				EndIf
			Else
				ApMsgStop(STR0185,STR0037)			//"Produto (Aditivo) È incompatÌvel com o tanque do bem."###"ATEN«√O"
...
FunÁ„o para buscar o combustÌvel utilizado no abastecimento do folha.
...
Verifica o combustÌvel digitado.
...
Function MNT656COMB(oOwner)
	...
	If Empty(NGSEEK('TQI',cPosto+cLoja+cTanque+cTTAComb,1,'TQI->TQI_CODCOM')) // Valida CombustÌvel x Tanque posto
		MsgStop(STR0212,STR0037) //"CombustÌvel do abastecimento incompatÌvel com o tanque do posto."###"ATEN«√O"
		Return .F.
	EndIf
	...
	If !Empty(oOwner:aCols[1][nPOSFROTA]) // Verifica se est· alterando o combustÌvel depois de informado o bem.
		cChaveTT8 := xFilial("TT8") + oOwner:aCols[1][nPOSFROTA] + "1" + cTTAComb
		dbSelectArea("TT8")
		dbSetOrder(2)
		If !dbSeek( cChaveTT8 )
			MsgStop(STR0054,STR0037) //"CombustÌvel do tanque do posto È incompatÌvel com o combustÌvel do bem."###"ATEN«√O"
			Return .F.
		EndIf
	EndIf
...
Soma todas as saÌdas de combustÌvel da folha editada

@author Wexlei Silveira
@since 06/04/2017
@return nSomaComb, Total de combustÌvel de saÌda da folha
/*/
//-------------------------------------------------------------------
...
Valida a sobreposiÁ„o de contadores na folha de abastecimento.
...
Function MNT656CONT()
	...
	//Busca a posiÁ„o atual dos contadores digitados.
	If TIPOACOM
			...
			If oBrw1:aCols[nX][nPOSDATAB] == dDataAb
				If TIPOACOM2 .And. nSegCont > 0
					If oBrw1:aCols[nX][nPOSHORAB] > cHoraAb .And. oBrw1:aCols[nX][nPOSCONT2] < nSegCont
						MsgAlert(STR0235, STR0138) //Contador menor a um abastecimento posterior. ## AtenÁ„o
						lRet := .F.
					ElseIf  oBrw1:aCols[nX][nPOSHORAB] < cHoraAb .And. oBrw1:aCols[nX][nPOSCONT2] > nSegCont
						MsgAlert(STR0236, STR0138) //Contador maior a um abastecimento anterior. ## AtenÁ„o
						lRet := .F.
					EndIf
				EndIf
			...
			ElseIf oBrw1:aCols[nX][nPOSDATAB] > dDataAb
				If TIPOACOM2
					If oBrw1:aCols[nX][nPOSCONT2] < nSegCont
						MsgAlert(STR0235, STR0138) //Contador maior a um abastecimento anterior. ## AtenÁ„o
						lRet := .F.
					EndIf
				EndIf
				If TIPOACOM
...
Faz as validaÁıes dos dados que ir„o compor o cabeÁalho

@author  Maicon AndrÈ Pinheiro
@since   18/04/2018
@param   cCampo, C, Coluna da tabela a ser pesquisada no SX3
@return  aArray, a, Array contendo as posiÁıes do aHeader
		 Pos, nome, desc, tipo
...
Static Function fValidHead(cCampo)
	...
	Local cCstWhen  := ""
	Local cValidAdd := "" // ValidaÁ„o adicional
...
Habilita ediÁ„o de contador e traz o ultimo registro do historico

@param   lWhen, lÛgico, Verifica se È chamado pelo When do campo.
...
FunÁ„o que agrupa validaÁıes realizadas ao salvar a tela de abastecimentos.
@type static
...
@param  nOpcX   , N˙merico, Indica operaÁ„o realizada.
@return N˙merico, Indica se oprocesso foi realizado con Íxito.
/*/
//----------------------------------------------------------------------------
