...
Function MDTIntEsoc( cEvento, nOper, xFicMed, aFuncs, lEnvio, oModelTNC, oModelTNY, cMsgInc, cChvRJE )
	...
	Private lMDTA005 := FWIsInCallStack( "MDTA005" ) // Cadastro de Fichas Médicas
	...
	ElseIf ( cEvento == "S-2210" .Or. cEvento == "S-2220" ) .And. Len( aFuncs ) == 0 //Valida se as fichas médicas 
		lIntegra := .F.
	ElseIf cEvento == "S-2240" .And. lMDTA005 .And. Empty( IIf( lEnvio, TM0->TM0_MAT, M->TM0_MAT ) ) //Valida se a ficha médica é de funcionário
		lIntegra := .F.
	EndIf
...
Function MDTVerAPrp( cEvento, cMsgInc )
	...
	If lMiddleware //Validações de ambiente do Middleware
		...
		ElseIf FindFunction( "fVersEsoc" )
			...
			If !( ("S_01_00" $ cVersLyt .Or. "S_01_01" $ cTAFVLES ) .Or. ( "02_05_00" $ cVersLyt .And. fVldMDTAtu() ) )
				cIncons := STR0069 //"Os eventos de SST do eSocial somente serão enviados caso o leiaute seja o S-1.0 simplificado ou o 2.5 e o ambiente esteja atualizado com o pacote da simplificação"
	...
	Else //Validações de ambiente do TAF
		...
		ElseIf aTAF[ 2 ] <> "1.0" .And. aTAF[ 2 ] <> "1.1" //Verifica se o leiaute do eSocial é o mais atual
			...
			cCorrec := STR0014 //"Favor verificar"
		ElseIf !( ( "S_01_00" $ cTAFVLES .Or. "S_01_01" $ cTAFVLES ) .Or. ( "02_05_00" $ cTAFVLES .And. fVldMDTAtu() ) )
			cIncons := STR0069 //"Os eventos de SST do eSocial somente serão enviados caso o leiaute seja o S-1.0 simplificado ou o 2.5 e o ambiente esteja atualizado com o pacote da simplificação"
			cCorrec := STR0070 //"Favor configurar o ambiente nas condições citadas"
		EndIf
	EndIf
...
Function MDTEnvEsoc( cEvento, nOper, aFuncs, oModel, cChvRJE, cMsgInc, cDescAtiv )
	...
	For nCont := 1 To Len( aFuncs )
		...
		If cEvento == "S-2220"
			If IsInCallStack( "NGIMPRASO" ) //Caso for chamado pela impressão do ASO
...
Function MDTGeraXml()
	...
	Private lMDTA005	 := .F. //Define a variável de chamada da rotina de cadastro de ficha médica como .F.
...
Function MDTDadFun( cFicOrMat, lMat )
	...
	If lMat //Caso a matrícula seja passada por parâmetro
		cNumMat := cFicOrMat
	Else
...
Busca a informação do campo CM9_ID, referente ao ID do funcionário no SIGATAF

@return	 cIDFunc, Caracter, ID do funcionário no TAF

@sample	 MDTGetIdFun( "000000001" )

@param	 cMatricula, Caracter, Matricula do funcionário na SRA
@param	 cFilFun, Caracter, Filial do funcionário a ser considerada
...
Function MDTGetIdFun( cMatricula, cFilFun )
	...
	//Por padrão define a filial como sendo a atual
...
Function MDTBscDtEnv( aFuncs, nOper, lXml, dDtAtu )
	...
	// Indica se é alguma rotina de relacionamento do laudo com o risco
	Local lRelLau		:= ;
							FWIsInCallStack( 'MDT181REL' ) ; // Risco
	...
	ElseIf lMDTA005 //Ficha Médica x Tarefas

		If dDtIniTar >= dDtAtu //Lançamento posterior a Data Atual
			dDataEnv := dDtIniTar
		ElseIf dDtIniTar < dDtAtu .And. ( Empty( dDtFimTar ) .Or. dDtFimTar > dDtAtu ) //Lançamento contemplando a Data Atual
			dDataEnv := dDtAtu
		EndIf
...
@return	dDtAtu, Data, Data da exposição mais recente do funcionário

@param	cMatricula, Caracter, Matrícula do funcionário a ter a data buscada
...
Static Function fEpiEnv( cMatricula )
	...
	//Caso for envio via SIGATAF (necessário realizar tratamento para envio via Middleware no Else)
	If !lMiddleware
...
Static Function fVldFunDem( aFuncs, lImpASO )
	...
	For nCont := 1 To Len( aFuncs )
		...
		If !Empty( dDtDemis ) .And. cSitFolh == "D"
			...
			//Caso o ASO tenha sido emitido no dia da demissão ou posteriormente e o ASO for diferente de "Demissional"
