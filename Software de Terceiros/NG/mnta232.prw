...
Function MNTA232(cOrdem,cPlano)
	...
	Local lOpened := .T.
	Local lTudOk  := .F.
	Local xPrepar := If(Type("oMainWnd")!="O",MsgRun( STR0001 , STR0002 , {|| lOpened := !AbreEmpresa() }),Nil)	 //"Preparando Variaveis"###"Aguarde..."
	...
	//Objetos na tela
	Local oDlg, oPanel, oPanelTot, oScrollBox, oMenu
	Local oPnlBtn, oBtnVisual, oBtnEntrada, oBtnHist
	...
	Local cDirExe     := cRootPath+Substr(cStartPath,2,Len(cStartPath))
	...
	If !FindFunction( 'MNTAmIIn' ) .Or. MNTAmIIn( 19, 95 )
		...
		*/
		...
		oDtMov:SetFocus()
		ACTIVATE MSDIALOG oDlg ON INIT EnchoiceBar(@oDlg,{|| lTudOk :=.T.,If(MNT232FIT(),oDlg:End(),lTudOk := .F.)},{||nopcmo:=1,oDlg:End()})
		...
		If nFocus != Nil
			...
			If !lAreaTrans
				If nInvisible == 0
					...
					oTFocus := Nil
				Else
					/*If (nTempPos := aSCAN(aRodizio,{|aArray| aArray[1] == aShape[nFocus][__CODPNEU__] }) > 0
					aTempPneu := aClone(aPNEUSFIM[])
					EndIf*/
...
Static Function CriaCabecalho(oPanel,nAltura,nLargura)
	...
	@ nCabecY-009,nCabecX+220 MsGet oDtMov Var dDTDATEM Of oPanel Valid MNTA231DL() .And. If(NGFUNCRPO("U_MNTA2316",.F.),NGPONTOENTR("MNTA2316",.T.,{dDTDATEM,cHORALE1,0}),.T.) .And. fVerBemPai(oPanel,nCabecX,nCabecY,cIdShapBem,cIdShapCT1) Picture '99/99/9999' Size 45,08 HASBUTTON Pixel
	...
	@ nCabecY+006,nCabecX+220 MsGet cHORALE1 Of oPanel Picture '99:99' Valid fValHrL1(cHORALE1) .And. NGVALHORA(cHORALE1,.T.) .And. MNTA231HL(cHORALE1,1,dDTDATEM) .And.;
	If(NGFUNCRPO("U_MNTA2316",.F.),NGPONTOENTR("MNTA2316",.T.,{dDTDATEM,cHORALE1,1}),.T.) .And. MNTA232CCB(cCodBem, dDTDATEM, cHORALE1)  .And. fVerBemPai(oPanel,nCabecX,nCabecY,cIdShapBem,cIdShapCT1) Size 20,08 Pixel
...
Static Function CriaEixo(oTPanel,lStepe,nAtu,nTop,nLeft)
	...
	For nY := 1 To Len(aEstruturas[nPosEstru][nEstru][nAtu])
		...
		If !lStepe .Or. (lStepe .And. TQ0->TQ0_STEPES != "N" .And. Val(TQ0->TQ0_STEPES) >= nY)
			...
			aTail(aPNEUSINI)[__ITEMCTA__] := ST9->T9_ITEMCTA             //[26] Codigo do produto no estoque antigo
		EndIf
...
Static Function CriaRodape(oPanel,nLargura,nAltRod)
	...
	Local oScroll, oBmpPE
	Local nAltVidas := 20, nAltScroll := (nAltVidas/2)

	Local nLinBmp, nColBmp, nLinSay, nColSay, nBmpPorLin, nBmpPorAtu
	Local nX
...
Static Function ConsisPneu( cPneu, cLocaliz, lConfTela, lConfMov )
	...
	Local lEstNeg    := GetNewPar('MV_ESTNEG','N') == 'N'  // Verifica parametro que indica se aceita estoque negativo.
	...
	-------------------------------------------------------------------------------------------------------------*/
	If lPneEst .And. aScan( aPneusIni, {|x| Alltrim(x[__CODBEM__]) == Alltrim(cPneu)}) == 0
		If Empty(cEstoquST9)
			If !lConfTela
		...
		If !Empty(aPNEUSINI[xy][__CODBEM__])
			...
			Else //Troca de Localizacao
				If aPNEUSINI[xy][__LOCALIZ__] <> aPNEUSFIM[nPOS3][__LOCALIZ__]
					nPOS4 := ASCAN(aBEMLOC,{|x| x[2] == aPNEUSINI[xy][__CODBEM__]})
	...
	*/
	BEGIN TRANSACTION
		...
		For yx := 1 To Len(aARDELSTC)
			cBEMPCOM := cBemPai //aARDELSTC[yx][5] //If(Empty(aARDELSTC[yx][5]),cBEM,aARDELSTC[yx,5])
			dbSelectArea("STC")
			dbSetOrder(1)
				...
				If dbSeek(xFilial("STZ")+aARDELSTC[yx][__CODBEM__]+'E')
					While !Eof() .And. STZ->TZ_FILIAL == xFilial('STZ') .And. STZ->TZ_CODBEM == aARDELSTC[yx][__CODBEM__] .And. STZ->TZ_TIPOMOV = 'E'
						If Empty(STZ->TZ_DATASAI)
							nPOS4 := ASCAN(aBEMLOC,{|x| x[2] == aARDELSTC[yx][__CODBEM__]})
							RecLock("STZ",.F.)
							STZ->TZ_TIPOMOV := 'S'
		...
		For m := 1 to Len(aPNEUSFIM)
			If !Empty(aPNEUSFIM[m][__CODBEM__])
				cBEMPCOM := cBemPai //If(Empty(aARRAYTRB[m,5]),cBEM,aARRAYTRB[m,5])
				dbSelectArea("STC")
				dbSetOrder(1)
	...
	If lRet
		...
		For nInd1 := 1 To Len( aRetCon )
			...
			NGTRETCON( aRetCon[nInd1,1], aRetCon[nInd1,2], aRetCon[nInd1,3], aRetCon[nInd1,4], aRetCon[nInd1,4], , .T. )
