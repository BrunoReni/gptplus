...
Static Function CRIATRBOS(aSTJ, cTRBTEMP, oTmpMrk, oTmpFil)
	...
	#IFNDEF TOP
	cFiltro := ' TJ_FILIAL == "'+xFilial("STJ")+'" '
	cFiltro += ' .And. TJ_LUBRIFI <> "S" .And. TJ_TERMINO = "N" .And. TJ_SITUACA = "L" .And. TJ_ORDEPAI = "' + Space(Len(STJ->TJ_ORDEPAI)) + '" '
	cFiltro += ' .And. TJ_SERVICO <> "'+cSerefor+'" .And. TJ_SERVICO <>"'+cSercons+'" '
	bFiltraBrw := {|| FilBrowse("STJ",@aIndSTJ,@cFiltro)}
	Eval(bFiltraBrw)
	...
	While !Eof()
		...
		RecLock(cTRB402,.T.)
		For nX := 2 to 6
			(cTRB402)->&(aDBF[nX][1]) := &(aDBF[nX][5])
		Next
		For nX := 1 to Len(aCamposReal)
			(cTRB402)->&(aCamposReal[nX]) := &('STJ->'+aCamposReal[nX])
		Next
		(cTRB402)->(MsUnlock())
		dbSelectArea("STJ")
		dbSkip()
	...
	cQuery += "       TJ_SERVICO <> '"+cSerefor+"' And TJ_SERVICO <> '"+cSercons+"' And"
	cQuery += "       D_E_L_E_T_ != '*'"
