...
//      oBrwCambio:AddFilter(STR0006, "EEQ_EVENT <> '602' .And. EEQ_EVENT <> '605' .And. EEQ_EVENT <> '606' .And. EEQ_EVENT <> '609' .And. EEQ_TIPO <> 'F' .And. EEQ_TP_CON <> '3'", .T., .T.)  //"Parcelas de Câmbio" //NCF - 04/07/2019
      oBrwCambio:AddFilter(STR0006, "EEQ_EVENT <> '602' .And. EEQ_EVENT <> '605' .And. EEQ_EVENT <> '606' .And. EEQ_EVENT <> '609' .And. EEQ_TIPO <> 'F' ", .T., .T.)  //"Parcelas de Câmbio" //NCF - 04/07/2019
      aFilAF900 := {}
...
Private oProgress := EasyProgress():New()
      ...
      If lRet
            ...
            If lRet
                  If oMarca:LenMarcados() > 1
                        oProgress:SetProcess({|| lRet := IntegEEQ(oMarca:GetMarcados(), GetIntegFields("LIQUIDA",,lMutParc), 99, oProgress) }, STR0056 ) // "Executando Liquidação em Lote"
                        oProgress:Init()
                  Else
                        MsAguarde({|| lRet := IntegEEQ(oMarca:GetMarcados(), GetIntegFields("LIQUIDA",,lMutParc), 99) }, STR0057 ) // "Executando Liquidação da Parcela"
                  EndIf
                  If lRet
                        MsgInfo( STR0049 + cLote, STR0002 ) //"Operação concluída. Lote de referência: " , "Aviso"
                  EndIf
      ...
      Do Case
            Case cOpc == "CHAVE_EEQ"
                  ...
                  If EEQ->EEQ_TP_CON $ "3|4"
                        ...
                        aAdd(aFields, {"EEQ_MODAL" , if(M->EEQ_MODAL == nil, EEQ->EEQ_MODAL, M->EEQ_MODAL), Nil}) //no estorno não tem a variável em memória
                  EndIf
            Case cOpc == "CHAVE_EEC"
