	...
	If cTipo="3" //DIOPS
		If Pergunte("PDIOPSXML",.T.)
			...
			if lOk
				...
				cArq := ""
				aArqSch := directory(cDirDIOPS + "\Diops" + IIf(B3D->B3D_ANO == "2020", "2019", B3D->B3D_ANO) + ".xsd")
				If Len(aArqSch) == 0
					cArq := "Diops" + IIf(B3D->B3D_ANO == "2020", "2019", B3D->B3D_ANO) + ".xsd"
				EndIf

				If !Empty(cArq)
					cTexto += "O arquivo "+ cDirDIOPS +"\" + cArq + " não foi encontrado no servidor." + Chr(13)
					cTexto += Replicate("-",30) + Chr(13)
					cTexto += Replicate("-",30) + Chr(13)
					if !lAuto
						LogGerArq(cTexto)
...
Static Function PrintCab()
	...
	InsInXMLTemp(MontaTag(4,'cnpj','85340271000149'/*cCNPJ*/,.T.,.T.,.F.) )
...
Static Function TagContas(cConta,cTag)
	...
	If TemConta(cConta)
		...
		Do While !TRBCON->(Eof())
			...
			InsInXMLTemp(MontaTag(6,'saldoInicial',AllTrim(Str(TRBCON->B8A_SALANT)),.T.,.T.,.T.) )
			InsInXMLTemp(MontaTag(6,'debitos',AllTrim(Str(TRBCON->B8A_DEBITO)),.T.,.T.,.T.) )
			InsInXMLTemp(MontaTag(6,'creditos',AllTrim(Str(TRBCON->B8A_CREDIT)),.T.,.T.,.T.) )
			InsInXMLTemp(MontaTag(6,'saldoFinal',AllTrim(Str(TRBCON->B8A_SALFIN)),.T.,.T.,.T.) )
...
Static Function TemConta(cConta)
	...
	cSql := " SELECT B8A_CONTA,B8A_SALANT,B8A_DEBITO,B8A_CREDIT,B8A_SALFIN FROM " + RetSqlName("B8A")
...
Static Function FluxoCaixa()
	If TemFlxCaixa()
		...
		Do While !TRBFLX->(Eof())
			...
			InsInXMLTemp(MontaTag(6,'valor',AllTrim(Str(ABS(TRBFLX->B8H_VLRCON))),.T.,.T.,.T.) )
