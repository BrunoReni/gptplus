		...
		if lMontaXML
			...
			if cTipo == "3"
				...
				IF lUsaColab //TC2.0
					...
					ENDIF
				ELSE // WS TSS
					aXml := NfseXml(cCodMun, cTipo, (cAlias)->F3_ENTRADA, (cAlias)->F3_SERIE, (cAlias)->F3_NFISCAL, (cAlias)->F3_CLIEFOR, (cAlias)->F3_LOJA,cMotCancela,cRdMakeNFSe,aAIDF,aTitIssRet)
				Endif
			endif
		...
		For nX := 1 To Len(aRemessa)
			...
			If nTamXml <= TAMMAXXML
				...
				Else
					...
					cCodCanc := iif(((Type("aRemessa[nX][9]") == "U" .or. Empty(aRemessa[nX][9])) .AND. AllTrim(cCodMun) $ "3301702"),cCodigoC,aRemessa[nX][9]) 
					If !Empty(cCodCanc)
...
/*/
//-----------------------------------------------------------------------
function getRDMakeNFSe(cCodMun,cTipo)
