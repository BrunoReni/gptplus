				...
				If lRet .And. (nJ270Opc == MODEL_OPERATION_UPDATE .OR. ;
					...
					Else //Limpa os dados da verba antes de incluir
						...
						If oModelO0W:HasField("O0W_VLHIST")
							...
							oModelO0W:SetValue( "O0W_FCHIST" , 0      )  // F. Correção do valor histórico
	...
	If nPosProg > 0
		...
		While (cAlias)->(!Eof())
			...
			If aPrognTip[2] == "1"  // Redutor Provável
				...
				If lCmpO0WHis   // Redutor Histórico pedido
					O0W->O0W_VRDHIS := JA94CALRED(cCajuri, (cAlias)->NSY_COD, '1', '2',,cFase,.F., .F.)
				EndIf
