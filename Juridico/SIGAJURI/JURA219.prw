...
Default cNumProc := " "
	...
	If !(cAliasNZZ)->(Eof())
		...
		aDadosCLI := aClone( JClientNZZ((cAliasNZZ)->NZZ_REU) )

		//Busca Parte contraria envolvido polo ativo
		aPAtivo := aClone(TrataEntidade((cAliasNZZ)->NZZ_AUTOR, "1", {"autor", "reclamante"}))
		aAdd( aNT9, aClone(aPAtivo) )
		aSize( aPAtivo, 0)

		//Carrega parte contraria envolvido polo Passivo
		aPassivo := aClone(TrataEntidade((cAliasNZZ)->NZZ_REU, "2", {"reu", "reclamada"}))
		aAdd( aNT9, aClone(aPassivo) )
		aSize( aPassivo, 0)

		//Carrega parte contraria envolvido terceiro interessado
		aTerceiro := aClone(TrataEntidade((cAliasNZZ)->NZZ_ADVOGA, "3", {"advogado parte contraria"}))
...
/*/
//-------------------------------------------------------------------
Static Function TrataEntidade(cNomEnvolvido, cTipoInt, aBusNQA)
Local aTpEnv    := {}
Local aEnvNome  := {}
Local aResult   := {}
Local cEntida   := ""
...
Local aAuxBaVe  := StrToKarr( cNomeEnvol, "|")
	...
	For nCont:=1 To Len(aAuxBaVe)
		If !Empty(aAuxBaVe[nCont])
			...
			cQuery += JurFormat("NZ2_NOME", .T./*lAcentua*/, .T./*lPontua*/) + " LIKE '%" + cNomeEnvol + "%' OR "			
		EndIf
	Next nCont
...
Local nX        := 0
	...
	oResponse[1]['NSZ_DCLIEN'] := JurEncUTF8( AllTrim(aRet[10] )) //-- Descrição / Razão Social

	//-- Parte contraria Polo Ativo / Polo Passivo / Terceiro Interessado
	For nX := 1 To Len(aRet[11])
		If !Empty(aRet[11][nX][1])
			aAdd(oResponse[1]['Envolvidos'], JSonObject():New())
			aTail(oResponse[1]['Envolvidos'])['NT9_NOMEEN'] := JurEncUTF8( aRet[11][nX][2][1] ) //-- Nome

			If (Empty(aRet[11][nX][2][2]))
				aTail(oResponse[1]['Envolvidos'])['NT9_CGC']    := "00000000000"                //-- CPF / CNPJ
			Else
				aTail(oResponse[1]['Envolvidos'])['NT9_CGC']    := aRet[11][nX][2][2]           //-- CPF / CNPJ
			EndIf
			...
			aTail(oResponse[1]['Envolvidos'])['NT9_DESCEN'] := JurEncUTF8( aRet[11][nX][3][6] ) //-- Descrição do Tipo de Envolvimento
		EndIf
	Next nX
	
	oResponse[1]['NUQ_CTIPAC'] := aRet[12]                                    //-- Cod. Tipo de Ação
	oResponse[1]['NSZ_VLCAUS'] := aRet[13]                                    //-- Valor da causa
	oResponse[1]['NSZ_CMOCAU'] := aRet[14]                                    //-- Moeda da causa
	oResponse[1]['NZZ_DTAUDI'] := aRet[15]                                    //-- Data Audiência
	oResponse[1]['NZZ_HRAUDI'] := aRet[16]                                    //-- Hora Audiência
	oResponse[1]['NZZ_COD']    := aRet[17]                                    //-- código
