...
/*/
//-------------------------------------------------------------------
WSMethod GET CliJuri QUERYPARAM valorDig, valorCod, valorLoja, filtFilial WSREST WSPfsAppCP
Local oResponse  := JSonObject():New()
Local cAlias     := ""
Local cQuery     := ""
Local cQrySel    := ""
...
Local aFilFilt   := EmpFilUsu("SA1")
	...
	cQrySel +=" SELECT SA1.A1_COD, SA1.A1_LOJA, SA1.A1_NOME"
	...
	Else
		...
		If !Empty(cFilConcat)
			cQryWhr += " AND SA1.A1_FILIAL IN (" + SubStr(cFilConcat,1, Len(cFilConcat)-1) + ")"
		EndIf
	EndIf
...
Local oResponse  := JSonObject():New()
//Local cRecnoTit  := Self:recnoTitulo
	...
	While (cAliasDb)->(!Eof())
		...
		If (lRegSE2Ok .And. lRegDSDOk)
			...
			If ((lFiltraPag .And. nIndex > nRegMin .And. nIndex <= nRegMax) .Or. lExportRel)
				...
				oResponse['desdobramento'][nNumReg]['desdobDesNatureza'] = JConvUTF8((cAliasDb)->ED_DESCRIC)
				...
				oResponse['desdobramento'][nNumReg]['escritorio'] = JConvUTF8((cAliasDb)->NS7_NOME)
				oResponse['desdobramento'][nNumReg]['codCentroCusto'] = JConvUTF8((cAliasDb)->CCUSTO)
				oResponse['desdobramento'][nNumReg]['centroCusto'] = JConvUTF8((cAliasDb)->CTT_DESC01)

				// Participante/Solicitante
				oResponse['desdobramento'][nNumReg]['codSolicitante'] = JConvUTF8((cAliasDb)->CPART)
				oResponse['desdobramento'][nNumReg]['nomeSolicitante'] = JConvUTF8((cAliasDb)->PARTNOME)
				...
				oResponse['desdobramento'][nNumReg]['nomeParticipante'] = JConvUTF8((cAliasDb)->SOLICNOME)
				...
				oResponse['desdobramento'][nNumReg]['rateio'] = JConvUTF8((cAliasDb)->OH6_DESCRI)
				...
				oResponse['desdobramento'][nNumReg]['cliente'] = JConvUTF8((cAliasDb)->A1_NOME)
				oResponse['desdobramento'][nNumReg]['codCaso'] = JConvUTF8((cAliasDb)->CCASO)
				oResponse['desdobramento'][nNumReg]['tituloCaso'] = JConvUTF8((cAliasDb)->NVE_TITULO)
				oResponse['desdobramento'][nNumReg]['codTipoDespesa'] = JConvUTF8((cAliasDb)->CTPDSP)
				oResponse['desdobramento'][nNumReg]['tipoDespesa'] = JConvUTF8((cAliasDb)->NRH_DESC)
				...
				oResponse['desdobramento'][nNumReg]['historico'] = JConvUTF8((cEntidade)->&(cEntidade + "_HISTOR"))
...
Local cBdBase      := (Upper(TcGetDb()))
	...
	If lPrefixo
		cQrySel := " SELECT E2_PREFIXO "
	Else
		cQrySel := " SELECT SE2.E2_FILIAL, SE2.E2_PREFIXO, SE2.E2_FORNECE, SE2.E2_PARCELA,"
...
Default aBodFilOrd := { /*cFiltroSE2*/, /*cFiltroDSD*/, /*cOrderBy*/, , }
	...
	cQrySel := " SELECT DSD.SE2RECNO, DSD.VALOR, DSD.ORIGEM, DSD.IDDOC, DSD.CITEM,"
...
Local nValor := Val(StrTran(cSearchKey,",","."))
	...
	If nValor > 0 .And. cValtoChar(nValor) == cSearchKey // Garante que o cSearchKey contém somente caracteres numéricos.
...
@param cDescriNat   - palavra a ser pesquisada no Nome da natureza.
/*/
//-------------------------------------------------------------------
...
//-------------------------------------------------------------------
/*/{Protheus.doc} WSGetTit(cCodTitulo)
...
Default cCodTitulo := ""
	...
	BeginSql Alias cTmpAlias
		Select 
			...
			FK7_FILIAL,FK7_IDDOC,FK7_CHAVE, E2_ORIGEM, ED_CCJURI, ED_DESFAT
			%Exp:cExtFlds%
		From %Table:SE2% SE2
	...
	If (cTmpAlias)->(!Eof())
		...
		If ( nPosPagPix > 0 )
			oResponse['titulo']["FKF_PAGPIX"] := EncodeUTF8((cTmpAlias)->FKF_PAGPIX)
		EndIf
	Endif
...
/*/
//-------------------------------------------------------------------
WSMETHOD POST RetCNAB WSREST WSPfsAppCP
Local cBody       := Self:GetContent()
Local cFilAntOld  := cFilAnt
Local cEmpAntOld  := cEmpAnt
Local oRequest    := JsonObject():New()
Local oResponse   := JSonObject():New()
Local cMsgSuccess := ""
...
Private aFA205R  := {}
	...
	oRequest:FromJson(DecodeUTF8(cBody))
	cEmpAnt := oRequest['empresa']
	cFilAnt := oRequest['codigo']

	FINA435(oRequest['empresa'], oRequest['codigo'])

	cEmpAnt := cEmpAntOld
	cFilAnt := cFilAntOld

	cMsgSuccess := I18N(STR0099 , { oRequest['empresa'], oRequest['codigo'] }) //"Empresa #1 Filial #2 processado com sucesso!"
	oResponse:FromJSon('{"message": "' + cMsgSuccess + '", "code": "200"}')
	
	// Retorna para a Empresa/Filial original (Erro no reaproveitamento da Thread)
	RpcSetEnv(cEmpAntOld, cFilAntOld, Nil, Nil, Nil, "WSPfsAppCP")
	Self:SetResponse(oResponse:toJson())
	oResponse := Nil
Return .T.
