...
Local cSituacC  := ""
Local cLib      := ""
Local lWebApp   := GetRemoteType(@cLib) == 5 .Or. "HTML" $ cLib // WebApp
...
If lWebApp
	aAdd( aRotina, {"Anexos", "JurAnexos('NX0', NX0->NX0_COD, 1, xFilial('NX0'), .T.)", 0, 2, 0, Nil })
EndIf
...
If !lAutomato
	If !ApMsgYesNo( STR0080 + cLit + CRLF + STR0081 ) //"Confirma a troca para situação "###"Atenção! Todas as pré-faturas marcadas serão trocadas."
				...
				If lVldModel						
					...
					Else
						...
						If nQtde > 1 .And. nContador < nQtde .And. lMostraMsg .And. ;
						    !ApMsgYesNo( I18N(STR0375, {NX0->NX0_COD}), STR0376 )// "Houveram problemas na alteração da pre-fatura #1, deseja continuar o processamento para as demais pré-faturas selecionadas?" - "Deseja alterar as próximas faturas?"
							lNext := .F.
							Exit
						Else
							lMostraMsg := .F.
...
/*/
//-------------------------------------------------------------------
Static Function JA202REFAZ(cCPreFt, lResumo, cResult, cCrysPath, lAutomato, lReport, lAtual)
Local aResult     := {}
Local aArea       := GetArea()
Local aAreaNX0    := NX0->(GetArea())
Local cParams     := ""
		...
		If !lResumo
			...
			If !lImgRept
				...
				If cResult == '3' // Gera relatório de faturamento em Word"
					...
					EndIf
					If lWebAppJob
						J026Anexar("NX0", xFilial("NX0"), NX0->NX0_COD, "", cDestPath + cArquivo + ".doc", .T.)
					EndIf
				EndIf
			...
			If lRet .And. lWebAppJob
				J026Anexar("NX0", xFilial("NX0"), NX0->NX0_COD, "", cDestPath + cArquivo + ".pdf", .T.)
			EndIf
...
For nNX8 := 1 To oModelNX8:GetQtdLine()
	...
	If nNX1Count == oModelNX1:GetQtdLine() //Se todos os casos forem removidos, as parcelas e contrato também serão marcados para retirar
		If !lAllFX
...
Else
	...
	If lRet
		...
		While !NX0->(EOF())
			...
			If lProcessa
				Processa( {|| lRet := J202ADDREL(NX0->NX0_COD, cCbxResult, lAutomato, lAtual)}, STR0147, STR0148 + " " + NX0->NX0_COD, .F.) //"Aguarde..." ### "Refazendo a Pré-Fatura "
			Else
				lRet := .T.
			EndIf
...
Adiciona a tabela temporária a fatura a ser gerada o relatÃ³rio.
...
/*/
//-------------------------------------------------------------------
Function J202ADDREL(cCodPre, cResult, lAutomato, lAtual)
Local lRet     := .F.
Local aArea    := GetArea()
Local nRec     := 0
Local oFilaExe := JurFilaExe():New( "JURA202", "2", cValToChar(ThreadID())) //2=Impressão
...
Local cRotina := ProcName(0)
	...
	ElseIf lWebApp .And. cCbxResult == "Impressora"
		lRet := JurMsgErro(STR0395, cRotina, STR0396) // "Uso não permitido da opção 'Impressora' via browser." # "Opções disponíveis: 'Word', 'Nenhum'."
	ElseIf lWebApp .And. cCbxResult == "Tela"
		lRet := JurMsgErro(STR0397, cRotina, STR0396) // "Uso não permitido da opção 'Tela' via browser." # "Opções disponíveis: 'Word', 'Nenhum'."
	EndIf
Return lRet
