...
Else
	...
	@ 100,130 Button STR0100 Size 037,012 PIXEL OF oDlg  Action (lRet := J203BOkVld(cCbResult), IIf(lRet, oDlg:End(), Nil))  //"Emitir"
				...
				For nFat := 1 To Len(aFaturas)
					...
					If NXA->NXA_TIPO == "FT"
						...
						If aParams[14] == 'S' .And. NXA->NXA_FPAGTO == "3" // Emite Pix
							J203ADDREL("P", aParams, , cRotina, lAutomato)
						EndIf
					EndIf

					J203ADDREL("V", aParams, , cRotina, lAutomato)
...
Default cFila    := ''
	...
	Action (	Iif(Len(aUtiliza) == 0, JurMsgErro(STR0136, "J203UTIAD"), ; // "Não há itens para remover"
...
/*/
//-------------------------------------------------------------------
Function JA203TIT(cEscrit, cFatura, cTipoEmi, cChkBolPix)
Local aArea       := GetArea()
Local aAreaNS7    := NS7->( GetArea() )
Local aAreaSA1    := SA1->( GetArea() )
		...
		If cTipoEmi == "1" //Fatura -> Gera o(s) titulo(s) financeiros
			...
			For nI := 1 To Len( aParcelas )
				...
				If lRet
					...
					If NXA->NXA_FPAGTO == "3" .And. AI0->AI0_RECPIX == "1" .And. cChkBolPix == "S" // Pix
						_lPix := .T.
					EndIf
...
If (NXA->( dbSeek( xFilial("NXA") + cEscrit + cFatura  ) ))
	...
	ElseIf cTpConF == '2' // Cotação do titulo diária (faz a conversão do valor na moeda nacional para a cotação diária na moeda do titulo)
		...
		aValConv  := JA201FConv(NXA->NXA_CMOEDA, cMoedNac, nValorHon, "8", NXA->NXA_DTEMI, NXA->NXA_CFILA,/*cpreft*/, /*cXfilial*/ , NXA->NXA_CESCR, NXA->NXA_COD)	//Utilizado a opção '8' para considerar a mesmas cotações negociadas na fatura - o 'cpreft' é o código da fatura + escritório
		nValorHon := Round(aValConv[1], TamSX3("E1_VALOR")[2])
		If nValorHon > 0
			nTaxa  := aValConv[3]
		EndIf

		aValConv  := JA201FConv(NXA->NXA_CMOEDA, cMoedNac, nValorDes, "8", NXA->NXA_DTEMI, NXA->NXA_CFILA,/*cpreft*/, /*cXfilial*/ , NXA->NXA_CESCR, NXA->NXA_COD)	//Utilizado a opção '8' para considerar a mesmas cotações negociadas na fatura - o 'cpreft' é o código da fatura + escritório
...
Função para buscar os percentuais de impostos
	...
	If cTpUnif $ "23" // Unifica
		...
		If Len(aDirPdf) > 0
			For nArq := 1 To Len(aDirPdf)
				...
				If lCarta .Or. lRelat .Or. lCompDesp .Or. ((lBolet .Or. lPix) .And. cTpUnif == "3")
...
//-------------------------------------------------------------------
/*/{Protheus.doc} J203BOkVld
Função de validação do botão 'Emitir' da fatura
...
/*/
//-------------------------------------------------------------------
Static Function J203BOkVld(cCbxResult)
Local lRet    := .T.
Local cLib    := ""
Local lWebApp := GetRemoteType(@cLib) == 5 .Or. "HTML" $ cLib // WebApp + WebAgent
Local cRotina := ProcName(0)

	If lWebApp .And. cCbxResult == "Impressora"
		lRet := JurMsgErro(STR0307, cRotina, STR0308) // "Uso não permitido da opção 'Impressora' via browser." # "Opções disponíveis: 'Word', 'Nenhum'."
	ElseIf lWebApp .And. cCbxResult == "Tela"
		lRet := JurMsgErro(STR0309, cRotina, STR0308) // "Uso não permitido da opção 'Tela' via browser." # "Opções disponíveis: 'Word', 'Nenhum'."
	EndIf
...
@paran cOrigem, Origem da despesa
...
/*/
//-------------------------------------------------------------------
Static Function J203GetAnx(cFilDesp, cCodDesp, cOrigem, cCodFat, cEscritorio, aTpImg)
Local aArea       := GetArea()
Local aFiltros    := {}
...
Local nTamNumExt  := TamSx3("NUM_EXTEN")[1]
	...
	If (cOrigem $ "NVY|OHB")
		...
		If cOrigem == 'NVY' // Anexos originados na NVY
		...
		ElseIf cOrigem == 'OHB'// Anexos originados na OHB
			cSqlFrm +=   " INNER JOIN " + RetSqlName("OHB") + " OHB ON( OHB.OHB_FILIAL = NUM.NUM_FILENT"
			cSqlFrm +=                                            " AND OHB.OHB_CODIGO = NUM.NUM_CENTID"
			cSqlFrm +=                                            " AND OHB.D_E_L_E_T_ = ' ')"
			cSqlFrm +=   " INNER JOIN " + RetSqlName("NVY") + " NVY ON( NVY.NVY_FILLAN = OHB.OHB_FILIAL"
			cSqlFrm +=                                            " AND NVY.NVY_CLANC  = OHB.OHB_CODIGO"
			cSqlFrm +=                                            " AND NVY.D_E_L_E_T_ = ' ')"
		EndIf

		cSqlWhr := " WHERE NUM.D_E_L_E_T_ = ' '" 
		cSqlWhr +=   " AND NUM.NUM_ENTIDA = '" + cOrigem + "'"
		cSqlWhr +=   " AND LOWER(NUM.NUM_EXTEN) IN (" + cTpExten + ")"

	ElseIf ( cOrigem == "NZQ" )
		cSql    := " SELECT COALESCE(NUMOHB.NUM_DOC, NUMOHF.NUM_DOC, NUMOHG.NUM_DOC, '') NUM_DOC,"
		cSql    +=        " COALESCE(NUMOHB.NUM_EXTEN, NUMOHF.NUM_EXTEN, NUMOHG.NUM_EXTEN, '') NUM_EXTEN,"
		cSql    +=        " COALESCE(NUMOHB.NUM_NUMERO, NUMOHF.NUM_NUMERO, NUMOHG.NUM_NUMERO, '') NUM_NUMERO,"
		cSql    +=        " COALESCE(NUMOHB.NUM_ENTIDA, NUMOHF.NUM_ENTIDA, NUMOHG.NUM_ENTIDA, '') NUM_ENTIDA,"
		cSql    +=        " COALESCE(NUMOHB.D_E_L_E_T_, NUMOHF.D_E_L_E_T_, NUMOHG.D_E_L_E_T_, ' ') D_E_L_E_T_"
		cSqlFrm +=   " FROM " + RetSqlName("NVY") + " NVY"
		cSqlFrm +=   " LEFT JOIN " + RetSqlName("OHB") + " OHB"
		cSqlFrm +=          " ON (OHB.OHB_FILIAL = NVY.NVY_FILLAN"
		cSqlFrm +=         " AND OHB.OHB_CODIGO = NVY.NVY_CLANC"
		cSqlFrm +=         " AND OHB.D_E_L_E_T_ = ' ')"
		cSqlFrm +=   " LEFT JOIN " + RetSqlName("NUM") + " NUMOHB"
		cSqlFrm +=          " ON (NUMOHB.NUM_FILENT = OHB.OHB_FILIAL"
		cSqlFrm +=         " AND NUMOHB.NUM_CENTID = OHB.OHB_CODIGO"
		cSqlFrm +=         " AND NUMOHB.NUM_ENTIDA = 'OHB'"
		cSqlFrm +=         " AND NUMOHB.D_E_L_E_T_ = ' ')"
		cSqlFrm +=   " LEFT JOIN " + RetSqlName("FK7") + " FK7"
		cSqlFrm +=          " ON (FK7.FK7_CHAVE  = NVY.NVY_CPAGTO"
		cSqlFrm +=         " AND FK7.FK7_ALIAS  = 'SE2'"
		cSqlFrm +=         " AND FK7.D_E_L_E_T_ = ' ')"
		cSqlFrm +=   " LEFT JOIN " + RetSqlName("OHF") + " OHF"
		cSqlFrm +=          " ON (OHF.OHF_IDDOC  = FK7.FK7_IDDOC"
		cSqlFrm +=         " AND OHF.D_E_L_E_T_ = ' ')"
		cSqlFrm +=   " LEFT JOIN " + RetSqlName("NUM") + " NUMOHF"
		cSqlFrm +=          " ON (NUMOHF.NUM_FILENT = OHF.OHF_FILIAL"
		cSqlFrm +=         " AND NUMOHF.NUM_CENTID = OHF.OHF_IDDOC || OHF.OHF_CITEM"
		cSqlFrm +=         " AND NUMOHF.NUM_ENTIDA = 'OHF'"
		cSqlFrm +=         " AND NUMOHF.D_E_L_E_T_ = ' ')"
		cSqlFrm +=   " LEFT JOIN " + RetSqlName("OHG") + " OHG"
		cSqlFrm +=          " ON (OHG.OHG_IDDOC  = FK7.FK7_IDDOC"
		cSqlFrm +=         " AND OHG.D_E_L_E_T_ = ' ')"
		cSqlFrm +=   " LEFT JOIN " + RetSqlName("NUM") + " NUMOHG"
		cSqlFrm +=          " ON (NUMOHG.NUM_FILENT = OHG.OHG_FILIAL"
		cSqlFrm +=         " AND NUMOHG.NUM_CENTID = OHG.OHG_IDDOC || OHG.OHG_CITEM"
		cSqlFrm +=         " AND NUMOHG.NUM_ENTIDA = 'OHG'"
		cSqlFrm +=         " AND NUMOHG.D_E_L_E_T_ = ' ')"

		cSqlWhr := " WHERE NVY.D_E_L_E_T_ = ' '"
		cSqlWhr +=   " AND LOWER(COALESCE(NUMOHB.NUM_EXTEN,"
		cSqlWhr +=                      " NUMOHF.NUM_EXTEN,"
		cSqlWhr +=                      " NUMOHG.NUM_EXTEN,"
		cSqlWhr +=                      " '')) IN (" + cTpExten + ")"
	EndIf

	cSqlWhr +=   " AND NVY.NVY_FILIAL = ?"
	Aadd(aFiltros, cFilDesp)

	cSqlWhr +=   " AND NVY.NVY_COD = ?"
	Aadd(aFiltros, cCodDesp)

	cSql := cSql + cSqlFrm + cSqlWhr
	cSql := ChangeQuery(cSql)

	dbUseArea( .T., "TOPCONN", TcGenQry2(,, cSQL, aFiltros), cAlias, .T., .T.)
...
/*/
//-------------------------------------------------------------------
Static Function J203Comprov(cFatura, cEscritorio)
Local aArea      := GetArea()
Local aFiles     := {}
Local aOrigem    := {"NVY", "NZQ", "OHB"}
...
Local lRetDocs   := .F.
			...
			If (cAliasFat)->(!Eof())
				...
				While (cAliasFat)->(!Eof()) 
					For nI := 1 To Len(aOrigem)
						lAchou := J203GetAnx((cAliasFat)->NVY_FILIAL, (cAliasFat)->NVY_COD, aOrigem[nI], cFatura, cEscritorio, aTpImg)
				...
				cImgName := STR0310 + "(" + cEscritorio + "-" + cFatura + ").pdf" // 'Comprovantes de despesas da fatura '
	...
	Else
		...
		JurConout(I18N(STR0311, {"Mogrify.exe", "Appserver"}))
	EndIf
