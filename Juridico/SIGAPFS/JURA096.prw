...
If lRet .And. FindFunction("JurVldPIX") // ProteÁ„o
	lRet := JurVldPIX() //ValidaÁ„o do banco e cliente para pagamentos PIX
EndIf
	...
	If !Empty(aCasosAlt)
		...
		For nX := 1 To Len(aCasosAlt)
			If NVE->(DbSeek(xFilial("NVE") + aCasosAlt[nX][1] + aCasosAlt[nX][2] + aCasosAlt[nX][3]))
				RecLock("NVE", .F.)
				NVE->NVE_SITCAD := "2"
				If NVE->(ColumnPos("NVE_DTEFT")) > 0 //ProteÁ„o 12/05/2023
					NVE->NVE_DTEFT := Date()
				EndIf
				NVE->(MsUnLock())
...
If lAutomato .Or. ApMsgYesNo(STR0089) //"Deseja revalorizar os Time-Sheets dos casos do contrato selecionado?"
	...
	If lLock
		// Time Sheets n„o vinculados a prÈ-faturas ou minutas
		...
		cQuery    +=  " UNION "
		// Time Sheets vinculados somente a prÈ-faturas
		cQuery    += " SELECT NUE_CPREFT, NUE.R_E_C_N_O_  NUERECNO, NUE_DATATS, NUE_COD, '2' TEMMINUTA "
		cQuery    +=   " FROM " + RetSqlName("NUE") + " NUE "
		cQuery    +=  " WHERE NUE.D_E_L_E_T_ = ' ' "
		cQuery    +=    " AND NUE.NUE_FILIAL = '" + xFilial( "NT0" ) +"' "
		...
		cQuery    +=  " UNION "
		// Time Sheets vinculados a minutas de prÈ-faturas e minutas sÛcio
		...
		lTemPreft  := !Empty(cCodPre)
		 //STR0085 -> "Existe(m) prÈ-fatura(s) para este contrato. Ao efetivar a revalorizaÁ„o as prÈ-fatura(s) ter„o o status atualizados para situaÁ„o 'Alterada'. Deseja continuar? Caso seja selecionada a opÁ„o 'N„o', ser„o revalorizados apenas time sheets que n„o possuem vÌnculos com prÈ-faturas."
		 //STR0266 -> "Existe(m) minuta(s) e prÈ-fatura(s) para este contrato. Ao efetivar a revalorizaÁ„o, a(s) minuta(s) ser·(„o) cancelada(s) e as prÈ-fatura(s) ter·(„o) o status atualizado para situaÁ„o 'Alterada'. Deseja continuar? Caso seja selecionada a opÁ„o 'N„o', ser„o revalorizados apenas time sheets que n„o possuem vÌnculos com prÈ-faturas/minutas."
		cMsgCancel := IIf(lTemMinuta, STR0266, STR0085)
		If lOk
			If lTemPreft .And. !lAutomato
			...
			For nI := 1 To Len(aTimeSheets)
				...
				If lOk .And. lTsVincPre
					lOk := !(JurGetDados("NX0", 1, xFilial("NX0") + aTimeSheets[nI, 2], "NX0_SITUAC") $ "4|C|F") // Emitir Fatura | Em Revis„o | Aguardando SincronizaÁ„o
					If !lOk
				...
				If lOk
					If lTsVincPre // Time Sheet vinculado a PrÈ-fatura
						If lLCPRE .And. lCancela
							If lTemMinuta .And. cPrefCanc <> aTimeSheets[nI, 2]
								lOk := J202CanMin(aTimeSheets[nI, 2], STR0267) // "RevalorizaÁ„o dos TimeSheets - JURA096"
							EndIf
							If lOk
								If cPrefCanc <> aTimeSheets[nI, 2] // Muda situaÁ„o das prÈ-faturas que n„o tem minutas vinculados aos timesheets.
									J144AltPre(aTimeSheets[nI, 2], STR0267, .T., .T.) // "RevalorizaÁ„o dos TimeSheets - JURA096"
								EndIf
	...
	If !lAutomato .And. lRet .And. !lTodosTSBloq .And. Empty(cCodTSPre) .And. Len(aTsOk) > 0
		MsgInfo(STR0117) // "Time Sheet revalorizado"
	ElseIf !lAutomato .And. lRet .And. !lTodosTSBloq .And. Empty(cCodTSPre) .And. Len(aTsOk) == 0
		MsgInfo(STR0268) // "N„o h· Time Sheets pendentes para serem revalorizados."
	EndIf
...
If lRet
	...
	If oModelNT1:GetValue("NT1_SITUAC") == "2" .Or. !Empty(cPrefat) .And. cSituac $ "4|5|7|9|A|B|C|F"
		...
		If "DELETE" == cAction
			lRet := .F.
			cMsgSoluc := STR0180 + AllTrim(oModelNT1:GetValue("NT1_PARC")) + STR0181 + cPreFat + STR0182 + Tabela("JS", cSituac, .F.) + STR0184 // "A parcela '" ### "' est· vinculada a prÈ-fatura '" ### "' com a situaÁ„o '" ### "' e n„o poder· ser alterada!"
		EndIf
	Else
...
If !lRet .And. lShowMsg
	JurMsgErro(STR0195,, cMsgSoluc) // "OperaÁ„o de AlteraÁ„o/Exclus„o cancelada!"
EndIf
...
If cParcSit == '1' .And. NX0->(DbSeek(xFilial("NX0") + cPreFat))
	...
	If AllTrim(NX0->NX0_SITUAC) $ "2|3|D|E" .And. !J96VerAltM(oModelNT1) // PrÈ-fatura em (An·lise - Alterada - Revisada - Revisada com RestriÁıes)
	...
	ElseIf AllTrim(NX0->NX0_SITUAC) == '6' // Minuta Emitida
		...
		EndIf
	ElseIf AllTrim(NX0->NX0_SITUAC) $ "4|5|7|9|A|B|C|F" .And. !J96VerAltM(oModelNT1) // Emitir Fatura - Emitir Minuta - Minuta Cancelada - Minuta SÛcio - Minuta SÛcio Emitida - Minuta SÛcio Cancelada - Em Revis„o - Aguardando SincronizaÁ„o e houve alteraÁ„o na NT1
		ApMsgInfo(STR0180 + AllTrim(cParcela) + STR0181 + cPreFat + STR0182 + AllTrim(cSituacao) + STR0184) // "A parcela '" ### "' est· vinculada a prÈ-fatura '" ### "' com a situaÁ„o '" ### "' e n„o poder· ser alterada!"
		lRet := .F.
	EndIf
