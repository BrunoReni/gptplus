...
If NXA->NXA_TIPO == "FT" .And. FindFunction("J203UpdPix"); 
	...
	.And. NXA->NXA_FPAGTO == "3" .And. (IsBlind() .Or. ApMsgYesNo("Essa fatura pode ter chave Pix gerada! Deseja realmente alterar as informações e cancelar o Pix?", STR0178)) // "Essa fatura pode ter chave Pix gerada! Deseja realmente alterar as informações e cancelar o Pix?" # "Atenção"
...
If NXA->NXA_SITUAC $ '1|3' // 1=Válida # 3=Faturada (Somente Minuta de Pré)
	...
	@ 040,010 Button STR0055 Size 037,012 PIXEL OF oDlg Action (lRet := J204BOkVld(cCbResult), IIf(lRet,oDlg:End(), Nil))  //"Emitir"
	...
	If aRelat[4] //Boleto ou Pix
		...
		Else
			...
			If aParams[14] == 'S' .And. NXA->NXA_FPAGTO == "3" .And. lAutoPix // Emite Pix
				J203ADDREL("P", aParams, , "JURA204")
			EndIf
		EndIf
	EndIf

	If aRelat[5]
		// Gera o relatório de Comprovante de Desespesas
		J203ADDREL("V", aParams, , "JURA204")
	EndIf
...
Local lPDUserAc := Iif(FindFunction("JPDUserAc"), JPDUserAc(), .T.) // Indica se o usuário possui acesso a dados sensíveis ou pessoais (LGPD)
	...
	@ 110, 129 Button STR0055 Size 037,012 PIXEL OF oDlg  Action  (lRet := J204BOkVld(cCbResult), IIf(lRet, oDlg:End(),  Nil))  //"Emitir"
	...
	If lRet
		If lCkRelat .OR. lCkCarta .OR. lCkRecibo .Or. lCkGerBolPix .Or. lCkCmpDesp
			...
			aParams[22] :=	" "
		Else
			ApMSgInfo( I18N(STR0107, {STR0060, STR0061, STR0062, STR0063, STR0261, STR0281}) ) 
			            // "Selecione pelo menos uma das opções: #1, #2, #3, #4, #5 ou #6."
			            // Carta de Cobrança, Relatório, Recibo, Boleto, Pix ou Comprovantes de despesa
			lRet := .F.
		EndIf
	EndIf
...
If Len(aDirPdf) > 0
	...
	For nFor := 1 To Len(aDirPdf)
		...
		If lEmissao .And. !lArqAdici
			If (lNoTpUnif .Or. lArqUnifi .Or. ((lArqBolet .Or. lArqPix) .And. cTpUnif == "2") ) .and. !lConferFat
				cEmail := "1"
			Else
				cEmail := "2"
			EndIf
...
//-------------------------------------------------------------------
/*/{Protheus.doc} J204BOkVld
Função de validação do botão 'Emitir' do refazer da fatura
e do relatório de conferência.
...
/*/
//-------------------------------------------------------------------
Static Function J204BOkVld(cCbxResult)
Local lRet    := .T.
Local cLib    := ""
Local lWebApp := GetRemoteType(@cLib) == 5 .Or. "HTML" $ cLib // WebApp + WebAgent
Local cRotina := ProcName(0)
Local lRelatConf := FwIsInCallStack("JA204Confe")

	If lWebApp .And. cCbxResult == "Impressora"
		lRet := JurMsgErro(STR0273, cRotina, IIf(lRelatConf, STR0271, STR0274)) // "Uso não permitido da opção 'Impressora' via browser." # "Opções disponíveis: 'Tela','Word'." # "Opções disponíveis: 'Word', 'Nenhum'." 
	ElseIf lWebApp .And. cCbxResult == "Tela" .And. !lRelatConf
		lRet := JurMsgErro(STR0275, cRotina, STR0274) // "Uso não permitido da opção 'Tela' via browser." # "Opções disponíveis: 'Word', 'Nenhum'."
	EndIf
Return lRet
