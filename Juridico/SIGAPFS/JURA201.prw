...
If lVldUser .And. oFilaExe:OpenWindow(.T.) //Indica que a tela está em execução para Thread de relatório
	...
	SetKEY(VK_F8 , {|| J201F8()})
	SetKEY(VK_F9 , {|| J201F9()})
	SetKEY(VK_F11, {|| J201SaveLOG(LOG := !LOG), MsgInfo(STR0076 + IIF(LOG, STR0079, STR0080)) }) // "O log de emissão esta: " e "ligado" "desligado"
...
If aRet[1]
	Processa( { || cRet := Runproc(lAutomato) }, STR0043, STR0044, .T. ) //"Aguarde"###"Emitindo pré-faturas ..."
Else
	If !Empty(aRet[2])
		ApMsgStop(aRet[2])
	EndIf
...
If lWebApp .And. cCbxResult == "Impressora"
	ApMsgStop(STR0146 + CRLF + CRLF + STR0147) // "Não é possível emitir pré-fatura com a opção de resultado 'Impressora' via browser." # "Opções disponíveis: 'Tela', 'Word', 'Nenhum'."
	lRet := .F.
EndIf

If 	lWebApp .And. (cCbxResult == "Tela" .Or. cCbxResult == "2") .And. THREAD
	ApMsgStop(STR0148 + CRLF + CRLF + STR0149) // "Não é possível emitir pré-fatura com a opção de resultado 'Tela' quando habilitado emissão em segundo plano via browser." # "Opções disponíveis: 'Word', 'Nenhum'."
	lRet := .F.
EndIf
		...
		If !J201IsDelPre(aPres[nI]) .And. !JurIN( JurGetDados("NX0", 1, xFilial("NX0") + aPres[nI], "NX0_SITUAC"), {"8", " "} ) // Diferente de substituida ou em branco
			...
			*/
			...
			If cTipoImp == '3' // Gera relatório de faturamento em Word"
				...
				EndIf
				// Grava anexos para Pré-Fatura
				If lRet .And. lWebAppJob
					J026Anexar("NX0", xFilial("NX0"), aPres[nI], "", cDestPath + cArquivo + ".doc", .T.)
				EndIf
			EndIf
			...
			// Grava anexos para Pré-Fatura
			If lRet .And. lWebAppJob
				J026Anexar("NX0", xFilial("NX0"), aPres[nI], "", cDestPath + cArquivo + ".pdf", .T.)
			EndIf
