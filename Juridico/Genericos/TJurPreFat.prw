...
Static Function JA201ATS(oParams)
	...
	If !Empty(oParams:GetDIniH()) .AND. !Empty(oParams:GetDFinH())
		cQueryTS += " NT1.NT1_DATAFI >= (CASE WHEN TMP.MISTO = '1' THEN '19000101' ELSE '" + DtoS(oParams:GetDIniH()) + "' END) AND "
		cQueryTS += " NT1.NT1_DATAFI <= (CASE WHEN TMP.MISTO = '1' THEN '22000101' ELSE '" + DtoS(oParams:GetDFinH()) + "' END) AND "
	EndIf
...
Static Function JA201AFA(oParams)
	...
	If !Empty(oParams:GetCliente()) .AND. !Empty(oParams:GetLoja()) .AND. Empty(oParams:GetCasos())
		cQueryFA +=   " AND NVV.NVV_CCLIEN = '"+ oParams:GetCliente() +"' " // SE O PARÂMETRO POR POR CLIENTE, EXIGE CLIENTE /LOJA
...
Method GetQryTmpCtr(lFilter) Class TJPREFATPARAM
	...
	cQueryCT +=         " WHERE NTH.NTH_FILIAL = '" + xFilial("NTH") + "' "
	...
	If !Empty(cContratos)
		If Self:GetTDContr()
			...
			cQueryCT +=                      " WHERE NT0_FILIAL = '" + xFilial("NT0") + "' "
			cQueryCT +=                        " AND NT0_COD IN (" + cContratos + ") "
