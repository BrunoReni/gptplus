...
@param  nRegCmp  , numerico, Recno do Título que está sendo usado para compensar
@param  lExclusao, Boolean, indica se esta sendo realizada a exclusão / cancelamento da baixa
...
@param nRecnoSE2   - Recno dao título - (SE2)
...
// Não executar nada quando for uma compensação e estiver posicionado no PA ou NDF,
// pois a compensação faz duas baixas, a do PA ou NDF e a do Título a Pagar.
...
If lRet .And. !lReturn
	...
	// E2_SALDO == 0 é utilizado para não gerar lançamentos de impostos a cada baixa parcial do
	// título principal e o "lCancelBaix" para indentificar que se trata de um estorno e os impostos
	// devem ser estornados.
	//---------------------------------------------------------------------------------------------
	If JCPVlLiqui(nRecnoSE2) != JCPVlBruto(nRecnoSE2)
		If lCompensac
			If lCancelBaix
				lExistImp := SE2->E2_SALDO == SE2->E2_VALOR .And. JLancImpCP()
			Else
				lExistImp := !JLancImpCP()
			EndIf
		Else
			lExistImp := SE2->E2_SALDO == SE2->E2_VALOR
		EndIf
	EndIf
	...
	If lCompensac
		...
		If lCancelBaix
			...
			SE2->(DbSeek(SE5->E5_FILIAL + SE5->E5_DOCUMENT))
		Else
			SE2->(dbGoTo(nRegCmp))
		EndIf
			...
			For nPag := 1 To Len(aNatPag)
				...
				If JurGetDados("SED", 1, xFilial("SED") + cNatPag, "ED_CCJURI") == "7" .And. cNatPag == cNatTrans // Tratamento quando for uma compensação de PA
					...
					If lExistImp .And. lExecImp // Cria lançamentos de impostos no momento da baixa do título principal
						JImpLanc(@aLancDiv, cNatTrans, nRecnoSE2)
...
/*/
//-----------------------------------
Static Function JImpLanc(aLancDiv, cNatTrans, nRecnoSE2)
Local aArea     := GetArea()
Local aAreaSE2  := SE2->(GetArea())
Local aAreaOHF  := OHF->(GetArea())
Local aRecImp   := JRecFilho(nRecnoSE2)
Local cHistLanc := ""
Local cNatImp   := ""
Local nImp      := 0
	...
	If Len(aRecImp) > 0
		...
		For nImp := 1 To Len(aRecImp)
			...
			If OHF->(DbSeek(SE2->E2_FILIAL + cIdDocPag))
				...
				aAdd(aLancDiv, {cNatImp, cNatTrans, SE2->E2_VALOR, cHistLanc})
			EndIf
		Next
	EndIf
...
Valida forma de pagamento para PIX

@param oModel    , Objeto do modelo de dados de Clientes

@return lFPagto  , Se verdairo a forma de pagamento é válida
		...
		If cFPagto == "3" // Pagar em PIX
			...
			If (SA6->(DbSeek(xFilial("SA6") + cChaveBco)))
				...
				If !(F70->(DbSeek(xFilial("F70") + SA6->A6_COD + SA6->A6_AGENCIA + SA6->A6_DVAGE + SA6->A6_NUMCON + SA6->A6_DVCTA)))
					lFPagto := JurMsgErro(STR0145, , STR0146) // "Banco não configurado para Pagamento em PIX." # "Para uso da forma de pagamento igual a: 3=PIX, preencha as chaves PIX no cadastro do banco."
				EndIf
			EndIf
