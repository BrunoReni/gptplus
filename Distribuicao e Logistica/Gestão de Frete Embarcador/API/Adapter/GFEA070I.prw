		...
		if self:cTipRet = '1'
			...
			if !((cAliasGW6)->(EOF()))
				for nJ := 1 to Len(aSelFields)
					...
					if aSelFields[nJ] = 'InternalId'
						Self:oEaiObjSn2['InternalId'] := (cAliasGW6)->&(Self:oFieldsJson['BranchId']) + "|" + (cAliasGW6)->&(Self:oFieldsJson['IssuerCode']) + "|" + (cAliasGW6)->&(Self:oFieldsJson['InvoiceSeries']) + "|" + (cAliasGW6)->&(Self:oFieldsJson['InvoiceNumber']) + "|" + (cAliasGW6)->&(Self:oFieldsJson['IssueDate'])
					else
						cField := aSelFields[nJ]
						cValue := iif(Self:oFieldsJson[cField] = NIL, NIL, (cAliasGW6)->&(Self:oFieldsJson[cField]))
				...
				self:oEaiObjSn2['AccountingMovements'] := {}
				Aadd(self:oEaiObjSn2['AccountingMovements'], JsonObject():New())
				...
				WHILE ((cAliasGW7)->(!Eof())) 		
					...
					IF (cAliasGW7)->(GW7_FILIAL) 	== (cAliasGW6)->&(Self:oFieldsJson['BranchId']) .AND. ;
					   ...
					   (cAliasGW7)->(GW7_DTEMIS)  	== (cAliasGW6)->&(Self:oFieldsJson['IssueDate']) 		
					 	...
					 	self:oEaiObjSn2['AccountingMovements'][nX]['LedgerAccount']		:= (cAliasGW7)->(GW7_CTACTB)
					 	self:oEaiObjSn2['AccountingMovements'][nX]['CostCenter']		:= (cAliasGW7)->(GW7_CCUSTO)
				 		self:oEaiObjSn2['AccountingMovements'][nX]['BusinessUnit']      := (cAliasGW7)->(GW7_UNINEG)
				 		self:oEaiObjSn2['AccountingMovements'][nX]['Value']          	:= (cAliasGW7)->(GW7_VLMOV)
				 		self:oEaiObjSn2['AccountingMovements'][nX]['TransactionType']   := (cAliasGW7)->(GW7_TRANS)			 					 					 		
					Endif
					(cAliasGW7)->(DbSkip())
					nX++							
				end
				
				self:oEaiObjSn2['FreightDocuments'] := {}
				Aadd(self:oEaiObjSn2['FreightDocuments'], JsonObject():New())
				...
				WHILE ((cAliasGW3)->(!Eof())) 		
					...
					IF (cAliasGW3)->(GW3_FILFAT)	== (cAliasGW6)->&(Self:oFieldsJson['BranchId']) .AND. ;
					   ...
					   (cAliasGW3)->(GW3_DTEMFA)  	== (cAliasGW6)->&(Self:oFieldsJson['IssueDate']) 	
					 	...
					 	self:oEaiObjSn2['FreightDocuments'][nX]['DocumentSerie']		:= (cAliasGW3)->(GW3_SERDF)
					 	self:oEaiObjSn2['FreightDocuments'][nX]['DocumentNumber']		:= (cAliasGW3)->(GW3_NRDF)
				 		self:oEaiObjSn2['FreightDocuments'][nX]['ElectronicAccessKey']  := (cAliasGW3)->(GW3_CTE)	
					Endif
					(cAliasGW3)->(DbSkip())
					nX++							
				end
				
				self:oEaiObjSn2['ShippingIssuer'] := {}
				Aadd(self:oEaiObjSn2['ShippingIssuer'], JsonObject():New())
				...
				WHILE (cAliasGU3)->(!Eof()) 
				 	...
				 	If (cAliasGU3)->GU3_FILIAL == FWxFilial("GU3") .AND. ;
					   (cAliasGU3)->GU3_CDEMIT == (cAliasGW6)->&(Self:oFieldsJson['IssuerCode'])				
					 	...
					 	self:oEaiObjSn2['ShippingIssuer'][nX]['FederalID']	:= (cAliasGU3)->(GU3_IDFED)
