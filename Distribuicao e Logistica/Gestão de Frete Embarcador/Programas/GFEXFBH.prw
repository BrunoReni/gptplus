...
Function GFECalcFrete()
	...
	Local lAplicValRom := !(lSimulacao .AND. iTipoSim == 0)
	Local lAplicViag   := .T.
	...
	While !GFEXFB_3EOF(lTabTemp, cTRBAGRU, @aAgrFrt, 0)
		...
		While  !GFEXFB_3EOF(lTabTemp, cTRBUNC, @aTRBUNC2, 6) .AND. ;
			...
			While !GFEXFB_3EOF(lTabTemp, cTRBTCF, @aTRBTCF1, 5) .AND. GFEXFB_5CMP(lTabTemp, cTRBTCF, @aTRBTCF1, 5,"NRCALC") == GFEXFB_5CMP(lTabTemp, cTRBUNC, @aTRBUNC2, 6,"NRCALC")
				...
				If nQtdeMin   > 0 .AND.;
						nVlFrMinT == 0 .AND.;
						Empty(cCompFrGar)	// Componente Frete Garantia
				...
				If cCalcDtPrev == "0"
					...
					If GV8->(dbSeek(xFilial("GV8") + cCdTrpBase + cNrTabBase + GFEXFB_5CMP(lTabTemp, cTRBTCF, @aTRBTCF1, 5,"NRNEG") + GFEXFB_5CMP(lTabTemp, cTRBTCF, @aTRBTCF1, 5,"NRROTA")))
						If lSimulacao
							...
							If GFEXFB_CSEEK(.F.,, @aTRBGRB1, 4,{GFEXFB_5CMP(lTabTemp, cTRBTCF, @aTRBTCF1, 5,"NRGRUP")})
								...
								if GFEXFB_CSEEK(lTabTemp, cTRBTRE, @aTRBTRE2, 7,{GFEXFB_5CMP(.F.     ,        , @aTRBGRB1, 4,"CDTPDC"	), ;
									...
									aTabPrazo[9] := POSICIONE("GU3", 1, xFilial("GU3") + GFEXFB_5CMP(lTabTemp, cTRBTRE, @aTRBTRE2, 7,"CDTRP"), "GU3_MODAL")
								EndIf
							EndIf
						Else
							GWU->(dbSetOrder(1))
						...
						If aRetTabPrazo[5] == .T.
							...
							GFEXFB_DRECLOCK(lTabTemp,cTRBUNC,.F.)
							...
							GFEXFB_EMSUNLOCK(lTabTemp,cTRBUNC)
						Else
				...
				If Len(aNegMin) > 0
					...
					If nVlCompMin >= 0 .And. nVlCompMin < nVlFretMin
						...
						If GV9->(dbSeek(xFilial("GV9") + aNegMin[1] + aNegMin[2] + aNegMin[3]))
							...
							GFEXFB_1AREA(lTabTemp,cTRBCCF, @aTRBCCF2)
			...
			If lAplicValRom
				...
				GFEXFB_1AREA(lTabTemp,cTRBUNC, @aTRBUNC3)
													...
													Else
														...
														If lRatClass
															...
															nY := aScan(aCompDia,{|x| x[_NRCALC] == GFEXFB_5CMP(lTabTemp, cTRBUNC, @aTRBUNC3, 6,"NRCALC");
																...
																.And. GV9->GV9_FRMCRC <> "1"})

															If nY > 0 .And. aCompDia[nY,_VALOR] > 0
																nPerc := nPerc *  (aValDiaria[nPos,3] / aCompDia[nY,_VALOR])
															EndIf

															nVlrCpo := aValDiaria[nPos,3] //GFEXFB_5CMP(lTabTemp, cTRBCCF, @aTRBCCF1, 9,"VALOR")
														Else
															nVlrCpo := aValDiaria[nPos,3]
														EndIf

														nPosRat := Ascan(aCount, {|x| x[1] == aValDiaria[nPos,2]})

														If nPosRat == 0
															Aadd(aCount, {aValDiaria[nPos,2], 1,0})
														...
														If aValDiaria[nPos,8] == aCount[nPosRat,2] .And. nVlrCpo != 0 //Verifica se é o ultimo rateio do componente para realizar o arredondamento do valor
															If Alltrim(GFEXFB_5CMP(lTabTemp, cTRBCCF, @aTRBCCF1, 9,"CDCOMP")) == Alltrim(aValDiaria[nPos,2]) .And. nVlrCpo != aValDiaria[nPos,3]
																nVlrCpo := aValDiaria[nPos,3]
															EndIf
															GFEXFB_5CMP(lTabTemp, cTRBCCF, @aTRBCCF1, 9,"VALOR",nVlrCpo-aCount[nPosRat,3])
														Else
															nValCompRat := (ROUND(nVlrCpo * nPerc,2))
															GFEXFB_5CMP(lTabTemp, cTRBCCF, @aTRBCCF1, 9,"VALOR",nValCompRat)
															aCount[nPosRat,3] += nValCompRat
														EndIf
													...
													If (nPos := aScan(aValCalc,{|x| x[1] == GFEXFB_5CMP(lTabTemp, cTRBUNC, @aTRBUNC2, 6,"NRCALC")})) > 0
														...
														If nPosMaior > 0
															...
															If nPosCalc > 0
																...
																oGFEXFBFLog:setTexto(STR0238 + cValToChar(aValCalc[nPos,7]) + CRLF) //"        Valor de frete original: "
											...
											For nX := 1 to len(aTRBUNC2)
												if aTRBUNC2[nX,25] == "0"
													AADD(aTRBUNC1, {aTRBUNC2[nx,01], aTRBUNC2[nx,02], aTRBUNC2[nx,03], aTRBUNC2[nx,04], aTRBUNC2[nx,05], aTRBUNC2[nx,06], aTRBUNC2[nx,07],;
														...
														aTRBUNC2[nx,22], aTRBUNC2[nx,23], aTRBUNC2[nx,24], aTRBUNC2[nx,25], aTRBUNC2[nx,26], aTRBUNC2[nx,27]})
												EndIf
											Next Nx
											aTRBUNC2 := aClone(aTRBUNC1)
											aTRBUNC3 := aClone(aTRBUNC1)
			...
			if GFEXFB_CSEEK(lTabTemp, cTRBDOC, @aDocCar2, 1,{GFEXFB_5CMP(lTabTemp, cTRBTRE, @aTRBTRE1, 7,"CDTPDC"), GFEXFB_5CMP(lTabTemp, cTRBTRE, @aTRBTRE1, 7,"EMISDC"), GFEXFB_5CMP(lTabTemp, cTRBTRE, @aTRBTRE1, 7,"SERDC"), GFEXFB_5CMP(lTabTemp, cTRBTRE, @aTRBTRE1, 7,"NRDC")})
				...
				if GFEXFB_CSEEK(lTabTemp, cTRBITE, @aTRBITE1, 8,{	GFEXFB_5CMP(lTabTemp, cTRBDOC, @aDocCar2, 1,"CDTPDC"), ;
					...
					aRetICMS := GFEFnIcms(cTmpTrp, ; // Código do transportador
						...
						GFEXFB_5CMP(lTabTemp, cTRBAGRU, @aAgrFrt, 0,"CDTPOP"),; //Posicione(cTRBAGRU,1,GFEXFB_5CMP(lTabTemp, cTRBDOC, @aDocCarg, 1,"NRAGRU"),"CDTPOP"),; //Tipo de Operação do Agrupador do Documento de Carga
...
@version 1.0
----------------------------------------------------------------------------*/
Static Function GFEVlBasComp(cNrCalc, aParComp, cCdclfr, cCdtpop)
...
Return aValor
					...
					@version 1.0
					----------------------------------------------------------------------------*/
				Static Function GFEPISCOF(cNRCalc)
					...
					----------------------------------------------------------------------------*/
				Static Function GFEGrvComImp(cNrCalc, cCdComImp, nVlImposto, cTotFre, cBasImp, cBaPiCo, cFreMin, cIdMin, nVlFrMi, cCdclfr, cCdTpOp)
					Local aSeek
					Default cCdclfr := ""
					Default cCdTpOp := ""
				...
				Static Function GFEAjusTre(lAltera)
					...
					@version 1.0
					----------------------------------------------------------------------------*/
				Static Function GFERatImp (cNrCalcImp, aParamComp, nVlFrete, nVLImposto,cCdclfr,cCdtpop)
	...
	While !GFEXFB_3EOF(lTabTemp, cTRBUNC, @aTRBUNC2, 6) .AND. GFEXFB_5CMP(lTabTemp, cTRBUNC, @aTRBUNC2, 6,"NRAGRU") == cNrAgrup
		...
		If nPos > 0
			...
			If !GV9->(dbSeek(xFilial("GV9") + GFEXFB_5CMP(lTabTemp, cTRBTCF, @aTRBTCF1, 5,"CDTRP") + GFEXFB_5CMP(lTabTemp, cTRBTCF, @aTRBTCF1, 5,"NRTAB") + GFEXFB_5CMP(lTabTemp, cTRBTCF, @aTRBTCF1, 5,"NRNEG")))
				...
				If !Empty(aTabInfo)
					GV9->(dbSeek(xFilial("GV9") + aTabInfo[2] + GFEXFB_5CMP(lTabTemp, cTRBTCF, @aTRBTCF1, 5,"NRTAB") + GFEXFB_5CMP(lTabTemp, cTRBTCF, @aTRBTCF1, 5,"NRNEG")))
				EndIf
			EndIf
...
Return //GFETratUniCal
				...
				Static Function ClcPercRed(cAtrCal)
					Local nPerc := 0
					cAtrCal := AllTrim(cAtrCal)

					If Val(cAtrCal) == 10 .Or. Val(cAtrCal) == 11 // 10=Peso.Liq 11=Qt.Serv
						nPerc := 0
					Else
