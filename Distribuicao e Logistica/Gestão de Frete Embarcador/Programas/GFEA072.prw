...
//Os registros que estiverem com checkbox marcado chamarão a função GFEA070DX
Function GFEA072DE()
	Local aRet		:= {}
	Local cTxtAP	:= ""
	Local cGW6Pro	:= GetNextAlias()
	...
	While !(cGW6Pro)->( Eof() )
		...
		If ValType(aRet) == "A"
			If aRet[1]				
				...
				Else
					cTxtAP += "Fatura " + GW6->GW6_NRFAT + " não foi enviada para desatualização no Financeiro por causa do seguinte erro: " + aRet[3][2][6] + CRLF 
				EndIf
			EndIf
		EndIf
	    (cGW6Pro)->( dbSkip() )
