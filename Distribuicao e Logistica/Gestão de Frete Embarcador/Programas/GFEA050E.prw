...
Function GFEA050E(cRomaneio,lSimula,nOperation)
	...
	Local oModelAnterior:= FwModelActive()
	Local oExecView
	Public nRomaneio := ""
	Private lAltOrdem := .F.
	...
	If GFXTB12117("GVX") .AND. GFXTB12117("GVY") .AND. GFXTB12117("GVZ")//Suavização para rotina de Vale Pedagio.
		nRomaneio := cRomaneio
		...
		If POSICIONE("GWN",1,xFilial("GWN") + nRomaneio,"!EOF()")
			oView	:= ViewDef()
			oModel	:= ModelDef()
		...
		//Restaura Model
		If oModelAnterior <> Nil
			oModelAnterior:Activate()
		EndIf
	Else
