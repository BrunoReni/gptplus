...
Private nSize     := 0
	...
	If lRet
		...
		If ValType(oXML)=="O"
			...
			ElseIf ValType(XmlChildEx(oXML,"_ENVICTE")) == "O" .And. (ValType(XmlChildEx(oXML:_enviCTe,"_CTE")) == "O" ) //-- Arquivo de Evento Cte
				GFEA118XML(cXMLFile,cDirLido,cDirErro,@aProc,@aErros,oXML:_enviCTe:_Cte,.F.)
			ElseIf ValType(XmlChildEx(oXML,"_ENVICTE")) == "O" .And. (ValType(XmlChildEx(oXML:_enviCTe,"_CTE")) == "A" ) //-- Arquivo de Evento Cte
...
Static Function CteTPDF(oCTE, aGXG, aGXH)
	...
	codCidIni := XmlValid(oCTe,{"_INFCTE","_IDE"},"_cMunIni")
	codCidFim := XmlValid(oCTe,{"_INFCTE","_IDE"},"_cMunFim")
	nomCidIni := FwQtToChr(XmlValid(oCTe,{"_INFCTE","_IDE"},"_xMunIni"))
	nomCidFim := FwQtToChr(XmlValid(oCTe,{"_INFCTE","_IDE"},"_xMunFim"))
...
//
Static Function ValidEmis(cIdFed,cMsgTra)
Local lAchou    := .T.
Local cEmitente := ''
Local cAliasGU3 := Nil
			...
			While (cAliasGW1)->(!Eof())
				...
				If GWU->(dbSeek(GW1->GW1_FILIAL + GW1->GW1_CDTPDC + GW1->GW1_EMISDC + GW1->GW1_SERDC + GW1->GW1_NRDC) .And. !Empty(GWU->GWU_CDTRP))
					While GWU->(!Eof()) .And. GWU->GWU_FILIAL==GW1->GW1_FILIAL .And. GWU->GWU_CDTPDC==GW1->GW1_CDTPDC .And. GWU->GWU_EMISDC==GW1->GW1_EMISDC .And. GWU->GWU_SERDC==GW1->GW1_SERDC .And. GWU->GWU_NRDC==GW1->GW1_NRDC
						...
						If ValType(oCTe)=="O" .And. Alltrim(cCnpjTran) == Alltrim(XmlValid(oCTe,{"_INFCTE","_EMIT"},"_CNPJ"))
							nRecnoGW1 := (cAliasGW1)->RECNOGW1
						EndIf
