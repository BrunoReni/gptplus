...
//---------------------------------------------------------------------------------------------------
/*/{Protheus.doc} GFEXFB_1AREA
//TODO Seleciona a area da tabela temporia. O array não precisa fazer tratamento.
...
@version 1.0
/*///------------------------------------------------------------------------------------------------
function GFEXFB_1AREA(lTabTmp, cTabTmp, aTabTmp)
	IF lTabTmp
		dbSelectArea(cTabTmp)
	Else
		// Não precisa fazer seleção do array
	ENDIF
Return
...
function GFEXFB_5CMP(lTabTmp, cTabTmp, aTabTmp, nArray, cCampo, cValor)
	...
	Private cTabTmp2 := cTabTmp
	IF lTabTmp
		if cValor == Nil
			Return &("(cTabTmp2)->" + cCampo)
		Else
			&("(cTabTmp2)->" + cCampo) := cValor
		EndIf 
	Else
		if cValor == Nil
...
/*/{Protheus.doc} GFEXFB_DRECLOCK
//TODO Similar ao comando RecLock. Para Array não ha necessidade de fazer tratamento
@author andre.wisnheski
@since 22/06/2015
@version 1.0
@param lTabTmp, Logico, Indica se é tabela temporária ou array
@param cTabTmp, Caracter, Nome da tabela temporia que será manipulada
@param lInsEdt, Logico, Indica Insert (.T.) Edit (.F.)
@type function
/*/
function GFEXFB_DRECLOCK(lTabTmp, cTabTmp, lInsEdt)
	IF lTabTmp
		RecLock(cTabTmp,lInsEdt)
	EndIf
Return

/*/{Protheus.doc} GFEXFB_EMSUNLOCK
//TODO Similar ao comando MsUnLock. Para Array não ha necessidade de fazer tratamento
@author andre.wisnheski
@since 22/06/2015
@version 1.0
@param lTabTmp, Logico, Indica se é tabela temporária ou array
...
@type function
/*/
function GFEXFB_EMSUNLOCK(lTabTmp, cTabTmp)
	IF lTabTmp
		(cTabTmp)->(MsUnLock())
	EndIf
Return
