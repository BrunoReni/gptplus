...
Function GFEA061J(nTpOp as numeric)
	...
	Else
		...
		ElseIf nTpOp == 2
			...
			oNewPage:SetConstruction({|Panel, nId |CriarPg(Panel, 1, 2,@cTargetXml,,@cTargetLog, @lSim, @lLibre)})
			...
			oNewPage := oStepWiz:AddStep("2")
			oNewPage:SetStepDescription("Confirmar Dados do Arquivo")
			oNewPage:SetConstruction({|Panel, nId | CriarPg(Panel, 2, 2,@cTargetXml,,@cTargetLog, @lSim, @lLibre)})
			oNewPage:SetNextAction({|| Processa({|| lRet := ImpTbXml(cTargetXml, cTargetLog, lSim, lLibre), IIF(!lSim,"Aguarde, Importando...", "Aguarde, Simulando...")}),  .T.})
		EndIf
...
/*/
//-------------------------------------------------------------------
Static Function CriarPg(oPanel, nId , nTpOp, cTarget,cName, cTargetLog, lSim, lLibre)
...
Static Function GetFileDir(cFullName)
    Local cPath as char
...
/*/
//-------------------------------------------------------------------
Static Function RetTpOriDsT(cTp)
	Local cRet as char
...
/*/
//-------------------------------------------------------------------
Static Function VldOriDst(cOriDst, cTp)
	Local cRet as char
...
/*/
//-------------------------------------------------------------------
Static Function RetSimNao(cValue)
	Local cRet as char
...
/*/
//-------------------------------------------------------------------
Static Function ImpTbXml(cTargetDir, cLogDir, lSim, lLibre)
...
Private aLog       as Array
	...
	For nI := 1 to nLinha
		...
		If !ExistCpo("GV2", Alltrim(oXmlParser:GetCol("COMPON",nI)))
			cLogTxt += LogMessage("ERRO: Componente não está cadastrado na rotina de Componentes de Frete."+CRLF)
			cLogTxt += LogMessage("AVISO: Fim da Operação do registro número "+cValToChar(nImp)+"." + CRLF + CRLF)
			lErro := .T.
			...
			If !lEncGV8
				...
				Do Case
					...
					Case cTpOri == '4'
						...
						cQuery += "   AND GV8_CDUFOR  = '"+PadR(cOri ,TamSX3("GV8_CDUFOR")[1])+"'"
					Case cTpOri == '5'
				...
				Do Case
					...
					Case cTpDst == '4'
						...
						cQuery += "   AND GV8_CDUFDS  = '"+PadR(cDst ,TamSX3("GV8_CDUFDS")[1])+"'"
					Case cTpDst == '5'
    ...
    If Len(aNewRow) == Len(::aColumns)
        ...
        For nX := 1 to Len(aNewRow)
        	...
        	If Alltrim(::aColumns[nx]) == "Valor Fixo" .Or. ;
				...
				Alltrim(::aColumns[nx]) == "Unit Tarifa Extra"
...
Static Function criaGV9(cChave, lSim)
	Local cChGV6     as char
	Local oModelGV1  as object
	Local lFound    := .F.
	Local aArea     := GV9->(GetArea())

	lGV6   := .F.

	oModelGV1 := oModelTRF:GetModel("DETAIL_GV1")
...
Static Function criaGV6(cChave, lSim)
