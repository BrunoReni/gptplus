	...
	For nI := 1 To oModelGWU:Length()
		...
		Else 
			...
			If (cAliasQry)->(!EoF())
				...
				GWU->(dbSeek(xFilial("GWU")+oModelGWU:GetValue('CDTPDC',nI)+oModelGWU:GetValue('EMISDC',nI)+oModelGWU:GetValue('SERDC',nI)+oModelGWU:GetValue('NRDC',nI)+(cAliasQry)->GWU_SEQ))
				RecLock("GWU",.F.)
	 ...
	 For nI := 1 To oModelGWU:Length() 
		If oModelGWU:isdeleted(nI) // verifica se a linha foi deletada
			...
			If oModelGWU:GetValue('SEQ',nI) == "01"
				...
				Return .F.
			ElseIf oModelGWU:GetValue('SEQ',nI) == "02"
				Help( ,, 'HELP',, "Para os documentos com tipo de frete CIF/FOB/Consig redespacho, não é possível excluir o segundo trecho do documento de carga. ", 1, 0,)
				Return .F.
			EndIf
