	...
	For nI := 1 To Len(aIdDCF)
	  ...
	  TcSetField(cAliasQry,'QTDORI','N',TamSX3('DCT_QTORIG')[1],TamSX3('DCT_QTORIG')[2])
		Do While (cAliasQry)->(!Eof())
			...
			If (cAliasQry)->QTD_MNTVOL > 0 .And. Self:oMovServic:ChkMntVol()
			...
			If lRet .And. (cAliasQry)->QTD_DISSEP > 0 .And. Self:oMovServic:ChkDisSep()
			...
			If lRet .And. (cAliasQry)->QTD_CONFEXP > 0 .And. Self:oMovServic:ChkConfExp()
...
Default cTipReab := "D"
	...
	If lConsCap 
		...
		If lRet .And. oEstFis:GetTipoEst() == '2' .And. ((Self:oMovServic:ChkReabast() .And. cTipReab == 'D') .Or. Self:oMovServic:ChkArmaz())
...
Default lDescBlq := .F.
	...
	If lRet .And. Self:oMovPrdLot:HasRastro() .And. !Empty(cLoteCtl)
		...
		If (cAliasQry)->(!Eof())
			...
			If Self:oMovServic:ChkTransf() .And. !(Self:oMovEndOri:GetArmazem() == Self:oMovEndDes:GetArmazem()) .AND. Self:oMovEndDes:GetTipoEst() = 5 .AND. lMovEst
