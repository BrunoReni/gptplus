...
Local lMsgprlt  := ""
	...
	If cTipInv == "2" .And. WmsArmUnit(cLocal) .And. WmsEndDoca(cLocal,cEnd)
		WmsMessage(WmsFmtMsg(STR0122),WMSXFUNA09,1,lExibMsg) //Não é permitido inventariar endereços do tipo 'DOCA' em armazéns unitizados controlados pelo WMS.
		lRet := .F.
	EndIf
...
Local nDec := TamSX3('D14_QTDEST')[2]
	...
	EndSql
	If (cAliasQry)->(!Eof())
		WmsMessage(WmsFmtMsg(STR0123,{{"[VAR01]",cProduto}}),WMSXFUNA10,1,lExibMsg) // "Existe divergência de saldo entre a D14 e SB2 para o produto : [VAR01]. Consulte documentação referente análise de estoque por endereço."
		lRet := .F.
	EndIf
	(cAliasQry)->(DbCloseArea())
	...
	If lRet .And. Rastro(cProduto)
		...
		EndSql
		If (cAliasQry)->(!Eof())
			WmsMessage(WmsFmtMsg(STR0124,{{"[VAR01]",cProduto}}),WMSXFUNA11,1,lExibMsg) // "Existe divergência de saldo entre a D14 e SB8 para o produto : [VAR01]. Consulte documentação referente análise de estoque por endereço."
			lRet := .F.
		EndIf
		(cAliasQry)->(DbCloseArea())
	...
	If lRet .AND. !lSLdDif
		...
		If (cAliasQry)->(!Eof()) .And. (cAliasQry)->NR_COUNT > 0
			WmsMessage(WmsFmtMsg(STR0126,{{"[VAR01]",cProduto}}),WMSXFUNA13,1,lExibMsg) //"Existe divergência Kardex(D13) endereço para o produto: [VAR01] .Consulte documentação referente análise de estoque por endereço. "
			lRet := .F.
		EndIf
		(cAliasQry)->(dbCloseArea())
	EndIf
...
Valida se a estrutura informada para o produto possui sequencia de abastencimento.
