...
Local nQtdSld  := 0
	...
	Do While lRet .And. (cAliasSDC)->(!Eof())
		...
		lEmpenho := !Empty((cAliasSDC)->DC_LOCALIZ)
					...
					If lOrigPerda
						...
						If TableInDic("SMN")
							If nRecnoSBC > 0 .And. nRecnoSD4 > 0
								...
								//--Grava tabela de estorno de apontamento de perda, utilizada como referencia em caso de exclusão da perda
								RecLock('SMN', .T.)
								SMN->MN_FILIAL := xFilial("SMN")
								SMN->MN_SEQSD3 := SBC->BC_SEQSD3
								SMN->MN_SEQSBC := SBC->BC_NUMSEQ
								SMN->MN_TRTSD4 := SD4->D4_TRT
								SMN->MN_QTDSD4 := nQtdEmp
								SMN->(MsUnlock())
...
Local nQtdBx 	:= 0
	...
	Do While lRet .And. (cAliasD13)->(!Eof())
		...
		If lEmpenho .OR. lSemSDC
			...
			lRetEmp := oEstEnder:UpdSaldo('499',.F./*lEstoque*/,.F./*lEntPrev*/,.F./*lSaiPrev*/,lEmpenho /*lEmpenho*/,.F./*lBloqueio*/,.F./*lEmpPrev*/,.F./*lMovEstEnd*/)
