	...
	If cField == "D0C_PRODUT"
		...
		If IntWMS(oModelD0C:GetValue("D0C_PRODUT"))
			...
			If lRet 
				...
				oSeqAbast:SetEstFis(oEstEnder:oEndereco:GetEstFis())
				If !oSeqAbast:LoadData(2)
					oModel:GetModel():SetErrorMessage( , , oModel:GetId() , '', WMSA52014, WmsFmtMsg(STR0019,{{"[VAR01]",oModelD0C:GetValue('D0C_PRODUT')}}), STR0020, '', '') // O produto [VAR01] não possui a estrutura cadastrada do endereço destino informado. // Informe outro endereço ou cadastre a estrutura para o produto.
					lRet := .F.
				EndIf
				If lRet
					If !(oSeqAbast:GetTipoEnd() == '4')
