...
CLASS WMSDTCEstoqueEndereco FROM LongNameClass
	...
	METHOD MakePerda(nRecnoSBC,cProg,nDifD4)
	...
	METHOD WmsEmpPrde()
	METHOD MakeFtMI(nRecnoSD2, cIdDCF,cEnder)
	METHOD UndoFtMi(nRecno, lEmpenho)
...
Default dDataMov := StoD("")
	...
	oMovEstEnd:SetDtEsto(IIF(!Empty(dDataMov), dDataMov, dDataBase))
...
/*/
//----------------------------------------
METHOD MakePerda(nRecnoSBC,cProg,nDifD4) CLASS WMSDTCEstoqueEndereco
Local lRet       := .T.
	...
	If lRet
		...
		If Len(aProduto) > 0
			For nProduto := 1 To Len(aProduto)
				...
				If cProg == "MATA685" .AND. lEmpSd4
					lRet := Self:UpdSaldo("999",.T. /*lEstoque*/,.F. /*lEntPrev*/,.F. /*lSaiPrev*/,.T./*lEmpenho*/,.F. /*lBloqueio*/,.F. /*lEmpPrev*/,.T./*lMovEstEnd*/,dDataMov)
				Else
				...
				If lRet .And. lRastro .And. cProg == "MATA685" .And. !lEmpSd4 .And. nDifD4 > 0
					//--Se houve quebra da SD4 no PCP(sigacusa) por falta de saldo, realiza a baixa do empenho
					Self:SetQuant(QtdComp(nDifD4))
...
/*/
//----------------------------------------
METHOD MakeFtMI(nRecnoSD2, cIdDCF, cEnder) CLASS WMSDTCEstoqueEndereco
Local lRet       := .T.
Local aProduto   := {}
Local nProduto   := 0
Local oMovEstEnd := WMSDTCMovimentosEstoqueEndereco():New()
...
/*/
//----------------------------------------
METHOD UndoFtMi(nRecno, lEmpenho) CLASS WMSDTCEstoqueEndereco
Local lRet       := .T.
Local aProduto   := {}
