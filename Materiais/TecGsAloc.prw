...
defCodLim( paramX ) <-- retorna o valor paramX e altera o retorno do método para paramX
...
method projAloc() class GsAloc
    ...
    If ::isActive()
        ...
        If lCodLim .And. FindFunction("At190QryAge")
            ...
            If cCodLim <> ::defCodLim()
                If At190QryAge(.T.,cCodTFF)
                    ::defMessage(STR0043) //"Alteração do Limite de Marcação na aba de Recursos Humanos do Orçamento de Serviços afetará a agenda do atendente."
                EndIf
            EndIf
        EndIf
		...
		For nJ := 1 To LEN(aAtend)
            ...
            If IsInCallStack("AT190JPRJLAC") .And. lAloRes
...
method gravaAloc( lInsert ) class GsAloc
    ...
    Local nGrupo        := 0
    Local cCodLim       := ""
    Local lAlRes        := IsInCallStack("AT190JYCMT")
    Local lOrcABQ       := .F.
    Default lInsert := .T.
    Static __AbbNumero := ""
        ...
        If !lInsert .OR. lAlRes .OR. (!(::defCob()) .And. ::insertTGY()) .Or. ((::defCob()) .And. ::insertTGZ())
			...
			For nX := 1 To LEN(::aPosFlds)
				...
				If nX == 1
                    ...
                    if lAlRes 
                        lOrcABQ := ATGsCkhOrc(TFL->TFL_CODPAI)
    
                        iF lOrcABQ
                            __AbbNumero	    := ATGsCkhABQ(TFL->TFL_CODPAI)
                        else
			...
			If Len(aGravar) > 0 
				...
				Begin Transaction
					::apagaRT()
				At330GvAlo(aAtdXCalen,"CN9",cTipoMv,.F.,@aInserted,.F.,.F.,.F.,,cItmRota,lPostoLib,__AbbNumero)
                    __AbbNumero := 0
...
@description Retorna a descrição da TCU
...
cQry += " WHERE ABQ.ABQ_FILIAL = '" +  xFilial('ABQ') + "' AND "
cQry += " ABQ.D_E_L_E_T_ = ' ' "
