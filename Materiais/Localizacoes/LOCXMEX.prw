...
@return lDup,lógico, .T. cuando el Folio Fiscal (UUID) se encuentro en otro documento
@example
DupUUID(_cUUID)
@see (links_or_references)
/*/
...
Funcion utilizada para agregar campos al encabezado de Notas Fiscales para México.
...
Function LxCposMex(aCposNF, cFunName, cTablaEnc)
	...
	Default nNFTipo := aCfgNF[SnTipo]

	lComExt := !(nNFTipo == 21 .And. !lCfdi40)
	// Tracker Pedido de Venta; consulta factura o remisión de salida
	cFunName := IIf(cFunName=="LOCXDLGNF" .And. FunName()=="MATA410", IIf(nNFTipo==1, "MATA467N", "MATA462N") , cFunName)
	// Tracker Solicitud de Compra, Generar Cotizaciones, Pedido de Compra o Remisión; consulta factura o remisión de entrada
	cFunName := IIf((cFunName=="LOCXDLGNF" .And. FunName()$"MATA110|MATA131|MATA121|MATA102N") .Or. (FunName()=="MATA102N" .And. nNFTipo==10), IIf(nNFTipo==10, "MATA101N", "MATA102N") , cFunName)
	...
	If cTablaEnc == "SF2" .And. cFunName $ "MATA467N|MATA465N|MATA462N" .And. ( nNFTipo == 1 .Or. nNFTipo == 2 .Or. nNFTipo == 21 .or. nNFTipo == 50 )
		...
		If cCFDUso <> "0"
			...
			EndIf
			//Número de documento a Sustiuir
		...
		EndIf
		If  cGlobal == "S" .And. nNFTipo == 01 .And. cCFDUso <> "0" .And. lCfdi40
			If SF2->(ColumnPos("F2_GLOBAL")) > 0
				aSX3 := LxSX3Cache("F2_GLOBAL")
				AAdd(aCposNF,{FWX3Titulo("F2_GLOBAL"),"F2_GLOBAL",aSX3[1],aSX3[2],aSX3[3],aSX3[4],aSX3[5],aSX3[6],"SF2",aSX3[7],,,,,,aSX3[8]})
			EndIf
			If SF2->(ColumnPos("F2_IDCBAJA")) > 0
				aSX3 := LxSX3Cache("F2_IDCBAJA")
				AAdd(aCposNF,{FWX3Titulo("F2_IDCBAJA"),"F2_IDCBAJA",aSX3[1],aSX3[2],aSX3[3],aSX3[4],aSX3[5],aSX3[6],"SF2",aSX3[7],,,,,,aSX3[8]})
			EndIf
	...
	Elseif cTablaEnc == "SF1" .And. cFunName $ "MATA465N" .And. nNFTipo == 4
		...
		If cCFDUso <> "0"
			...
			If  SF1->(ColumnPos("F1_TPDOC")) > 0 .And. nNFTipo == 04
				aSX3 := LxSX3Cache("F1_TPDOC")
				AAdd(aCposNF,{FWX3Titulo("F1_TPDOC"),"F1_TPDOC",aSX3[1],aSX3[2],aSX3[3],aSX3[4],aSX3[5],aSX3[6],"SF1",aSX3[7],,,,,,aSX3[8]})
			EndIf
		EndIf
	EndIf
Return
	...
	If SF3->(ColumnPos("F3_MOTIVO")) > 0 .And. SF3->(ColumnPos("F3_STATUS")) > 0 .And. SF3->(ColumnPos("F3_CNATREC")) > 0 .And. SF3->(ColumnPos("F3_CODNFE")) > 0
		...
		If SF3->(dbSeek(cFilSF3+cNumDoc+cSerie))
			RecLock("SF3",.F.)
				...
				SF3->F3_STATUS := IIF((cCanEst == "0" .OR. cCanEst == "") .AND. cCanAcep == "2","A","S")
...
If lRet .And. nNFTipo == 1 .And. lCfdi40 .And. cGlobal =='S' .And. aCabNota[2][nPosGlobal] == "1" .And.;
...
Default aDadosD := {}
	...
	If &(cPrefNF+"CVEPED") == "A1" .And. Empty(&(cPrefNF+"TIPOPE")) .And. (!Empty(&(cPrefNF+"CERORI")) .Or. !Empty(&(cPrefNF+"NUMCER")) .Or. !Empty(&(cPrefNF+"EXPCONF")) .Or. !Empty(&(cPrefNF+"INCOTER")) .Or. !Empty(&(cPrefNF+"SUBDIV")) .Or. !Empty(&(cPrefNF+"OBSERV")) .Or. !Empty(&(cPrefNF+"TCUSD")) .Or. !Empty(&(cPrefNF+"TOTUSD")))
		MSGINFO(STR0166)
		Return .F.
	EndIf
