...
For nY := 1 To oMdlTXS:Length()
	...
	If !Empty( oMdlTXS:GetValue("TXS_CODPRD") )
		...
		If TXS->( ColumnPos('TXS_INSALU') ) .And. !(Empty(oMdlTXS:GetValue('TXS_INSALU')) )
			lSetValue := lSetValue .And. oModelTFF:SetValue('TFF_INSALU', oMdlTXS:GetValue('TXS_INSALU'))
		EndIf

		If TXS->( ColumnPos('TXS_GRAUIN') ) .And. !(Empty(oMdlTXS:GetValue('TXS_GRAUIN')) )
			lSetValue := lSetValue .And. oModelTFF:SetValue('TFF_GRAUIN', oMdlTXS:GetValue('TXS_GRAUIN'))
		EndIf

		If TXS->( ColumnPos('TXS_PERICU') ) .And. !(Empty(oMdlTXS:GetValue('TXS_PERICU')) )
	...
	If lSetValue .And. lExistTXX
		lSetValue := At984AVrb(oModelFull,oMdlFac,oModelTWO,oMdlTXS:GetValue('TXS_CODIGO'),cCodTWO)
	Endif
...
/*/
//------------------------------------------------------------------------------
Static Function At984AVrb(oModelFull,oMdlFac,oModelTWO,cCodRH,cCodTWO)
Local lSetValue 	:= .T.
Local nY 			:= 0
Local cChvItem 		:= ""
...
For nY := 1 To oModFacMat:Length()
	...
	If !Empty(oModFacMat:GetValue('TXX_BENEFI'))
		...
		cChvItem	:= cCodFac + oModFacMat:GetValue('TXX_CODIGO') + oModelTWO:GetValue('TWO_ITEM')
