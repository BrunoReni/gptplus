	...
	While TGT->(!EOF()) .And. xFilial("TGT") == TGT->TGT_FILIAL .And.;
		...
		If TGT->TGT_EXCEDT == "1"
			...
			If cChaveTFF <> xFilial(cTabela) + cCodTFF + cTabela
				...
				aTFFVal := U_GetDtTFF(cTabela, 1 ,xFilial(cTabela) + cCodTFF, cContr, TGT->TGT_DTINI,TGT->TGT_DTFIM,cEncerr = '1',dDtEncerr)
			EndIf 
			If !Empty(aTFFVal)
				aMesVlr := U_ValRetMes(aTFFVal,nMes,TGT->TGT_INDICE,TGT->TGT_VALREA)
				If !Empty(aMesVlr)
					nValRet := aMesVlr[1,2]
					Exit
				Endif
