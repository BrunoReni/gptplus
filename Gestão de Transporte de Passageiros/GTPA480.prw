...
BeginSql Alias cAliasTmp
    ...
    SELECT 'REC_BILHETE' AS TIPO,
           GIC_COLAB AS COLAB,
           SUM(GIC.GIC_VALTOT) AS TOTAL
    FROM %Table:GIC% AS GIC
    ...
    GROUP BY GIC.GIC_COLAB
    UNION
    SELECT 'REC_ADICIONAIS' AS TIPO,
           GZT_COLAB AS COLAB,
           SUM(GZT_VALOR) AS TOTAL
    FROM %Table:GZT% GZT
    ...
    GROUP BY GZT_COLAB
    UNION
    SELECT 'REC_TAXAS' AS TIPO,
           G57_EMISSO AS COLAB,
           SUM(G57.G57_VALOR) AS TOTAL
    FROM %Table:G57% G57
