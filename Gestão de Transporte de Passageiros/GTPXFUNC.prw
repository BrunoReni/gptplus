...
Function GTPXGerViag(cCodLin,cSentido,cCodHor,dDtIni,dDtFim,aRecurso,cCodExcl,cTipo, cCodContr, cKmProvavel,nQtdCarros)
...
Function GTPNewTempTable(cQuery,cAlias,aIndex,aFldConv,oTable,lForceRemake)
	...
	If ( Len(aDivQry) > 0 )
		...
		If ( TcSQLExec(cQryIn) < 0 )
			...
			Iif( !IsBlind(), MsgAlert( TCSQLError() ), nil)
...
Function GTPxTmpTbl(cAliasAux,aIndex)
	...
	lRemake := AdmDiffArray(aStruct,aStructTmp) .Or. (Valtype(oTableTmp) <> "O")
