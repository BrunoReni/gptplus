...
FwMsgRun(,{|| lRet := CancelaGCT(cCodCN9)}, STR0004, STR0006) // "Solicitando cancelamento do contrato no GCT...") //"Aguarde", "Solicitando cancelamento do contrato no GCT..."
...
@see (links_or_references)
/*/
Static Function CancelaGCT(cCodCN9)
Local lRet := .F.

CN9->(dbSetOrder(1))

If CN9->(dbSeek(xFilial("CN9") + cCodCN9))
