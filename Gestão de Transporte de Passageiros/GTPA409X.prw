...
Local dDtFim        := nil
Local aErro			:= {}
Local lNewVldDoc	:= FindFunction('GTPVLDDOCS') .And. AliasInDic('H68')
...
BeginSql Alias cAliasTmp 
	...
	FROM %Table:GYN% AS GYN
	INNER JOIN %Table:GYD% AS GYD ON GYD.GYD_FILIAL = %xFilial:GYD%
...
/*/
//------------------------------------------------------------------------------
Static Function GtpTXbDem(cCodColab,dDtRef)
Local lDemit 	 := .T.
...
/*/
//------------------------------------------------------------------------------
Static Function GtpRestriRH(cCodColab,dDtRef,cMotivo)
Local lRestrRH 	 := .T.
Local cTmpResTri := GetNextAlias()
Local cWhere	 := "%%"	
