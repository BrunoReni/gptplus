...
Local aRotina := {}
	...
	ADD OPTION aRotina TITLE STR0085			 		ACTION 'A421EncFic("4")'	OPERATION 4 ACCESS 0 // #Gerar Titulo //#Enc. Ficha
...
Static Function ViewDef()
	...
	If AliasInDic('H6M') .And. AliasInDic('H6N') .And. GI6->(FieldPos('GI6_CTRCXA')) > 0
		oView:AddUserButton("Caixa do Colaborador", "GTPA421",{|oModel| GTPA481(oModel)}, "Caixa do Colaborador",,) 
	Endif
	...
	If ( GI6->(FieldPos("GI6_USAGTV")) > 0 )
		...
		oView:AddUserButton("Leg. Anexo","GTPA421",{|oVw| LegAnexoGTV(oVw)},"Anexos")//, , {MODEL_OPERATION_INSERT, MODEL_OPERATION_UPDATE}) // "Vincular Cheque"
	EndIf
...
Static Function A421Commit(oModel)
	...
	If !FwIsInCallStack("GTFECHACX") .AND. !FwIsInCallStack("GTPPROCREAB") .And. !FwIsInCallStack("GA700MrkFch") 
		...
		If ( Len(aNewGZE) > 0 )
			...
			cMsgYesNo := "Há novos depósitos do tipo GTV (Guia de Transporte de Valores). "
			cMsgYesNo += "Os documentos de GTV podem ser anexados "
			cMsgYesNo += "na base de conhecimento. " + CHR(13) + CHR(10)
			...
			cMsgYesNo += "Deseja anexar os documentos GTV na base de conhecimento agora? "
...
Static Function HasNewGTV(oSubGZE,aNewGZE,aDelGZE)
	...
	If ( Len(aNewGZE) == 0 .And. nOper != MODEL_OPERATION_DELETE)
		...
		For nI := 1 to oSubGZE:Length()
			...
			If ( oSubGZE:GetValue("GZE_FORPGT",nI) == "5" )
				//Não foi inserido
...
lStrGYV := GTPxVldDic('GYV', aFldsGYV, .T., .T.)
	...
	If nOperation == MODEL_OPERATION_INSERT
		...
		If GIC->(FIELDPOS("GIC_EMPRJI")) > 0 .AND. GIC->(FIELDPOS("GIC_INTEGR")) > 0 .AND.;
			...
			//Deixado sem filial na GIC, pois pode existir bilhete para esse processo em outra filial
			BeginSql Alias cAliasOut
				...
				ON GI6.GI6_FILIAL = %xFilial:GI6%
					...
					AND GI6.%NotDel%
				WHERE GIC.GIC_AGENCI = %Exp:cAgencia%
					AND GIC.GIC_DTVEND BETWEEN %Exp:DtoS(dDataIni)% AND %Exp:DtoS(dDataFin)%
					AND GIC.GIC_NUMFCH = ''
			...
			If (cAliasOut)->VLRTOTAL > 0
				...
				oGridGZGR:SetValue("GZG_DESCRI", POSICIONE('GZC' ,1 ,XFILIAL('GZC') + StrZero(30, TamSx3('GZC_CODIGO')[1]) , 'GZC_DESCRI'))
				oGridGZGR:SetValue("GZG_TIPO"  , "1")
				oGridGZGR:SetValue("GZG_CARGA" , .T.)
				...
				oGridGZGD:SetValue("GZG_DESCRI", POSICIONE('GZC' ,1 ,XFILIAL('GZC') + StrZero(31, TamSx3('GZC_CODIGO')[1]) , 'GZC_DESCRI'))
				oGridGZGD:SetValue("GZG_TIPO"  , "2")
				oGridGZGD:SetValue("GZG_CARGA" , .T.)
		...
		If AliasInDic('H6M') .And. AliasInDic('H6N') .And.;
			...
			If (cAliasH6N)->H6N_VLPEND > 0
					...
					oGridGZGD:SetValue("GZG_DESCRI", POSICIONE('GZC' ,1 ,XFILIAL('GZC') + StrZero(32, TamSx3('GZC_CODIGO')[1]) , 'GZC_DESCRI'))
					oGridGZGD:SetValue("GZG_TIPO"  , "2")
					oGridGZGD:SetValue("GZG_CARGA" , .T.)
...
Static Function VerDocGTV(oGrid,cField,nLineGrid,nLineModel)
	...
	If ( cField == "ANEXO" )
		...
		If ( oModel:GetModel("GZEDETAIL"):GetValue("GZE_FORPGT",nLineModel) == "5" )
			...
			If ( !lAnexo .And. lNewGze )
				...
				cMsgErro += "não é possível anexar o documento da GTV na base de conhecimento."
			
				cMsgSolu := "Durante a Confirmação da ficha (botão 'Confirmar'), "
				cMsgSolu += "os documentos de GTV, poderão ser anexados caso deseje-se "
		...
		Else
			...
			cMsgErro := "Somente os depósitos com GTV (Guia de Transporte de Valores) "
			cMsgErro += "permitem anexar o documento na Base de Conhecimento"
			
			cMsgSolu := "Se é desejado efetuar o anexo da guia de transporte de valores, "
			cMsgSolu += "selecione o tipo de depósito e forma de depósito como '5=GTV'."
