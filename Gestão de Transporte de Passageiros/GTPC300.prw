...
If ValType(oViewMonitor) == 'O' .AND. oViewMonitor:IsActive()
	...
	If ( cGC300Focus == "GYNDETAIL" )
		...
		aAdd(aCores, {"BR_BRANCO", "Sem ocorrências"})
...
If GYD->(FieldPos('GYD_TIPLIN')) > 0 .And. GYD->(FieldPos('GYD_NRMOTO')) > 0
	...
	BeginSql Alias cAliasTmp 
		...
		INNER JOIN %Table:GY0% AS GY0 ON GY0.GY0_FILIAL = %xFilial:GY0%
		...
		AND GYD.%NotDel%
		INNER JOIN %Table:GI2% AS GI2 ON GI2.GI2_FILIAL = %xFilial:GI2%
