...
//	U_AJREMOCAO()							// CRIA PARÂMETROS, TABELAS, CONSULTAS E ETC - DESCONTINUADO, POIS ESTÁ NO WIZARD DE INSTALAÇÃO.
	...
	//IF !LVERZBX 							// HABILITA CONTROLE DE MINUTA
		//MSGALERT("MV_LOCX097 ==> "+STR0133, STR0002) //Controle de minuta desabilitado.###Rental
		//RETURN NIL
	//ENDIF
	...
	//AADD(AROTINA,{ OemToAnsi(STR0008) ,"LOCA00524"   ,0,6}) //"IMPRIMIR MINUTA"
	//AADD(AROTINA,{"IMPR.PARTE DIARIA","LOCA00502",0,6})
	//AADD(AROTINA,{"MOBILIZ/DESMOB."  ,"LOCA00501" ,0,6})
...
LOCAL AAREA   := GETAREA()
LOCAL NMOBDES := AVISO( STR0009, STR0010, {STR0011,STR0012,STR0013}) // "IMPRESSÃO DE MOB./DESMOB." - "ROTINA PARA IMPRESSÃO DE CONTRAPESOS, SELECIONE A OPÇÃO:"

IF ! STRZERO(NMOBDES,1) $ "1;2"	 										// CANCELADO PELO USUÁRIO
	RETURN NIL 
ENDIF 

U_RELMINUEQ( 1 , IIF( NMOBDES==1 , STR0014 , STR0015) ) 	// 1 ABRE DIALOGO DE IMPRESSAO //"MOBILIZAÇÃO" - 
...
CASE FwIsInCallStack("LOCA00506") //CANCELA MINUTA //UPPER(CPERG)==UPPER("LOCA00506")
	CQRY += " AND FPF_STATUS = '1' AND FPF_MOTCAN = '"+CMOTI+"' " + CCRLF 
...
IF SELECT("TRBFPO") > 0					// TRBZLG - alias anterior
	TRBFPO->(DBCLOSEAREA())
ENDIF

TCQUERY _CQUERY NEW ALIAS "TRBFPO"

IF TRBFPO->(EOF())
	IF FPO->( RECLOCK("FPO", .T.) )
		FPO->FPO_FILIAL	:= XFILIAL("FPO")
		FPO->FPO_FROTA  := FPF->FPF_FROTA 	// FQ5->FQ5_GUINDA
		FPO->FPO_CODCLI	:= POSICIONE("FP0",1,XFILIAL("FP0")+FQ5->FQ5_SOT,"FP0_CLI")
		...
		IF NPOSBARRA > 0 					// VERIFICA SE EXISTE REVISÃO
			IF FPF->FPF_TIPOSE == "E"
				...
				FPO->FPO_REVISA := STRZERO(POSICIONE("FP0",3,FQ5->FQ5_FILORI+FQ5->FQ5_AS+FQ5->FQ5_VIAGEM,"FP0_REVNAS"),2)
	 ...
	 //	IF ! FQ5->( DBSEEK( XFILIAL("FQ5") + GDFIELDGET("FPF_AS", NI), .T. ) )	// POSICIONA NA DTQ
		...
		ELSE
			...
			CASE FPF->FPF_TIPOSE == "E" 				// EQUIPAMENTO
				...
				_CTPPROJ := POSICIONE("FPA" , 3 , FwxFilial("FPA")+FPA->FPA_AS+FPA->FPA_VIAGEM , "FPA_TIPOSE") 
				IF ! LOCA00517( _CTPPROJ , GDFIELDGET("FPF_FROTA",NI) , GDFIELDGET("FPF_DATA",NI) , GDFIELDGET("FPF_HORAI",NI) , GDFIELDGET("FPF_HORAF",NI) ) 
					...
					FP0->(DBSETORDER(3)) 				// FP4_FILIAL + FP4_AS + FP4_VIAGEM
				    //IF FP4->( DBSEEK(XFILIAL("FP4") + FQ5->FQ5_AS + FQ5->FQ5_VIAGEM) ) 
					IF FP0->( DBSEEK(XFILIAL("FP0") + FQ5->FQ5_AS + FQ5->FQ5_VIAGEM) ) 
						IF _LGERZLG
							//IF FP4->FP4_TIPOSE != "M" 	// M=MAO DE OBRA
							IF FP0->FP0_TIPOSE != "M" 	// M=MAO DE OBRA
	...
	IF ATABAUX[NPOS,NPOSMAR]==CMARCA
		...
		CASE ATABAUX[NPOS,NPOSSTA]=="3"  //1=PREVISTA,2=CONFIRMADA,3=BAIXADA,4=ENCERRADA,5=CANCELADA,6=MEDIDA
			IF ! FVERZLG(ATABAUX[NPOS,NPOSNAS],ATABAUX[NPOS,NPOSDAT])  //VERIFICA SE EXISTE PROGRAMAÇÃO
				CSTATUS := "2"
			ELSE
		//		MSGALERT("EXISTE PROGRAMAÇÃO DIÁRIA NO DIA "+DTOC(ATABAUX[NPOS,NPOSDAT])+". ESTORNO NÃO EXECUTADO." , "Rental")
				CSTATUS := "2"
			ENDIF
		...
		CASE ATABAUX[NPOS,NPOSSTA]=="5"  //1=PREVISTA,2=CONFIRMADA,3=BAIXADA,4=ENCERRADA,5=CANCELADA,6=MEDIDA
			...
			IF LEN(AVERZBX)==0
				CSTATUS := "1"
			ELSE
				//MSGALERT(STR0091+DTOC(ATABAUX[NPOS,NPOSDAT])+STR0092+ALLTRIM(AVERZBX[1,3])+STR0093 , STR0002) //"EXISTE MINUTA NO DIA "###" AS: "###". ESTORNO NÃO EXECUTADO."###"Rental"
				cErro 	:= STR0091+DTOC(ATABAUX[NPOS,NPOSDAT])+STR0092+ALLTRIM(AVERZBX[1,3])+STR0093
				CSTATUS := ""
			ENDIF
			IF EMPTY(CSTATUS)
				...
				IF LEN(AVERZLM)==0
					CSTATUS := "1"
				ELSE
					//MSGALERT(STR0094+DTOC(ATABAUX[NPOS,NPOSDAT])+STR0092+ALLTRIM(AVERZLM[1,2])+STR0093 , STR0002) //"EXISTE MEDIÇÃO NO DIA "###" AS: "###". ESTORNO NÃO EXECUTADO."###"Rental"
					cErro 	:= STR0094+DTOC(ATABAUX[NPOS,NPOSDAT])+STR0092+ALLTRIM(AVERZLM[1,2])+STR0093
					CSTATUS := ""
				ENDIF
			ENDIF
			//se tiver erro, mostra mensagem
			If Empty(CSTATUS)
				MsgAlert(cErro,STR0002)
			EndIf
		ENDCASE
						...
						IF CSTATUS == "1" 
						 //	FKILLPROG( FPF->FPF_TIPOSE, FPF->FPF_MINUTA )	// REMOVE PROGRAMACAO
							IF ATABAUX[NPOS,NPOSSTA] == "5"
								...
								_CQRYZLG := " SELECT R_E_C_N_O_ AS REG FROM " + RETSQLNAME("FPO") + " " + CRLF
...
//	ENDIF
//ELSE
//	MSGALERT("ESTORNO EXECUTADO COM SUCESSO." , "Rental")
//ENDIF
...
IF !EMPTY(CFAMILIA) .AND. LRET .AND. SUBSTR(CAS,2,1) <> "2"
	IF CFAMILIA <> ST9->T9_CODFAMI
		MSGALERT(STR0098 , STR0002) //"A FAMÍLIA DO BEN DA TROCA NÃO É CORRESPONDENTE AO BEM ORIGINAL."###"Rental"
		LRET := .F.
	ENDIF
ENDIF

IF ST9->T9_TIPOSE <> "E" .AND. LRET .AND. SUBSTR(CAS,2,1) <> "2"
	MSGALERT(STR0099 , STR0002) //"ESTE RECURSO NÃO É UM EQUIPAMENTO."###"Rental"
	LRET := .F.
ENDIF
/*
IF (CEQUIATU <> CEQUINOV) .AND. LRET
	MSGALERT("O RECURSO SELECIONADO ("+ALLTRIM(ATROCA[2])+"), NÃO TEM O MESMO TIPO OU CONFIGURAÇÃO DO QUE FOI VENDIDO ("+ALLTRIM(ATROCA[1])+")." , "Rental")
	LRET := .F.
ENDIF
*/
...
IF UPPER(CROTINA) == "LOCA048"
	CQRY += " AND ZBX.FPF_AS = '" + CAS + "'" + CCRLF
	// 29/07/2022 - Jose Eulálio - SIGALOC94-414 - Ajustar medição para quando colocar a AS carregar o grid de medição com as minutas baixadas do período.
	//CQRY += " AND ZBX.FPF_DATA BETWEEN '" + DTOS(DDATAD) + "' AND '" + DTOS(DDATAA) + "'" + CCRLF
	CQRY += " AND ZBX.FPF_DATA <= '" + DTOS(DDATAA) + "' " + CCRLF
	CQRY += " AND ZBX.FPF_STATUS = '3' " + CCRLF
ELSE
	...
	*/
	...
	// alterado por frank em 14/10/22
	CQRY += " AND ( ( ZBX.FPF_DATA BETWEEN '" + DTOS(DDATAD) + "' AND '" + DTOS(DDATAA) + "' AND ZBX.FPF_HORAI > '" + CHINI + "' AND ZBX.FPF_HORAI < '" + CHFIM + "' ) " + CRLF
	CQRY +=   " OR  ( ZBX.FPF_DATA BETWEEN '" + DTOS(DDATAD) + "' AND '" + DTOS(DDATAA) + "' AND ZBX.FPF_HORAF > '" + CHINI + "' AND ZBX.FPF_HORAF < '" + CHFIM + "' ) " + CRLF
	CQRY +=   " OR  ( ZBX.FPF_DATA BETWEEN '" + DTOS(DDATAD) + "' AND '" + DTOS(DDATAA) + "' AND ZBX.FPF_HORAI = '" + CHINI + "' AND ZBX.FPF_HORAF = '" + CHFIM + "' ))" + CRLF
...
IF CHORAI <> "  :  " .AND. CHORAF <> "  :  " .AND. !EMPTY(CHORAI) .AND. !EMPTY(CHORAF)
	...
	IF _CTEMP1 == _CTEMP2  .OR. (_CTEMP1 == "0000" .AND. _CTEMP2 == "2400")
		MSGALERT(STR0121 , STR0002) //"O HORÁRIO INICIAL E FINAL NÃO PODEM SER IGUAIS."###"Rental"
		IF _CTEMP1 == "2400"
...
IF CHALMI <> "  :  " .AND. CHALMF <> "  :  " .AND. !EMPTY(CHALMI) .AND. !EMPTY(CHALMF)
	...
	IF _CTEMP1 == _CTEMP2  .OR. (_CTEMP1 == "0000" .AND. _CTEMP2 == "2400")
		...
		IF _CTEMP1 == "2400"
		//	MSGALERT("O HORÁRIO DE ALMOÇO INICIAL 24:00 É CONSIDERADO NA VALIDAÇÃO COMO 00:00" , "Rental")
		ELSEIF _CTEMP2 == "2400"
		//	MSGALERT("O HORÁRIO DE ALMOÇO FINAL 24:00 É CONSIDERADO NA VALIDAÇÃO COMO 00:00" , "Rental")
		ENDIF
		RETURN .F.
	ENDIF
	   
	IF _CTEMP1 > _CTEMP2
		MSGALERT(STR0126,STR0002) //"OS HORÁRIOS DE ALMOÇO NÃO PODEM CARACTERIZAR A MUDANÇA DE DIAS."###"Rental"
		IF _CTEMP1 == "2400"
		 //	MSGALERT("O HORÁRIO DE ALMOÇO INICIAL 24:00 É CONSIDERADO NA VALIDAÇÃO COMO 00:00" , "Rental")
		ELSEIF _CTEMP2 == "2400"
		 //	MSGALERT("O HORÁRIO DE ALMOÇO FINAL 24:00 É CONSIDERADO NA VALIDAÇÃO COMO 00:00" , "Rental")
...
IF CHJANI <> "  :  " .AND. CHJANF <> "  :  " .AND. !EMPTY(CHJANI) .AND. !EMPTY(CHJANF)
	...
	IF _CTEMP1 == _CTEMP2  .OR. (_CTEMP1 == "0000" .AND. _CTEMP2 == "2400")
		MSGALERT(STR0129,STR0002) //"O HORÁRIO DE JANTAR INICIAL E FINAL NÃO PODEM SER IGUAIS."###"Rental"
		IF _CTEMP1 == "2400"
		 //	MSGALERT("O HORÁRIO DE JANTAR INICIAL 24:00 É CONSIDERADO NA VALIDAÇÃO COMO 00:00","Rental")
		ELSEIF _CTEMP2 == "2400"
		 //	MSGALERT("O HORÁRIO DE JANTAR FINAL 24:00 É CONSIDERADO NA VALIDAÇÃO COMO 00:00","Rental")
		ENDIF
		RETURN .F.
	ENDIF
	
	IF _CTEMP1 > _CTEMP2
		MSGALERT(STR0130 , STR0002) //"OS HORÁRIOS DE JANTAR NÃO PODEM CARACTERIZAR A MUDANÇA DE DIAS."###"Rental"
		IF _CTEMP1 == "2400"
		 //	MSGALERT("O HORÁRIO DE JANTAR INICIAL 24:00 É CONSIDERADO NA VALIDAÇÃO COMO 00:00" , "Rental")
		ELSEIF _CTEMP2 == "2400"
		 //	MSGALERT("O HORÁRIO DE JANTAR FINAL 24:00 É CONSIDERADO NA VALIDAÇÃO COMO 00:00" , "Rental")
...
LOCAL CPERG     := "LOCP075" //PADR("LOCA00524",LEN(SX1->X1_GRUPO))
...
ELSE
	MSGALERT(STR0134 , STR0002) //"NÃO HÁ DADOS, VERIFIQUE OS FILTROS."###"Rental"
ENDIF
...
WHILE !FQ5->(EOF()) .AND. FQ5->(FQ5_SOT+FQ5_OBRA) == CPROJET+COBRA
 ...
 //	IF FQ5->FQ5_TPAS <> "G"
	IF ( FP0->FP0_TIPOSE != "E" .and. FP0->FP0_TIPOSE != "L" )
		FQ5->(DBSKIP())
		LOOP
	ENDIF 
...
WHILE .T.
	...
	IF LEN(ACOLS)==0
		MSGALERT("NÃO EXISTEM MINUTAS NO INTERVALO SELECIONADO." , "Rental")
		RETURN
	ENDIF
...
IF EMPTY(CZLGSTATUS)
	MSGALERT('FAVOR INFORMAR O STATUS.' , "Rental")
	RETURN .F.
ENDIF
...
DO CASE
CASE NPOSAUX==0
	MSGALERT("ATENÇÃO ==> FAVOR SELECIONAR PELO MENOS UMA MINUTA." , "Rental")
	RETURN(.F.)
ENDCASE
...
IF NGRAVADOS>0
	MSGALERT("ACEITE DE MINUTAS EXECUTADO COM SUCESSO." , "Rental")
ELSE
	MSGALERT("NENHUM ACEITE FOI EXECUTADO." , "Rental")
ENDIF
...
IF FVERZLG(CNRAS,DDATA) 		// VERIFICA SE EXISTE PROGRAMAÇÃO
	MSGALERT("EXISTE PROGRAMAÇÃO DIÁRIA NO DIA "+DTOC(DDATA)+". MINUTA DESSE DIA NÃO FOI ACEITA." , "Rental")
	LRET := .F.
ENDIF
...
// Funcao para identificar se utiliza o processo de minuta
// Autor: Frank Fuga em 11/11/2022
...
// Funcao para atualiza o status da medicao no momento da inclusao do pedido de vendas
// rotina utilizada no ponto de entrada M410STTS
// Autor: Frank Fuga em 11/11/2022
// As variaveis de area (posicionamento das tabelas estao no M410STTS)
Function LOCA00526()
Local _lMinuta  := .F.
Local _lMedicao := .F.
...
// Funcao para atualiza o status da medicao no momento da exclusao do pedido de vendas
// rotina utilizada no ponto de entrada MA410DEL
// Autor: Frank Fuga em 11/11/2022
// As variaveis de area (posicionamento das tabelas estao no MA410DEL)
Function LOCA00527()
Local _lMinuta  := .F.
Local _lMedicao := .F.
