	...
	WHILE INT077->(!EOF())							                                                                   	//LACO ENQUANTO NAO FOR O FIM DA TABELA TEMPORARIA INT077(QUERY) 
		...
		IF !EMPTY(INT077->FPU_DTFIN) 
			...
			IF !EMPTY(cDataIn)
				...
				cBody += " 		<td width='120' align='center'><b>"+dDtVenc+"</b></td> "   	               //DATA DE VENCIMENTO DA INTEGRACAO
...
WHILE QRYAS->(!EOF())  
    ...
    If cAs <> QRYAS->NR_AS
		...
		cQRY+= " OR '" + DTOS(DATE()) + "' BETWEEN  '" + DTOS(STOD(QRYAS->DATALIMITE)-7) + "' AND FPU_DTLIM ) "       + CRLF
