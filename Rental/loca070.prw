	...
	IF FQD->(DBSEEK(XFILIAL("FQD") + ST9->T9_STATUS))
		...
		IF CSTSATU == "70" .OR. CSTSATU == "00" .OR. EMPTY(CSTSATU)
			IF FQD->(DBSEEK(XFILIAL("FQD") + M->T9_STATUS))
				...
				IF CSTSNEW <> "00" .AND. !EMPTY(CSTSNEW)
					MSGALERT(STR0001 + ALLTRIM(FQD->FQD_STATQY) + " - " + ALLTRIM( Posicione("TQY",1,xFilial("TQY")+_cStsNew,"TQY_DESTAT") ) , STR0002)  //"NÃO É POSSÍVEL ALTERAR PARA O STATUS "###"GPO - VALSTST9.PRW"
					LRET := .F.
				ENDIF
			ENDIF
		ENDIF
