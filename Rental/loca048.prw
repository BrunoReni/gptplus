...
ENCHOICEBAR(OTELA,{||(LRET := FSALVAR(OTELA),FSAIR(OTELA,LRET))},{||(FSAIR(OTELA))},,ABNTENCHOI)
...
@ 003 ,008 TO 043,500 OF OPANEL PIXEL
...
ELSE
	//@ 058 ,400 SAY OEMTOANSI("HORÕMETRO ATUAL" ) SIZE 050,8  OF OPANEL3 PIXEL 
	@ 058 ,400 SAY OEMTOANSI(STR0065 ) SIZE 050,8  OF OPANEL3 PIXEL // "CONT.ATUAL"
	@ 055 ,440 MSGET OPOSCONT  VAR _NPOSCONT PICTURE PESQPICT("FPN","FPN_POSCON") VALID IIF(_NPOSCONT>=_NCONANT,(_NVLRHOR:=CALCHOR(),(NVALTOT:= ( NVALSERV+NVALISS + NVALFRE + NVALSEG + _NVLRHOR + NVALACRE + NVALARRE) - NVALDESC)),(MSGALERT("O HORÕMETRO N√O PODE SER MAIOR QUE O HORÕMETRO ANTERIOR!","AtenÁ„o!"),.F.)) SIZE 050,8  OF OPANEL3 PIXEL
ENDIF
...
FOR NPOS:=1 TO LEN(ACOLS)
	IF ! ACOLS[NPOS,LEN(AHEADER)+1]	// N√O EST¡ DELETADO
		...
		FPP->(MSUNLOCK())
		IF LVERZBX //.AND. FP0->( (FP0_TIPOSE=="G" .OR. FP0_TIPOSE=="E") .AND. DTOS(FP0_DATINC)>="20150819")  //HABILITA CONTROLE DE MINUTA removido por Frank
...
cQuery := " SELECT COUNT(FPF_FROTA) AS TOT FROM "+RETSQLNAME("FPF")+" "+ CRLF
cQuery += " WHERE 	FPF_FILIAL = '"+FwxFilial("FPF")+"' AND "+ CRLF
cQuery += " 		FPF_DATA BETWEEN '" + cDtIni + "' AND '" + cDtFim + "' AND "+ CRLF 
cQuery += " 		(	(FPF_HORAI >= '" + cHoraIni + "' AND FPF_HORAI <= '" + cHoraFim + "') OR "+ CRLF 
cQuery += " 			(FPF_HORAF >= '" + cHoraIni + "' AND FPF_HORAF <= '" + cHoraFim + "')) AND "+ CRLF 
cQuery += " 		FPF_FROTA = '" + cBem + "' "
//desconsidera AS origem, se informado
If !Empty(cAS)
	cQuery += " AND FPF_AS <> '" + cAs + "' "
EndIF
cQuery += " AND D_E_L_E_T_ = ' ' "+ CRLF
...
Default cPedido := ""
	If !empty(cPedido)
		...
		cQuery += " WHERE FPN_FILIAL = '"+FwxFilial("FPN")+"' AND "
		cQuery += " FPN_NUMPV = '"+cPedido+"' "
