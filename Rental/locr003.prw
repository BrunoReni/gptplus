 ...
 //	OREPORT:SAY( NLIN, NMINLEFT+100, TRANSFORM(TRBFAT->F2_VALBRUT,"@E 9,999,999,999.99"), OFONT2:OFONT ) COMENTADO POR GUILHERME CORONADO
	...
	WHILE TRBPRD->(!EOF())
		...
		*/
		...
		_cQuery := "SELECT C6_SERIE,C6_PRODUTO,C6_XAS,C6_ITEM,C6_QTDVEN,C6_PRCVEN,C6_VALOR,C6_XPERLOC "
		...
		WHILE !(cAliasSc6)->(EOF())//DENNIS CALABREZ - CHAMADO 30566 08/03/23 - ALTERADO O ALIAS DE SC6 PARA TRBSC6
			IF (cAliasSc6)->C6_SERIE == MV_PAR03
				...
				FPA->(DBSETORDER(3))
				FPA->(DBSEEK(XFILIAL("FPA")+(cAliasSc6)->C6_XAS))
				//DENNIS CALABREZ - CHAMADO 30566 08/03/23 - ALTERADO O ALIAS DE SC6 PARA TRBSC6
				AADD(_APRODUTOS,{(cAliasSc6)->C6_ITEM,SB1->B1_DESC,CVALTOCHAR((cAliasSc6)->C6_QTDVEN),TRANSFORM((cAliasSc6)->C6_PRCVEN,"@E 9,999,999,999.99"),TRANSFORM((cAliasSc6)->C6_VALOR,"@E 9,999,999,999.99"),(cAliasSc6)->C6_XPERLOC    })
			ENDIF
			(cAliasSc6)->(DBSKIP())
		ENDDO 
