...
_CQUERY += "WHERE  "
_CQUERY += "	FP0.FP0_FILIAL = '" + _CFILATU + "' AND "
_CQUERY += "	FP0.FP0_PROJET BETWEEN '" +      MV_PAR01  + "' AND '" +      MV_PAR02  + "' AND "
_CQUERY += "	FP0.FP0_DATINC BETWEEN '" + DTOS(MV_PAR03) + "' AND '" + DTOS(MV_PAR04) + "' AND "
_CQUERY += "	FP0.FP0_STATUS = '2' AND "								// EM APROVAÇÃO
_CQUERY += "	FP0.FP0_TIPOSE = '" + MV_PAR05 + "' AND "				// TIPO DO SERVIÇO
...
CASE (_NOPC == 1 .OR. _NOPC ==2) .AND. MSGYESNO(STR0020 + IIF(_NOPC == 1,STR0021,STR0022) + STR0023 )	//APROVAÇÃO E REPROVAÇÃO
	...
	If ( cUsuBlq == "1" )
		Help(Nil,Nil,alltrim(upper(Procname())),; // "RENTAL: "
			Nil,STR0004+" "+STR0040,1,0,Nil,Nil,Nil,Nil,Nil,; 
			{STR0021})
	Else
		TRB->(DBGOTOP())
		WHILE TRB->(!EOF())
			IF TRB->FP0_OK == _CMARCA
				If FP0->(dbSeek(FwxFilial("FP0")+TRB->FP0_PROJET))
					...
					If ( cLvlApp <= cLvlPrj )
						Help(Nil,Nil,alltrim(Upper(Procname())),; // "RENTAL: "
							Nil,STR0004+" "+STR0041,1,0,Nil,Nil,Nil,Nil,Nil,; 
							{STR0042})					
					Else
						...
						Else //Aprovação por Margem
							...
							Else
								//Mensagem de aviso - não tem alçada de aprovação
								Help(Nil,Nil,alltrim(Upper(Procname())),; // "RENTAL: "
									Nil,STR0004+" "+STR0043,1,0,Nil,Nil,Nil,Nil,Nil,; 
									{STR0021})
							EndIf							
						EndIf
					EndIf
				...
				/*IF _NOPC == 1 .AND. _NLIMITE < _NAPROVA
					...
					Help(Nil,Nil,alltrim(upper(Procname())),; // "RENTAL: "
							Nil,STR0004 + " "+ STR0017 +" "+ ALLTRIM(TRANSFORM(_NLIMITE,"@E 999,999,999.99")) + STR0019 + ALLTRIM(TRB->FP0_PROJET) + " (R$ " + ALLTRIM(TRB->FP0_COL03) + ").",1,0,Nil,Nil,Nil,Nil,Nil,; 
							{""})						 
				ELSE
		...
		If FWIsInCallStack("LOCA013")
			...
			Else
					...
					If nOldDesc <> nNewDesc .or. nNewMargem <> nOldMargem
						If aAprovInfo[nPosTpApr] $ "3" // Ambos
							If nNewDesc > nOldDesc .or. nNewMargem <  nOldMargem
								Help(Nil,Nil, "LOCA057_F1",Nil, STR0059,1,0,Nil,Nil,Nil,Nil,Nil, {STR0060}) //"Usuario do tipo ambos com desconto acima do anterior, ou margem abaixo da anterior."###"Erro 1 - valido apenas para usuario do tipo desconto, ou margem."
				...
				Else
					...
					Help(Nil,Nil, "LOCA057_F6",Nil, STR0067,1,0,Nil,Nil,Nil,Nil,Nil, {STR0073}) //"O status não permite a geração do contrato."###"Altere o status do orçamento pela rotina de manutenção."
					Return .F.
				EndIF
			EndIF
		EndIF
...
//Dados da FPR para aprovação
cQryExe := " SELECT FPR_NIVEL, FPR_TIPAPR, FPR_PRCDES, FPR_PRCMRG, FPR_MSBLQL  "+ CRLF
cQryExe += " FROM " + RETSQLNAME("FPR") + " FPR " + CRLF
cQryExe += " WHERE 	FPR_FILIAL = '" + FwxFilial("FPR") + "' AND " + CRLF
cQryExe += "		D_E_L_E_T_ = '' AND " + CRLF
cQryExe += "		FPR_CODUSR = '" + __cUserID + "' " + CRLF
