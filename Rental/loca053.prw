...
ENDIF
//ifranzoi - 08/07/2021
//06/10/2022 - Jose Eulalio - Retirada do campo customizado RA_DSHEET e RA_TSHEET
...
_CQUERY += " INNER JOIN " + RETSQLNAME("SRJ") + " SRJ ON SRA.RA_FILIAL = '"+FwxFilial("SRA")+"' AND SRJ.RJ_FILIAL = '"+FwxFilial("SRJ")+"' AND SRA.RA_CODFUNC = SRJ.RJ_FUNCAO AND SRJ.D_E_L_E_T_ = '' "
_CQUERY += " LEFT  JOIN " + RETSQLNAME("FPQ") + " FPQ ON FPQ.FPQ_MAT = SRA.RA_MAT  " //ifranzoi - 07/07/2021
_CQUERY += "                                         AND FPQ.FPQ_DATA   = '" + DTOS(DDATABASE) + "' AND FPQ.D_E_L_E_T_ = '' "
_CQUERY += " INNER JOIN " + RETSQLNAME("FQ1") + " FQ1 ON FQ1_CODUSR='"+_CUSER+"' AND FQ1.FQ1_NOMPRO='LOCA053' AND FQ1.FQ1_CC=SRA.RA_CC AND FQ1.D_E_L_E_T_ = ' ' "
...
_CQUERY += "   WHERE (SRA.RA_DEMISSA  = '' OR SRA.RA_DEMISSA BETWEEN  '"+DTOS(_DTATVI)+"' AND '"+DTOS(_DTATVF)+"'  ) "    
...
// 07/11/2022 - Jose Eulalio - SIGALOC94-554 - Lui passou orientação para excluir tal validação
/*
_DFOLMES := GETMV("MV_FOLMES")     
_DFOLMES :=_DFOLMES+"01"x
...
ODLGLO := MSNEWGETDADOS():NEW(NLIN1,NCOL1 ,NLIN2  ,NCOL2  ,NSTYLE,"LOCA05305()"  ,       ,			,       ,       ,    ,"LOCA05304()"  ,         ,.T.   ,OFOLDER:ADIALOGS[NFOLDER],AHEADER,ACOLS)
...
IF _COPC == "2" //.OR. _COPC == "9" 			// STATUS OU VALIDAÇÃO DA LINHA OK
	...
	If IsInCallStack("LINHAOK")
		IF EMPTY(_CSTATUS)
			...
			_LRET := .F.
		ENDIF
		IF EMPTY(_CAS) .AND. _CSTATUS$"OBRA|INTEGR"
			MSGALERT(STR0040) //"Status obra: AS em branco, favor selecione"
			_LRET := .F.
		ENDIF
	EndIf
	/*IF !EMPTY(_CAS) .AND. !_CSTATUS$"OBRA|INTEGR"
...
IF _COPC == "3" .OR. _COPC == "9" 			// AS OU VALIDAÇÃO DA LINHA OK
	...
	CASE _cCampo == "FPQ_AS" .And. IsInCallStack("ValidGet")
		If !(_CSTATUS $ "OBRA|INTEGR") .And. !EMPTY(&(ReadVar()))
...
CASE NOPC == 1	// INTEGRAÇÃO
	...
	*/
	...
	AADD(AROTINA,{STR0048 , ; //"GERA INTEGRAÇÃO"
	"LOCA05310(3, MV_PAR01, MV_PAR02, MV_PAR03, MV_PAR04, 'INTEGR', 'N', MV_PAR05, MV_PAR06, NIL)",0,3} ) 	// GERAR OS LANÇAMENTOS DE INTEGRAÇÃO NO ZLO
...
CASE NOPC == 2	// CÓPIA AS
	...
	*/
	...
	_CQUERY := "SELECT DISTINCT SRA.RA_MAT,    SRA.RA_OKTRANS, SRA.RA_FILIAL,  SRA.RA_NOME , SRA.RA_CODFUNC, "
	_CQUERY += "                SRA.RA_CC     , SRJ.RJ_FUNCAO,  SRJ.RJ_DESC , ZLO.FPQ_STATUS, ZLO.FPQ_VT   , "
	_CQUERY += "                ZLO.FPQ_HORAS, ZLO.FPQ_PROJET, ZLO.FPQ_OBRA ,  ZLO.FPQ_DESC, SRA.RA_DEMISSA, ZLO.FPQ_HRINI, "
	_CQUERY += "                ZLO.FPQ_HRFIN, ZLO.FPQ_HRINI, ZLO.FPQ_HRFIN "
	...
	_CQUERY += "        ((SRA.RA_DEMISSA  BETWEEN  '"+DTOS(_DTATVI)+"' AND '"+DTOS(_DTATVF)+"' ) OR SRA.RA_DEMISSA = '' ) "  // NAO MOSTRA DEMITIDOS 
	_CQUERY := CHANGEQUERY(_CQUERY)
	TCQUERY _CQUERY NEW ALIAS "QRY"
...
CASE NOPC == 3	// COPIA PROJETO
	...
	*/
	...
	_CQUERY += "                ZLO.FPQ_HRINI, ZLO.FPQ_HRFIN, ZLO.FPQ_HRINI, ZLO.FPQ_HRFIN "
	_CQUERY += "FROM   " + RETSQLNAME("SRA") + " SRA INNER JOIN "
	_CQUERY +=             RETSQLNAME("FPQ") + " ZLO ON "
	_CQUERY += "       ZLO.D_E_L_E_T_ = '' AND "
	_CQUERY += "       SRA.RA_MAT = ZLO.FPQ_MAT AND "
...
CASE NOPC == 4	// GERAÇÃO
	...
	*/
	...
	_CQUERY += " 	SRA.RA_FILIAL = '"+FwxFilial("SRA")+"' AND SRJ.RJ_FILIAL = '"+FwxFilial("SRJ")+"' AND " 
	//_CQUERY += " 	SRA.RA_CODFUNC = SRJ.RJ_FUNCAO AND SRJ.D_E_L_E_T_ = '' AND SRA.D_E_L_E_T_ = '' AND SRA.RA_TSHEET = 'S' "
...
ELSEIF NTIPO == 5
	...
	AADD(APARAMBOX,{1,STR0098           , SPACE(05) , "@E 99:99"        , "ATVLDHORA(MV_PAR04)" , ""     , ""  , 30 , .T.}) // "HORA FIM"
	//ifranzoi - 08/07/2021
	//AADD(APARAMBOX,{1,STR0103             , SPACE(06) , "@!"		       , "EXISTCPO('SX5','75' + MV_PAR05).AND. TRIM(MV_PAR05) != '000005'","75"     ,""  ,20, .T.}) // TIPO CARACTERE "STATUS"
	AADD(APARAMBOX,{1,STR0103             , SPACE(06) , "@!"		       , "EXISTCPO('SX5','QV' + MV_PAR05).AND. TRIM(MV_PAR05) != 'INTEGR'","QV"     ,""  ,20, .T.}) // TIPO CARACTERE "STATUS"
	//AADD(APARAMBOX,{1,"Nº AS"              , SPACE(27) , "@!"              , "LOCA05308(MV_PAR06,.F.,TRIM(MV_PAR05)=='000004')" ,"FQ5AS" ,""  ,80, .F.}) // TIPO CARACTERE
	//AADD(APARAMBOX,{1,STR0099              , SPACE(27) , "@!"              , "LOCA05308(MV_PAR06,.T.,If(AllTrim(MV_PAR05 == 'OBRA', .T., .F.))" ,"FQ5AS" ,""  ,80, .F.}) // TIPO CARACTERE "Nº AS"
	AADD(APARAMBOX,{1,STR0099              , SPACE(27) , "@!"              , "LOCA05308(MV_PAR06,.T.,If(AllTrim(MV_PAR05) == 'OBRA', .T., .F.),If(!(AllTrim(MV_PAR05) $ 'OBRA|INTEGR'), .T., .F.))" ,"FQ5AS" ,""  ,80, .F.}) // TIPO CARACTERE "Nº AS"
...
cQryExe += " WHERE FPQ_FILIAL = '" + cFilFpq + "' AND FPQ_FILMAT = '" + cFilMat + "' AND
cQryExe += " FPQ_MAT = '" + cMatFpq + "' AND FPQ_DATA = '"+Dtos(dDataFpq)+"' AND D_E_L_E_T_ = ' ' "
