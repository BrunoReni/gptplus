...
Else
	cQuery += " SELECT R_E_C_N_O_ RECNOAA3 FROM " + RetSqlName("AA3") + CRLF
	cQuery += " WHERE 	AA3_CODPRO = '" + cCodProd + "' AND "  + CRLF
	If !Empty(cCodBem)
		cQuery += " 		AA3_NUMSER = '" + cCodBem + "' AND "  + CRLF
	EndIf
	//cQuery += " 		AA3_EQALOC = '" + cEqAloc + "' AND "  + CRLF
	cQuery += " 		AA3_FILIAL = '" + xFilial("AA3") + "' AND "  + CRLF
	cQuery += " 		D_E_L_E_T_ =  ' ' " 
...
cQuery += " SELECT MAX(AA3_NUMSER) NUMSER FROM " + RetSqlName("AA3") + CRLF
cQuery += " WHERE "  + CRLF
cQuery += " 		AA3_NUMSER LIKE '" + cNewProd + cCliCod + cCliLoja + "%'  "  + CRLF
...
While lBusca
	...
	//Query não considera nem filial, nem delete, pois o número deve ser único
	cQuery := " SELECT AA4_NSERAC NSERAC FROM " + RetSqlName("AA4") + CRLF
	cQuery += " WHERE "  + CRLF
	cQuery += " 		AA4_NSERAC = '" + cIdAa3 + "'  "  + CRLF
...
//Query não considera nem filial, nem delete, pois o número deve ser único
cQuery += " SELECT MAX(AA4_NSERAC) NSERAC FROM " + RetSqlName("AA4") + CRLF
cQuery += " WHERE "  + CRLF
cQuery += " 		AA4_NSERAC LIKE '" + cNewCod + "%'  "  + CRLF
...
While lBusca
	...
	//Query não considera nem filial, nem delete, pois o número deve ser único
	cQuery := " SELECT AA3_NUMSER NSERAC FROM " + RetSqlName("AA3") + CRLF
	cQuery += " WHERE "  + CRLF
	cQuery += " 		AA3_NUMSER = '" + cIdAA4 + "'  "  + CRLF
...
cQuery := " SELECT DISTINCT FP0_PROJET, FP0_NOMECO, FP0_CLI, FP0_LOJA, FP1_CLIORI, FP1_LOJORI,FQ7_LCCDES, FQ7_LCLDES, A1_NOME " + CRLF
cQuery += " FROM " + RetSqlName("FP0") + " FP0 " + CRLF
cQuery += " INNER JOIN " + RetSqlName("SA1") + " SA1 " + CRLF
cQuery += " 	ON A1_FILIAL = '" + xFilial("SA1") + "' " + CRLF
cQuery += " 	AND A1_COD = '" + cCodCli + "' " + CRLF
cQuery += " 	AND A1_LOJA = '" + cLojCli + "' " + CRLF
cQuery += " 	AND SA1.D_E_L_E_T_ = ' '  " + CRLF
cQuery += " INNER JOIN " + RetSqlName("FP1") + " FP1 " + CRLF
cQuery += " 	ON FP0_FILIAL = FP1_FILIAL " + CRLF
cQuery += " 	AND FP0_PROJET = FP1_PROJET " + CRLF
cQuery += " 	AND A1_COD = FP1_CLIORI " + CRLF
cQuery += " 	AND A1_LOJA = FP1_LOJORI " + CRLF
cQuery += " 	AND FP1.D_E_L_E_T_ = ' '  " + CRLF
cQuery += " LEFT JOIN " + RetSqlName("FQ7") + " FQ7 " + CRLF
cQuery += " 	ON FP0_FILIAL = FQ7_FILIAL " + CRLF
cQuery += " 	AND FP0_PROJET = FQ7_PROJET " + CRLF
cQuery += " 	AND FP1_OBRA = FQ7_OBRA " + CRLF
cQuery += " 	AND A1_COD = FQ7_LCCDES " + CRLF
cQuery += " 	AND A1_LOJA = FQ7_LCLDES " + CRLF
cQuery += " 	AND FQ7.D_E_L_E_T_ = ' '  " + CRLF
