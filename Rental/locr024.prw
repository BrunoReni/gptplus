...
If SA1->(dbSeek(xFilial("SA1")+_PCOD+_PLOJ))
	...
	_pLIN++
	@ _pLIN,014 PSay STR0010 + Transform(SA1->A1_CGC,"@R 99.999.999/9999-99") + STR0011 + AllTrim(SA1->A1_INSCR) 	// "CNPJ "###" IE "
Else
...
_cQUERY := "SELECT FPQ.FPQ_FILIAL , FPQ.FPQ_MAT , FPQ.FPQ_DATA , FPQ.FPQ_STATUS , FPQ.FPQ_AS , FPQ.FPQ_PROJET , FPQ.FPQ_OBRA , FPQ.FPQ_DESC , FPQ.FPQ_VT , FPQ.FPQ_HORAS , " + CRLF 
_cQUERY += "       SRA.RA_FILIAL  , SRA.RA_MAT  , SRA.RA_NOME  , SRA.RA_CODFUNC , DTQ.FQ5_AS , DTQ.FQ5_CONTRA , " + CRLF 
_cQUERY += "       DTQ.FQ5_CODCLI , DTQ.FQ5_LOJA , SA1.A1_COD , SA1.A1_LOJA, DTQ.FQ5_OBRA     "                   + CRLF 
_cQUERY += "FROM   " + RetSQLName("FPQ") + " FPQ "                                                                + CRLF 
_cQUERY += "       INNER JOIN " + RetSQLName("SRA") + " SRA ON SRA.D_E_L_E_T_ = '' AND SRA.RA_FILIAL  = '"+xFilial("SRA")+"' AND FPQ.FPQ_MAT    = SRA.RA_MAT "     + CRLF 
_cQUERY += "       LEFT  JOIN " + RetSQLName("FQ5") + " DTQ ON DTQ.D_E_L_E_T_ = '' AND FPQ.FPQ_AS     = DTQ.FQ5_AS     AND FPQ.FPQ_PROJET = DTQ.FQ5_CONTRA " + CRLF 
//_cQUERY += "       LEFT  JOIN " + RetSQLName("AAM") + " AAM ON AAM.D_E_L_E_T_ = '' AND DTQ.FQ5_CONTRA = AAM.AAM_CONTRT "                                     + CRLF 
_cQUERY += "       LEFT  JOIN " + RetSQLName("SA1") + " SA1 ON SA1.D_E_L_E_T_ = '' AND DTQ.FQ5_CODCLI = SA1.A1_COD     AND DTQ.FQ5_LOJA   = SA1.A1_LOJA "    + CRLF 
_cQUERY += "WHERE  FPQ.D_E_L_E_T_ = ''                             AND "                                          + CRLF 
_cQUERY += "       FPQ.FPQ_MAT    BETWEEN '" +      MV_PAR03  + "' AND '" +      MV_PAR04  + "' AND "             + CRLF 
_cQUERY += "       FPQ.FPQ_PROJET BETWEEN '" +      MV_PAR05  + "' AND '" +      MV_PAR06  + "' AND "             + CRLF 
_cQUERY += "       FPQ.FPQ_DATA   BETWEEN '" + DtoS(MV_PAR01) + "' AND '" + DtoS(MV_PAR02) + "' AND "             + CRLF 
_cQUERY += "       FPQ.FPQ_STATUS >= 'OBRA  ' AND FPQ.FPQ_STATUS  <= 'OBRASL' AND "                               + CRLF 
_cQUERY += "       FPQ.FPQ_PROJET <> '                      ' "                                                   + CRLF 
