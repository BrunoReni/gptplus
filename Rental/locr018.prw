	...
	If !empty(TRB->FPA_GRUA)
		...
		If dbSeek(xFilial("ST9")+TRB->FPA_GRUA) 								// --> Campo: "EQUIPAMENTO"
			If !Empty(ST9->T9_CODFA) 
				@ nLin,91 PSay ST9->T9_CODFA 									// NOVA IMPRESSAO DO EQUIPAMENTO POR COD FANTASIA
			Else 
...
cQuery     += "        FPA_ULTFAT As FPA_ULTFAT , FPA_DTPRRT As FPA_DTPRRT , FPA_AS    As NUM_AS, FPA_DNFRET As FPA_DNFRET, FPA_QUANT "                                                + CRLF
