...
If Len( aGrau ) > 0
	For nX := 1 To Len( aGrau )
		If  Empty(cFiltro) .Or. ;
			...
			oGrau["name"] := AllTrim( EncodeUTF8(aGrau[nX,4]) )
...
If nLenCNH > 0
	For nX := 1 To nLenCNH
		...
		oCNH["name"] := AllTrim( EncodeUTF8( aBoxCNH[nX,3] ) )
...
Local aArea       := GetArea()
Local aEstCiv     := FWGetSX5("33")
Local aListEstCiv := {}
Local nLen        := Len( aEstCiv )
Local nX          := 1
...
If nLen > 0
	For nX := 1 To nLen
		If Empty( cName ) .Or. ;
		( ( cName $ AllTrim( aLogr[nX,5] ) .Or. ( cName $ AllTrim( aLogr[nX,6] ) ) ) )
			nCount++
			If ( nCount >= nRegIni .And. nCount <= nRegFim )
