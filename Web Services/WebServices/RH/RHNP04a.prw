...
@return:	Dados dos funcionários no formato - "employeesRequisitionsResponse"
/*/
Function fDetailReq(cBranchVld, cMatSRA, aIdReq)
...
If lChangeEmp := !(cEmpAnt == cEmpReq)
	fNewEmpFiles(cEmpReq, cFilReq, {"SRA","SRJ","SQB","CTT"})
EndIf

If SRA->(dbSeek(cFilReq+cMatReq))
	//Dados do funcionário
	oItens["role"] 				:= Alltrim( EncodeUTF8( FDesc("SRJ", SRA->RA_CODFUNC, "RJ_DESC",,cFilReq ) ) )
	oItens["departament"] 		:= Iif( !Empty(SRA->RA_DEPTO), ;
										Alltrim( EncodeUTF8( FDesc("SQB", SRA->RA_DEPTO, "QB_DESCRIC",,cFilReq ) ) ), ;
										EncodeUTF8( STR0028 ) )
	oItens["costCenter"] 		:= Alltrim( EncodeUTF8( FDesc("CTT", SRA->RA_CC, "CTT_DESC01",,cFilReq ) ) )
	...
	oItens["newRole"] 				:= IIf( !Empty(cNewCargo), ;
											Alltrim( EncodeUTF8( FDesc("SQ3", cNewCargo, "Q3_DESCSUM",,cFilReq ) ) ), ;
											EncodeUTF8( STR0028 )) // Não informado.
	oItens["newFunction"] 			:= IIf( !Empty(cNewFunc), ;
											Alltrim( EncodeUTF8( FDesc("SRJ", cNewFunc, "RJ_DESC",,cFilReq ) ) ), ;
											EncodeUTF8( STR0028 ) ) // Não informado.
	oItens["proposedSalary"]		:= nNewSal


	dDtSolic 						:= PosAlias( "RH3", cCodReq, cFilReq, "RH3_DTSOLI", 1 )
...
If lChangeEmp
	fNewEmpFiles(cSavEmp, cBranchVld, {"SRA","SRJ","SQB","CTT"})
EndIf
...
/*/
Function fDetailTrnsf(cBranchVld, cMatSRA, aIdReq)
	...
	If SRA->(dbSeek(cFilReq+cMatReq))
		//Dados do funcionário
		oItem["role"] 				:= Alltrim( EncodeUTF8( FDesc("SRJ", SRA->RA_CODFUNC, "RJ_DESC", ,cFilReq ) ) )
		...
		aSM0Data := FWSM0Util():GetSM0Data( ;
						IIf( !Empty(cEmpPara), cEmpPara, cEmpReq ), ;
						Iif( !Empty(cFilPara), cFilPara, cFilReq ), ;
...
@return:	Lista com os cargos.
/*/
Function fSalaryTypes(aQryParam)
...
lChangeEmp := !( cCodEmp == cEmpAnt )

If lChangeEmp
	fNewEmpFiles(cCodEmp, cCodFil, {"SX5"})
EndIf

aGetTipos := FWGetSX5( cTabela )

If lChangeEmp
	fNewEmpFiles(cSavEmp, cSavFil, {"SX5"})
EndIf
...
/*/
Function fCategoryTypes(aQryParam)
...
If Len(aId) >= 3 .And. !(aId[1] == cEmpAnt)
	fNewEmpFiles(aId[1], aId[2], {"SX5"})
	lChangeEmp := .T.
EndIf
