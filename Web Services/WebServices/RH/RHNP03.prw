...
Function GetAnnualRec( nTipo, cMatSRA, cBranchVld, aEvents, cAnoBas, cFile, cQryParam, aTransf, aIdFunc, aMultV, lMultV )
	...
	Local nTamTransf := 0
	Local nPosDel	 := 0
	Local lAtual	 := .T. //Variável de controle para saber se o cliente possui os fontes do server atualizado, mas o client está desatualizado
	Local cRelRpo	 := GetRpoRelease()
	Local lRelease	 := ( cRelRpo == "12.1.023" .Or. cRelRpo == "12.1.017" ) // Tratativa para releases 23 ou anterior, pois nesse caso o frontend é diferente e não tem o qryparam ID
	...
	If !lAtual .And. !lRelease
		aAdd( aLog, STR0018 ) //"Durante o processamento ocorreram erros que impediram a gravação dos dados. Contate o administrador do sistema."
		aAdd( aLog, "" )
		aAdd( aLog, STR0020 ) //"Possíveis causas do problema:"
		aAdd( aLog, "- " + STR0021 ) //"Verifique se os seus arquivos do Portal MeuRH estão atualizados."
...
If len(aPaymentId) < 11 .or. empty(aPaymentId[11])
   cCompany := cEmpAnt
   PaymentReceipt:Branch := cFilFun 	// Filial do Funcionário
Else   
...
Static Function GetPaymRec(aPaymentId, cFile, aMsgDefault)
    ...
    For nX := 1 To 2
	    ...
	    Else
			...
			If nX == 2
				aAdd( aMsgDefault, "" )
				aAdd( aMsgDefault, STR0029 ) //"Durante a execução do download verifique se o arquivo PDF está sendo gerado no local indicado"

				If lRecibo .And. lRdMake
					aAdd( aMsgDefault, "" )
					aAdd( aMsgDefault, STR0030 ) //"Caso o recibo seja personalizado, atualize o sistema com uma versão mais recente do arquivo RECIBO.PRX disponível na Central de Downloads da Totvs"
				EndIf
			EndIf
	    EndIf
