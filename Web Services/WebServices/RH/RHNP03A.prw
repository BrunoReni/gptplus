...
/*/
Function fGetFileAnnualRec(xCodFil, cCodMat, cAnoBas, cArqLocal, cFileName, cExtFile, xCodEmp, lJob, cUID)
	...
	For nX := 1 To 3
		...
		If !File( cArqLocal + cFileName + cExtFile + '*' )
			GPEM580(.T., xCodFil, cCodMat, cAnoBas, .F., .T., cFileName + cExtFile + cValToChar(nX) )
		EndIf
