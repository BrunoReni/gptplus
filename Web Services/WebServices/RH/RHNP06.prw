...
WSMETHOD GET GetTotSummPeriod PATHPARAM employeeId WSREST Timesheet
	...
	oItem["initPeriod"] 				:= StoD( Format8601(.T.,Self:initPeriod,,.T.) )
	oItem["endPeriod"]  				:= StoD( Format8601(.T.,Self:endPeriod,,.T.) )
...
WSMETHOD GET clockings PATHPARAM employeeId WSREST Timesheet
	...
	If lAuth
		...
		For nX := 1 To Len(aProcFun)
			If !(cEmpAnt == cEmpAtu)
				aDataAux := GetDataForJob( "3", {aProcFun[nX,1], aProcFun[nX,2], initPeriod, endPeriod, lDivergent}, cEmpAtu )
			Else
				aDataAux := getClockings( aProcFun[nX,1], aProcFun[nX,2], initPeriod, endPeriod, lDivergent)
			EndIf
		...
		If RH3->( dbSeek( cFilRH3 + cCodRH3 )) //Filial+Cod RH3
			...
			Else
				...
				If lRet
					...
					DbSelectArea("RGK")
					DBSetOrder(1)
					If RGK->( dbSeek( cFilRH3 + RH3->RH3_MAT + cCodRH3 ) )
						RecLock( "RGK", .F. )
						RGK->( dbDelete() )
						RGK->( MsUnlock() )
					EndIf
