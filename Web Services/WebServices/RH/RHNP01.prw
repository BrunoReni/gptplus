...
WSMETHOD GET getTeam WSRECEIVE coordinatorId WSSERVICE Team
	...
	Local oItem      := JsonObject():New()
	Local oPhone     := JsonObject():New()
	Local oEmail     := JsonObject():New()
	Local oContacts  := JsonObject():New()
	...
	If Len(Self:aUrlParms) > 1 .And. !Empty(Self:aUrlParms[2]) .And. !("current" $ Self:aUrlParms[2])
		If Len( aIdFunc := STRTOKARR( ::aUrlParms[2], "|" ) ) > 1
			If ( cBranchVld	== aIdFunc[1] .And. cMatSRA == aIdFunc[2] )
				...
				fPermission(cBranchVld, cLogin, cRD0Cod, "teamManagement", @lHabil)
				If !lHabil
	...
	If lChangeEmp
		aCoordTeam := fTeamEmp( cEmpFunc, cMatSRA, {"", cOrgCFG, cVision, cBranchVld, cMatSRA, , , , , cBranchVld, cMatSRA, , , , , .T., aEmpresas, aQryParam, @lHasNext} )
	Else
		aCoordTeam := APIGetStructure("", cOrgCFG, cVision, cBranchVld, cMatSRA, , , , , cBranchVld, cMatSRA, , , , , .T., aEmpresas, aQryParam, @lHasNext)
	EndIf
		...
		For nX := 1 To Len(aCoordTeam)
			...
			For nY := 1 To Len(aCoordTeam[nX]:ListOfEmployee)
				...
				If lContinua .and. !(cEmpStruct+cFilStruct+cMatStruct == cEmpFunc+cBranchVld+cMatSRA) //(Nao considera a propria matricula)
					...
					DbSelectArea("SR3")
					SR3->( dbSetOrder(1) )
					SR3->( dbSeek( oEmployee:EmployeeFilial + oEmployee:Registration ) )
					While SR3->(!Eof())  .And.  SR3->(R3_FILIAL+R3_MAT) == oEmployee:EmployeeFilial + oEmployee:Registration
						If SR3->R3_DATA > dDtUltSal
							dDtUltSal := SR3->R3_DATA
						EndIf
...
WSMETHOD GET delaysAbsences WSREST Team
	...
	cMRrhKeyTree	:= fMHRKeyTree(cBranchVld, cMatSRA)
	aCoordTeam		:= APIGetStructure(cCodRD0, cOrgCFG, cVision, cBranchVld, cMatSRA, , , , , cBranchVld, cMatSRA, , , , , .T., aEmpresas)
	lContinua 		:= Len(aCoordTeam) > 0 .And. !ValType( aCoordTeam[1] ) == "L" //Verifica se carregou dados da hierarquia.
...
Function fGetSubsEligible( cBranchVld, cMatSRA, aGetStruct, oItem, lRet, aQryParam, lMorePages, cOrgCFG )
	...
	If cOrgCFG == "0"
		...
		If !Empty(cFilGest) .And. !Empty(cMatGest)
			...
			For nX := 1 To Len( aGetGestor[1]:ListOfEmployee )
				...
				cNameFun 	:= UPPER( AllTrim( EncodeUTF8(aGetGestor[1]:ListOfEmployee[nX]:Name)) )
		...
		If RH3->( dbSeek(xFilial("RH3", cBranch) + cRH3Cod ) )
			...
			If lContinua
				...
				If oItemDetail:hasProperty("approved")
					...
					lisApprove := oItemDetail["approved"]
...
If oStVacWkfl == Nil .Or. cGetVacWKF <> cEmpTeam
	...
	cQryObj += " RH3.RH3_FILIAL, RH3.RH3_CODIGO, RH3.RH3_MAT, RH3.RH3_DTSOLI, RH3.RH3_VISAO, RH3.RH3_FILINI, RH3.RH3_MATINI, RH3.RH3_EMP,
	cQryObj += " RH3.RH3_FILAPR, RH3.RH3_MATAPR, RH3.RH3_STATUS, RH3.RH3_TIPO, RH3.RH3_NVLINI, RH3.RH3_NVLAPR, RH3.R_E_C_N_O_"
	cQryObj += " FROM " + RetFullName('SRA', cEmpTeam) + " SRA "
...
While (cQuery)->(!Eof())
	...
	If Len(aPerFerias) > 0 
		...
		cCondition	:= GetCondition(dDouble, dRiskDouble, aPerFerias[1,3], aPerFerias[1,4])
...
Function SetJson(oAbsences,aAbsences,aOcurances,nX,aConflict)
	...
	If !Empty(aOcurances)
		...
		If !lConcessive
			aDateGMT := LocalToUTC( DTOS((aOcurances[nX][28][2]) - aOcurances[nX][29] + 1), "12:00:00" )
...
Function fGetdelaysAbsences( cBranchVld, cMatSRA, lTotalize, aCoordTeam, aQryParam, lNextPage )
	...
	Local cSPHWhere		:= ""		
	Local cIdAll		:= ""
	Local cIdDelay		:= "'011N','012A','013N','014A','019N','020A'" 	//Identificadores de atrasos
	Local cIdAbsence	:= "'007N','008A','009N','010A'"				//Identificadores de faltas
	...
	//Posicoes usadas no array da estrutura do time para query
	Local nPosEmp 		:= 1
	Local nPosFilQry	:= 4
	Local nPosMatQry	:= 5
	Local nPosDtIni 	:= 6
	...
	For nX := 1 To Len( aQryParam )
		Do Case
			...
			Case UPPER(aQryParam[nX,1]) $ "NAME||SEARCH" 
				cName		:= UPPER( cValToChar( AllTrim(aQryParam[nX,2]) ) )
	...
	If !Empty(cOcurrency)
		cIdAll := If( UPPER(cOcurrency) == "DELAY", cIdDelay, cIdAbsence )
	Else
		cIdAll := cIdDelay +","+ cIdAbsence
	EndIf	

	//Filtra por nome
	If !Empty(cName)
		cFilter += " AND RA_NOME LIKE '%" + cName + "%' "
	EndIf
	...
	//Formata os dados da estrutura hierarquica para uso na query
	aDataQry := fQueryStructure(aCoordTeam, cEmpAnt, cBranchVld, cMatSRA)

	For nX := 1 To Len(aDataQry)
		If Empty(cDtIni) .And. Empty(cDtFim)
			If aDataQry[nX, 1] == cEmpAnt
				GetPonMesDat( @dIniPonMes, @dFimPonMes, aDataQry[nX, 2] )
				aDataQry[nX, 6] := dToS(dIniPonMes)
				aDataQry[nX, 7] := dToS(dFimPonMes)
			Else
				aPeriods := GetDataForJob( "1", {aDataQry[nX, 2], aDataQry[nX, 3], Nil}, aDataQry[nX, 1] )
				If Len(aPeriods) > 0
					aDataQry[nX, 6] := dToS(aPeriods[1,1])
					aDataQry[nX, 7] := dToS(aPeriods[1,2])
				EndIf
			EndIf
		Else
			aDataQry[nX, 6] := cDtIni
			aDataQry[nX, 7] := cDtFim
		EndIf
	Next nX
	...
	cJoinSPC := "%" + FWJoinFilial("SPC", "SP9") + "%"
	cJoinSPH := "%" + FWJoinFilial("SPH", "SP9") + "%"

	For nX := 1 To Len(aDataQry)

		cSPCWhere := "%"
		cSPCWhere += "PC_FILIAL IN (" + aDataQry[nX, nPosFilQry] + ") AND "
		cSPCWhere += "PC_MAT IN (" + aDataQry[nX, nPosMatQry] + ") AND "
		cSPCWhere += "PC_DATA >= '" + aDataQry[nX, nPosDtIni] + "' AND "
		cSPCWhere += "PC_DATA <= '" + aDataQry[nX, nPosDtFim] + "' AND "
		cSPCWhere += "P9_IDPON IN (" + cIdAll + ") AND "
		cSPCWhere += " SRA.D_E_L_E_T_ = ' ' AND SPC.D_E_L_E_T_ = ' ' AND SP9.D_E_L_E_T_ = ' ' "
		cSPCWhere += cFilter
		cSPCWhere += "%"

		cSPHWhere := "%"
		cSPHWhere += "PH_FILIAL IN (" + aDataQry[nX, nPosFilQry] + ") AND "
		cSPHWhere += "PH_MAT IN (" + aDataQry[nX, nPosMatQry] + ") AND "
		cSPHWhere += "PH_DATA >= '" + aDataQry[nX, nPosDtIni] + "' AND "
		cSPHWhere += "PH_DATA <= '" + aDataQry[nX, nPosDtFim] + "' AND "
		cSPHWhere += "P9_IDPON IN (" + cIdAll + ") AND "	
		cSPHWhere += " SRA.D_E_L_E_T_ = ' ' AND SPH.D_E_L_E_T_ = ' ' AND SP9.D_E_L_E_T_ = ' ' "
		cSPHWhere += cFilter
		cSPHWhere += "%"		

		__cSRAtab := "%" + RetFullName("SRA", aDataQry[nX, nPosEmp]) + "%"
		__cSPCtab := "%" + RetFullName("SPC", aDataQry[nX, nPosEmp]) + "%"
		__cSPHtab := "%" + RetFullName("SPH", aDataQry[nX, nPosEmp]) + "%"
		__cSP9tab := "%" + RetFullName("SP9", aDataQry[nX, nPosEmp]) + "%"
		...
		BeginSql Alias cAliasQry
			...
			SELECT             
				SPC.PC_FILIAL FILIAL, 
				SPC.PC_MAT MAT, 
				SPC.PC_DATA DATAMOV, 
				SPC.PC_PD PD, 
				SPC.PC_PDI PDI,
				SPC.PC_QUANTC QUANTC, 
				SPC.PC_QUANTI QUANTI,
				SPC.PC_ABONO ABONO, 
				SPC.PC_QTABONO QTABONO, 
				SP9.P9_CODIGO,
				SP9.P9_IDPON, 
				SP9.P9_DESC,
				SRA.RA_NOME
			FROM 
				%exp:__cSPCtab% SPC
			INNER JOIN %exp:__cSP9tab% SP9
				ON %exp:cJoinSPC% AND SP9.%NotDel% AND SPC.PC_PD = SP9.P9_CODIGO
			...
			SELECT             
				SPH.PH_FILIAL FILIAL,
				SPH.PH_MAT MAT,
				SPH.PH_DATA DATAMOV, 
				SPH.PH_PD PD, 
				SPH.PH_PDI PDI, 
				SPH.PH_QUANTC QUANTC, 
				SPH.PH_QUANTI QUANTI,
				SPH.PH_ABONO ABONO, 
				SPH.PH_QTABONO QTABONO,
				SP9.P9_CODIGO,
				SP9.P9_IDPON, 
				SP9.P9_DESC,
				SRA.RA_NOME
			FROM 
				%exp:__cSPHtab% SPH
			INNER JOIN %exp:__cSP9tab% SP9
				ON %exp:cJoinSPH% AND SPH.PH_PD = SP9.P9_CODIGO
			INNER JOIN %exp:__cSRAtab% SRA
				ON SPH.PH_FILIAL = SRA.RA_FILIAL AND SPH.PH_MAT = SRA.RA_MAT
			WHERE
				%Exp:cSPHWhere%
		...
		While !(cAliasQry)->(Eof())
			...
			//Atribui o valor na posicao correta conforme o tipo (Falta ou atraso)
			nPosVal := If( (cAliasQry)->P9_IDPON $ cIdDelay, nPosDelay, nPosAbsence )
			...
			If lTotalize
				...
				Else
					...
					Else
						aEventos[nReg, nPosVal] := aEventos[nReg, nPosVal] + fConvhR(nSaldo,"D",,5)
					EndIf
				EndIf
			Else
				If !cLastMat == (cAliasQry)->FILIAL + (cAliasQry)->MAT + (cAliasQry)->DATAMOV
					nReg ++
					aAdd( aEventos,	{ aDataQry[nX, nPosEmp], (cAliasQry)->FILIAL, (cAliasQry)->MAT, AllTrim((cAliasQry)->RA_NOME), (cAliasQry)->DATAMOV, 0, 0 } ) 
				EndIf
				...
				Else
					aEventos[nReg, nPosVal] := aEventos[nReg, nPosVal] + fConvhR(nSaldo,"D",,5)
				EndIf
				cLastMat := (cAliasQry)->FILIAL + (cAliasQry)->MAT + (cAliasQry)->DATAMOV
			EndIf
	...
	Else
		...
		ASort( aEventos,,, { |x, y| x[5]+x[5] < y[5]+x[4] })
...
/*/{Protheus.doc} fQueryStructure
	Gera um array com dados das filiais e matriculas a partir da estrutura do time, para uso em query
	@author	Marcelo Silveira
	@since	29/10/2020
	@param	aCoordTeam = Array com a estrutura da equipe do gestor
	@return aDataQuery = Array com os dados de filial e matricula formatados para uso em query
/*/
Function fQueryStructure( aCoordTeam, cEmpReq, cFilReq, cMatReq )
...
DEFAULT cMatReq 	:= ""
	...
	For nX := 1 To Len( aCoordTeam[1]:ListOfEmployee )
		...
		//Não considera a matricula que esta fazendo a requisicao
		If cEmpAtu+cFilAtu+cMatAtu == cEmpReq+cFilReq+cMatReq
			Loop
		EndIf
		
		If (nPos := aScan( aDataQuery, {|x| x[1]+x[2] == cEmpAtu + cFilAtu} ) ) == 0
			nReg ++
			aAdd( aDataQuery, { ;
				aCoordTeam[1]:ListOfEmployee[nX]:EmployeeEmp, ;
				aCoordTeam[1]:ListOfEmployee[nX]:EmployeeFilial, ;
				aCoordTeam[1]:ListOfEmployee[nX]:Registration, ;
				"'" + aCoordTeam[1]:ListOfEmployee[nX]:EmployeeFilial + "'", ;
				"'" + aCoordTeam[1]:ListOfEmployee[nX]:Registration + "'", ;
				"", ; //Vazio para guardar a data inicio do periodo de apontamento
				"" ; //Vazio para guardar a data final do periodo de apontamento
		 	} )
		Else
			aDataQuery[nReg,5] += ",'" + aCoordTeam[1]:ListOfEmployee[nX]:Registration + "'"
		EndIf
...
/*/
Function fTeamEmp(cNewEmp, cNewFil, aParams)

	Local aTeam			:= {}
	Local cSavFil       := cFilAnt
	Local cSavEmp       := cEmpAnt

	fNewEmpFiles(cNewEmp, cNewFil,,.T.)

	aTeam := APIGetStructure( aParams[1], aParams[2], aParams[3], aParams[4], aParams[5], aParams[6], aParams[7], aParams[8], aParams[9], aParams[10], aParams[11], aParams[12], aParams[13], aParams[14], aParams[15], aParams[16], aParams[17] )
...
*/
Local lProjSRF		:= SuperGetMv("MV_PROJSRF",,.F.,cFilTeam)
Local cDia2501		:= If( lProjSRF, SuperGetMv("MV_DIA2501",,"N",cFilTeam), "N")
...
If !Empty(cMatTeam)
	...
	While (cQuery)->(!Eof())
		...
		If !empty((cQuery)->RF_DATAINI)
			If lProjSRF
				fChkLicRem( StoD( (cQuery)->RF_DATAINI ), StoD( (cQuery)->RF_DATAINI ) + (cQuery)->RF_DFEPRO1 - 1 , @nDMes12, @nDMes01, cDia2501)
			EndIf
			dIniVacDate := cToD( Substr((cQuery)->RF_DATAINI,7,2) + "/" + Substr((cQuery)->RF_DATAINI,5,2) + "/" + Substr((cQuery)->RF_DATAINI,1,4) )
			dEndVacDate := (dIniVacDate + (cQuery)->RF_DFEPRO1) - 1 + nDMes12 + nDMes01
			If !Empty(dDtIni) .And. !Empty(dDtFim)
		...
		If !empty((cQuery)->RF_DATINI2)
			If lProjSRF
				fChkLicRem( StoD( (cQuery)->RF_DATINI2 ), SToD( (cQuery)->RF_DATINI2 ) + (cQuery)->RF_DFEPRO2 - 1 , @nDMes12, @nDMes01, cDia2501)
			EndIf
			dIniVacDate := cToD( Substr((cQuery)->RF_DATINI2,7,2) + "/" + Substr((cQuery)->RF_DATINI2,5,2) + "/" + Substr((cQuery)->RF_DATINI2,1,4) )
			dEndVacDate := (dIniVacDate + (cQuery)->RF_DFEPRO2) - 1 + nDMes12 + nDMes01
			If !Empty(dDtIni) .And. !Empty(dDtFim)
				lAchou := ( dIniVacDate >= dDtIni .And. dEndVacDate <= dDtFim  )
			EndIf
			If lAchou .And. dIniVacDate >= dDate
		...
		If !empty((cQuery)->RF_DATINI3)
			If lProjSRF
				fChkLicRem( StoD( (cQuery)->RF_DATINI3 ), StoD( (cQuery)->RF_DATINI3 ) + (cQuery)->RF_DFEPRO3 - 1 , @nDMes12, @nDMes01, cDia2501)
			EndIf
			dIniVacDate := cToD( Substr((cQuery)->RF_DATINI3,7,2) + "/" + Substr((cQuery)->RF_DATINI3,5,2) + "/" + Substr((cQuery)->RF_DATINI3,1,4) )
			dEndVacDate := (dIniVacDate + (cQuery)->RF_DFEPRO3) - 1 + nDMes12 + nDMes01
			If !Empty(dDtIni) .And. !Empty(dDtFim)
				lAchou := ( dIniVacDate >= dDtIni .And. dEndVacDate <= dDtFim  )
			EndIf
			If lAchou .And. dIniVacDate >= dDate
