...
Method BuscaDados(oFiltros, cTipo, cSubTipo) Class ProducaoCapacidadeRecurso
    ...
    oJsonRet["gauge"]["label"]      := "% Utilização"
    ...
    oJsonRet["tags"][nPosTag]["texto"]      := cCodRecur
    oJsonRet["tags"][nPosTag]["colorTexto"] := ""
    aAdd(oJsonRet["tags"], JsonObject():New())
    nPosTag++
    oJsonRet["tags"][nPosTag]["icone"]      := "po-icon-star-half"
    oJsonRet["tags"][nPosTag]["texto"]      := "Horas Produtivas: "+cValToChar(nHrProd)+" horas"
    oJsonRet["tags"][nPosTag]["colorTexto"] := ""
    aAdd(oJsonRet["tags"], JsonObject():New())
    nPosTag++
    oJsonRet["tags"][nPosTag]["icone"]      := "po-icon-star-filled"
    oJsonRet["tags"][nPosTag]["texto"]      := "Capacidade: "+cValToChar(nHrDispRec)+" horas"
    oJsonRet["tags"][nPosTag]["colorTexto"] := ""
    FwFreeArray(aNiveis)
...
Method BuscaDetalhes(oFiltros, nPagina) Class ProducaoCapacidadeRecurso
    ...
    oDados["tags"][nPosTag]["texto"]        := cCodRecur
    oDados["tags"][nPosTag]["colorTexto"]   := ""
...
Method CargaMonitor() Class ProducaoCapacidadeRecurso
    ...
    If !PCPMonitorCarga():monitorAtualizado("ProducaoCapacidadeRecurso")
        ...
        aAdd(aTags, JsonObject():New())
        aTags[2]["texto"]      := "GUIL01"
        aTags[2]["colorTexto"] := ""
        aTags[2]["icone"]      := "po-icon-manufacture"
        aAdd(aTags, JsonObject():New())
        aTags[3]["texto"]      := "Horas Produtivas: 110 horas"
        aTags[3]["colorTexto"] := ""
        aTags[3]["icone"]      := "po-icon-star-half"
        aAdd(aTags, JsonObject():New())
        aTags[4]["texto"]      := "Capacidade: 200 horas"
        aTags[4]["colorTexto"] := ""
        ...
        oProdGauge["thresholds"]["0"]["color"]      := COR_VERMELHO_FORTE
        oProdGauge["thresholds"]["0"]["bgOpacity"]  := 0.2
        oProdGauge["thresholds"]["40"]              := JsonObject():New()
        oProdGauge["thresholds"]["40"]["color"]     := COR_AMARELO_QUEIMADO
        oProdGauge["thresholds"]["40"]["bgOpacity"] := 0.2
        oProdGauge["thresholds"]["75"]              := JsonObject():New()
        oProdGauge["thresholds"]["75"]["color"]     := COR_VERDE_FORTE
        oProdGauge["thresholds"]["75"]["bgOpacity"] := 0.2
        ...
        oProdGauge["markers"]["40"] :=  JsonObject():New()
        oProdGauge["markers"]["40"]["color"] := COR_PRETO
        oProdGauge["markers"]["40"]["size"]  := 6
        oProdGauge["markers"]["40"]["label"] := "40"
        oProdGauge["markers"]["40"]["type"]  := "line"
        oProdGauge["markers"]["75"] :=  JsonObject():New()
        oProdGauge["markers"]["75"]["color"] := COR_PRETO
        oProdGauge["markers"]["75"]["size"]  := 6
        oProdGauge["markers"]["75"]["label"] := "75"
        oProdGauge["markers"]["75"]["type"]  := "line"
        ...
        oCarga:setaTitulo("Produção X Capacidade")
        oCarga:setaObjetivo("Acompanhar o percentual de utilização do recurso com apontamentos de produção com base na capacidade calculada para o período.")
        ...
        oCarga:setaPropriedade("03_NIVEIS","Atenção;Urgente","Níveis (% Utilização)",1,30,0,"po-lg-6 po-xl-6 po-md-6 po-sm-12")
...
Method ValidaPropriedades(oFiltros) Class ProducaoCapacidadeRecurso
    ...
    If aRetorno[1] 
        If !oFiltros:HasProperty("03_NIVEIS") .Or. oFiltros["03_NIVEIS"] == Nil .Or. Empty(oFiltros["03_NIVEIS"])
            aRetorno[1] := .F.
            aRetorno[2] := "O campo 'Níveis' deve ser preenchido com 2 valores separados por ponto e vírgula (ex.: '75;50'), definindo os percentuais de utilização do recurso que indicam Atenção e Urgência, respectivamente."
        Else 
            aNiveis := STRTOKARR( oFiltros["03_NIVEIS"], ";" )
            If Len(aNiveis) <> 2
                aRetorno[1] := .F.
            ...
            Else
                ...
                Else
                    If Val(aNiveis[1]) <= Val(aNiveis[2])
                        aRetorno[1] := .F.
                        aRetorno[2] := "No campo 'Níveis', o primeiro valor referente ao status 'Atenção' deve ser maior que o segundo, que representa 'Urgência'."
                    EndIf
                EndIf
            EndIf
        EndIf
