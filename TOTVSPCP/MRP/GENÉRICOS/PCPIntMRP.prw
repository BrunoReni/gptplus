...
Function PInMrpPend(nCode, oJsonRet, lReproc, cApi, aPend, cTipo, cUUID, lIntegra)
	...
	*/
	If !Empty(cUUID)
		...
		//Verifica também pendências de estoque não eliminadas, e elimina.
...
@since 06/05/2022
@version P12.1.33
@param lSchdl  , Lógico, Indica se está rodando em schedule
...
Static Function procSchdl(aAPI, lSchdl, lAbreTela, cTicket, lReItg)
	...
	If lAbreTela .And. Len(aApisPend) > 0
		...
		If !PCPLockPen("VALID", cTicket, aApisPend)
			...
			//Se aguardou o término do processamento, refaz a validação para abrir o pcpa712
			If lAbreTela
