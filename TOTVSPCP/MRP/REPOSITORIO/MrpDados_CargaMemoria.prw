...
CLASS MrpDados_CargaMemoria FROM LongNameClass
	...
	METHOD addLogDocumento(cFilAux, cProduto, cOpcional, dData, cDocum, cTabela, nQuant)
	...
	METHOD montaJoinHWA(cProdCol, cTicket, lJoinSMM, cColFil, cTabela, oMultiEmp, lJoinSMB)
...
@return Nil
/*/
Method addLogDocumento(cFilAux, cProduto, cOpcional, dData, cDocum, cTabela, nQuant) Class MrpDados_CargaMemoria
	Local cTipoDoc := ""
	Local cTipoReg := ""
	Local cIndex   := ""

	If Self:utilizaSMV()
		//cTipoDoc = Tipo de documento (1=OP;2=SC;3=Pedido de compra;4=Empenho;5=Demanda;)
		//6=Saldo inicial;7=Saldo Rejeitado;8=Em Terceiro;9=De Terceiro;10=Saldo Bloqueado)
		//cTipoReg = Tipo de registro (1=entrada;2=saída;3=saldo)
		Do Case
			Case cTabela == "T4Q"
				cTipoDoc := "1"
				cTipoReg := "1"
...
METHOD carregaRegistros() CLASS MrpDados_CargaMemoria
	...
	Self:oDados:oLogs:log("carregaRegistros;inicio;" + Time(), "41")
	...
	Self:oDados:oLogs:log("carregaRegistros;fim;" + Time(), "41")
	Self:oDados:oLogs:log(STR0080 + cValToChar(MicroSeconds() - nTempoIni), "41") //"Tempo total da carga "
...
METHOD registraDocumentos(cTabela) Class MrpDados_CargaMemoria
	...
	If Self:utilizaSMV() .And. Len(Self:aDocumentos) > 0
		...
		If lError
			Self:oDados:oLogs:log(I18N(STR0104, {cChave, cTabela}) , "E") //"Erro ao gravar lista. Chave[#1[ID CHAVE]#]. Tabela[#2[ID TABELA]#]."
		Else
			...
			If lError
				Self:oDados:oLogs:log( I18N(STR0104, {cChave, "TABELAS"}) , "E") //"Erro ao gravar lista. Chave[#1[ID CHAVE]#]. Tabela[#2[ID TABELA]#]."
			EndIf
...
METHOD registraProcessados() CLASS MrpDados_CargaMemoria
	...
	If oStatus:preparaAmbiente(Self:oDados)
		...
		If !Self:atualizaRegistrosNoBanco("T4J", cUpdate, cWhere)
			Self:oDados:oLogs:log(STR0057 + TCSQLError(), "E") //"Erro ao atualizar os registros que foram processados: "
		EndIf
	EndIf
...
METHOD percentualAtual(oDados, nPercent) CLASS MrpDados_CargaMemoria
	...
	If nPercent > 0 .AND. nPercent < 100 .AND. nPercent > oDados:oProdutos:getflag("memoryLoadPercentage")
		...
		//::log(STR0052 + cValToChar(nPercent) + "%" + " - " + STR0053 + "["+cValToChar(nProdCalc + nInexistente)+"] / " + STR0054 + "["+cValToChar(nProdutos)+"] (" + Time() + ")", "%")
	Else
...
/*/
METHOD montaJoinHWA(cProdCol, cTicket, lJoinSMM, cColFil, cTabela, oMultiEmp, lJoinSMB) CLASS MrpDados_CargaMemoria
	Local aFiliais := {}
	Local cAlias   := ""
	Local cColuna  := ""
	Local cQuery   := ""
	...
	If lFlagOk
		While oStatus:getStatus("niveis") != "9" .AND. oStatus:getStatus("status") != "4"
			...
			If lProcPrinc .And. (((MicroSeconds() - nSecLog) / 15) > 1)
				Self:oDados:oLogs:log(STR0105, "RN", , .T.) //"Processando o recalculo de niveis."
