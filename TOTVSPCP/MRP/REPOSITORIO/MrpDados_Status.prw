	...
	If AllTrim(oDados:oParametros["cAutomacao"]) != "1"
		...
		If lOk
			If nOk > 0
				cQuery += ","
			EndIf
			cQuery += " HW3_STATCM = (CASE WHEN HW3_STATCM < '" + cAux + "' OR HW3_STATCM IS NULL THEN '" + cAux + "' ELSE HW3_STATCM END) "
			nOk++
		EndIf
	...
	BEGIN SEQUENCE
		If Type("cEmpAnt") == "U"
			If Empty(oDados:oParametros["cEmpAnt"]) .Or. Empty(oDados:oParametros["cFilAnt"])
				oDados:oLogs:log(STR0058, "E") //"Erro ao preparar o ambiente. Parâmetros 'cEmpAnt' e 'cFilAnt' não informados."
				lRet := .F.
			Else
				RpcSetType(3)
