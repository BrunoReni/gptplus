...
/*/
METHOD new(oParametros, oPeriodos, oLogs, lRecursiva) CLASS MrpDados
...
METHOD inicializaTabelas(lCreate) CLASS MrpDados
	...
	If lCreate .AND. (::oParametros["nOpcCarga"] != 2) //!Carga Movimento em Memória
		...
		If VarIsUID( cChaveExec + "UIDs_PCPMRP")
			...
			If VarIsUID(cChaveExec + "UIDs_PCPMRP") .AND. !VarClean(cChaveExec + "UIDs_PCPMRP") .And. Self:oLogs:logValido("E")
			...
			If !Empty(aSessoes)
				...
				For nInd := 1 to Len(aSessoes)
					If !VarClean( aSessoes[nInd][1] ) .And. Self:oLogs:logValido("E")
		...
		If !VarSetUID( cChaveExec + "UIDs_PCPMRP", .T.) .And. Self:oLogs:logValido("E")
	...
	::oRastreioEntradas := MrpData_Global():New( cChaveExec, "RASTRO_ENTR", lCreate, ::oLogs, oStatus)
	::oRastreioEntradas:setlOrder( .F. )

	::oTransferencia := MrpData_Global():New( cChaveExec, "TRN", lCreate, ::oLogs, oStatus)
...
METHOD gravaSaidaEstrutura(cFilAux, dData, cComponente, nQuantidade, nPerMinimo, cIDOpc, lMantTrava, lFazLock) CLASS MrpDados
	...
	If !::atualizaMatriz(cFilAux, dData, cComponente, cIDOpc, {"MAT_SAIEST"}, {nQuantidade}, @cChave, lMantTrava, lFazLock, .T.)
		If Self:oLogs:logValido("E")
	...
	While !(nPerMinimo == Nil .and. nPerMaximo == Nil .and. nUltPerCal == Nil)
		...
		If nPerMinimo != Nil .AND. nPerMinimo != -1
			...
			ElseIf lError
				lError := .F.
				Sleep(50)
				::oLogs:log(RTrim(cProduto) + STR0084 , "22") //"Não encontrado na tabela de produtos."
				Loop
			...
			If nPerMinAtu > nPerMinimo;  //Atualiza para tras
			   ...
			   .OR. (!lReinicia .AND. nPerMinAtu <  nPerMinimo .AND. nThreadRes > 0 .AND. nThreadRes == ThreadID()) //Atualiza para frente
				...
				If nPerMinimo < ::oParametros["nPeriodos"] .And. Self:oLogs:logValido("23")
		...
		If nUltPerCal != Nil .AND. nUltPerCal != -1
			...
			EndIf
			lAtual     := .T.
			If nUltPerCal < ::oParametros["nPeriodos"] .And. Self:oLogs:logValido("24")
...
METHOD destruir(lForca) CLASS MrpDados
	...
	StartJob("PCPCleanVr", GetEnvServer(), .F., ::oParametros["cChaveExec"], ::oParametros["ticket"], ::oParametros["lAguardaDescarga"], (Self:lCreate .OR. lForca))
...
@version 1.0
/*/
Function PCPCleanVr(cChaveExec, cTicket, lAguardaDescarga, lLimpaTudo)
	...
	If lLimpaTudo
		...
		If VarIsUID( cChaveExec + "UIDs_PCPMRP" )
			...
			If VarIsUID(cChaveExec + "UIDs_PCPMRP")
				If !VarClean(cChaveExec + "UIDs_PCPMRP")
					If soDominio:oLogs:logValido("E")
						soDominio:oLogs:log("PCPCleanVr - VarClean(UIDs_PCPMRP) FALSE", "E")
					EndIf
					StartJob("PCPCleanVr", GetEnvServer(), .F., cChaveExec, cTicket, lAguardaDescarga, lLimpaTudo)
			...
			For nInd := 1 To nTotal
				...
				If (cChaveExec $ cGlobalKey .OR. oStatus:getStatus("status") != "4") .AND. VarIsUID( cGlobalKey )
					If VarClean( cGlobalKey )
						If soDominio:oLogs:logValido("LM")
							soDominio:oLogs:log("PCPCleanVr - VarClean( cGlobalKey ) TRUE: '" + AllTrim(cGlobalKey) + "'", "LM")
						EndIf
					ElseIf soDominio:oLogs:logValido("E")
						soDominio:oLogs:log("PCPCleanVr - VarClean( cGlobalKey ) FALSE: '" + AllTrim(cGlobalKey) + "'", "E")
					EndIf
				EndIf
			Next nInd
			aSessoes := FwFreeArray(aSessoes)
		ElseIf soDominio != Nil .AND. soDominio:oLogs:logValido("LM")
			soDominio:oLogs:log("PCPCleanVr - NAO Entrou '" + cChaveExec + "UIDs_PCPMRP" + "'", "LM")
		EndIf
		...
		If VarIsUID(cChaveExec)
			If VarClean(cChaveExec) 
				If soDominio != NIL .and. soDominio:oLogs:logValido("LM")
					soDominio:oLogs:log("MrpData_Global - VarClean(cChaveExec) TRUE: '" + AllTrim(cChaveExec) + "'", "LM")
				EndIf
			Else
				If soDominio != NIL .and. soDominio:oLogs:logValido("E")
		...
		If !lAguardaDescarga .AND. soDominio != Nil
			...
			While oStatus:getStatus("finalizado") != "true" .And. oStatus:getStatus("status", @lOk) != "4"
				If !lOk
					soDominio:oLogs:log("Termino Descarga Memoria: '" + AllTrim(cTicket) + "'", "R")
					Exit
				EndIf
				Sleep(50)
			End
