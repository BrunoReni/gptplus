...
METHOD consumirAlternativos(cFilAux, cProduto, nSaldo, nPeriodo, cProdOrig, lWait, cIDOpc, nSaiEstrut, aBaixaPorOP, aMinMaxAlt, lTranAlt) CLASS MrpDominio_Alternativo
	...
	ElseIf nSaiEstrut > 0
		For nIndex := 1 to nTotal
			...
			If cRegra $ "|2|3|"
				//::oDominio:oLogs:log("Força consumo do alternativo, produto '" + AllTrim(cProduto) + "'", "33")
				lCompraAlt := .T.
			EndIf
		Next
	EndIf
		...
		For nInd := 1 to Len(aAlteDeman)
			...
			If aRegistro != Nil
				...
				If !Self:reservaAlternativo(cFilAux, aRegistro[2])
					...
					If lLog5
						::oDominio:oLogs:log(STR0175 + AllTrim(aRegistro[2]), "5") //"Não obteve reserva do produto alternativo: "
					EndIf
