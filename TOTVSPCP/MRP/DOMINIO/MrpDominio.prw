...
METHOD loopNiveis() CLASS MrpDominio
	...
	Local lDelegar        := .T.
	Local lCacheLocal     := .F.
	Local lLogPerc        := .T. //::oLogs:logValido("%") - Forçado para exibir em tela.
				...
				If !(AllTrim(aBaixaPorOP[nIndex][ABAIXA_POS_TIPO_PAI]) $ "|1|"+scTextPPed+"|"); //"PMP|Ponto Ped."
				   .AND. aBaixaPorOP[nIndex][ABAIXA_POS_QTD_SUBSTITUICAO] >= 0
					...
					ElseIf "|TRANF" $ "|"+AllTrim(aBaixaPorOP[nIndex][ABAIXA_POS_TIPO_PAI]);
						   ...
						   //Se for transferência de produção a necessidade será sempre a qtd a ser transferida, já atribuida na variável nQtdNec
	...
	For nInd := 1 to nTotal
		...
		If !lVersao .OR. Empty(aVersao) .OR. Empty(aVersao[nPosRevisao])
			...
			aVersao  := {cRevPad,;
						...
						"",""}       //TODO - Inserir regra de local padrão
		Else
		...
		For nPerAux1 := nPerIndMin to nPerIndMax
			...
			If !lErrorAlt
				...
				If lWait
					...
					lReturn := .F.
					If lLog5
						oLogs:log(STR0174 + AllTrim(cChaveProd) + STR0018 + cValToChar(nPeriodo), "5") //"Interrompendo cálculo. Não conseguiu desfazer substituição dos produtos alternativos, produto: " + " - Periodo: "
					EndIf
					If nThreads > 1
			...
			If lUsaME .And. lReturn .And. (nSaldo + nQtrSai) < 0
				...
				If nSaldo + nQtrSai + nSaidSubs < 0
					...
					If lWait
						...
						lRetPerCal := .T.
						If lLog5
							oLogs:log(STR0169 + AllTrim(cChaveProd) + STR0018 + cValToChar(nPeriodo), "5") //""Interrompendo cálculo produto, não conseguiu consumir o estoque em outra filial, produto: " + " - Periodo: "
						EndIf
						If nThreads > 1
							oLiveLock:setResult(cChaveProd, 1, .F., .T., .T.)
						EndIf
			...
			If lExplode
				If !lErrorMAT .And. nNecess != 0
					If (lSubProduto .AND. soDominio:oSubProduto:processar(cFilAux, cProduto, cIDOpc, nPeriodo, nNecess))
						...
						lReturn := .F.
						If lLog5
							oLogs:log(STR0148 + AllTrim(cChaveProd) + STR0018 + cValToChar(nPeriodo), "5") //"Interrompe cálculo após gerar necessidade do subproduto: "
						EndIf
