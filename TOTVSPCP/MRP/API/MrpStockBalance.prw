...
Function MrpSBVLD(oMRPApi, cMapCode, oItem)
	...
	If cMapCode == "fields"
        ...
        If lRet .And. oItem["lot"] == Nil
			lRet := .F.
			oMRPApi:SetError(400, STR0007 + " 'lot' " + STR0008) //"Atributo 'XXX' Nao foi informado."
		EndIf

        If lRet .And. oItem["availableQuantity"] == Nil
			lRet := .F.
			oMRPApi:SetError(400, STR0007 + " 'availableQuantity' " + STR0008) //"Atributo 'XXX' Nao foi informado."
		EndIf

        If lRet .And. oItem["consignedOut"] == Nil
			lRet := .F.
			oMRPApi:SetError(400, STR0007 + " 'consignedOut' " + STR0008) //"Atributo 'XXX' Nao foi informado."
		EndIf

        If lRet .And. oItem["consignedIn"] == Nil
			lRet := .F.
			oMRPApi:SetError(400, STR0007 + " 'consignedIn' " + STR0008) //"Atributo 'XXX' Nao foi informado."
		EndIf

        If lRet .And. oItem["unavailableQuantity"] == Nil
			lRet := .F.
			oMRPApi:SetError(400, STR0007 + " 'unavailableQuantity' " + STR0008) //"Atributo 'XXX' Nao foi informado."
		EndIf
