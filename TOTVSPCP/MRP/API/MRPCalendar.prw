...
Function MrpCAVLD(oMRPApi, cMapCode, oItem)
	...
	If cMapCode == "fields"
		...
		If Empty(oItem["calendar"])
			oMRPApi:SetError(400, STR0007 + " 'calendar' " + STR0008) //"Atributo 'XXX' não foi informado."
			Return .F.
		EndIf

		If Empty(oItem["date"])
			oMRPApi:SetError(400, STR0007 + " 'date' " + STR0008) //"Atributo 'XXX' não foi informado."
			Return .F.
		EndIf

		If Empty(oItem["startTime"])
			oMRPApi:SetError(400, STR0007 + " 'startTime' " + STR0008) //"Atributo 'XXX' não foi informado."
			Return .F.
		Else
			If !FormatHrOk(oItem["startTime"])
				oMRPApi:SetError(400, STR0007 + " 'startTime' " + STR0009) //"Atributo 'XXX' deve ser informado no formato de hora (hh:mm)."
				Return .F.
			EndIf
		EndIf

		If oItem["endTime"] == Nil
			oMRPApi:SetError(400, STR0007 + " 'endTime' " + STR0008) //"Atributo 'XXX' não foi informado."
			Return .F.
		Else
			If !FormatHrOk(oItem["endTime"])
				oMRPApi:SetError(400, STR0007 + " 'endTime' " + STR0009) //"Atributo 'XXX' deve ser informado no formato de hora (hh:mm)."
				Return .F.
			EndIf
		EndIf

		If Empty(oItem["interval"])
			oMRPApi:SetError(400, STR0007 + " 'interval' " + STR0008) //"Atributo 'XXX' não foi informado."
			Return .F.
		Else
			If !FormatHrOk(oItem["interval"])
				oMRPApi:SetError(400, STR0007 + " 'interval' " + STR0009) //"Atributo 'XXX' deve ser informado no formato de hora (hh:mm)."
				Return .F.
			EndIf
		EndIf

		If (SubHoras(oItem["startTime"], oItem["endTime"])) > 0
			oMRPApi:SetError(400, STR0007 + " 'startTime' " + STR0010 + " 'endTime'.") //"Atributo 'XXX' deve ser menor ou igual ao 'endTime'."
			Return .F.
		EndIf

		If Hrs2Min(oItem["endTime"]) - Hrs2Min(oItem["startTime"]) - Hrs2Min(oItem["interval"]) < 0
			oMRPApi:SetError(400, STR0011) //"O período informado é inválido. ('endTime' - 'startTime' - 'interval') deve ser maior ou igual a zero."
			Return .F.
		EndIf

		If Empty(oItem["totalHours"])
			oMRPApi:SetError(400, STR0007 + " 'totalHours' " + STR0008) //"Atributo 'XXX' não foi informado."
			Return .F.
		Else
			If !FormatHrOk(oItem["totalHours"])
				oMRPApi:SetError(400, STR0007 + " 'totalHours' " + STR0009) //"Atributo 'XXX' deve ser informado no formato de hora (hh:mm)."
				Return .F.
			EndIf
		EndIf
	EndIf
