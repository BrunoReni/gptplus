...
Function MrpPCVLD(oMRPApi, cMapCode, oItem)
	...
	If cMapCode == "fields"
		If lRet .And. Empty(oItem["code"])
			lRet := .F.
			oMRPApi:SetError(400, STR0007 + " 'code' " + STR0008) //"Atributo 'XXX' Nao foi informado."
		EndIf

		If lRet .And. Empty(oItem["request"])
			lRet := .F.
			oMRPApi:SetError(400, STR0007 + " 'request' " + STR0008) //"Atributo 'XXX' Nao foi informado."
		EndIf

		If lRet .And. Empty(oItem["product"])
			lRet := .F.
			oMRPApi:SetError(400, STR0007 + " 'product' " + STR0008) //"Atributo 'XXX' Nao foi informado."
		EndIf

		If lRet .And. Empty(oItem["quantity"])
			lRet := .F.
			oMRPApi:SetError(400, STR0007 + " 'quantity' " + STR0008) //"Atributo 'XXX' Nao foi informado."
		EndIf

		If lRet .And. oItem["receivedQuantity"] == Nil
			lRet := .F.
			oMRPApi:SetError(400, STR0007 + " 'receivedQuantity' " + STR0008) //"Atributo 'XXX' Nao foi informado."
		EndIf
		If lRet .And. Empty(oItem["warehouse"])
			lRet := .F.
			oMRPApi:SetError(400, STR0007 + " 'warehouse' " + STR0008) //"Atributo 'XXX' Nao foi informado."
		EndIf
		If lRet .And. Empty(oItem["type"])
			lRet := .F.
			oMRPApi:SetError(400, STR0007 + " 'type' " + STR0008) //"Atributo 'XXX' Nao foi informado."
		EndIf

		If lRet .And. !("|"+AllTrim(oItem["type"])+"|"$"|1|2|")
			lRet := .F.
			oMRPApi:SetError(400, STR0007 + " 'type' " + STR0009) //"Atributo 'XXX' " deve ser preenchido com '1' para Firme e '2' para Previsto."
		EndIf
	EndIf
Return lRet
