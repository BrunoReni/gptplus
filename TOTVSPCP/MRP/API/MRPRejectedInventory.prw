...
Function MrpRIVLD(oMRPApi, cMapCode, oItem)
	...
	If cMapCode == "fields"
        ...
        If lRet .And. Empty(oItem["product"])
			lRet := .F.
			oMRPApi:SetError(400, STR0009 + "product" + STR0010) //"Atributo 'XXX' Nao foi informado."
		EndIf

		If lRet .And. Empty(oItem["quantity"])
			lRet := .F.
			oMRPApi:SetError(400, STR0009 + "quantity" + STR0010) //"Atributo 'XXX' Nao foi informado."
		EndIf

		If lRet .And. Empty(oItem["warehouse"])
			lRet := .F.
			oMRPApi:SetError(400, STR0009 + "warehouse" + STR0010) //"Atributo 'XXX' Nao foi informado."
		EndIf

		If lRet .And. Empty(oItem["invoiceDate"])
			lRet := .F.
			oMRPApi:SetError(400, STR0009 + "invoiceDate" + STR0010) //"Atributo 'XXX' Nao foi informado."
		EndIf

		If lRet .And. oItem["returnedQuantity"] == Nil
			lRet := .F.
			oMRPApi:SetError(400, STR0009 + "returnedQuantity" + STR0010) //"Atributo 'XXX' Nao foi informado."
		EndIf

		If lRet .And. Empty(oItem["code"])
			lRet := .F.
			oMRPApi:SetError(400, STR0009 + "code" + STR0010) //"Atributo 'XXX' Nao foi informado."
		EndIf
