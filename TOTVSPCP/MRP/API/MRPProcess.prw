...
#DEFINE CAMPOS_LOGICOS "|demandsProcessed|lPorNivel|lAguardaDescarga|lLogML|lGeraDoc|eventLog||lUsesProductIndicator|lRastreiaEntradas|"
#DEFINE CAMPOS_NUMERICOS "|nThreads|nThreads_RAS|nThreads_MAT|nThreads_AGL|nPilhaFunc|"
...
Function MrpPCalcul(oBody, cAutomacao)
	...
	ElseIf VarBeginT( cMyUID, cMyKey )
		...
		Else
			...
			oPCPError := PCPMultiThreadError():New(cErrorUID)
			oPCPError:startJob("MrpProcExe", GetEnvServer(), .F., Nil, Nil, oBody:toJson(), cTicket,,,,,,,,,, "PutGlbValue('THREAD_MASTER_"+cTicket+"','1')")
...
Function MrpVld(oMRPApi, cMapCode, oItem)
	...
	ElseIf cMapCode == LISTA_DE_PARAMETROS
		...
		If lRet .And. Empty(oItem["value"]) .And. Empty(oItem["list"])
			lRet := .F.
...
@version P12
@param cTicket  , Character, Ticket do MRP
@param cCodParam, Character, Código do parâmetro
@param cParamVal, Character, Valor do parâmetro
@return Nil
/*/
Function MrpAddPar(cTicket, cCodParam, cParamVal)
