...
Function MrpBROVLD(oMRPApi, cMapCode, oItem)
	...
	If cMapCode == "fields"
		...
		If Empty(oItem["routing"])
			oMRPApi:SetError(400, STR0008 + " 'routing' " + STR0009) //"Atributo 'XXX' não foi informado."
			Return .F.
		EndIf

		If Empty(oItem["operation"])
			oMRPApi:SetError(400, STR0008 + " 'operation' " + STR0009) //"Atributo 'XXX' não foi informado."
			Return .F.
		EndIf

		If Empty(oItem["component"])
			oMRPApi:SetError(400, STR0008 + " 'component' " + STR0009) //"Atributo 'XXX' não foi informado."
			Return .F.
		EndIf
