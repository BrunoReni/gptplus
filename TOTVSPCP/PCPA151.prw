...
METHOD processarProduto(cProduto, cLocal, nSaldoIni) CLASS SugestaoLotesEnderecos
	...
	oJsData["oUpdSB2"]["local"  ] := cLocal
	
	oJsData["oUpdSB2"]["previsto"] := JsonObject():New()
	oJsData["oUpdSB2"]["previsto"]["nQtdEmpOrig"] := 0
	oJsData["oUpdSB2"]["previsto"]["nQtdEmpReal"] := 0
	
	oJsData["oUpdSB2"]["firme"] := JsonObject():New()
	oJsData["oUpdSB2"]["firme"]["nQtdEmpOrig"] := 0
	oJsData["oUpdSB2"]["firme"]["nQtdEmpReal"] := 0
...
METHOD avaliaSaldos(oJsData, cAliasSD4, oJsInfoPrd, lPrepEmpen, lBaixaEmp) CLASS SugestaoLotesEnderecos
	...
	oJsData["oUpdSB2"][cTipoOp]["nQtdEmpOrig"] += nEmpOrig
	...
	If lPotencia
		...
		oJsInfoPrd["nQtd"] := oJsData["nSldSB2"]
	EndIf
	...
	For nIndLote := 1 To nLotes
		...
		If lPotencia
			...
			nEmpPot += oSelf:aplicarPotencia(aSaldo[xPOS_aLotes_QUANTIDADE], nPotencia)
		EndIf
		...
		nEmpReal                       += oSelf:aplicarPotencia(aSaldo[xPOS_aLotes_QUANTIDADE], aSaldo[xPOS_aLotes_POTENCIA])
		...
		If aSaldo[xPOS_aLotes_QUANTIDADE] != 0
			...
			oJsData["oUpdSB2"][cTipoOp]["nQtdEmpReal"] += aSaldo[xPOS_aLotes_QUANTIDADE]
		EndIf
	Next nIndLote
...
METHOD gravaAlteracoes(oJsData) CLASS SugestaoLotesEnderecos
	BEGIN TRANSACTION
		...
		If (oJsData["oUpdSB2"]["previsto"]["nQtdEmpOrig"] < oJsData["oUpdSB2"]["previsto"]["nQtdEmpReal"]) .Or. (oJsData["oUpdSB2"]["firme"]["nQtdEmpOrig"] < oJsData["oUpdSB2"]["firme"]["nQtdEmpReal"])
			Self:updSB2(oJsData["oUpdSB2"])
		EndIf
	END TRANSACTION
Return
...
METHOD aplicarPotencia(nQuant, nPotencia) CLASS SugestaoLotesEnderecos
	...
	If nPotencia > 0
		nReal := Round(nQuant * (nPotencia / 100), oSelf:nDecD4Qtd)
	Else
		nReal := Round(nQuant, oSelf:nDecD4Qtd)
	EndIf
...
@return Nil
/*/
METHOD updSB2(oUpdSB2) CLASS SugestaoLotesEnderecos

	dbSelectArea("SB2")
	SB2->(DbSetOrder(1))
	If SB2->(DbSeek(Self:cFilSB2+oUpdSB2["product"]+oUpdSB2["local"]))
		If oUpdSB2["previsto"]["nQtdEmpOrig"] < oUpdSB2["previsto"]["nQtdEmpReal"]
			GravaB2Emp("+", oUpdSB2["previsto"]["nQtdEmpReal"] - oUpdSB2["previsto"]["nQtdEmpOrig"], "P", .F., 0)
		EndIf
		
		If oUpdSB2["firme"]["nQtdEmpOrig"] < oUpdSB2["firme"]["nQtdEmpReal"]
			GravaB2Emp("+", oUpdSB2["firme"]["nQtdEmpReal"] - oUpdSB2["firme"]["nQtdEmpOrig"], "F", .F., 0)
		EndIf
	EndIf
