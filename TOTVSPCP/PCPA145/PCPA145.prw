...
METHOD processar() CLASS ProcessaDocumentos
	...
	If !oPCPError:possuiErro() .And. FindFunction("P145GrvRas") .And. AliasInDic("SMH")
		lExecRas := .T.
...
METHOD getDocsAglutinados(cDocMRP, cProduto, lEmpenho) CLASS ProcessaDocumentos
	...
	If aRegs[1]
		...
		While nPos <= nTamRegs
			...
			*/
			If !Empty(cDocPai)
				aAdd(aDocs, {cDocPai, oJson["items"][nPos]["allocation"], oJson["items"][nPos]["trt"]})
			Else
				If aDocDePara[1]
...
Function P145AtuSta(cTicket, nStatus, lIntgracao)
	...
	If PCPLockControl():semaforo("LOCK", cChaveLock) .And. HW3->(dbSeek(xFilial("HW3")+cTicket))
		...
		If lIntgracao
			PutGlbValue("P145STATUS", cStatus)
			...
			EndIf
		Else
			cStatusGlb := GetGlbValue("P145STATUS")
			If !Empty(cStatusGlb)
...
Function P145EndLog()
	...
	If GetGlbValue("P145MAIN") == "FIM" .And. GetGlbValue("P145JOBINT") == "FIM" .And. GetGlbValue("PE_145GER") == "FIM"
