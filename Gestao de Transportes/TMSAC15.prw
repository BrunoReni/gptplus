...
If lTM15TpDoc
	aRetPE := ExecBlock("TM15TpDoc",.F.,.F.,{AClone(aDocs),nPeso,nValTot})
...
If lTMAC15JV
    cBodyPE := ExecBlock("TMAC15JV", .F., .F., { cJson } )
...
METHOD GetREPOM( cPath, cPathPar, cToken, cQueryPar, cVersion, cMessage ) CLASS TMSBCARepomFrete
...
Local cPath     := ::url_repom
Local cToken    := ::access_token
Local cPathPar  := "/ShippingFuelBenefit/GetByHiredDriverNationalId/"
Local cGet      := ""
Local aArea     := GetArea()
Local cJson     := ""
Local aCards    := {}
...
cPathPar += cHiredNID + "/" + cDriverNID
cGet := ::GetREPOM( cPath, cPathPar, cToken, "", "2.4" )
...
Local cToken    := ::access_token
Local cVersion  := ::api_version
Local cPathPar  := "/Shipping/FuelConsume/ByShipping/"
Local cGet      := ""
Local cMessage  := ""
...
Default cShippID     := ""
    ...
    If !Empty(cShippID)
        cPathPar += cShippID
        cGet := ::GetREPOM( cPath, cPathPar, cToken, "", cVersion, @cMessage )
