		...
		If !Empty(cCodRRE)
			...
			If DJ4->(dbSeek(xFilial('DJ4')+cCodRRE))
				...
				For nCntFor2 := 1 To Len(aVeiculo)
					...
					//-- Valor da Mercadoria
					nSoma   := (1 / (10 ^ TamSX3("DJ5_VALMER")[2]))
					nMinAnt := 0
					...
					Next nCntFor1
					//-- Peso Cubado
					nSoma   := (1 / (10 ^ TamSX3("DJ5_PESOM3")[2]))
					nMinAnt := 0
					...
					For nCntFor1 := 1 To nMaxLin
						Aadd(aVetFaixa,{aValMer[nCntFor1,1],aValMer[nCntFor1,2],aValMer[nCntFor1,3],aValMer[nCntFor1,4],;
																				...
																				aValMer[nCntFor1,5]})
						lValMer := Iif(!lValMer,(nValor  >= aVetFaixa[nCntFor1,3] .And. nValor  <= aVetFaixa[nCntFor1,4]) ,lValMer)
						lPeso   := Iif(!lPeso  ,(nPeso   >= aVetFaixa[nCntFor1,5] .And. nPeso   <= aVetFaixa[nCntFor1,6]) ,lPeso)
						lPesoM3 := Iif(!lPesoM3,(nPesoM3 >= aVetFaixa[nCntFor1,7] .And. nPesoM3 <= aVetFaixa[nCntFor1,8]) ,lPesoM3)
						lIdVeic := Iif(!lIdVeic,(nIdVeic >= aVetFaixa[nCntFor1,9] .And. nIdVeic <= aVetFaixa[nCntFor1,10]),lIdVeic)
						If lValMer .And. lPeso .And. lPesoM3 .And. lIdVeic .And. !lRegDJ5
							nLinAtu := nCntFor1
							lRegDJ5 := .T.
						EndIf
					Next nCntFor1
					...
					If lRegDJ5
						lBloqueia := .T.
						If aVetFaixa[nLinAtu,11] == '2'  //1=Sim,2=Nao
							lBloqueia := .F.
						EndIf
					EndIf
					...
					If lBloqueia
						...
						While (cQryDJ6)->(!Eof())
							If (cQryDJ6)->DJ0_TIPO == '1'	//-- Veiculo
								...
								EndIf
							Else    //Motorista
								If Len(aMotorista[nCntFor2,7]) > 0
									aAdd(aRet,{ ;
										DJ4->DJ4_CODRRE, ;
										DJ4->DJ4_CODCLI, ;
										DJ4->DJ4_LOJCLI, ;
...
Local lGDifal     := .F.
Local lGFcpDif    := .F.
Local cUFOrigem   := MaFisRet(,"NF_UFORIGEM")
Local cNumero     := ''
Local cLcPadTit   := ''
Local nVlrIcm     := 0
Local nMes        := 0
...
Function TMSMntJSon(cCodFon,cCodReg,cBase,cConteudo, lEndPoint )
