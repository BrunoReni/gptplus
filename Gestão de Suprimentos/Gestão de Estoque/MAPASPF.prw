...
Static Function ProcessTN(oMapas, cMapCfop)
    ...
    EndIf
    cQuery += "WHERE SF1.F1_FILIAL = '" + oMapas:cFilSF1 + "' "
    cQuery += "AND SD1.D1_FILIAL = '" + oMapas:cFilSD1 + "' "
    cQuery += "AND SB1.B1_FILIAL = '" + oMapas:cFilSB1 + "' "
    cQuery += "AND SB5.B5_FILIAL = '" + oMapas:cFilSB5 + "' "
    cQuery += "AND SF4.F4_FILIAL = '" + oMapas:cFilSF4 + "' "
    cQuery += "AND SB1.B1_GRUPO BETWEEN '" + oMapas:cGrupoDe + "' AND '" + oMapas:cGrupoAte + "' "
    cQuery += "AND SB1.B1_COD BETWEEN '" + oMapas:cProdDe + "' AND '" + oMapas:cProdAte + "' "
    If oMapas:lTNFiltDtN
        cQuery += " AND SD1.D1_DTDIGIT "
    Else
        cQuery += " And Coalesce(SD1COMPL.D1_DTDIGIT, SD1.D1_DTDIGIT) "
    EndIf
    cQuery += " BETWEEN '" + DtoS(oMapas:dDataDe) + "' AND '" + DtoS(oMapas:dDataAte) + "' "
    cQuery += "AND SD1.D1_QUANT > 0 "
    cQuery += "AND (SD1.D1_OP = ' ' OR SF4.F4_PODER3 <> 'D' OR SC2.C2_TPPR <> 'E') "
    cQuery += "AND SB5." + oMapas:cProdPF + " IN ('S', 's') "
    cQuery += "AND SF4.F4_ESTOQUE = 'S' "
    cQuery += "AND SF1.D_E_L_E_T_ = ' ' "
    cQuery += "AND SD1.D_E_L_E_T_ = ' ' "
    cQuery += "AND SB1.D_E_L_E_T_ = ' ' "
    ...
    cQuery += "INNER JOIN " + oMapas:cSqlNameB5 + " SB5 "
    cQuery += "ON SB5.B5_COD = SB1.B1_COD "
    cQuery += "WHERE SF2.F2_FILIAL = '" + oMapas:cFilSF2 + "' "
    cQuery += "AND SD2.D2_FILIAL = '" + oMapas:cFilSD2 + "' "
    cQuery += "AND SB1.B1_FILIAL = '" + oMapas:cFilSB1 + "' "
    cQuery += "AND SB5.B5_FILIAL = '" + oMapas:cFilSB5 + "' "
    cQuery += "AND SF4.F4_FILIAL = '" + oMapas:cFilSF4 + "' "
    cQuery += "AND SB1.B1_GRUPO BETWEEN '" + oMapas:cGrupoDe + "' AND '" + oMapas:cGrupoAte + "' "
    ...
    While !(cAliasMov)->(EoF())
        ...
        If cChave != cSFAnt // O processamento a seguir só é necessário 1 vez por NF
            ...
            If !Empty(cCNPJT) .And. cCNPJT == oMapas:cCnpjFil
                cCodTran := Iif((cAliasMov)->ENTSAI == "E", "A", "F")
            ElseIf (!Empty(cCNPJT) .And. cCNPJT == cCNPJ) .Or. Empty(cCNPJT)
                cCodTran := Iif((cAliasMov)->ENTSAI == "E", "F", "A")
            Else
            ...
            If oMapas:lTN .And. !lIntTms .And. lEmpTran .And. cModalTran $ "RO/AQ/FE/AE";
                ...
                oMapas:GravaTN(aRecTN, aRecCC, aRecTM)
...
METHOD ProcessaUC() CLASS MAPASPF
    ...
    If ::lUCREPrdCt
        cQuery += "SF5.F5_CODIGO = (SELECT TOP(1) D3_TM FROM " + ::cSqlNameD3 + " SD3A "
        cQuery += "WHERE SD3A.D3_FILIAL = '" + ::cFilSD3 + "' AND "
        cQuery += "SD3A.D3_CF LIKE 'PR%' AND "
        cQuery += "SD3A.D3_ESTORNO <> 'S' AND "
        cQuery += "SD3A.D3_EMISSAO BETWEEN '" + DtoS(::dDataDe) + "' AND '" + DtoS(::dDataAte) + "' AND "        
...
METHOD GeraTXT(cArqDest, cDir) CLASS MAPASPF
    ...
    If nHandle != -1
        ...
        cMapas += Iif(::lMVN, "1", "0")
        cMapas += Iif(::lMVI, "1", "0") // Movimentação Internacional: cenário não coberto no momento
        cMapas += Iif(::lUP, "1", "0")
        cMapas += Iif(::lUT, "1", "0")// Transformação: cenário não coberto no momento
        cMapas += Iif(::lUC, "1", "0")
        cMapas += Iif(::lFB, "1", "0")
        cMapas += Iif(::lTN, "1", "0")
        cMapas += Iif(::lAM, "1", "0") // Transporte Internacional: cenário não coberto no momento
