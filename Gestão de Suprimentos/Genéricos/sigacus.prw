...
Else
	...
	While !( cAliasSB8 )->( Eof()) .And. cFilialSB8+cSeek == ( cAliasSB8 )->B8_FILIAL+( cAliasSB8 )->B8_PRODUTO+( cAliasSB8 )->B8_LOCAL
		...
		If cProg == 'A241' .Or. cProg == "A261"
			...
			If QtdComp(nSalLote,.t.) > QtdComp(0) .and. Empty(cOP)
			...
			EndIf
		EndIf
		If QtdComp(nSalLote,.t.) > QtdComp(0) .Or. ((cProg == "A270" .And. !lParam) .Or. (cProg == "A685" .And. !lParam) .Or. ((cProg == "A240" .Or. cProg == "A241") .And. SF5->F5_TIPO == "D") .Or. (cProg == "A242" .And. cCpo == "M->D3_LOTECTL"))
