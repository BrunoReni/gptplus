...
Local cPerApur  := ""
Local cAliasTmp	:= CriaTrab(Nil,.F.)
Local cTamOP    := Space(TamSX3("D3_OP")[1])
Local aAreaK270 := {}
...
EndIf
cQuery += "INNER JOIN ("
cQuery +=      "SELECT B2_FILIAL, B2_COD, B2_LOCAL, MAX(B9_DATA) B9_DATAMAX, B9_QINI "
cQuery +=      "FROM "+ RetSqlName("SB2") +" SB2 "
cQuery +=      "LEFT JOIN "+ RetSqlName("SB9") +" SB9 on "
cQuery +=         "SB9.B9_FILIAL = '"+xFilial("SB9")+"' "
...
cQuery +=      "GROUP BY B2_FILIAL, B2_COD, B2_LOCAL, B9_QINI ) SB2_B "
cQuery += " ON SB2_B.B2_COD = SB1.B1_COD "
If lAliasD3E
...
oBulk := FwBulk():New(cTableProd, 1000)
...
If lExistArq
	...
	While !(cArqTMP)->(EOF())
		...
		oBulk:AddData({"H010",;
					...
					(cArqTMP)->TOTAL,;
					(cArqTMP)->TOTAL,;
					((cArqTMP)->TOTAL/(cArqTMP)->QUANTIDADE)})
