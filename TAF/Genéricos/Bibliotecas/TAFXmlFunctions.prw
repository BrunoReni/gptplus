...
/*/
//---------------------------------------------------------------------------
Static Function ProcXML( cAliasTb, cLayout, cRegNode, cFunctionXML, nTopSlct, cPath, cStatus, lLote, nEscopo, oBrowse, lFilBrw, lFilLote, lPerLote )
	...
	Local cMsgProc		:= ""
	Local cSigla		:= Iif( nEscopo == 5, "R-", "S-" )
	Local cFilPerg		:= ""
	Local cQryFil       := ""
	Local cPerLote      := ""
	Local cFilLote     	:= ""
...
/*/
//---------------------------------------------------------------------------
Function TAFQryXMLeSocial( cBancoDB, nTopSlct, cAliasTb, cStatus, aEventos, aIDsTrb, cRecNos, cMsgProc, cLayout, aFiliais, lAllEventos, lJob, nEscopo, lCommit, lProtul, oTabFilSel, lXmlLote,cPeriod, cEventos,lApi,aMotiveCode,cQueryBc, cFilLote, cPerLote, dDataIni, dDataFim )
	...
	local cTipoEvt		:= ""
	Local lTotal		:= .F.
	Local lOrdena		:= IsInCallStack("TAFPROC4TSS")
	Local lOrdDesc      := .F.
	Local cCodCM6       := ""
	Local lProthRem		:= .F.
	Local cMotiveCode	:= ""
	...
	For nX := 1 To Len(aEventos)
		...
		If !Empty(aEventos[nX][4])
			...
			If TAFAlsInDic(cAliasTb) .And. !Empty(cLayout)  .And. (lProthRem .OR. !( cLayout $ "S-5001|S-5002|S-5003|S-5011|S-5012|S-5013|TAUTO" ))
				...
				If cTipoEvt != "C" .and. !( cLayout $ "S-5003|S-5013" )
...
/*/
//---------------------------------------------------------------------------
function TAFQryMonTSS( cBancoDB , nTopSlct , cAliasTb , cStatus , aEvtsESoc , aIDsTrb , cRecNos, cMsgProc,cLayoutESocial,aFiliais,lAllEventos,dDataIni,dDataFim, lMV, oTabFilSel,lReavPen)
	...
	local cAuxFil			:= ""
	local nEventos			:= 0
	local cEventoMarcado	:= ""
	local cIniEsoc			:= SuperGetMv('MV_TAFINIE',.F.," ")
	Local cVerSchema		:= SuperGetMv('MV_TAFVLES',.F.,"02_03_00")
	Local lOrdena			:= IsInCallStack("TAFPROC4TSS")
	Local lOrdDesc          := .F.
	Local cCodCM6           := ""
	Local cQry2				:= ""
	...
	For nI := 1 To Len(aEventosParm)
		...
		If nEventos > 0 .And. aEventosParm[nI][2] //Só considero os tipos de eventos selecionados na tela de filtro
			...
			For nX := 1 To nEventos
				...
				cEventoMarcado	:= aEventosParm[nI][1][nX][12] .Or. lAllEventos //Determina se o evento foi marcado no browse
				cLayout			:= aEventosParm[nI][1][nX][02] 					//Descrição do Evento

				If cEventoMarcado .And.  !( cLayout $ "S-5001|S-5002|S-5003|S-5011|S-5012|S-5013" )
					...
					If AliasInDic(cAliasTb) .And. !Empty(cLayout) .And. IIf(lEvtTrb,!Empty(cCmpTrab),.T.)
						...
						If  cTipoEvt == "E" .And. !Empty(AllTrim(cCmpData)) .And. cAliasTb != "CMJ"
							cQry += " AND " + cCmpData + " >= '" + DtoS(dDataIni) + "'"
							cQry += " AND " + cCmpData + " <= '" + DtoS(dDataFim) + "'"
						EndIf
						
						If cLayout $ "S-1200|S-1210|S-1295|S-1299|S-2299|S-2399"
						...
						/*
						If  cLayout == "S-2100"
							cQry += " AND '" + cIniEsoc + "' >= '" + DtoS(dDataIni) + "'"
							cQry += " AND '" + cIniEsoc + "' <= '" + DtoS(dDataFim) + "'"
						EndIf
						*/
						...
						If cLayout == "S-2230"
							If TafColumnPos( "CM6_XMLREC" )
								cQry += " ORDER BY CM6_DTAFAS, CM6_XMLREC"
							EndIf
						EndIf
						*/
					Else
...
//-------- Tratamento para utilizar sempre o mesmo ID no envio e gera
	...
	If cTipo $ "MENSAL"
		...
		Else
			...
			Else
				cIdeEvento += xTafTag( "nrRecibo", Substr(( cAlias )->( &( cAlias + "_PROTPN" ) ),1,23) )
			EndIf
		EndIf
...
Function XmlErpxTaf(cAlias, cFunGerXml, cLayout, cFilReg)
	...
	Local cXmlErp 		:= FwCutOff(GetXmlSt2(cAlias, (cAlias)->(Recno()), cEmpAnt + cFilReg  ))
...
/*/
//---------------------------------------------------------------------
Static Function GetXmlSt2(cAlias, nRecno, cFilReg)
...
Function TAfxmltss(cAlias,nRecno)
	...
	Local cTptXml		:= "Tipo XML"
	Local cXmtaf		:= "Xml TAF"
...
If Len(aMotiveCode) > 0
	...
	If (cAlias)->(!EOF())
		While (cAlias)->(!EOF())
			cMotiveCode += 	iif(Empty(cMotiveCode),'','|') + ;
							(cAlias)->C8N_ID
			(cAlias)->(DbSkip())
		EndDo
	EndIf
