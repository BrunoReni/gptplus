...
Function TAFDesEven( cVerAnt as character, cVerAtu as character, cLayESoc as character, dDataLim as date )
	...
	If TafLayESoc(cLayESoc) .And. DToS(dDataLim) > "20220309"
		...
		cMsg += ' ' + STR0062 + cVerAnt + '.' + '</p>'			// "Este comportamento se deve ao fato deste leiaute ter sido descontinuado após a versão "
		cMsg += '	<p>' + STR0063 + cVerAnt + '.' + '</p>' 	// "Caso seja necessário fazer a manutenção dos registros deste evento, o parâmetro MV_TAFVLES deve ser alterado para a versão "																																																																								
		cMsg += '	<p>' + STR0064 + cVerAnt + 	'.' + '</p>'	// "A alteração deste parâmetro pode causar erros de integração ou transmissão no caso de ocorrência de utilização destas funcionalidades com o ambiente configurado para a versão " 
		cMsg += '</div>'
