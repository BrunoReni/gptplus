				...
				For nK := 1 to Len( aV70 )
					...
					If !SkipRule( cEventCode, aV70[nK,1], cPeriodAux )
						...
						If !lJoin
							...
							If cEventCode $ "S-1200|S-1202|S-1207|S-1210|S-1260|S-1270|S-1280|S-1295|S-1298|S-1299|S-2501"
								cQuery += "WHERE " + cAlias + "." + cAlias + "_FILIAL = '" + cBranch + "' "
...
Static Function GetData( cAliasQry as character, aV70 as array, cPeriodAux as character, cEventCode as character, cBranch as character, cAlias as character, cObriEvt as character )
	...
	If cEventCode $ "S-2205|S-2206|S-2306"
		DBSelectArea( cAlias )
		( cAlias )->( DBSetOrder( 1 ) )
	...
	ElseIf cEventCode $ "S-1200|S-1202|S-1207|S-1210|S-1260|S-1270|S-1280|S-1295|S-1298|S-1299" 	
			cTimeLim := DToS( MonthSum( SToD( ( cPeriodAux + RTrim( aV70[3] ) ) ), 1 ) )
	Else
		If !( cEventCode $ "S-2190|S-2210" )
			cTimeLim := DToS( MonthSum( SToD( Left( ( cAliasQry )->DTEVENT, 6 ) + RTrim( aV70[3] ) ), 1 ) )
		EndIf
	EndIf
	...
	ElseIf cEventCode == "S-2299" .and. aV70[1] == "001"
		cTimeLim := DToS( DaySum( SToD( ( cAliasQry )->DTEVENT ), 10 ) )
	...
	ElseIf cEventCode == "S-2300"
		If aV70[1] == "001"
			cTimeLim := SubStr( DToS( MonthSum( SToD( ( cAliasQry )->DTEVENT ), 1 ) ), 1, 6 ) + "15"
		ElseIf aV70[1] == "002"
			cTimeLim := DToS( LastDate( MonthSum( SToD( cObriEvt ), 1 ) ) )
		EndIf
	ElseIf cEventCode == "S-2230"
		If ( cAliasQry )->XMLREC == "INIC" .or. ( cAliasQry )->XMLREC == "COMP"
			...
			ElseIf aV70[1] == "002"
				...
				If Len( aCM6 ) > 0
					...
					If nTime >= 3 .and. nTime <= 15
						cTimeLim := DToS( MonthSum( SToD( Left( ( cAliasQry )->DTEVENT, 6 ) + RTrim( aV70[3] ) ), 1 ) )
					Else
						cTimeLim := DToS( MonthSum( SToD( Left( ( cAliasQry )->DTEVENT, 6 ) + "15" ), 1 ) )
					EndIf
				EndIf
			ElseIf aV70[1] == "003"
				aCM6 := Next2230( ( cAliasQry )->TRABAL, ( cAliasQry )->DTEVENT,, aV70 )
				...
				If Len( aCM6 ) > 0
					...
					If nTime > 15
						lr2230 := .T.
						cTimeLim := DToS( DaySum( SToD( ( cAliasQry )->DTEVENT ), 15 ) )
					Else
						lr2230 := .F.
					EndIf
				EndIf
			...
			ElseIf aV70[1] == "008"
				lr2230 := .T.
				cTimeLim := DToS( DaySum( SToD( ( cAliasQry )->DTEVENT ), 90 ) )
		...
		ElseIf ( cAliasQry )->XMLREC == "TERM"
			lr2230 := .T.
			cTimeLim := DToS( MonthSum( SToD( Left( ( cAliasQry )->DTFAFA, 6 ) + RTrim( aV70[3] ) ), 1 ) )
		EndIf
...
Static Function Next2230( cIDEmployee as character, cPeriodFrom as character, cPeriodTo as character, aV70 as array, lAfast as logical )
	...
	If aV70[1] == "004" .and. lAfast
		aCM6 := Next2230( cIDEmployee, aCM6[1,4], DToS( DaySum( SToD( aCM6[1,4] ), 60 ) ), aV70, .F. )
