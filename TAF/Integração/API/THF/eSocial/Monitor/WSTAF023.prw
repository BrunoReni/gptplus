...
Static Function WS023Event(oResponse, nNewPos, cPeriod, cInFiliais, cTableEvent, cEvent, cTypeEvent, cCampoTable, cPeriodFrom, cPeriodTo, aMotives, cInFilC1E, aFilterStatus)
    ...
    If cTypeEvent == "T"
        ...
        If !(lHasFilStatus == .T. .And. aScan(aFilterStatus, "4") == 0)        
    ...
    Else
        ...
        If TafColumnPos( cTableEvent + "_STASEC" )
            ...
            For nZ := 1 To Len(aDtRefEve)
                ...
                If !Empty(aEvento[6]) 
                    ...
                    Else
                        ...
                        cQuery += " SUBSTRING(A." + aDtRefEve[nZ] + ",3, 4) = '" + cPeriod + "' "     
...
Static Function checkRequest(oRequest)
    ...
    If Empty(oRequest["branches"])
        cMessage += " Filial não informado no parâmetro 'branches'. "
    EndIf
    ...
    If !Empty(aGroups)
        ...
        For nX := 1 To Len(aEvents)
            ...
            If AccessEvt(aEvents[nX])
                ...
                If AScan(aGroups, aEvents[nX][12]) > 0
...
Static Function SSTEvt()
    ...
    Local cSSTEvt := "S-2210/S-2220/S-2240"
