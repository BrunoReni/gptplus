		...
		Else
			...
			Else
				...
				If PrepEnv( cEmpRequest, cFilRequest )
					...
					Else
						StartJob( "WS052EXEC", GetEnvServer(), .F., cEmpRequest, cFilRequest, cRequestID )
...
/*/
//---------------------------------------------------------------------
Function WS052EXEC( cEmpRequest as character, cFilRequest as character, cRequestID as character )
	...
	If V3J->( MsSeek( xFilial( "V3J" ) + PadR( cRequestID, TamSX3( "V3J_ID" )[1] ) ) )
		...
		GetQuery(cBranch, cCPF, cName, cCatNumber, cPeriodFrom, cPeriodTo, @aResponse)
...
Static Function GetMonitor( oResponse as object, aKey as array)
	...
	If GetQuery(cBranch, , , , , , @aResponse, cId, cVersao)
...
/*/
//---------------------------------------------------------------------
Static Function GetData( cAliasQry as character )
	...
	cAccDate         	:= SUBSTR( ( cAliasQry )->DTACID, 0, 4 ) + "-"+ SUBSTR( ( cAliasQry )->DTACID, 5, 2 ) + "-"+ SUBSTR( ( cAliasQry )->DTACID, 7, 2 )//DToC( SToD( ( cAliasQry )->DTACID ) )
	cDeathDate       	:= IIF( Empty( ( cAliasQry )->DTOBIT ), "", DToC( SToD ( ( cAliasQry )->DTOBIT ) ) )
	...
	cBirthDate			:= DToC( SToD( Iif( cPredecessorType == "S2190", ( cAliasQry )->DATANA , ( cAliasQry )->DTNASC ) ) ) 
	...
	If cCodCountry == '01058' 
		cPlaceInsc		:= Transform( AllTrim( ( cAliasQry )->NRIACI ), "@R 99.999.999/9999-99" )
	...
	cNote := CM0->CM0_OBSMEM

	cServiceDate		:= DToC( SToD( ( cAliasQry )->DTATEN ) )
	...
	cReceiving			:= DToC( SToD( ( cAliasQry )->DTRECP ) )
	...
	If cTpInsc == "CNPJ"
		cCGC				:= Transform( aEmpregador[2], "@R 99.999.999/9999-99" )
	Else
	...
	If TafColumnPos("CM0_ULTTRB")
		cDayWork	:= DToC( SToD( ( cAliasQry )->ULTTRB ) )
...
/*/
Static Function GetQuery(cBranch as character, cCPF as character, cName as character, cCatNumber as character, cPeriodFrom as character, cPeriodTo as character, aResponse as array, cId as character, cVersao as character)
	...
	If lRet 
		...
		cQuery  += " ,C9V.C9V_ESTCIV AS ESTCIVIL "
		...
		lRet := IIf(Eof(),.F.,.T.)
		While ( cAliasQry )->( !Eof() )
			aAdd( aResponse, GetData( cAliasQry ) )
			( cAliasQry )->( DBSkip() )
		EndDo
		( cAliasQry )->( DBCloseArea() )
	EndIf 
