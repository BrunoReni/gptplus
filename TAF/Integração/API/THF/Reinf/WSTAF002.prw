...
Else
	...
	Else
		...
		If PrepEnv( cEmpRequest, cFilRequest )
			...
			//-------------------------------------------------------------------
			aApurac := TAFGetEvt( self:period, self:event, "", aFiliais, 1, "", "", "", "", self:page, self:pageSize, .T. )
			...
			ElseIf self:event $ "R-1070" .and. !Empty( aApurac )
				WS0021070( aApurac, oResponse, oBody )
			...
			ElseIf self:event $ "R-2050" .and. !Empty( aApurac )
				WS0022050( aApurac, oResponse, oBody )
			ElseIf self:event $ "R-2055" .and. !Empty( aApurac )
				WS0022055( aApurac, oResponse, oBody, self:period, self:event, aFiliais )
			...
			ElseIf self:event $ "R-2060" .and. aApurac[6]
				WS0022060( aApurac, oResponse, self:period, oBody )
			ElseIf self:event $ "R-3010" .and. !Empty( aApurac )
				WS0023010( aApurac, oResponse, oBody )
...
/*/
//---------------------------------------------------------------------------------------------------------------
WSMETHOD GET invoiceDetail QUERYPARAM companyId, period, event, id, branchId, page, pageSize WSREST WSTAF002
...
/*/
//---------------------------------------------------------------------------------------------------------------
static function TafGetEvt(cPerApu, cEvent, cId, aFiliais, nLevel, cChvNF, cItemNF, cNumProc, cFilDoc, nPage, nSize, lNewQuery, lAll, cCodPart)
    ...
    case cEvent $ "R-4010"
        For nX:=1 To Len(aFiliais)
            aAdd(aFil,{ aFiliais[nX][9] , aFiliais[nX][3] })
        Next nX
        aAliasQry := WSQry4010(substr(cPerApu,3,4) + substr(cPerApu,1,2), aFil, aInfEUF, nPage, nSize, lAll )
				...
				if lvalidOk
					...
					For nX := 1 to Len( aProcFil )
						...
						If !Empty(cCNPJ)
							...
							TAFAPR4020( self:period, aFil, lValid, @lSucesso, cCNPJ, @aIDLog,,,lReinf212)
						EndIf	
						If !Empty(cCodPar)
							TafPopTmp( @oTmpCGC , @cCodPar, 60)
							//Chamada da apuracao por do fornecedor exterior que nao possui CNPJ nem NIF
							TAFAPR4020( self:period, aFil, lValid, @lSucesso, '', @aIDLog, cCodPar,,lReinf212)
						EndIf	
						If !Empty(cNif)
							TafPopTmp( @oTmpCGC , @cNif, TamSx3("C1H_NIF")[2])
							//Chamada da apuracao por do fornecedor exterior que nao possui CNPJ mas possui NIF
							TAFAPR4020( self:period, aFil, lValid, @lSucesso, '', @aIDLog, '', cNif,lReinf212)
						EndIf	
...
/*/
//---------------------------------------------------------------------------------------------------------------
Static Function RetFil(aLoadFil, aFilial, cEvent, lReinf212, cIdEvAdic)
...
Static Function RetErAl(cBranch, cEvent, aIdLog)
	...
	While (cAlErros)->(!Eof())
        For nX := 1 to Len(aIdLog)
            if (cAlErros)->PROCID == aIdLog[nX][1]
