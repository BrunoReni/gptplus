...
TTALK "v1";
WSSYNTAX "/api/tsi/v1/TSIIntegrationErrors/reprocessInvoice/?{companyId}";
PRODUCES APPLICATION_JSON
...
/*/
//---------------------------------------------------------------------
WSMETHOD POST ReprocessInvoice WSRECEIVE companyId WSREST TSIIntegrationErrors
Local lRet             := .T.
	...
	Else
		...
		If PrepEnv( cEmpRequest, cFilRequest )
            ...
            if oBody['reprocessAll']
                TSIV5RC20():New( cEmpAnt+cFilAnt, .T. )
                oResult["status"]   := .T.
                ...
                for nR := 1 to len(oBody['items'])
                    TSIV5RC20():NEW(cFilAnt+cEmpAnt,.T.,oBody['items'][nR])
                    oResult["status"]   := .T.
