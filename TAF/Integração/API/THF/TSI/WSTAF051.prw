...
::cQuery += cIsNullSQL + "((SELECT CASE WHEN C20.C20_CODSIT IN ('000003','000004') THEN 'Sim' ELSE 'Não' END FROM " + RetSqlName( 'C20' ) + " C20 WHERE C20.R_E_C_N_O_ = TAB2.C20REC),'Não') CANCTAF, "
...
::cQuery += "OR ( SF3.F3_CFO <= " + cIndCFOP + " AND SF3.F3_TIPO IN ('B','D') ) "
...
::cQuery += TafSF3LJoin( cDbType, cSubStr, cConcat, cIndCFOP, cNoLock )
...
/*/
//---------------------------------------------------------------------
Function TafSF3LJoin( cDbType, cSubStr, cConcat, cIndCFOP, cNoLock )
...
cCasePart += " OR ( SF3.F3_CFO <= " + cIndCFOP + " AND SF3.F3_TIPO IN ('B','D') ) "
...
cQry += " THEN 'F'" + cConcat + "RTRIM(SF3.F3_CLIEFOR)" + cConcat + "RTRIM(SF3.F3_LOJA) "
cQry += " ELSE 'C'" + cConcat + "RTRIM(SF3.F3_CLIEFOR)" + cConcat + "RTRIM(SF3.F3_LOJA) END AND "
cQry += " C1H.D_E_L_E_T_ = ' ' "
