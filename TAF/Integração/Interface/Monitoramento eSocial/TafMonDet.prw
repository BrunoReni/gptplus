...
/*/
//-------------------------------------------------------------------
Function consultaTotalizador(aIDsSel,oTabFilSel,dDtIni,dDtFim,cIDBrowse)
	...
	Local aData := GetApoInfo("TAFA423.PRW")
	...
	Define MsDialog oDlg Title "Seleção de Status" From 0,0 To 180,500  Pixel

	oSayMsg := TSay():New(045,020,{||cMsgAmb},oDlg,,,,,,.T.,,,200,030,,,,,,.T.)
	...
	If lOk 
		...
		Else
			...
			FWMsgRun(,{|oMsgRun| TAFProc5Tss(.F.,aEvents,'4',aIdsSel,,lEnd,,paramFiliais,paramDataInicial,paramDataFim,,,,,,@oTabFilSel,,,oMsgRun)},"Consulta de Totalizadores", "Realizando Consulta no Servidor TSS ..." )
...
Static Function mostraTotErro()
	...
	Local cIdUnic := ""
	Local cEvtRetXml := "S-1200|S-1210|S-1295|S-1299|S-2299|S-2399"
	...
	Else
		...
		MsgInfo("Somente os eventos S-1200,S-1210,S-1295,S-1299,S-2299 e S-2399 retornam totalizadores.","Totalizadores")
	EndIf
...
/*/
//-------------------------------------------------------------------
Static Function TafGrvTot(aFiliais,dDtIni,dDtFim,aEvents)
...
If MsgYesNo("Este procedimento realizará a substituição de todos os totalizadores do período e filial(ais) selecionada(s), deseja prosseguir ?","Eventos de Totalizadores")
	...
	If lForce
		For nI := 1 To Len(aFiliais)
			...
			If aEvents[nX][3] $ "C91|T3P|T72|CUO" //S-1200, S-1210, S-1295 e S-1299
