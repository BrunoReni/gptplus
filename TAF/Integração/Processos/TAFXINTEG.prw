...
Function xTafCabXml(cXmlLayout as character, cAlias as character, cLayout as character, cTagReg as character,;
	...
	If cLayout $ "1202|1207|2210|2220|2240|2231|2400|2405|2410|2416|2418|2420"
		lSimplif := .T.
	Elseif cLayout $ "2500|2501|3500"
		cVerSchema := "S_01_01_00"
	EndIf
	...
	If cTipo $ "EVENTO|MENSAL"
		...
		If lSimplif
			...
			cEventos :=  "1295|1298|1299|3000|3500"
	...
	If !(cLayout $ "5001|5002|5003|5011|5012|5013")
...
/*/
//-------------------------------------------------------------------
Function xTafGerXml( cXml, cLayout, cPath, lLote, lRetMsg, nSeq, cFile, cSigla, cFil, lXmlErp, aArquivos )
...
*/
//-------------------------------------------------------------------
Function FTafVldOpe( cAlias, nInd, nOpc, cFilEv, aIncons, aChave, oModel, cNomModel, cCmpsNoUpd , nIndIDVer, lRuleEvCad, aNewData, cMatTrab, nRecnoReg, lTermAfas, cCmpsRemSimp )
...
/*/
//-------------------------------------------------------------------
Function FTafGetVal( cInfoObjto, cTipoCmp, lInfoConv, aIncons, lInfoObrig, cNomeNo, cNomCmp, lFormatNum, lEmpty, lAtributo, cTag )
	...
	If !lInfoConv
		...
		If !lAtributo .And. oDados:XPathHasNode( cInfoObjto  )

			If  "/matricula" $ cInfoObjto
				If lEspM
					cRet := AllTrim( oDados:XPathGetNodeValue( cInfoObjto ) )
				Else
					cRet := RTrim( oDados:XPathGetNodeValue( cInfoObjto ) )
				EndIf
			Else
				cRet := AllTrim( oDados:XPathGetNodeValue( cInfoObjto ) )
			EndIf
			...
			If cTipoCmp == "D"
				cRet := TAFGetData( cRet, "DDMMAAAA", "/" )
			EndIf
			// Ajuste efetuado para contemplar a verificação de varios pagamentos no XML S-1200 na Integracao.
		ElseIf cInfoObjto == "/eSocial/evtRemun/dmDev/codCateg" .And. ValType(oDados:XPathGetAttArray( "/eSocial/evtRemun/dmDev" )) == "A"
			//Retorna a primeira posição da ARRAY, pois nao existe varias categorias.
			cArrObjto	:=  "/eSocial/evtRemun/dmDev[1]/codCateg"
			cRet := Alltrim( oDados:XPathGetNodeValue( cArrObjto ) )

		ElseIf lAtributo .And. oDados:XPathHasAtt( cInfoObjto )
			cRet := oDados:xPathGetAtt( cInfoObjto, cTag )
		...
		ElseIf lInfoObrig
			Aadd( aIncons, " O Nó " + cInfoObjto + " não foi preenchido e é uma informação " +;
	...
	If lLaySimplif .And. (cTipoCmp == "C" .AND. Valtype(cInfoObjto) == "C")
		If "/nrInsc" $ cInfoObjto
			cRet := alltrim(SUBSTR(cRet, 1,14))
		EndIf
	EndIf
	...
	If !Empty( cRet )
		...
		If Valtype( cRet ) <> cTipoCmp
			If cTipoCmp == "D"
				cRet := CToD( cRet )

			ElseIf cTipoCmp == "N"
				If lFormatNum .AND. At(',',cRet) > 0
					cRet := Val( StrTran( StrTran( cRet, ".", "" ), ",", "." ) )
...
Function FTafGetFil( cSeekFil, aIncons , cAliasTAF, lEsocial )
	...
	If !Empty( cFilTAF )
		cFilTAF := PadR( cFilTAF, FWSizeFilial() )
	EndIf
	RestArea(aArea)
...
/*/
//-------------------------------------------------------------------
Function FCModelInt( cAlias, cFonte, oModel, nOperation,cId,lRegTrans )
	...
	If nOperation == 3 .and. !lRegTrans
		If !(cFonte $ "|TAFA275|TAFA276|TAFA277|TAFA261|") //Não é necessário verificar duplicidade de ID para os Eventos S-2205, S-2206, S-2230, S-2306.
			cIdAux 	:= oModel:GetModel("MODEL_"+cAlias):GetValue(cAlias+"_ID",cFonte)
...
Function FGetIdInt(cFieldLay1 as character, cFieldLay2 as character, cInfo1 as character, cInfo2 as character, lIdChave as logical,; 
	...
	If !Empty( cInfo1 ) .Or. !Empty( cInfo2 )
		...
		ElseIf cFieldLay1 == "nrOC"	.OR. cFieldLay1 == "nrCRM" .or. cFieldLay1 == "nrOc"
			cAlias := "CM7"
			cCmpTab1 := "CM7_CODIGO"
			nOrder   := 2 //4
			cFonte   := "TAFA262"
		...
		ElseIf (cFieldLay1 == "matricula")
			...
			nOrder   := 11
			/*
			//Fatores de Riscos Ambientais ( Tabela T12 )
				ElseIf cFieldLay1 == "codFatRis"
				cAlias   := "T12"
				cCmpTab1 := "T12_CODFAT"
				nOrder   := 2
			*/
		...
		If TafIndexInDic(cAlias, nOrder, .T.)
			...
			If Empty( cId )  .And. (!Empty(cInfo1) .Or. !Empty(cInfo2))
				...
				Else
					//cId := cTxtErr + IIf(!Empty(cInfo1), Substr(StrZero(Val(cInfo1),6),4,3), Substr(StrZero(Val(cInfo2),6),4,3))
...
/*/
//-------------------------------------------------------------------
Function FAltRegAnt( cAlias as Character, cOper as Character, lGravaDt as Logical, cDtOldFim as Character, cDtNewReg as Character, cDtOldReg as Character )
	...
	If !lGoExtemp .Or. cAlias $ 'T1V|T1U|T0F|V73|V76|V77|V78'
		...
		If cAlias == "CM6"
			...
			CM6->( DBSetOrder( 3 ) )
			If CM6->( MsSeek( xFilial( 'CM6' ) + CM6->CM6_ID + "1" ) )
				RecLock( cAlias, .F. )
...
/*/
//-------------------------------------------------------------------
Function TAFRegStat( nOpc, cAlias, aChave, nInd, cId, cVerAnt, aIncons, cProtocolo, lRegTrans, cEvento, nIndIDVer, cChave, oModel, aDatas, lRuleEvCad, aNewData, cMatTrab, cNomModel, nRecnoReg, lTermAfas )

	Local oModelDel		:=	Nil
	Local cTags			:=	""
	Local cInfo			:=	""
	Local cCmpInd		:=	cAlias + "_FILIAL"
	Local nlY			:=	0
	Local lFindReg		:=	.T.
	Local lFindCod		:=	.F.
	local lGerNewReg	:=	.F.
...
//- Caso nao tenha enviado o codigo, nao deve realizar acoes
	...
	If !Empty( cChave )
		...
		ElseIf cNomModel $ "TAFA261|TAFA257"
		...
		Else
			...
			If lFindCod
				...
				If aTafRot[4] $ ( "S-5001|S-5002|S-5011|S-5012|S-5003" )
					...
					ElseIf !( (cAlias)->&( cAlias + "_STATUS") ) $ ( "2|6|4|7" ) .AND. !lTermAfas
						...
						If AllTrim((cAlias)->&( cAlias + "_EVENTO" )) == "I"
							...
							If nOpc == 3
								...
								ElseIf cNomModel == "TAFA261"
									nOpc := 4 //A operação será de alteração (Exclusivo para o evento S-2230)
	   ...
	   /*
			If !Empty(cMatTrab)
	   		//Se for um evento de cadastramento/alterações do funcionário
	   		//Verifica se a matricula já existe para outro funcionário;
	   	   TafExtMat(cChave, cMatTrab, @nOpc, @aIncons)
			EndIf*/
...
/*/
//-------------------------------------------------------------------
Function TAFRastro( cAlias, nIndice, cChave, lInatRgAnt, lMens, oBrw )
...
Function xTAFFunLay(cLayout,nOpc,cTagEvt)
	...
	"2240|2365|2400|2405|2420|2440|2600|2620|2399|2800|2820|3000|4000|5001|5002|5003|5011|5012|5013|1295|2260|2221|2231|2410|2416|2418|2500|2501|3500|"
...
// Inclusão normal
	...
	ElseIf lRegTrans
		...
		/*If Empty((cAlias)->&( cAlias + '_DTFIN' ))
			TafCloseTb("I",,,,aDatas[2],aNewData[1],aDatas[1],@cDtFinAt)
			(cAlias)->&( cAlias + '_DTFIN' ) := cDtFinAt
		EndIf*/
...
/*/
//-------------------------------------------------------------------
Function TAFLoadModel(cModel,nOperation)
...
/*/
//-------------------------------------------------------------------
Function TAFIdFunc(cCPF, cMat, cInconMsg, nSeqErrGrv)

	Local nI 			:= 0
	Local nID			:= 0
	Local nVersao		:= 0
	Local nTamFil   	:= TamSx3("C9V_FILIAL")[1]
	Local cFunc			:= ""
	Local cID			:= ""
	Local cEvento		:= ""
	Local cVersao		:= ""
	...
	If TafColumnPos("C9V_MATTSV") .And. lLaySimplif
