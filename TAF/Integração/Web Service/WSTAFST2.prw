...
WSMETHOD GET QUERYPARAM ticketCode, registryKey, sourceBranch, startRecNo, searchMode, queryElements, lotQuantity, returnRetResponse WSREST  WSTAFST2
	...
	cUId := FWUUID(AllTrim(Str(Randomize(1,999999))) + Lower(AllTrim(::ticketCode) + AllTrim(::registryKey)) + AllTrim(Str(ThreadID()))) 
	   ...
	   TAFCALLIPC(cFuncIPC,cFuncREST,cUId,cChave,cValorR,aQryParam)
	  	...
	  	If !Empty(aRetorno)
			If aRetorno[1]
				...
				TafConOut(CRLF + 'tafuuid_response: ' + cUId + ' | ' + cResponse)
				lRet := .T.
			Else
...
/*/
//---------------------------------------------------------------------------
Static Function WsSearchDePara(sourceBranch,aEmpresas,cEmpTAF,lRetFilTAF,cFilTAF)
