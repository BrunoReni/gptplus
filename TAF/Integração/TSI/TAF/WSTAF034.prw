...
Ws034Proc( oBjWS034, lERP, aJsonDoc, oTsiNFis, oModel, oMldC20, oMldC30, oMldC35, oMldC39, oMldT9Q, oMldC2F, oMldC2D, oMldC21, oMldC29 )
...
aStructC29 := TsiStruct( C29->(DBStruct()), oHashC29 )
...
while (cAlias)->(!eof())
    if C20->(DbSeek( (cAlias)->C20_FILIAL + (cAlias)->C20_CHVNF ))
        ...
        if nPageAux <= nPageSize
            ...
            /*-----------------------------------
            |     FATURAS DO DOCUMENTO FISCAL   |
            ------------------------------------*/
            oMdlC29 := oModel:GetModel("MODEL_C29")
            nQtNt := len(oJsonResp[cTagNf])
            oJsonResp[cTagNf][nQtNt]['ticketsByInvoice'] := {}
            getJson(@oJsInfTick, oHashC29, aStructC29, @oMdlC29, 'ticketsByInvoice' )
            oJsonResp[cTagNf][nQtNt]['ticketsByInvoice'] := oJsInfTick['ticketsByInvoice']
            oJsInfTick['ticketsByInvoice'] := {}
        else
            lHasNext := .T.
        EndIf
...
@return Nil, nulo, não tem retorno. 
//----------------------------------------------------------------------*/
Function Ws034Proc( oBjWS034, lERP, aJsonDoc, oTsiNFis, oModel, oMldC20, oMldC30, oMldC35, oMldC39, oMldT9Q, oMldC2F, oMldC2D, oMldC21, oMldC29 ) 
...
for i := 1 to nLenJSon
    ...
    if ValType(xRet) == "U" .And. ValType( oBjJson["invoice"] ) == 'A'  //JsonObject populado com sucesso
        ...
        for nQtd := 1 to nAte
            ...
            if !Ws034PutPost( oBjJson["invoice"][nQtd], @aRetJs, @oJsonResp, @oModel, @oMldC30, @oMldC35, @oMldC2F, @oMldC39, @oMldC2D, @oMldT9Q, @oMldC21, @oMldC20, @oTsiNFis, @oMldC29 )
...
@return Nil, nulo, não tem retorno. 
//----------------------------------------------------------------------*/
Function Ws034PutPost(aObjJson, aRetJs, oJsonResp, oModel, oMldC30, oMldC35, oMldC2F, oMldC39, oMldC2D, oMldT9Q, oMldC21, oMldC20, oTsiNFis, oMldC29 )
...
if Empty(cWarning) //caso nao encontre inconsistencias
    ...
    cChv += padr(aObjJson['taxDocumentNumber'],_nTmNUMDOC)
        ...
        BEGIN TRANSACTION
            ...
            If !lSameStamp
                ...
                If lSucessItem //Retorno VldData
                    TsiNFCommit(oModel,aRetJs,lSucessItem,cKey,aOBjJson)
                else
...
static function TsiNFCommit(oModel,aRetJs,lSucessItem,cKey,aOBjJson)
    ...
    If ValType( oModel) == 'O' .and. FwFormCommit( oModel )
        ...
        TafConout('confirmando gravacao - '+aOBjJson['taxDocumentNumber']+"- Thread " + cValtoChar(ThreadId()),1,.f.,"TSI")
        ClearV5R( "C20", cKey )
        //ClearTmpTSI(aObjJson,oTsiNFis)
    else 
        Tafconout('erro FwFormCommit - '+aOBjJson['taxDocumentNumber']+"- Thread " + cValtoChar(ThreadId()),1,.f.,"TSI")
    Endif
    
Return

/*----------------------------------------------------------------------
{Protheus.doc} ClearTmpTSI()
Deleta registro da tabela temporária
@author Renan Gomes
@since 26/07/2022
@return Nil, nulo, não tem retorno.
//----------------------------------------------------------------------
/*
static function ClearTmpTSI(oJson,oTsiNFis)
Local cQuery := ""

cQuery := "DELETE FROM "+oTsiNFis:oTempTable:GetTableNameForQuery()+" WHERE "
cQuery += "NFISCAL = '"+alltrim(oJson['taxDocumentNumber'])+"' AND "
cQuery += "SERIE = '"+alltrim(oJson['taxDocumentSeries'])+"' AND "
cQuery += "CLIFOR = '"+alltrim(oJson['clieFor'])+"' AND "
cQuery += "LOJA = '"+alltrim(oJson['loja'])+"' "

TcSqlExec(cQuery) //Executa o DELETE
...
@return Nil, nulo, não tem retorno.
//----------------------------------------------------------------------*/
static function GrvFisDoc( oModel, oMldC30, oMldC35, oMldC2F, oMldC39, oMldT9Q, oMldC2D, oMldC21, aObjJson, aRetJs, nI, nOperation, cKey, lSucessItem, oMldC20, oTsiNFis, oMldC29 )
...
/*----------------------------------------------------------------
|                CADASTRO DE FATURAS ( T013AI )                  |
-----------------------------------------------------------------*/
nAteA := 0
if valtype( aObjJson["ticketsByInvoice"] ) == 'A'
    nAteA := len( aObjJson["ticketsByInvoice"] )
endif
for nlA := 1 to nAteA
    if (nlA > 1);oMldC29:AddLine();endif
    TafIncReg( @oModel, aObjJson["ticketsByInvoice"][nlA], oHashC29, @aRetJs, @lSucessItem, 'MODEL_C29', aInfo , oTsiNFis)
    if !lSucessItem; lErro := lSucessItem ; endif
next nlA
...
Local cSeek    := ''
Local cKey     := ''
Local cTamTPInsc := lTrim(cValToChar(GetSx3Cache('T9C_TPINSC','X3_TAMANHO')))
...
HMSet(oHashC20, '#F3#C20_IDOBR' , "GetIDCached( 'T9C', padr(aObjJson['registrationType']," + cTamTPInsc + ") + padr(aObjJson['cnoNumber']," + cTamNRInsc + "), 3 ,oTsiNFis:GetHashCahed('T9C'))" )                //T9C_FILIAL, T9C_ID
...
HMSet(oHashC35, '#F3#C35_CST'   , "ValCodCst(aObjJson['cst'], aObjJson['taxCode']  )" )//C14_FILIAL, C14_CODIGO
...
HMSet(oHashC2F, '#F3#C2F_CST'   , "ValCodCst( aObjJson['cst'], aObjJson['taxCode'] )")               //C14_FILIAL, C14_CODIGO
...
Local oHashC29 := HMNew( )

//Campos das informações das faturas do documento fiscal.
SetHashKey(oHashC29, 'ticketIssuer',         'C29_INDEMI')
SetHashKey(oHashC29, 'ticketType',           'C29_INDTIT')
SetHashKey(oHashC29, 'ticketDescription',    'C29_DESCRI')
SetHashKey(oHashC29, 'ticketNumber',         'C29_NUMTIT')
SetHashKey(oHashC29, 'numberOfInstallments', 'C29_QTDPAR')
SetHashKey(oHashC29, 'totalTicketValue',     'C29_VLRTOT')
SetHashKey(oHashC29, 'valueOfDiscounts',     'C29_VLRDES')
SetHashKey(oHashC29, 'ticketNetValue',       'C29_VLRLIQ')
...
@return Nil, nulo, não tem retorno.
//----------------------------------------------------------------------*/
static function ValCodCst(cCodCst,cCodTrib)
Local cAliasCst  := ''
Local cIDTrib    := ''
...
if !empty(cCodTrib)
    cIdTrib		:= GetAdvFval( 'C3S', 'C3S_ID', xFilial( 'C3S' ) + cCodTrib , 1 )
