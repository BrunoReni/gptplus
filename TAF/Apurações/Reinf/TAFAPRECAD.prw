...
/*/
//-------------------------------------------------------------------
Function QryApRecAd(cPerApu, lApur, lIdProc , aFil, cReg, lApi)
	...
	If !__lApurBx
		If lApur 
			cQuery	+= "  'NFS' AS ROTINA, C20.C20_FILIAL AS FIL, '0' AS ID, C20.C20_NUMDOC AS NUMDOCTO, C20.C20_CHVNF AS CHVNF, C1H.C1H_CNPJ AS CNPJORIREC,  C30.C30_TPREPA AS TPREPASSE, "
			cQuery  += "  CASE WHEN C1H.C1H_CNPJ = ' ' AND C1H.C1H_PAISEX != '' AND C1H.C1H_PEEXTE = '2' THEN C1H.C1H_ID ELSE ' '     END AS CIDEXTE, "
			cQuery  += "  CASE WHEN C1H.C1H_CNPJ = ' ' AND C1H.C1H_PAISEX != '' AND C1H.C1H_PEEXTE = '2' THEN C1H.C1H_CODPAR ELSE ' ' END AS CCODPAR, "
			cQuery  += "  CASE WHEN C1H.C1H_CNPJ = ' ' AND C1H.C1H_PAISEX != '' AND C1H.C1H_PEEXTE = '2' THEN C1H.C1H_NOME ELSE ' '   END AS CDESPART, "
		...
		cQuery	+= " C30.C30_NUMITE = C35.C35_NUMITE AND C35.C35_CODTRI = '000013' AND C35.D_E_L_E_T_ = ' ' "
		
		cQuery	+= " INNER JOIN " + RetSqlName("C1H") + " C1H ON C1H.C1H_ID = C20.C20_CODPAR AND "
		
		If cReg == "R-2030"
			cQuery 	+= " C1H.C1H_PPES IN ('2','3') AND "
			cQuery 	+= " (C1H.C1H_CNPJ <> '' OR (C1H.C1H_CNPJ = '' AND C1H.C1H_PAISEX != '' AND C1H.C1H_PEEXTE = '2')) "
		ElseIf cReg == "R-2040"
			cQuery 	+= " C1H.C1H_PPES = '2' AND C1H.C1H_CNPJ <> '' AND "
			cQuery 	+= " C1H.C1H_INDDES = '1' "
		EndIf
		...
		// Where
		cQuery	+= " WHERE C20.C20_FILIAL IN " + cFiliais + " AND C20.D_E_L_E_T_ = ' ' AND "
	...
	If lApur 
		...
		EndIf	
		cQuery	+= "  'FAT' AS ROTINA, LEM.LEM_FILIAL AS FIL, LEM.LEM_ID AS ID, LEM.LEM_NUMERO AS NUMDOCTO, LEM.LEM_NUMERO AS CHVNF, C1H.C1H_CNPJ AS CNPJORIREC, T5M.T5M_TPREPA AS TPREPASSE, "
		cQuery  += "  CASE WHEN C1H.C1H_CNPJ = ' ' AND C1H.C1H_PAISEX != '' AND C1H.C1H_PEEXTE = '2' THEN C1H.C1H_ID     ELSE ' ' END AS CIDEXTE, "
		cQuery  += "  CASE WHEN C1H.C1H_CNPJ = ' ' AND C1H.C1H_PAISEX != '' AND C1H.C1H_PEEXTE = '2' THEN C1H.C1H_CODPAR ELSE ' ' END AS CCODPAR, "
		cQuery  += "  CASE WHEN C1H.C1H_CNPJ = ' ' AND C1H.C1H_PAISEX != '' AND C1H.C1H_PEEXTE = '2' THEN C1H.C1H_NOME   ELSE ' ' END AS CDESPART, "
	...
	ElseIf cReg == "R-2040"
		cQuery 	+= " AND C1H.C1H_PPES = '2' AND C1H.C1H_CNPJ <> '' "
	...
	// Where
	cQuery	+= " WHERE LEM.LEM_FILIAL IN " + cFiliais + " AND LEM.D_E_L_E_T_ = ' ' AND "
	...
	If __lApurBx
		cQuery += " T5P.T5P_DTPGTO BETWEEN '" + cDataIni + "' AND '" + cDataFim + "' "
	Else
...
Static Function Apurac3040(cPerApu,cReg, aFil, aInfoEUF, aMovs)
	...
	cQuery := QryApRecAd(cPerApu, .T., .F. , aFil, cReg)
