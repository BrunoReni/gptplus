...
Begin Sequence
	...
	DbSelectArea("T39")
	DbSetOrder(2)
	If DbSeek(xFilial("T39") + cAnoReg)
		While !EOF() .AND. cAnoReg == T39->T39_ANOREF .AND. xFilial("T39") == T39->T39_FILIAL
			//Regime do Registro	- N = Normal, Estimativa e Outros;S = Simples Nacional
			cTipRegim := Iif ( T39->T39_TIPREG == '01', "N" , "S" )
			T39->(DbSkip())
		EndDo
	EndIf
	...
	If aWizard[2][10] == "1 - Sim"
		...
		If !Empty(cItens)
			cWhereCpl += "AND C1L.C1L_ID NOT IN (" + cItens + ") "
		EndIf
		cWhereCpl += "%"
	Else
