...
Static __lBlind         := Nil
Static __nDiasAtr       := Nil      
Static __lFGerApr       := Nil
Static __oProxSeq       := Nil
Static __cSquen         := Nil
...
ADD OPTION aRotina TITLE STR0017	ACTION 'F677APROVA'			OPERATION  8	ACCESS 0  	//"Aprovação"
...
/*/
//-------------------------------------------------------------------
Function  F677AVLMod(oModel As Object)
				...
				If lRet
					...
					If lRet
						...
						While lRet .And. (cAliasTrb2)->(!Eof())
							...
							If cSeq == "1"
								F677MsgMail(1, (cAliasTrb2)->PARTIC, Nil)
							EndIf
	...
	If !Empty( cEventCode ) .And. ChkEventReg( cEventCode, cUserCode )
		...
		If lAddMessage
			If FwLibVersion() >= "20220905"
				...
				jNotification["click_action"] := "FCM_PLUGIN_ACTIVITY"
