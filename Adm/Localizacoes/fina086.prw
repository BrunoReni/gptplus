			...
			If cPaisLoc $ "ARG" .And. Empty(FE_DTESTOR) .And. lEstorna		// Gera um registro de estorno das retenções.
				...
				IF SFE->FE_TIPO == "B"
					...
					If cRETOP == "2" 
						SFE->(DbSkip())
						LOOP
					Elseif cRETOP == "3" .and. !VigValida(SFE->FE_EMISSAO,dDatabase) .and. cTipoSFH == "I"
						SFE->(DbSkip())
						LOOP
					Endif 
				EndIf 
...
Static Function xDelSE5(nHdlPrv,cLotecom,cArquivo,aFlagCTB)
	...
	*/
	...
	Do While (xFilial("SE5")==E5_FILIAL.And.cNumOP==E5_ORDREC)
		...
		If E5_RECPAG=="P"
			lAtuFKs := (cPaisLoc $ "MEX" .And. Empty(SE5->E5_BANCO) .And. AllTrim(SE5->E5_TABORI) $ "FK2|FK5" .And. Empty(SE5->E5_SITUACA))
...
If !(TRB->SITUACAO $ "02|04|05|06|07")  // accede para documentos activos
	...
	If nNumReg > 0
		If aScan(aMarca,{|x| x[1] > 0}) <= 0
			aMarca := {}
		Else
			If TRB->TIPODOC == 'PA' .AND. !F086VlPaEs(TRB->NUMERO) // desmarca el documento
				TRB->(RecLock("TRB",.f.))
				TRB->MARK := " "
				TRB->(MsUnlock())
	...
	If !EMPTY(TRB->MARK) .AND. TRB->TIPODOC == 'PA'
		If !F086VlPaEs(TRB->NUMERO)
			...
			If MSSEEK(xFilial("SEK") + TRB->NUMERO + TRB->TIPODOC + TRB->PREFIXO + TRB->NUMDOC + TRB->PARCELA + TRB->TIPO )
				...
				EndIf
				//Por medio de la función F086VlMvPa() se valida que el PA no contenga compensaciones
				If F086VlMvPa(TRB->NUMERO, TRB->TIPO, TRB->PARCELA, TRB->PREFIXO)
					nAncho :=  290
					nLargo := 350
				...
				Else
					// Si el PA contiene compensación(es), se detona mensaje al usuario
					MsgInfo(STR0148) // "El importe de este anticipo ya fue utilizado, total o parcialmente, para el pago de otros documentos. No se permite cancelación."
				EndIf
			EndIf
...
/*/{Protheus.doc} F086VlMvPa
	Identifica si existen compensaciones
	hechas sobre un PA en particular
	@type  Function
	@author Oswaldo Diego
	@since 16/02/2023
	@version 12.127 o Superior
	@param cPa     , caracter, Número de PA
	@param cTipo   , caracter, Tipo
	@param cParcela, caracter, Parcela
	@param cPrefijo, caracter, Prefijo
	@Return lCompen, lógico, Verdadero si el PA no tiene compensaciones, de lo contrario, falso
	@example
	F086VlMvPa(cPa, cTipo, cParcela, cPrefijo)
	/*/
Static Function F086VlMvPa(cPa, cTipo, cParcela, cPrefijo)
	Local cAliasQry	:= GetNextAlias()
	Local lCompen   := .F.
	Local aArea     := GetArea()

	BeginSQL Alias cAliasQry
		SELECT COUNT(1) COMPEN
		FROM %Table:SEK%
		WHERE EK_FILIAL   = %xfilial:SEK%  AND
			  EK_ORDPAGO <> %Exp:cPa%      AND
			  EK_NUM      = %Exp:cPa%      AND
			  EK_TIPO     = %Exp:cTipo%    AND
			  EK_PARCELA  = %Exp:cParcela% AND
			  EK_PREFIXO  = %Exp:cPrefijo% AND
			  EK_CANCEL   = 'F'
	EndSQL

	(cAliasQry)->(DBGOTOP())

	lCompen := (cAliasQry)->COMPEN == 0
