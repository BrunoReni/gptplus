	...
	For nj := 1 to Len( aSE2[ni][1])
		...
		SE2->(DbSetOrder(1))
			...
			If aSE2[ni][1][nj][_JUROS  ] > 0  // existem juros
				AAdd(aInfoFK6, {})
				nArFK6		:= Len((aInfoFK6))
				nVlJurTit	:= Round(xMoeda( aSE2[ni][1][nj][_JUROS  ], aSE2[ni][1][nj][_MOEDA  ], 1,,5,aTxMoedas[aSE2[ni][1][nj][_MOEDA  ]][2]),MsDecimais(1))
				...
				AAdd(aInfoFK6[nArFK6], {"FK6_VLMOE2", aSE2[ni][1][nj][_JUROS  ]})
			...
			If aSE2[ni][1][nj][_MULTA  ] > 0 //Existe Multa
				AAdd(aInfoFK6,{})
				nArFK6		:= Len((aInfoFK6))
				nVlMulTit	:= Round(xMoeda( aSE2[ni][1][nj][_MULTA  ], aSE2[ni][1][nj][_MOEDA  ], 1,,5,aTxMoedas[aSE2[ni][1][nj][_MOEDA  ]][2]),MsDecimais(1))
				...
				AAdd(aInfoFK6[nArFK6], {"FK6_DATA"  , dDataBase	})
				AAdd(aInfoFK6[nArFK6], {"FK6_MOEDA" , StrZero( aSE2[ni][1][nj][_MOEDA  ],2)})
				AAdd(aInfoFK6[nArFK6], {"FK6_VLMOE2", aSE2[ni][1][nj][_MULTA  ]})
				AAdd(aInfoFK6[nArFK6], {"FK6_TXMOED", aTxMoedas[aSE2[ni][1][nj][_MOEDA  ]][2]} )
				AAdd(aInfoFK6[nArFK6], {"FK6_ORIGEM", cFunName	})
				 ...
				 If cPaisLoc == "PER"
			    	...
			    	Else
			    		If aSE2[nA][1][nC][_MOEDA] == 1
				    		If (AllTrim(aSE2[nA][1][nC][_TIPO]) $ "TX|NF|FT")
...
Local aRateioGan  :=	{}
Local aAreaSA2
// Variables referentes al calculo de acumulado de IVA.
Local nA,nZ,nX, nValAcDia, nValAcMes, aAreaSFE, dDtPesq, dDataFim, dDataAtu
Local lAchou, nVlAcmPg, lReten
	...
	If nOpca==1
	   aAreaSA2	:=	SA2->(GetArea())
		If RateioCond(@aRateioGan)
			...
			If cPaisLoc == "PAR" .and. Len(aSE2Tmp[1]) >0
				// Valida el acumulado por dia y mes 
				nValAcDia := 0 
				nValAcMes := 0
				aAreaSFE := SFE->(GetArea())
				SFE->(DbSetOrder(3))
				...
				SFE->(DbGotop())
				lAchou:=.F.
				SFE->(dbSeek(xFilial("SFE")+SA2->A2_COD+SA2->A2_LOJA))

				While !lAchou .AND.  dDtPesq<=dDataFim
					If SFE->(dbSeek(xFilial("SFE")+SA2->A2_COD+SA2->A2_LOJA+dtos(dDtPesq)))       
						lAchou:=.T.
					EndIf
					dDtPesq:=dDtPesq+1
				Enddo      
				...
				If lAchou  
					While lAchou .And. SFE->(!Eof()) .and. (xFilial("SFE")+SA2->A2_COD+SA2->A2_LOJA==SFE->FE_FILIAL+SFE->FE_FORNECE+SFE->FE_LOJA) .AND. SFE->FE_EMISSAO<=dDataFim
						If SFE->FE_TIPO=="I"
							If SFE->FE_EMISSAO==dDatabase
								nValAcDia:=nValAcDia+SFE->FE_VALBASE 
								nValAcMes:=nValAcMes+SFE->FE_VALBASE 	 
				...
				EndIf
				RestArea(aAreaSFE)
				// Valida el acumulado por Orden de pago
				nVlAcmPg := 0
				For nZ := 1 To Len(aSE2Tmp[1])		
					If Len(aSE2Tmp[1,nZ,_RETIVA]) > 0
						For nA := 1 To Len(aSE2Tmp[1,nZ,_RETIVA])              
							If aSE2Tmp[1,nZ,_RETIVA,nA,4] >0              
								nValor	:= 0
								nValor	:= Round((aSE2Tmp[1,nZ,_RETIVA,nA,3]), msdecimais(aSE2Tmp[1,nZ,4]))
								nVlAcmPg:= nVlAcmPg +  nValor
							EndIf	
						Next nA
					EndIf
				...
				If  nVlAcmPg >= GetNewPar("MV_VLRIVAT",0) .or. ((nValAcDia+nVlAcmPg)>=GetNewPar("MV_VLRIVAD",0)) .OR. ((nValAcMes+nVlAcmPg)>=GetNewPar("MV_VLRIVAM",0)) 
					lReten:=.T.
				EndIf
				If !lReten .and. Len(aSE2Tmp[1]) > 0
					For nX := 1 to Len(aSE2Tmp[1])
						For nZ := 1 To Len(aSE2Tmp[1,nX,_RETIVA])		
							If Len(aSE2Tmp[1,nX,_RETIVA]) > 0
								aSE2Tmp[1,nX,_RETIVA,nZ,4]:= 0
								aSE2Tmp[1,nX,_RETIVA,nZ,6]:= 0
							EndIf
						Next nZ
					Next nX
				EndIf
...
Function F85aMovFK(cModelo, cCamposE5, aInfoFK2, aInfoFK5, aInfoFK6)
	...
	If  (lFinm020 .Or. lFinm030) .And. !Empty(cCamposE5)
		...
		oModel:SetValue( "MASTER", "NOVOPROC"	, .T. )		//Nuevo proceso
