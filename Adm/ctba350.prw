...
IF nOpca == 1
	...
	Else
		...
		For nX := 1 TO If(lFilde_Ate, 16, 14)  //parametro filial de / ate = mv_par15 / mv_par16
...
lPrintR := .T.
If lProcessa .And. TRB350REL->(LastRec()) == 0 .Or. __lResSld
	If !__lBlind
		lPrintR := .F.
...
If TCSPExist("CTB350REL_" + cEmpAnt )
	...
	If Empty(aResult)
  		conout(STR0072) //"Falha na chamada do processo de inclusão dos dados para o relatorios. Gravação será efetuada de forma padrão."
   		lUsaProc := .F.
	EndIf
...
ßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßß
*/
Function ct350roda( cFilCT2, lEnd,aErro,aErroTexto,cTpSldAtu,lEfLote,lEfDoc,lMostraLct,lSimula, lCTBA351, aRegMin, aRegMax )
...
If lCTBA351
	...
	CT2->(dbSeek(aRegMin[1] + DTOS(aRegMin[2]) + aRegMin[3] + aRegMin[4] ))
EndIf
...
-------------------------------------------------------------------------*/
Static Function CriaTmp( aCampos, nThread)
Local aArea	:= GetArea()
Local cArqAlias	:= "TRB350REL"
Local cError    := ""

If nThread > 1
	If TcVersion() < '21.1.1.4'  //tabelas temporarias somente a partir desta versao do dbaccess
		nThread := 1 //reatribui variavel nThread para 1 esta eh passado por referencia
	EndIf
EndIf
...
Else
	//criacao da tabela temporaria compartilhada por multiplas threads
	_cTableTmp := Ctb350TmpC(aCampos, @cError)  //variavel static recebe nome real da tabela
...
/*-------------------------------------------------------------------------
Funcao        : Ctb350TmpC - Cria Tmp de forma a compartilhar com multiplas threads
Autor         : Totvs
Data          : 27/10/2022
Uso           : Cria a tabela temporaria a ser usada na impressão do relatorio em multiplas threads
-------------------------------------------------------------------------*/
Function Ctb350TmpC(aCampos, cError) 
Local cTableTmp
Local cSetupTime
...
cSetupTime   := TCConfig("SETTEMPKEEPALIVE=18000") // Setar timeout de inatividade em segundos (5 Horas)

If cSetupTime == "OK - SETTEMPKEEPALIVE TO 18000"

    FWDBCreate(cTableTmp, aCampos, "TOPCONN", .T.)      // Criação da tabela
    ...
    DBCreateIndex(cTableTmp+"1", "DDATA+LOTE+DOC")
...
Else
    ...
    Conout(cSetupTime)
    cError += "CTB350 - Processing Error: " + "Could not set timeout time in temp table: "
    cError += cTableTmp +". For more details, check file -> console.log"  
	Conout(cError)
...
BEGIN SEQUENCE
	IF TcSqlExec(cStatement) <> 0
		UserException( STR0081 + CRLF + TCSqlError()  + CRLF + ProcName(1) + CRLF + cStatement ) //"Erro na instrução de execução do SQL"
		lRetorno := .F.
	Endif
RECOVER
	lRetorno := .F.
...
Else
	If _cTableTmp <> Nil  //quando eh por mulithread o nome deve pegar da variavel static _cTableTmp
		cNomeTab := _cTableTmp
	EndIf
EndIf
