...
If lProcedure
	If SPSMigrated()//se jแ estแ usando o novo processo de procedures
		...
		EndIf
		//*Verifica se a procedure estแ instalada e com mesma assinatura
		If __lCTBM300 
			//*Caso alguma das rotinas acima esteja em execu็ใo nใo permitir a execu็ใo da rotina via procedure.               
			If  CtbIsCube() .or. _lJobs  .or. ctb300val(MV_PAR03,MV_PAR04,dDataIni, dDataFim,lTdsMoedas,mv_par12)
			  lProcedure:= .F.
			Endif 
		EndIf	
	else
		lProcedure := .F. // execu็ใo em advpl
		If !isBlind()
			Help(" ", 1, "NOPROC25", , STR0116, 2, 0,,,,,,)//Para execu็ใo da procedure ้ necessแrio a instala็ใo do novo processo 25
		EndIF	
	EndIf
EndIF	
If lProcedure .and. __lCTBM300 //se regra de procedure e se estแ com a mesma assinatura
