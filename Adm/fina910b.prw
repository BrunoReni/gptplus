		...
		While fReadLn(nHdlFile,@cLinha,nTam)
			...
			ElseIf lTemHead .And. Alltrim(aLinha[1]) == "10"	// Detalhes do arquivo							
				...
				If Len(aLinha) == nPosToReg
					...
					lContinua := Fa910Cont(aLinha[DT_DTVND], cTef, cNSUSitef, cParc, cCodAut, cCodLoj)
...
@version 1.0
/*/
Static Function Fa910Cont(cDtVend As Character, cTef As Character, cNSUSitef As Character, cParc As Character, cCodAut As Character, cCodLoj As Character) As Logical

	Local cQry 		As Character
	Local cAlsFIF	As Character
	Local lRet 		As Logical
	Local lContinua	As Logical

	cQry		:= ""
	cAlsFIF		:= GetNextAlias()
	lRet		:= .F.
	lContinua	:= .F.

	Default cDtVend := CtoD("")
	Default cTef := ""
	Default cNSUSitef := ""
	Default cParc := ""
	Default cCodAut := ""
	...
	If lContinua
		...
		If	__oTmpFIF == Nil
			...
			__oTmpFIF	:= FWPreparedStatement():New(cQry)
		EndIf

		__oTmpFIF:SetString(1, xFilial("FIF"))
		__oTmpFIF:SetString(2, cDtVend)
		__oTmpFIF:SetString(3, cTef)
		__oTmpFIF:SetString(4, cNSUSitef)
		__oTmpFIF:SetString(5, cParc)
		__oTmpFIF:SetString(6, cCodAut)
		__oTmpFIF:SetString(7, cCodLoj)

		cAlsFIF := MPSYSOpenQuery(__oTmpFIF:GetFixQuery())

		If (cAlsFIF)->REC > 0
			FIF->(DbGoTo((cAlsFIF)->REC))
			lRet	:= .T.
		EndIf
