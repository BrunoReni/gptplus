...
*/
//----------------------------------------------------------------------
Function FINGRVFK7(cAlias As Character, cChave As Character, cFilMov As Character) As Character
	Local aAreaAnt	:= GetArea()
	Local cRet 		:= ""
	Local lChave	:= .T.
	Local cFilFK7	:= ""
    ...
    If __lFK7Cpos .And. FindFunction("FinFK7Cpos") .And. FExecFixN("2")
        cNewCh := StrTran(cChave, "|", "")
        DbSetOrder(3)
        lRet := MsSeek(cFilFK7 + cAlias + cNewCh)
    Else
        DbSetOrder(2)
        lRet := MsSeek(cFilFK7 + cAlias + cChave)
    EndIf   
