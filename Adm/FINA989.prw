...
For nFil := 1 To Len( aFils )
	...
	For nCart := 1 To 2
		...
		| movimentações já integradas e que foram excluídas no ERP.  			              |
		| Obs: Foi disponibilizado apenas na opção "banco-a-banco" (cTpSaida="2") pois a      | 
		| geração da T999 não entra no filtro do range de datas definido no Wizard, então 	  |    
		| como a integração via TXT não grava a flag de integração, acabaria sempre enviando. |
		-------------------------------------------------------------------------------------*/	
	...
	If nCart == 1 // A PAGAR
		...
		If SA2->(MsSeek( xFilial("SA2") +  (cAliasQry)->COD + (cAliasQry)->LOJA))
			...
			If __lTemDKE //DKE: Tabela complementar da SA2	
				...
				Endif
				If Empty(cIseImun)
					cIseImun := "1" //Entidade não isenta (Tributação normal);
				EndIf
			EndIf
		...
		If lTribFKW .and. Len(aT154Trb) > 0
			...
			*/
			...
			For nI := 1 To Len(aT154Trb)
				...
				If aT154Trb[nI][1] == "12" //Gera T154AG e tabelas filhas somente para IR na emissão
...
Function FFinT158( nRecno As Numeric, nCart As Numeric, cAliasQry As Character)
	...
	If !_lPCCBaixa .or. lTitPA
		lTemNatR := FTemFKW(cIdDoc)	//Verificação via FKW quando ambiente for de "PCC na emissao" ou o titulo for PA (não há FKY nesses casos)
	Else
		lTemNatR := FTemFKY(cIdDoc)	//Verificação via FKY para as demais situações	
	Endif
	...
	If lTemNatR //Verifica se o titulo possui vinculo com naturezas de rendimento 
		...
		While (cAliasBx)->(!Eof())
			...
			AAdd( aRegs, { cReg,;								   //01 - REGISTRO - Tipo de Registro
						(cAliasQry)->E2_NUM,;				       //02 - NUMERO - Número da fatura/recibo
...
Função para verificar se existem dados na tabela FKH para realizar 
a integração.
...
Static Function TrataAcao( cTpAtrb As Char, cFiltro As Char ) As Char
	...
	ElseIf cFiltro == "ISENCAO"
		...
		ElseIf cTpAtrb == "022" //AJUDA DE CUSTO
			cRet := "8"
		ElseIf cTpAtrb == "023" //RENDIMENTOS PAGOS S/ RETENÇÃO DO IR - LEI 10.833/2003  
			cRet := "9"
		EndIf
...
Static Function MntQryAdt(cAliasBx As Character)
	...
	If __oQryPA == Nil	
		...
		//Obs: Caso mudar alguma coluna da estrutura do SELECT acima, replicar também nas querys das funções MntQry158a e MntQry158b
...
Static Function MntQry158a(cAliasBx As Character)
	...
	If __oQry158a == Nil	
		...
		//Obs: Caso mudar alguma coluna da estrutura do SELECT acima, replicar também nas querys das funções MntQry158b e MntQryAdt
...
@param	cAliasBx - Alias da tabela temporaria
@return Nil	- Retorna .T. para final de execução
...
Static Function MntQry158b(cAliasBx As Character)
	...
	If __oQry158b == Nil	
		...
		cQuery += 						" AND A.FK2_IDDOC = FK2.FK2_IDDOC "
		cQuery += 						" AND A.D_E_L_E_T_ = ' '  ) SEQMAX "
		//Obs: Caso mudar alguma coluna da estrutura do SELECT acima, replicar também nas querys das funções MntQry158a e MntQryAdt
...
Verifica se há retenção/suspenção/isenção/dedução para o IRF com o 
...
Static Function FIrrfFKW(cAliasQry, aT154Trb, lFornPF, dDtEmis, lIsento, lOrigNF, lSusp) 
	...
	If __lRatIrf .and. lOrigNF
		...
		| as parcelas (MV_RATIRRF=T), o trecho abaixo irá aglutinar/somaro IR para gerar corretamente |
		---------------------------------------------------------------------------------------------*/	
		aIdDoc := {}
		If __oQryFK7 == Nil 
	...
	If FKG->( MsSeek( xFilial("FKG") + cIdDoc ) )
		...
		While FKG->( !Eof() ) .And. Alltrim( FKG->( FKG_FILIAL + FKG_IDDOC ) ) == Alltrim( xFilial("FKG") + cIdDoc )
			...
			ElseIf cTpAtrb $ "013" // Dedução por Pensão Alimenticia 
				...
				EndIf
				lDedPen := .T.				
			ElseIf cTpAtrb $ "006|015|016|017|018|019|020|021|022|023" //Isenção
	...
	While FKY->( !Eof() ) .and. FKY->FKY_IDFK2 == cIdMov
		...
		If nRenBrut > 0 
			AAdd( aT158Trb,{cTrib ,;  			//01 - Codigo que corresponde o tributo no layout TAF
							FKY->FKY_NATREN,;	//02 - Código da natureza de rendimento
							nRenBrut,; 			//03 - Rendimento bruto
							nRenTrib,;			//04 - Rendimento tributavel
							nValTrib,;			//05 - Valor do tributo
...
Static Function NatPropBX(cAliasBx As Char, cAliasQry As Char, aT158Trb As Array, lFornPF As Logical, dDataPg As Date, lIsento As Logical) As Logical
	...
	If __oQryFTR == Nil
		...
		cQry1 	+= 				" AND FK2.D_E_L_E_T_ = ' '),0) = 0 THEN 1 " // Título com 1 única baixa total
		...
		cQry1 	+= 				" AND FK2EST.FK2_TPDOC = 'ES' "
		cQry1 	+= 				" AND FK2EST.D_E_L_E_T_ = ' ') "
		cQry1 	+= 				" AND FK2.D_E_L_E_T_ = ' ') * FKW_PERC / 100 ) ) / ( FKW_PERC / 100) ) / FKW_BASETR " // Título total baixado com pagamentos parciais
		cQry1 	+= 		" END FATOR "
	...
	While (cAliasIR)->( !EOF() )
		...
		If !Empty((cAliasIR)->NUMPRO) //Verifica se houve retenção que deixou de ser efetuada em função de um processo adm/jud
			lSusp    := .T.
			nRenTrib := nRenBrut
		...
		If nRenBrut > 0
			AAdd( aT158Trb,{ cTrib ,;  			 //01 - Codigo que corresponde o tributo no layout TAF
							(cAliasIR)->NATREN,; //02 - Código da natureza de rendimento
							nRenBrut,; 			 //03 - Rendimento bruto
							nRenTrib,;			 //04 - Rendimento tributavel
							nValTrib,;			 //05 - Valor do tributo
							cNumPro,;		 	 //06 - Identificação do processo judicial/administrativo ou ato concessório
