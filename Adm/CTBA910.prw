...
If lCriouSDF .And. MsgYesNo(STR0088+CRLF+STR0089,STR0001)   //"Será necessário acesso exclusivo ao sistema." ##"Confirma o processamento de criação das Entidades no cadastro e execução UPDDISTR ?"##"Atenção" 
	...
	cTxtAux += STR0092+CRLF   //"Assistente de criação de Entidades Finalizado!"
	cTxtAux += STR0093+CRLF   //"Neste momento iremos executar a função UPDDISTR para atualização do dicionario de dados!"+CRLF 
	CONOUTR(cTxtAux)
...
For nInc := 1 To Len( aSM0 )
	...
	//Provisório - Tratamento para contornar erro na LIB
	__cUserID := "000000"
...
Local aEstrut	:= {}
Local cTexto	:= "Classe necessária não encontrada. Atualize a versão da LIB."
Local nX		:= 0
Local nY		:= 0
Local nZ		:= 0
Local cGrpNum	:= ""
