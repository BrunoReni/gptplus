...
Method listaDebitos() class PMobFinMod
	...
	While !TRB1->( Eof() )
		...
		self:listaDebitosMap['debitos'][nLen]['valor']			:= TRB1->E1_VALOR
...
Method detalheDebito() class PMobFinMod
	...
	if len(aMatTitulo) > 3
		...
		cSql += " E1_VENCTO, E1_BAIXA, E1_VALOR, E1_SALDO, E1_IRRF,  " 
		cSql += " SE1.R_E_C_N_O_ tituloId,	" + cEnt
		...
		cSql += " AND FK7.FK7_CHAVE  = SE1.E1_FILIAL+'|'+SE1.E1_PREFIXO+'|'+SE1.E1_NUM+'|'+SE1.E1_PARCELA+'|'+SE1.E1_TIPO+'|'+SE1.E1_CLIENTE+'|'+SE1.E1_LOJA  AND FK7.D_E_L_E_T_ = ' '"+ cEnt
		...
		self:detalheDebitoMap['detalhes'][1]['dataVencimento'] 	:= Alltrim(Transform(Dtos(TRB1->E1_VENCTO), "@R 9999-99-99"))
		self:detalheDebitoMap['detalhes'][1]['dataPagamento'] 	:= Iif(!Empty(TRB1->E1_BAIXA),Alltrim(Transform(Dtos(TRB1->E1_BAIXA), "@R 9999-99-99")), "")
		self:detalheDebitoMap['detalhes'][1]['valor'] 			:= TRB1->E1_VALOR
...
Method getMatTit() CLASS PMobFinMod
	...
	if self:lLoginByCPF
		...
		endIf
	endIf
	cSql += " SELECT BA1_CODINT, BA1_CODEMP, BA1_MATRIC FROM "+RetSqlName("BA1")+" BA1 "
	cSql += " WHERE BA1_FILIAL = '"+xFilial("BA1")+"' "
	cSql += " AND BA1_CPFUSR = '"+cCpf+"' "
	cSql += " AND BA1.D_E_L_E_T_ = ' ' "

	dbUseArea(.T.,"TOPCONN",TcGenQry(,,cSql),"TMPCPF",.T.,.F.)
    
	If !TMPCPF->(Eof())
		while !TMPCPF->( Eof() )

			cCodInt := Alltrim(TMPCPF->BA1_CODINT)
			cCodEmp := Alltrim(TMPCPF->BA1_CODEMP)
			cMatric := Alltrim(TMPCPF->BA1_MATRIC)
