...
Method grvPedido() Class SyncHndPLS
    ...
    Local cResponse := iif(self:lAuto, self:cJsAutPost, self:oClient:GetResult() )
	...
	if BNV->(MsSeek(xFilial("BNV")+self:cPedido))
		...
		aadd( aCampos,{ "BNV_STATUS", iif(self:lSuccess,self:_StSuccess,self:_StError) })
...
Method setupPost() Class SyncHndPLS
    ...
    if !empty(cEndPoint)
        ...
        if self:cCodTrans == _cancel_BEA_atu
            self:oClient:setPath(self:cVersao+"/"+self:cAPI+"/"+self:cChaveBNV+"/"+"cancel")
        else
            self:oClient:setPath(self:cVersao+"/"+self:cAPI)
        endif       
        ...
        self:logPlsToHat("URL API: "+cEndPoint+self:cVersao+"/"+self:cAPI)
		self:logPlsToHat("Json de envio: " + self:cJson)

        self:lSuccess := iif(self:lAuto,.T.,self:oClient:Post(self:aHeader))
