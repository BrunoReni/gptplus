...
plLogDet( cBusca, 'BAV_SEGMEN', 'BI3_CODSEG', 'Segmentacao', nil, cRotina )

plLogDet( cBusca+cCodPla, 'BAV_CODPLA', 'BA1_CODPLA|BA3_CODPLA', 'Plano', nil, cRotina )
...
if nTipo == 0
	...
	BAV->(dbSetOrder(1))
	if BAV->(msSeek(xFilial('BAV')+cBusca+cCodPla, .f.))

		if nTipo == 2
			cRet := if(empty(BAV->BAV_CTAENC), 'C->'+cBusca, BAV->BAV_CTAENC)
		else
			cRet := if(empty(BAV->BAV_CONTA), 'C->'+cBusca, BAV->BAV_CONTA)
		endIf

	elseIf BAV->(msSeek(xFilial('BAV')+cBusca+space(4), .f.))
...
if existBlock("PLCTPBUS")
	...
	cRet := execBlock("PLCTPBUS", .f., .f., { cRotina, 'BAV', cBusca, cRet } )
