...
±±³Fun‡„o	 ³ acdv150    ³ Autor ³ Sandro              ³ Data ³ 14/08/01 ³±±
±±ÃÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄ´±±
±±³Descri‡„o ³ Programa de transferencia (com Controle de Localizacao)    ³±±
...
Private cCB0ProdEnd := IIf( FindFunction( 'AcdGTamETQ' ), AcdGTamETQ(), Space(48) )
...
If ! SBE->(MsSeek(xFilial("SBE")+cArmazem+cEndereco))
	...
	elseIf Empty(cEndereco) .and. !Empty(cArmazem)
		If NNR->(MsSeek(xFilial("NNR")+cArmazem))
			...
			lRet := .F.
			VtAlert(STR0062, STR0013,.t.,4000,4)// armazem não localizado
		EndIf
	else
		VtAlert(STR0007, STR0013,.t.,4000,4) //'Endereco nao encontrado'###'Aviso'
		VTKeyboard(chr(20))
...
EndIf
If !lSemEnd 
	If lRet .And. ! CBEndLib(cArmazem,cEndereco) .Or. !ExistCpo("SBE",cArmazem+cEndereco,,,.F.) //5º parametro retira o Help
...
Else
	...
	Begin Sequence
	For nP := 1 to len(aItensPallet)
		cProduto :=  padr(aItensPallet[nP],nCodEtq2)
